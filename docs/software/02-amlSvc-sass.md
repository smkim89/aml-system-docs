...
**V71 runtime RLS 경계.** `%>`와 expression overlap은 leakproof가 아니어서 `aegis_app_runtime` RLS barrier 아래에서 V64 GIN을 잃었다. 후보 CTE를 고정 3열 SECURITY DEFINER 함수로 옮기고, NOLOGIN/non-super/BYPASSRLS 전용 owner에 watchlist 두 테이블 SELECT만 부여한다. 함수는 tenant GUC 일치 또는 elevated만 허용하며 missing/mismatch는 42501이다. 직접 테이블 RLS 정책 2종, candidate 의미·점수·응답은 불변이다. V71은 같은 release bootJar privileged runner로만 적용한다. Java `Properties` canonical parser가 URL credential override와 non-PHP profile을 거부하고 normalized config+Base64 app envelope를 만든다; container entrypoint는 envelope가 설정된 proof session에서만 decode하며 일반 기동은 기존 entrypoint 그대로다. runner는 Java V38+PHP repeatable `version/type/script/checksum` inventory를 적용한 뒤 app provision을 별도 JDBC transaction으로 수행한다. `aegis_aml_app`은 exact runtime membership/history-only direct ACL을 가지며 password probe는 `session_user`를 확인한다. 선존 recall/app role의 attribute·membership·current-DB/AML ownership·direct/default ACL이나 password mismatch는 자동 교정하지 않는다. V71만 적용된 정상 중간상태는 재실행으로 resume한다. 함수 planner GUC를 영구 강제하지 않는다.

배포 검증은 privileged init과 ordinary app을 같은 mandatory image `.Id`로 고정한 tenant_demo/PHP bounded non-super session에서 수행한다(RepoDigests가 있으면 exact 비교, 로컬 empty는 허용). Dilisense는 upstream key를 host streaming proxy에만 두고 AML에는 session inner key를 전달하며, REST sync가 실제 소비한 동일 JSONL을 0600 tee+raw version/SHA-256/byteCount로 결속한다. 저장 activeVersion은 `dilisense-<rawVersion>`이다. tenant_demo active-version join 기준 Dilisense PEP 90만·전체 150만 미만이면 성능 증적은 무효다. live plan runner는 recall-owner transaction에서만 `statement_timeout=10000`·`pg_trgm.word_similarity_threshold=0.30`을 LOCAL 적용하고 canonical `EXPLAIN (ANALYZE, BUFFERS)`의 V64 GIN 3종·entries Seq Scan 0·3초 이하를 판정한다(57014=`PLAN_TIMEOUT`). `aml-response-performance-v1`은 WLF 3회 protocol 성공과 median≤3초, CDD→RA≤3초, 국내 TM≤4초, 해외 TM≤6초 및 실패 시 관측 wall/trace latency와 partial/NOT_RUN 6건을 기록하는 운영 증적이며 엔진 케이스 카탈로그를 대체하지 않는다.
