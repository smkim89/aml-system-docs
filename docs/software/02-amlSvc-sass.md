...
| 명단·국가위험 관리 | `domain/watchlist`·`domain/policy/CountryRisk` | versioned import + diff + freshness monitoring + applied-snapshot gate |
...
활성 binding의 `timezone`은 규제 일경계용 **필수 유효 IANA region ID**다. DST 규칙을 보존하지 못하는 fixed-offset ID(`+10:00`, `GMT+10:00`)도 invalid다. PATCH null·blank·invalid/fixed-offset timezone은 write 전에 `400 AML.VALIDATION_ERROR`로 거절하고, nullable migration으로 남은 legacy missing/invalid/fixed-offset timezone은 `TenantPolicyUnboundException`→`422 AML.TENANT_POLICY_UNBOUND`로 인입을 거절한다(Manila 기본값 금지). production startup의 `ProductionSafetyValidator`도 elevated cross-tenant scan으로 ACTIVE binding의 null·blank·invalid/fixed-offset timezone을 같은 정책에 검증한다. `report_cutoff_time`은 nullable legacy compatibility를 유지하며 timezone의 대체값이 아니다.

**다통화(법인별 자국통화) REST 계약 확장(PLAN 20260818, U1/U18 — r16 이격 3).** ① `PATCH .../policy-binding` 은 **거래성 canonical 이벤트 보유 테넌트의 `baseCurrency` 변경을 fail-closed 로 거부**한다(`422 AML.TENANT_CURRENCY_HISTORY_LOCKED`) — 단일 통화 윈도우 보호(이력 있는 테넌트의 통화 소급 변경은 CTR/STR 임계·FX 해석을 소급 오염시킨다). `baseCurrency` 외 필드는 이력과 무관하게 허용한다. ② `report_cutoff_time` 값 기입은 **전용 서브리소스** `PUT .../policy-binding/report-cutoff-time`(항상-쓰기, 명시 `null` 허용)로 분리됐다 — `PATCH .../policy-binding` 은 더 이상 이 필드를 갱신하지 않는다(cutoff 무접촉). ③ 신규 raw read-back `GET .../policy-binding`(**12키** — `tenantId`/`jurisdiction`/`baseCurrency`/`reportingCurrency`/`timezone`/`policyPackCode`/`policyPackVersion`/`policyPackEffectiveFrom`/`reportCutoffTime`/`calendarCoverage`/`policyPackResolved`/`transactionalHistory`)이 `resolve()` 를 거치지 않고 tenant 행을 그대로 투영한다 — 4열(jurisdiction/baseCurrency/reportingCurrency/timezone) 미설정=422(미바인딩), 4열은 있으나 핀 Policy Pack revision 비-effective=200+`policyPackResolved=false` 로 구분한다(바인딩 존재와 pin 유효성 혼동 방지). ④ **엔진 CTR 임계 read 2종**(`GET /api/v1/admin/aml/ctr-thresholds`·`GET .../ctr-thresholds/{currency}`, `CtrThresholdAdminController`)이 신설됐다 — EXECUTED 반영값만·행 부재 시 합성 폴백 없음(목록은 생략, 단건은 404). bo-api 기준통화 프로파일 apply 오케스트레이션(API §3.16a)의 `STEP CTR_THRESHOLD` 판정 정본이다.
...
│   │                    #   AmlcCredential(feature/aml-reports-amlc-migration V57 — 테넌트별 AMLC 포털 로그인 계정 VO)
...
│   └── enums/          # EventFamily, CaseType, ScreeningStatus 등 닫힌 enum
│                        #   TmScenario 는 2026-08-01(V61)로 삭제됨 — scenarioCode 는 자유형 String(§12.1)
├── application/
...
│                  #   AmlcSubmissionPort(AMLC 포털 lodgement)·AmlcCredentialPort(계정 영속, V57),
...
│   ├── out/submission/ # MockRegulatorSubmissionAdapter (FIU acked/failed outcome, ProviderSvc 위임 무변경),
│   │                   #   MockAmlcSubmissionAdapter(mode=mock, 결정적 접수번호)·
│   │                   #   PlaywrightAmlcSubmissionAdapter(mode=browser, feature/aml-reports-amlc-migration §1.4-C —
│   │                   #   테넌트별 저장 계정으로 브라우저 자동화, AMLC 포털 직접 lodge — ProviderSvc 위임 아님)
...
**V71 runtime RLS 경계.** `%>`와 expression overlap은 leakproof가 아니어서 `aegis_app_runtime` RLS barrier 아래에서 V64 GIN을 잃었다. 후보 CTE를 고정 3열 SECURITY DEFINER 함수로 옮기고, NOLOGIN/non-super/BYPASSRLS 전용 owner에 watchlist 두 테이블 SELECT만 부여한다. 함수는 tenant GUC 일치 또는 elevated만 허용하며 missing/mismatch는 42501이다. 직접 테이블 RLS 정책 2종, candidate 의미·점수·응답은 불변이다. V71은 same-release bootJar privileged runner로만 적용한다. Java canonical parser가 URL credential override·CR/LF app credential·non-PHP profile을 거부하고 raw-main RO→generated-config RW/RO→proof-output RW 3-directory phase로 normalized config+Base64 envelope를 만든다. proof envelope가 없으면 image entrypoint는 기존 Java 기동을 그대로 수행한다. runner는 Java V38+PHP repeatable inventory를 적용한 뒤 app provision을 별도 JDBC transaction으로 수행하며 V71-only 상태는 재실행 가능하다. Spring ready proof writer는 실제 wrapped DataSource의 `session_user=aegis_aml_app`·`current_user=aegis_app_runtime`, inventory, recall-call evidence를 0600으로 기록하고 proof env가 없으면 no-op이다. 선존 recall/app role의 잘못된 ownership/ACL/password를 자동 교정하지 않으며 함수 planner GUC도 영구 강제하지 않는다.

배포 검증은 privileged init과 ordinary app을 같은 mandatory image `.Id`로 고정한 tenant_demo/PHP bounded non-super session에서 수행한다(RepoDigests가 있으면 exact, local empty 허용). State는 `up → capture-import → active-version-counts → replay-import → WLF-C21 → explain → C09 locked vitest → actual REST C01~C19 → parent-shell C20 restore → gate-ready → 나머지 RA/STR/CTR/performance`다. Dilisense proxy는 current-UID 0700 trusted root에서 symlink/hardlink/outside-root를 거부하고 동일 upstream stream을 capture/import 독립 digest로 누적한다. 저장 activeVersion은 `dilisense-<rawVersion>`이며 replay 진단은 C21 전까지 `PENDING_C21_PROOF`다. WLF-C21은 import/replay 각 시점의 서로 다른 idempotency key·동일 입력 screen snapshot, checksum-bound production parser 진단 2회, 독립 raw `functions>20` 전수계수, 서명된 admin REST browse/`entryIds` ordered `functions`/`refs`를 함께 결속한다. Actual REST C01~C19는 고유 same-source 4-eyes replay(C06), v1 raw 3필드 reveal digest/감사/역사 ID(C08), 동일 FP 승인 SUBMITTED→BO detail+locked Vitest(C09), Dilisense full-K 국가 분류/전건 replay(C19)를 포함한다. Sync DELISTED·후보 제외는 잠긴 F-012, PEP 경쟁자에 의한 제재 비밀림 방지는 잠긴 F-063이 별도 conjunction owner다. C20의 네 재기동은 저장 image ID/digest와 일치해야 하고 SUPPRESS/RISK_SIGNAL/invalid/restore 전이가 모두 닫혀야 한다. tenant_demo active-version PEP 90만·전체 150만, GIN3/no Seq/3초 JSON까지 있어야 gate-ready다. live plan은 recall-owner transaction에서만 timeout 10초·pg_trgm 0.30을 LOCAL 적용하고 57014를 `PLAN_TIMEOUT`으로 기록한다. `aml-response-performance-v1`은 write-once artifact로 WLF 3회 protocol 성공+median≤3초, CDD→RA≤3초, 국내 TM≤4초, 해외 TM≤6초와 실패 wall/trace latency·partial/NOT_RUN 6건을 안전한 0700 evidence root에 보존하며 엔진 카탈로그를 대체하지 않는다.
...
전역 cap 은 회수 순위를 `list_type` 과 무관하게 이름 점수·`entry_id` 로만 매겼다. PEP 물량은 제재 물량보다 자릿수가 크므로(dilisense 단일 소스만 약 150만 행), **흔한 이름의 PEP 후보가 cap 을 가득 채우면 진짜 제재 엔트리가 후보 집합에서 통째로 빠졌다** — 정책 평가에 도달조차 못 하므로 스크리닝은 `NO_MATCH` 가 되고 송금 게이트는 ALLOW 한다. 이것이 이 라운드에서 가장 무거운 규제 결함이었다(PEP 축 분리 정책 §10.3b 가 이를 증폭했다 — 정책 이전에는 살아남은 PEP 행이라도 `POSSIBLE_MATCH` 로 사람 눈에 띄었으나, 정책 도입 후에는 억제돼 아무것도 노출되지 않는다).

회수 SQL 은 `row_number() OVER (PARTITION BY axis ORDER BY strong DESC, sim DESC, entry_id)` 로 축별 랭킹을 매기고 축마다 `cap + 1` 행을 가져온다. 따라서 **제재·법집행은 오직 다른 제재·법집행에만 잘린다 — PEP 물량에는 절대 잘리지 않는다**(계약). 후보 총량 상한은 `cap` → `3 × cap` 으로 상수배 늘지만 세 축이 동시에 포화할 때만 도달하고, 회수는 **strictly additive** 다(구 전역 랭킹이 반환하던 행은 구성상 자기 축의 상위 `cap` 안에도 들므로 전부 그대로 반환된다).

절단은 축 단위로 귀속해 보고한다 — `candidateCapHit`(어느 축이든 잘림) + `truncatedAxes[]`(잘린 축) + `candidateCapScope`(`PER_LIST_TYPE_AXIS`, 과거 전역 cap 증거와 구분하는 키) + `axisCounts`(축별 최종 후보수). `SANCTIONS_LAW` 축 절단은 별도 `log.warn` 으로 승격한다 — PEP 축 절단은 150만 행 명단에서 흔한 정상 상태지만 제재 축 절단은 "제재 후보를 다 보지 못하고 내린 판정"이라 즉시 미탐 조사 대상이다. 회수 계약이 바뀌었으므로 `candidateStrategyVersion` 을 `wlf-cand-v1`(전역 cap) → **`wlf-cand-v2`**(축별 cap) 로 회전한다(키 이름은 불변, 기존 진단 키 삭제 0).
...
screening은 요청 시작에 ACTIVE WLF 전체 정의를 1회 immutable snapshot으로 pin하고 후보 엔트리마다 `list_type` profile로 점수를 계산한 뒤 최고 confidence band(동률이면 점수, 다시 동률이면 entryId 안정 순서)의 profile 임계로 `NO_MATCH`/`POSSIBLE_MATCH`를 결정하되, **이름 성분이 `highConfidenceThreshold` 이상이고 negative 신호가 0인 강한 이름 일치는 코로보레이터 유무와 무관하게 band를 `REVIEW`로 승격**(escalation floor, 사유 코드 `NAME_HIGH_CONFIDENCE`, 2026-07-29)하며 status는 band 파생을 유지한다. 오탐 화이트리스트 `AUTO_DISCOUNTED`는 승격보다 우선한다. 후보 선정의 band 비교는 승격 적격(이름 성분 ≥ `highConfidenceThreshold` ∧ negative 신호 0) 후보를 review band 미만·NO_MATCH band 초과의 **중간 순위**로 취급하며(적격 후보는 review band 이상 후보를 절대 밀어내지 못한다), 동률 시 점수 → entryId 안정 순서 비교는 불변이다. **후보가 0건이면 reviewThreshold가 0이어도 평가할 match가 없으므로 status와 `confidenceBand`를 모두 `NO_MATCH`로 고정**하고, 후보가 존재할 때만 score와 profile 임계를 비교한다. 승인 commit이 평가 중간에 발생해도 한 결과 안에 ruleVersion을 섞지 않는다. `highConfidenceThreshold` 이상도 자동 `TRUE_MATCH`가 아니라 `HIGH` evidence/band가 붙은 `POSSIBLE_MATCH`다. 진성 확정은 §10.4의 분석가 4-eyes 상태머신을 계속 따른다. 결과 JSONB `score_breakdown.appliedPolicy`에는 profile·config/rule version·두 임계·definitionHash·band를 snapshot해 과거 검토를 현재 설정으로 재계산하지 않는다. 외부 응답의 `scoreBreakdown`은 숫자 factor map을 유지하되 감사 스냅샷 중첩 블록(`candidateStrategy`·`nameMatcher`·승격 시 `nameEscalation`)을 병기하며, 정책 snapshot은 top-level `appliedPolicy`로 분리한다. 후보 recall 진단은 별도 `score_breakdown.candidateStrategy`(§10.2b·API §3.2)로 영속한다 — `candidateStrategyVersion`(`wlf-cand-v2`)·`matcherVersion`·`trgmFloor`·`candidateCap`·`candidateCapScope`·`phoneticEnabled`·`candidateCapHit`·`candidateCount`·`strategyCounts`·`axisCounts`·절단 시 `truncatedAxes[]`.
...
#### 10.3b PEP 축 분리 정책 (`PepAxisPolicy`, 정책 축 `wlf-pep-axis-v2`, 2026-08-12 사용자 결정)
...
|---|---|---|
| `CUSTOMER`(회원가입 CDD → 1차 RA) | 국적 또는 생년(年) 중 **1개 이상 코로보레이터**가 함께 일치할 때만 PEP 매치로 인정. 국적은 정확 일치(`countryScore = 1.0`), 생년월일은 `dobCorroboratorFloor`(기본 0.5 = 생년 일치) 이상. 요건 미달 원인이 **불일치**(등재 값과 실제로 다르다)일 때 | `PEP_CORROBORATION_REQUIRED` |
| `CUSTOMER` — 확인 불가 갈래 | 요건 미달 원인이 불일치가 아니라 **확인 불가**(명단·고객 어느 쪽에 값이 없거나 읽히지 않는다)일 때. 귀결은 `customer.unknown-outcome` 노브가 정한다 — **3값**(기본 `SUPPRESS` = 억제 / `MATCH` = 강등하지 않음 / `RISK_SIGNAL` = 억제하되 계량 가능한 신호로 표식, 아래 註) | `SUPPRESS` → `PEP_CORROBORATION_UNKNOWN` · `RISK_SIGNAL` → `PEP_CORROBORATION_UNKNOWN_RISK_SIGNAL` |
| `COUNTERPARTY`(해외송금 수취인) | 수취인은 구조적으로 이름과 송금 국가밖에 없고 그 국가는 등재 인물의 국적이 아니다 → 이름 일치를 **매치가 아니라 위험 신호**로만 기록. 점수·`matchedEntries` 계보는 보존되고 `status` 만 매치가 되지 않으며, 그 신호는 STR·2차 RA 로 **실제 소비**된다(아래 §수취인 PEP 위험 신호의 소비) | `PEP_NAME_RISK_SIGNAL` |

##### 코로보레이터 4상태 (`MatchScore.CorroboratorState`, v2 가 v1 에서 바꾼 것)

v1 은 "국적 점수 1.0 또는 생년 점수 ≥ floor 가 아니면 원인 불문 억제" 였다. 점수 `0.0` 하나에 ① 명단 값 결측 ② 고객 값 결측 ③ 실제 불일치 ④ 파싱 실패가 모두 접혀 있었고, 규제 관점에서 **"다른 사람이다"** 와 **"모른다"** 는 반대 상황인데 같은 `NO_MATCH` 로 뭉개졌다(진짜 PEP 가 조용히 미탐이 되는 경로). v2 는 매처가 코로보레이터별 상태를 함께 산출해 근거에 싣는다.

| 상태 | 뜻 |
|---|---|
| `MATCH` | 양쪽에 값이 있고 일치한다 |
| `MISMATCH` | 양쪽에 값이 있고 다르다 — **다른 사람이라는 적극적 증거** |
| `UNKNOWN_LIST_DATA` | 명단 엔트리에 쓸 수 있는 값이 없다(결측·공백·파싱 불가) |
| `UNKNOWN_TARGET_DATA` | 스크리닝 대상에 쓸 수 있는 값이 없다(결측·공백·파싱 불가) |

파싱 실패는 **값을 낸 쪽의 `UNKNOWN_*`** 으로 접는다 — 읽히지 않는 값은 다름의 증거가 아니다. 정책 하한(`dobCorroboratorFloor`)이 매처보다 엄격해 매처는 `MATCH` 인데 정책이 인정하지 않는 경우는 `MISMATCH` 로 보고한다(모르는 것이 아니라 요건 미달임을 분명히 한다).

CUSTOMER 축 판정 우선순위는 **MATCH > MISMATCH > UNKNOWN** 이다 — ① 요구 개수만큼 `MATCH` 면 정책 무개입(매치 승격), ② 부족하지만 `MISMATCH` 가 하나라도 있으면 `CUSTOMER_CORROBORATION_MISSING`, ③ 부족하고 전부 `UNKNOWN` 이면 **`customer.unknown-outcome` 이 귀결을 고른다** — `SUPPRESS`(기본) → `CUSTOMER_CORROBORATION_UNKNOWN`, `MATCH` → `UNCHANGED`(매치 유지), `RISK_SIGNAL` → `CUSTOMER_CORROBORATION_UNKNOWN_SIGNAL`. ②가 ③보다 앞서는 이유는 불일치가 **있는 증거**이고 결측은 **없는 증거**이기 때문이다(라이브 실측 탈락 4건이 전부 국적 불일치였다). 판정 enum 이름 `CUSTOMER_CORROBORATION_MISSING` 은 v1 값을 그대로 유지한다 — 엔진 케이스 카탈로그 WLF-C19 가 이 문자열을 기대값으로 고정하고 카탈로그는 append-only 다.

##### 확인 불가 귀결 `RISK_SIGNAL`(2026-08-13 신설, 3값화)

`SUPPRESS` 의 문제는 "매치로 안 올린다" 가 아니라 **관측·계량 자체가 사라진다**는 것이었다 — 사유 코드는 갈렸어도 그 행을 하류(2차 상시 RA 가중)가 소비하지 않으니 탐지 결과는 정책 도입 전과 바이트 동일했다. `RISK_SIGNAL` 은 status 를 올리지 않아 오탐 폭증(96.6만 행 PEP 명단의 동명이인을 매치로 승격) 없이 **미탐 방향만** 닫는다: 그 행에 신호 표식을 붙여 계량 대상으로 남긴다. 되돌리기 비용은 `MATCH` 보다 작다.

- **status·차단 계약 무변경** — `RISK_SIGNAL` 귀결도 `status` 는 `NO_MATCH` 이고 `WlfMatchVerdict` 의 억제 경로를 그대로 탄다. 달라지는 것은 **사유 코드**(`PEP_CORROBORATION_UNKNOWN_RISK_SIGNAL`)와 감사 근거의 **표식 2키**(`pepAxis.riskSignal=true` · `pepAxis.signalOrigin="PEP_CORROBORATION_UNKNOWN"`)뿐이다.
- **사유 코드와 origin 은 다른 네임스페이스다** — 사유 코드는 "이 행이 왜 매치가 아닌가", origin 은 "이 신호가 어느 계열인가" 를 말한다. 후자는 하류가 수취인 이름 신호(`PEP_NAME_RISK_SIGNAL`)와 구분해 등록하기 위한 키다.
- **STR 발동 게이트에는 연결하지 않는다** — `PepNameRiskSignal.carries()`/`presentIn()`/`strongest()`(수취인 축 코드만 본다)는 **무변경**이고, 확인 불가 신호는 별도 술어(`carriesUnknownCorroboration`/`unknownCorroborationPresentIn`/`unknownStrongest`)로만 판정한다. 따라서 **STR_PEP 발동 조건은 무변경**이며 STR DRAFT 볼륨이 늘지 않는다.
- **엔진 하류 소비는 아직 없다(사실 그대로 기록)** — `signalScaling` 스키마는 origin 키가 자유롭기 때문에 이 신호를 `PEP_CORROBORATION_UNKNOWN` origin 으로 등록할 **수는** 있으나, 2026-08-13 시점 엔진은 그 origin 의 알럿 계보를 만들지 않으므로 **RA 가중에 실제로 반영되는 경로는 없다**(정책 결정 대기). 현재 이 귀결이 실제로 바꾸는 것은 사유 코드·표식·화면 필터의 해상도까지다.
- **기본값은 `SUPPRESS` 유지** — 억제율을 전 테넌트에서 즉시 뒤집는 것은 라이브 오탐률 실측 없이는 되돌리기 비용이 큰 운영 결정이고, 케이스 카탈로그 WLF-C19 ③ 이 `unknownOutcome:"SUPPRESS"` 를 문자로 고정한다(append-only). 전환은 설정 1개이며 오타는 부팅에서 거부된다.

**정규화(입력 손실 제거)**: 코로보레이터 비교는 값객체로 한다. `BirthDate`(`domain/watchlist`)는 `yyyy` · `yyyy-MM-dd` · `dd/MM/yyyy` · `yyyyMMdd` 를 명시 파싱한다(구분자 `-`·`/`·`.` 허용, 문자열 앞 4자리 슬라이싱 폐기 — 명단 `07/09/1963` 과 고객 `1963-09-07` 이 같은 생년인데 불일치로 판정되던 결함). `CountryIso`(`adapter/out/feed` → `domain/watchlist` 이동)는 ISO2·ISO3·국가명을 정규화하고 **배열 교집합**으로 비교한다. dilisense 파서는 `date_of_birth`·`citizenship`/`jurisdiction` **배열 전체**를 보존한다(첫 값만 남겨 다중국적 일치를 잃던 결함).

> **본 문장이 실제로 참이 된 시점은 2026-08-13 이다(사실 정정 註).** 2026-08-12 등재 당시 파서는 두 필드를 **앞 5개로 잘라** 담고 있었으므로 위 "배열 전체" 서술은 **그 시점에는 거짓**이었고, 같은 날 연동 §7.4 는 반대로 `각 최대 5개` 라고 적어 두 정본이 서로 어긋나 있었다. 절단은 정보 손실에 그치지 않는다 — 코로보레이터를 성립시키는 값이 여섯 번째에 있으면 매처는 남은 5개만 보고 `UNKNOWN_*` 이 아니라 **`MISMATCH`** 로 판정하고, 아래 §10.3b 의 우선순위(일치 > 불일치 > 확인 불가)가 있는 증거를 우선하므로 `PEP_CORROBORATION_REQUIRED` 로 억제한다. 즉 **억제 결과와 억제 사유가 함께 거짓**이 되고, `customer.unknown-outcome=MATCH` 로 바꿔도 구제되지 않는다. 실 피드 census(선두 228,339 레코드 26.5% + delta 27,969 전수 계수)에서 `date_of_birth` 6개 이상 35건 실재(최대 13)·국적 초과 0·증가분 80값/1,120 B·인덱스 영향 0 임을 확인하고 상한을 제거했으며, 대신 길이 20 초과를 진단 `wideMultiValueFields` 로 **계수만 하고 값은 보존**한다(연동 §7.4). 이후 이 문장은 코드와 일치한다. **`wideMultiValueFields` 는 코로보레이터 2필드 전용 계수로 남는다** — 2026-08-13 에 PEP 직위 목록(`functions`)의 상한 5 도 제거했으나 그 광폭 계수는 **별도 키 `wideFunctionFields`** 다(연동 §7.4). 두 신호는 뜻도 조치도 다르다: 코로보레이터 광폭은 벤더 데이터 품질 사고 신호이고 절단되면 **미탐**을 만들지만, `functions` 는 경력이 긴 PEP 라는 뜻일 뿐 매칭 판정을 바꾸지 않으며 실 피드에서 20 초과가 **정상적으로** 나타난다(census 최대 60). 한 키로 합치면 조치가 필요한 미탐 위험 신호가 정상 데이터에 희석돼 읽을 수 없게 된다.

##### 제재 회귀 방지 불변식 — 2층

1. **회수 층(§10.2b-1)**: 후보 cap 을 명단축(`SANCTIONS_LAW`/`PEP`/`OTHER`)별로 적용해 **제재·법집행 후보가 PEP 물량에 잘리지 않게** 한다. v1 의 불변식은 "cap 을 통과한 후보 집합 내부"에서만 참이었고, cap 전단에서 제재가 탈락하면 정책 평가에 도달조차 못 했다(동일 정확 이름 PEP 201건 + SANCTIONS 1건이면 회수 200건이 전부 PEP).
2. **선택 층**: 축 판정은 **후보 선택 이전**에 후보별로 계산한다(`ScoredCandidate.verdict`). 선택 후 강등하면 매치가 될 수 없는 PEP 후보가 실제 제재 후보를 밀어내고 최상위를 차지한 뒤 강등돼 제재 매치가 통째로 사라진다(미탐). 강등 대상 후보는 점수와 무관하게 `WlfMatchVerdict.selectionRank = 0`(최하위 티어)이다.

##### 판정 단일화 — `WlfMatchVerdict` (simulate ↔ 실운영 parity)

밴드·§10.3a 이름 상향·PEP 축 판정을 결정하는 지점은 **순수함수 `WlfMatchVerdict.evaluate` 한 곳뿐**이다. 실운영 `WlfScreeningService` 와 admin `:simulate`(`ScreeningOperationsService`)가 같은 함수를 태우고, `PepAxisPolicy` 는 빈(`WlfPepAxisConfig` 단일 조립)으로 승격돼 두 경로가 **같은 인스턴스**를 주입받는다. 분리 이전에는 시뮬레이션이 정책도 §10.3a 이름 상향도 건너뛴 자체 계산을 해, 운영자가 시뮬레이션에서 본 `REVIEW` 가 실운영에서는 `NO_MATCH` 였다 — 임계 튜닝과 4-eyes 승인이 그 관측을 근거로 이뤄지므로 단순 표시 불일치가 아니라 **정책 결정을 오염시키는 결함**이다. simulate 의 what-if 임계(`similarityThreshold`)는 review 임계 override 로만 쓰이고 상위 밴드(`highConfidenceThreshold`) 판정은 override 대상이 아니다. simulate 응답도 실운영과 **같은 형태의 `pepAxis` 근거 블록**을 싣는다(엔진 `SimulateResult` 레코드 시그니처는 불변 — 기존 free-form `scoreBreakdown` 안의 가산 키).

**회수 옵션 단일화(2026-08-13 — 위 parity 의 두 번째 조립 지점 제거).** 판정 함수를 하나로 합쳐도 **두 경로가 서로 다른 후보 집합을 보면 결과는 여전히 반대로 갈린다.** 실제로 그랬다 — `:simulate` 는 `CandidateOptions.defaults()`(cap 200·trgm floor 0.30·phonetic on·statement timeout 10 s)를 자체 조립했고 실운영은 `aegis.aml.wlf.candidate.{cap,trgm-floor,phonetic-enabled,statement-timeout-ms}` 로 조립했다. 운영 cap 을 500 으로 올린 테넌트에서 §10.2b-1 축 rank 201 의 진성 제재 후보는 실운영 `POSSIBLE_MATCH` / 시뮬레이션 `NO_MATCH` 이고, cap 을 50 으로 낮추면 반대 방향(시뮬레이션만 없는 매치를 낸다)이며 `trgmFloor`·phonetic 토글도 같은 성질이다. 운영자는 이 시뮬레이션으로 보류율을 읽고 임계를 4-eyes 승인하므로 판정 함수 parity 만으로는 §10.3b 의 목적이 달성되지 않는다.

- **설계** — 실운영 유스케이스가 **자기 회수 계약을 공표**하고(`ScreenSubjectUseCase.candidateRecallOptions()`, 구현 `WlfScreeningService`) 시뮬레이션이 그 값을 **읽어 간다**. 조립 지점은 실운영 생성자 한 곳이며, 시뮬레이션은 값을 복제하지 않으므로 두 서비스가 각각 빈을 주입받는 형태와 달리 **배선·한정자 실수로도 갈릴 수 없고** 새 회수 knob 이 생기면 자동 반영된다.
- **회수 knob 은 what-if 입력이 아니다** — 엔진 REST·bo-api BFF·bo-web 시뮬레이션 화면·검증 러너 어느 호출자도 회수 파라미터를 보내지 않으며 what-if 축은 검토 임계(`similarityThreshold`) 하나다. 따라서 명시 입력을 신설하지 않고 운영 옵션을 공유한다.
- **암묵적 기본값 금지** — 대신 **어떤 옵션·절단 상태로 돌았는지를 응답에 싣는다**: `scoreBreakdown.candidateRecall`(API §3.2, 가산 키 1개) 에 `optionsSource="PRODUCTION_SCREENING"`(자체 기본값을 조립하지 않았다는 사실 자체의 기록)·4개 옵션값·`candidateCapScope`·`recalledCount`/`candidateCount`·`candidateCapHit`/`recallComplete`·절단 시 `truncatedAxes[]`. 종전 시뮬레이션은 회수 결과에서 후보 목록만 취해 `capHit` 를 통째로 버렸다 — **잘린 후보 집합 위에서 임계를 튜닝해도 알 수 없었다.**
- **무부수효과 불변** — `@Transactional(readOnly = true)` 유지, 새로 호출하는 것은 필드 반환 게터뿐이며 쓰기 포트(screening 영속·FP 화이트리스트·vault) 미참조.
- **부하 관점 기록** — 시뮬레이션의 회수 비용이 실운영과 같아진다(종전에는 시뮬레이션만 cap 200 으로 싸게 돌았다). cap 상향 결정 시 부하 검토 대상이 **두 경로**가 된다. 시뮬레이션은 admin 저빈도 호출이라 절대 비용은 실운영 스크리닝과 동일 수준이다.

##### `targetType` 신뢰 경계 (`ScreeningAxisResolver`)

`POST /api/v1/aml/screen` 은 body 의 `targetType` 을 도메인 입력으로 쓰고, COUNTERPARTY 로 표시된 PEP 는 코로보레이터와 무관하게 억제된다 — 즉 **회원을 COUNTERPARTY 로 선언하면** CUSTOMER 축의 요구를 우회해 강등을 얻을 수 있다(매핑 오류든 오용이든 결과는 같다). 등록 회원(`aml_customers`)의 키로 COUNTERPARTY 스크리닝이 들어오면 실효 축을 **CUSTOMER 로 되돌린다**. 방향이 언제나 더 엄격한 쪽이므로 이 방어가 오탐이어도 매치를 놓치지 않는다(반대로 강등을 허용하면 미탐이 된다). 요청을 거부하지는 않으며 불일치 사실은 사유 코드 `WLF_TARGET_ROLE_MISMATCH` 와 `pepAxis.declaredAxis`·`pepAxis.axisMismatch` 로 영속 근거에 남는다. 조회는 COUNTERPARTY 선언 + `targetRef` 존재일 때만 수행하고(`aegis.aml.wlf.axis-guard.enabled`, 기본 true), 후보 recall 은 **선언된 축** 기준을 유지한다(실효 축 정정은 정책 판정에만 적용).

##### 수취인 PEP 위험 신호의 소비 (`PepNameRiskSignal`)

수취인 축 강등이 점수를 스크리닝 행에 영속만 하고 아무 데서도 읽히지 않으면 사용자 결정의 "이름으로 위험 점수 가산"이 아니라 소비되지 않는 진단 메타데이터에 그친다. `PepNameRiskSignal` 은 사유 코드 `PEP_NAME_RISK_SIGNAL` **하나만** 보고 그 행을 정확히 골라(상태는 `NO_MATCH` 이고 CUSTOMER 축 억제 코드는 걸리지 않는다) 다음 두 경로로 소비한다.

1. **STR PEP 신호 집계** — `TransactionReportSideEffectRunner` 가 당사자 스크리닝에서 이 신호를 읽어 `pep` 신호를 세운다(KYC PEP 플래그·확정 PEP 매칭과 OR).
2. **STR 계보 + 2차 상시 RA 가산** — `StrEvaluationService` 가 히트 상태 게이트(`POSSIBLE_MATCH`/`TRUE_MATCH`/`ESCALATED`)의 **유일한 예외**로 이 행의 `matchedEntries` 를 계보로 승격하되 `origin=PEP_NAME_RISK_SIGNAL` 로 확정 매칭(`WATCHLIST_MATCH`)과 구분한다(`listType=PEP` 한정 — 제재 계보에는 절대 나타나지 않으며 대표 매칭 선택에서 확정 매칭을 밀어내지 않는다). 발동한 알럿의 가중을 통해 **송금인(회원)** 의 2차 상시 RA `TRANSACTION_BEHAVIOR` 로 가산된다 — 수취인은 CDD·1차 RA·재실사 주기를 갖지 않아 귀속시킬 RA 주체가 없고(2차 RA 행의 `target_ref` 는 언제나 회원 업무참조), "이 회원이 PEP 이름과 일치하는 수취인에게 송금한다"는 사실은 회원의 거래행태 위험이다. 여러 행이 있으면 **최댓값**(가장 강한 이름 일치)으로 대표한다 — 합산하면 같은 인물을 두 번 세는 착시가 된다.

> **그 가중은 2026-08-13 부터 `ruleSeverityWeights` 단독이 아니다(사실 정정).** 위 2번의 "기존 가중(`ruleSeverityWeights`)을 통해" 는 등재 당시에는 참이었으나, **신호 점수 비례 스케일링**(§11.3a)이 도입되면서 기여도에 **세 번째 항 `× m(nameScore) ≤ 1.0`** 이 곱해진다. 방향은 **깎기**다 — 배수 상한이 `1.0` 이라 어떤 입력에서도 현행보다 커지지 않으며, 강한 이름 일치(이름 점수 ≥ 상한)는 종전과 **같은** 가중을 받고 약한 일치만 점수에 비례해 감산된다. 이 항은 **테넌트 정본(`aml_risk_models.parameters.signalScaling`)이 그 origin 을 등록했을 때만** 1.0 이 아니므로, 키가 없는 테넌트의 동작은 위 서술 그대로다.
>
> 또한 이 스케일링이 소비하는 점수는 계보의 `matchScore`(= 합성 `overall`)가 아니라 **이름 하위점수 `nameScore`** 다 — 스크리닝 `score_breakdown.name` 을 계보 evidence 에 **additive** 로 운반한 값이며 `matchScore` 는 **무변경 보존**한다. 수취인 축의 `overall` 은 이름+국가+전화만 동봉해 실측 상한이 0.65 근방이라 "비례" 를 표현하지 못하지만, 이름 하위점수는 정확일치에서 1.0 까지 도달한다. 운반 부재·파싱 실패·범위 밖 값은 전부 배수 **1.0**(감산 0 = 안전 방향)이다.

**차단은 발생하지 않는다** — `sanctionHit`(제재 매칭)은 이 신호를 절대 읽지 않으므로 `RESTRICT`·HOLD 사유가 되지 않고, 스크리닝 `status` 도 `NO_MATCH` 그대로다. 제재는 여전히 유일한 차단 경로다.

**설명가능성**: 억제된 결과에만 `scoreBreakdown.pepAxis` 감사 블록이 붙고 사유코드가 발급된다 — `applied`·`policyVersion`·`axis`·(역할 불일치 시에만) `declaredAxis`·`axisMismatch`·`listType`·`decision`·`suppressedStatus`·`suppressedBand`·`riskScore`·`entryId`·`reviewThreshold`, CUSTOMER 축 **세 갈래**(`CUSTOMER_CORROBORATION_MISSING`·`CUSTOMER_CORROBORATION_UNKNOWN`·**`CUSTOMER_CORROBORATION_UNKNOWN_SIGNAL`**)는 `matchedCorroborators`·`acceptedCorroborators`·`requiredCorroborators`·`corroboration`(코로보레이터별 4상태 맵 `{COUNTRY, DOB}`)·`unknownOutcome` 추가, **`CUSTOMER_CORROBORATION_UNKNOWN_SIGNAL` 만** 표식 2키(`riskSignal=true`·`signalOrigin="PEP_CORROBORATION_UNKNOWN"`)를 더 싣는다(다른 판정의 근거 블록은 바이트 동일). 세 갈래를 한 술어(`Decision.involvesCorroboration()`)로 판정해 판정이 늘어도 근거 블록 조건을 한 곳에서만 고친다. 억제되지 않은 결과에는 블록이 없어 응답이 바이트 동일하다. 정책값은 전부 `aegis.aml.wlf.pep-axis.*` 설정으로 노출되며 기본값이 정책 정본이고, `enabled=false` 하나로 이전 동작에 완전히 복귀한다. **충족 불가능한 설정**(요구 코로보레이터 수 > 인정 코로보레이터 수)은 모든 PEP 를 조용히 미탐으로 만들므로 **부팅 시점에 거부**하며, `customer.unknown-outcome` 의 오타·미지원 값도 조용히 기본값으로 흘리지 않고 부팅에서 거부한다.

| 설정 키 | 기본 | 뜻 |
|---|---|---|
| `aegis.aml.wlf.pep-axis.enabled` | `true` | `false` = 정책 도입 이전 동작 완전 복귀 |
| `aegis.aml.wlf.pep-axis.customer.require-corroborator` | `true` | 회원 축 추가 식별자 요구 |
| `aegis.aml.wlf.pep-axis.customer.minimum-corroborators` | `1` | 요구 개수(인정 개수 초과 시 부팅 거부) |
| `aegis.aml.wlf.pep-axis.customer.accept-country` · `...accept-dob` | `true` · `true` | 인정 코로보레이터 |
| `aegis.aml.wlf.pep-axis.customer.dob-floor` | `0.5` | 생년(年) 일치까지 인정하는 하한 |
| `aegis.aml.wlf.pep-axis.customer.unknown-outcome` | `SUPPRESS` | 전부 확인 불가일 때의 귀결(`SUPPRESS`\|`MATCH`\|**`RISK_SIGNAL`**) |
| `aegis.aml.wlf.pep-axis.counterparty.risk-signal-only` | `true` | 수취인 축 이름 일치를 위험 신호로만 기록 |
| `aegis.aml.wlf.axis-guard.enabled` | `true` | `targetType` 신뢰 경계 |

**BO 노출**: 강등 건은 `status=NO_MATCH` 라 기본 검토 큐(`POSSIBLE_MATCH`)에서 빠지므로, bo-api 가 `pepAxis` 근거를 큐·상세·`:simulate` 응답에 실어 내리고 bo-web 이 배지·근거 카드·이력 탭 필터로 도달 가능하게 한다(기능정의서 §3.1·§3.3·§12-B.1).

**유효 정책 자체의 노출(2026-08-13 additive)**: 위 건별 근거는 **후보가 있었던** 스크리닝 행에만 붙으므로, 운영자가 "이 테넌트의 확인 불가 귀결이 지금 무엇인가" 를 알려면 억제된 건을 하나 찾아 열어야 했다. `GET /api/v1/admin/aml/wlf-engine-config` 응답에 **읽기 전용 `pepAxis` 유효정책 블록**(`enabled`·`policyVersion`·`unknownOutcome`·`customerRequiresCorroborator`·`customerMinimumCorroborators`·`acceptedCorroborators`·`dobCorroboratorFloor`·`counterpartyRiskSignalOnly`)을 실어 bo-api 가 필드 손실 없이 통과시키고 bo-web `/aml/wlf-engine` 화면 상단이 배지로 표기한다 — 임계 튜닝과 축 정책을 같은 화면에서 대조하기 위해서다. 이 값은 정책팩(4-eyes 승인 대상)이 아니라 런타임 설정(`aegis.aml.wlf.pep-axis.*`)이라 **표시 전용**이며, **테넌트별 4-eyes 편집 승격은 범위 밖**이다(policy-pack 스키마 변경 = 전 테넌트 재승인).

**실측(2026-08-12 라이브, v1 도입 시점 측정)**: 데모 회원 `POSSIBLE_MATCH` 6/9(66.7%) → 2/9(22.2%), 제재 회귀 0.

> **이 수치는 2026-08-13 이후 그대로 재현되지 않는다(데모 모집단 변경 註 — 엔진 회귀 아님).** 데모 회원 `M-1005` 의 합성 이름이 실 dilisense 명단 인물과 **이름 정확일치 + 국적(PH) 코로보레이터 충족**이라 축 정책이 개입하지 못하고 `POSSIBLE_MATCH`(0.65)로 확정됐고, 그 결과 해외송금 라이브 폐루프 게이트(2-leg 전건 `ALLOW` 요구)가 **엔진 결함이 아닌 이유로** 깨졌다. 2026-08-13 사용자 지시로 그 회원 이름을 **어느 명단에도 없는 합성 3토큰 이름**으로 교체했다(라이브 실측: 전 ACTIVE 소스 대상 `NO_MATCH`·최상위 점수 0.2241 < 검토 임계 0.65·`NAME_EXACT_MATCH` 부재. 게이트와 **동형 페이로드** — CUSTOMER 축·이름 토큰·**생년 동봉**·국가 — 로 질의). 이름 **외** 필드(국적·거주국·생년·성별·직업·자금원·KYC 등급)는 무변경이며 게이트 기대값은 한 줄도 완화하지 않았다. 부수 효과로 그 회원의 명단 floor → 당연고위험 자동상신 계보가 사라진다 — 데모가 **우연한 실명 충돌에 의존하고 있었다는 사실의 해소**이지 탐지 후퇴가 아니다.
...
**② 적용본 존재 게이트(`WatchlistReadinessGateAdapter`, DB §3.6a)**: tenant(+jurisdiction)의 각 활성 필수 PROD 정책은 일치 source의 `activeVersion`이 있어야 하며, NOT_APPLICABLE은 유효 waiver가 필요하다. 정책이 없으면 적용본 source ≥1건이면 통과하고 0건이면 fail-closed다. 적용본 부재만 503 `AML.SCREENING_UNAVAILABLE`; STALE/FAILED(IMPORTING 제외) 적용본은 200 실제 판정이다. 필수 정책 seed는 REST-only다.
...
**재스크린 워커 인프라 타임아웃 서킷·일시정지(2026-09-07, aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit)** — 후보 조회 `CandidateRecallTimeoutException`·DB 인프라 계열(SQLState 57/53/08, `QueryTimeoutException`·`CannotAcquireLockException`·`CannotCreateTransactionException` 등)은 주체 실패가 아니므로 `RescreenWorker` 가 **attempt 를 소모하지 않고** 타깃을 `RETRYING`(+쿨다운) 으로 되돌린다. 인프라 실패가 연속 `failure-threshold`(기본 3, 틱 경계와 무관하게 누적 — 성공·비인프라 결과에서만 리셋)이면 남은 claim 타깃을 해제하고 워커 서킷을 OPEN(쿨다운 기본 300s, half-open 실패 시 2배·최대 1800s) — OPEN 동안 claim 없음, half-open 은 1건만 claim, 성공 시 CLOSED. 틱 처리 시간이 `tick-budget-ms`(기본 60,000)를 넘으면 남은 타깃을 `tick-budget-release-delay-ms`(기본 5,000) 뒤 재시도로 해제 후 종료(서킷 open 해제는 쿨다운 적용). half-open 프로브는 **성공·NOT_APPLICABLE·비인프라 실패·claim 0(유휴)** 모두 CLOSED 로 복귀하고(poison subject 로 half-open 고착 방지) 인프라 실패만 재open(쿨다운 2배). 주의: 잡을 pause 해 half-open claim 이 0 이 되면 유휴 프로브로 CLOSED 되어 지수 쿨다운이 base 로 리셋된다(resume 후 재트립 시 다시 3연속 기준). `ScreeningUnavailableException`(readiness 게이트)은 제외(기존 attempt 소모). 상태 전이는 구조화 INFO 로그가 정본. **서킷 상태는 인스턴스(프로세스) 메모리 로컬**이라 다중 레플리카 배포에서는 인스턴스별로 독립적으로 열리고 닫힌다(클러스터 전역 상태 아님 — 각 인스턴스가 같은 DB 포화를 관측하면 각자 open). 운영자는 `POST /api/v1/admin/aml/watchlist-rescreen-jobs/{jobId}:pause|:resume`(`aml:admin:watchlist`, 감사 1건)로 잡을 일시정지할 수 있고, `paused_at IS NOT NULL` 잡의 타깃은 claim 에서 제외된다(V76). 프로퍼티 `aegis.aml.rescreen.circuit.*`.
...
| `factorBreakdown` | factor별 점수·근거. 요인 외에 2차(ONGOING) 계보 마커가 같은 맵에 실린다(`scenario`·`triggerAlerts`·`reviewShortened`·`baselineModelCode`·**`signalScaling`**(§11.3a) 등) — 이들은 기여 요인이 아니므로 BO 는 예약키로 요인 목록에서 배제하고 전용 필드로 파생한다 |
...
### 11.3a 신호 점수 비례 스케일링 (`signalScaling`, 2026-08-13 — ONGOING 선택키)

2차 상시 RA 의 알럿 기여도는 종전 `weightFor(ruleCode) × recencyMultiplier(ageDays)` 였고 그 집계는 **알럿 간 `max`** 다. 이 산식은 **이름만 닮은 신호**와 **확정 매칭**을 같은 `STR_PEP` 룰코드·같은 `ruleSeverityWeights` 로 취급하므로, 이름 점수 0.65 인 건과 0.99 인 건이 **똑같은 만점 가중**을 받았다. 스케일링은 그 계단을 없애는 세 번째 항이다.

```
contribution = weightFor(ruleCode) × recencyMultiplier(ageDays) × m(signalScore)
LINEAR : m(s) = min + (max - min) × clamp01( (s - floor) / (ceiling - floor) )
STEP   : m(s) = max                                    // 계단(현행) 동작 — 되돌리기 경로
```

- **방향은 「깎기」다(사용자 결정 2026-08-13)**. 배수는 `(0, 1.0]` 이고 상한 `1.0` 은 §10.3b 의 구조 불변식(이름 유사도 신호가 확정 매칭보다 무거워질 수 없다)이 요구하는 값이라 완화하지 않는다. 따라서 **어떤 입력에서도 기존 가중을 초과하지 않는다** — 강한 일치는 종전과 동일, 약한 일치만 감산된다. 성격은 가중 상향이 아니라 **오탐 억제**다.
- **스케일 입력은 이름 하위점수(`nameScore`)** — 스크리닝 `score_breakdown.name` 을 알럿 evidence 계보에 additive 로 운반한 값이며 `matchScore`(= 합성 `overall`)는 무변경 보존한다. 점수 부재·비유한·범위 밖은 배수 **1.0**.
- **대상은 계보 origin + 명시 룰 allowlist 둘 다 맞을 때뿐이다.** 대표 계보의 `origin` 이 스케일링 정의에 등록된 값이고(현재 운영값은 `PEP_NAME_RISK_SIGNAL`), 그 정의의 `rules` 가 발동 룰코드를 담을 때만 배수가 1.0 이 아니다. **확정 매칭(`WATCHLIST_MATCH`)·KYC 플래그(`KYC_PEP_FLAG`) 대표 알럿은 언제나 배수 1.0** — 그 알럿의 발동 원인은 회원의 확정 PEP 이므로 동행한 수취인 약한 이름 신호로 감산하면 확정 PEP 고객의 2차 RA 가 무관한 신호로 내려가는 **미탐**이 된다. 중첩 `additionalMatches[]` 순회는 대표가 신호일 때만 하고 복수면 **최대 점수 1건**(감산이 가장 작은 쪽 = 안전 방향)을 쓴다.
  > **그래서 대표 선택 규칙 자체가 이 게이트의 전제다(`WatchlistMatch.combine`, 도메인).** 회원 근거가 **확정**일 때만 대표를 회원으로 두고 수취인 매칭을 `additionalMatches` 로 밀며, 확정의 정의는 **실계보(`isLineage()`) 이거나 회원 자신의 확정 플래그**(STR_PEP 한정 — CDD KYC PEP)다. 종전에는 `isLineage()` 만 보아 **KYC 확정 PEP 회원인데 워치리스트 계보가 없으면** 확정 KYC 블록이 버려지고 수취인 이름 신호가 대표가 됐고, 그 evidence 루트 origin 이 `PEP_NAME_RISK_SIGNAL` 이라 게이트를 통과해 **확정 PEP 고객의 기여도가 감산**됐다(미탐). 제재 룰(`STR_SANCTION`)에는 이런 확정 플래그 축이 없으므로(확정 = 스크리닝 매칭뿐) 기존 동작 그대로다. 이 판정을 도메인에 둔 이유는 게이트 테스트가 **이 함수가 실제로 만드는 evidence** 로만 단언하게 하기 위해서다 — 손으로 지어낸 evidence 조합에 대한 단언은 프로덕션이 만들지 않는 형태를 지키는 공허한 단언이 된다.
- **곱은 `max` 집계 안쪽에 있다** — 다른 룰의 알럿이 최대 기여를 점유하면 배수를 바꿔도 RA 점수가 움직이지 않는다. 근거 블록의 `severityDriver` 가 그 사실의 기계 판정이다.
- **선택키 + 항등원 폴백** — `parameters.signalScaling` 이 없으면 배수는 **곱셈의 항등원 1.0** 이고 현행 동작과 바이트 동일하다. 이 상수는 튜닝값이 아니라 "스케일링이 설정되지 않았다" 의 정의이므로 「엔진에 숫자 상수 0개」 규약을 위반하지 않는다(0.25·0.65 같은 임의 기본값은 심지 않는다). **엔진 기본값은 현행 유지이고 비례는 4-eyes 파라미터로만 켠다** — 엔진 기본값을 비례로 바꾸면 아무도 승인하지 않은 가중 변경이 전 테넌트에 즉시 적용된다.
- **제재는 대상이 아니다(구조 강제)** — `rules` 에 `STR_SANCTION` 이 있으면 파싱·저장 검증이 **거부**한다. 차단(HOLD/BLOCK) 계약은 무접촉이다.
- **`floorScore`·`ceilingScore` 는 새로 지어낸 상수가 아니라 WLF 문턱 승계다(임의 상수 금지).** 데모·검증 스택의 운영값 `0.65`/`0.92` 는 이 테넌트가 이미 4-eyes 로 운영 중인 정책팩 문턱과 같은 값이다 — `floorScore` = `wlf.possible-threshold`(그 아래는 WLF 가 검토 후보로도 올리지 않는 자동 할인 대역 → **최대 감산**), `ceilingScore` = `wlf.true-threshold`(= `MatchRuleSet` 의 `highConfidenceThreshold`. §10.3a 의 이름 강한 일치 승격이 **이름 하위점수를 바로 이 문턱과 비교**하므로 그 이상은 WLF 자신이 고신뢰로 보는 대역 → **현행 가중 m=1.0**). 즉 배수 곡선의 두 끝이 WLF 판정 밴드와 정렬되고, 운영 테넌트가 문턱을 튜닝하면 이 대역도 같은 값으로 옮긴다. 같은 값을 화면 시드도 쓴다(정본 1개). 반면 `minMultiplier`(운영값 0.25 = 감산 상한 75%)는 **정책 선택**이다 — 0 이면 알럿이 발동했는데 RA 기여가 사라져 조사자가 근거를 못 보고, 1 이면 항등원이라 기능이 무의미하다. 라이브 이름 점수 분포가 쌓이면 4-eyes 로 재튜닝할 값이다.
- **불변식(fail-fast)**: `0 ≤ floorScore < ceilingScore ≤ 1` · `0 < minMultiplier ≤ maxMultiplier ≤ 1` · `rules` 비어있지 않고 중복 없음 · 룰코드 `(STR|CTR|FDS)_.+` · `STR_SANCTION` 금지 · `curve ∈ {LINEAR, STEP}` · origin 키 `[A-Z][A-Z0-9_]{0,63}`. 읽기(`OngoingRaParameters.from`)와 **쓰기**(`RiskModelDefinitionValidator` 허용키·하위키) 두 경로 모두에 걸린다 — 허용키에 넣지 않으면 4-eyes draft 저장 자체가 400 이라 "관리 메뉴에서 고친 점수가 엔진에 반영된다" 가 물리적으로 불가능하다.
- **근거·설명책임**: `factorBreakdown.signalScaling` 에 `{origin, alertId, ruleCode, rawScore?, multiplier, curve, floorScore, ceilingScore, minMultiplier, maxMultiplier, modelVersion, severityDriver}` 를 남기고(대표 = 스케일 대상 알럿 중 기여도 최대 1건), `TRANSACTION_BEHAVIOR` reason 문자열에도 배수가 실제로 감산했을 때만 "이름위험 배수 0.48" 을 덧붙인다(배수 1.0 이면 사유 문자열이 현행과 동일). raw PII 없음.
- **4-eyes 시뮬레이션 parity** — 시뮬레이션 표본 로더도 알럿 evidence 를 함께 읽어 **엔진과 같은 역파싱 계약**으로 표식을 복원한다. 이게 없으면 `signalScaling` 만 바꾼 초안의 시뮬레이션이 언제나 shift 0 으로 보여 checker 가 "영향 없음" 으로 승인한다.
- **되돌리기 3중**: ⓐ `curve: "STEP"`(한 값) ⓑ `signalScaling` 블록 삭제 후 모델 재활성화 ⓒ `aegis.aml.wlf.pep-axis.counterparty.risk-signal-only=false`(신호 자체를 끔).
- **Flyway 신규 0건** — 기존 `aml_risk_models.parameters` JSONB 안의 선택키다(DB §3.9).

---

## 12. Transaction Monitoring 모델

### 12.1 TM scenario (**V61 이후 자유형 String — 닫힌 enum `TmScenario` 폐기**)

`TmScenario` 는 **2026-08-01(V61, refactor/remove-legacy-tm-scenarios)로 완전 삭제**됐다 — v9.21 확정 사실(TM 알림 발동 정본은 CTR/STR 룰 카탈로그뿐, 거래 인입 경로에서 시나리오 정의가 전혀 발동하지 않음, F-025 실측)에 따라 `scenarioCode` 는 String 으로 전환했고, `aml_tm_scenarios` 의 레거시 정의 10종(닫힌 enum 시절 값, 실측 13행 — `STRUCTURING` v1/v2·`HIGH_RISK_CORRIDOR` v1/v2/v3 포함)도 DELETE 됐다(DB §5.6/§7 V61). 코드 형식 불변식은 `^[A-Z][A-Z0-9_]{2,64}$`(`TmScenarioDefinition` 생성자 검증)이며, 예약 코드 `CUSTOM_RULE` 은 `TmScenarioService.draft` 가 거부한다(설정형 STR/CTR 룰 결재 subjectRef prefix `ManageConfigurableReportRuleUseCase.APPROVAL_PREFIX="CUSTOM_RULE|"` 와의 오라우팅 방지 — `ApprovalDispatchService` 가 subjectRef 의 prefix 일치로 승인 라우팅을 분기하기 때문). 시나리오 정의는 여전히 tenant별 `aml_tm_scenarios`에 version·DSL·threshold로 저장되고 `TmScenarioDslParser`(`and`/`or`/`cmp`/`velocity`)·`TmEvaluationEngine`이 평가하지만, **신선 배포·V61 적용 후 정상 상태는 0행**이며 저작 경로(`TmScenarioAdminController` REST `draft`/`simulate`/`:activate`)로 자유형 코드를 신규 등록할 수 있다(**신규 저작 시나리오도 거래 인입 경로에서 자동 발동하지 않는다** — 발동 정본은 여전히 CTR/STR 룰 카탈로그뿐). 송금/월렛 reference scenario의 과거 seed(V19/V22/V26/V28) 는 역사적 기록이며 V45 production `SUPERSEDED` 격리·`demo` profile repeatable 복원 메커니즘은 무변경이나, V61 DELETE 후 해당 seed 코드는 더 이상 재활성화 대상이 없다(no-op).

> **레거시 6종 ACTIVE 시드(과거 사실, 제거됨) — 참고용.** 과거 hanpass-ph 데모(`tenant_demo`) ACTIVE 는 `STRUCTURING`(분할 거래, 24h 건수 ≥ 5)·`HIGH_RISK_CORRIDOR`(해외송금 단건 고액, `phpEquivalent ≥ 280000`)·`RAPID_MOVEMENT`(입금 후 즉시 이동, 2h 건수 ≥ 3 ∧ `phpEquivalent ≥ 56000`)·`REFUND_LAUNDERING`(반복 환류, 7d 건수 ≥ 6 ∧ `phpEquivalent ≥ 28000`)·`ROUND_TRIPPING`(순환/자기거래, 14d 건수 ≥ 4 ∧ `phpEquivalent ≥ 112000`)·`MULE_NETWORK`(다수 sender → 동일 수취인/계좌) 6종이었고, `SHELL_MERCHANT`(허위 가맹점)·`TRADE_MISPRICING`(인보이스 단가·수량 이상)·`CRYPTO_OFF_RAMP`(가상자산 현금화)·`INTERNAL_OVERRIDE_ABUSE`(내부자 승인 남용) 4종은 advanced-domain 비-hanpass 잔존이었다. **`phpEquivalent` 금액 feature 파생 로직 자체(`TmEvaluationService.buildSnapshot`, fds-svc `FeatureComputeAdapter.readPhpEquivalent` 대칭)는 무변경**이며 신규 저작 시나리오도 계속 이 feature 를 소비할 수 있다.
...
| Webhook (outbound) | OUT | screening/case/report 상태변경 콜백(서명·재시도/멱등) + egress SSRF policy(HTTPS-only prod·내부대역 차단·no-redirect, P0-17 — URL 원천=`aml_api_credentials.webhook_url`, **매 전송 직전 재검증만**·위반=delivery 실패 backoff. **(2026-08-10 정정)** 종전 근거 "런타임 등록 경로 없음"은 admin REST `GET\|PUT /api/v1/admin/aml/webhook-credential`(API §2.7a — 목적지·서명 시크릿 upsert, 쓰기 `aml:admin:policy`) 신설로 **더 이상 사실이 아니며**, 등재 시점 SSRF 검증은 **의도적으로 미도입**(전송 직전 재검증 계약 유지)) | `POST {customerWebhookUrl}` (계약=API §8·연동 §3.4) |
...
**신호 소비 단계 재무장(2026-08-13).** 위 durable retry 는 각 step 을 **독립적으로** 되살리는데, step 들이 서로 독립이 아닌 축이 하나 있다 — **WLF 스크리닝 결과를 읽어서 평가하는 하위 단계**다. 인입은 WLF → TM/CTR/STR/RA 순서로 실행하지만 WLF 실패가 형제 단계 진행을 막지 않으므로, **WLF 만 실패한 인입의 STR·2차 상시 RA 는 스크리닝 신호 없이 `SUCCEEDED` 로 끝난다.** 워커가 WLF step 만 claim 해 재시도하면 스크리닝 행은 뒤늦게 생기지만 이미 `SUCCEEDED` 인 하위 단계를 깨우는 경로가 없어 **그 위험 신호는 영원히 소비되지 않는다** — 수취인 PEP 이름 위험 신호(§10.3b)는 STR·2차 RA 에 도달하지 못하고, 더 위험하게는 송금인 제재 매칭이 `STR_SANCTION` 을 영영 발동시키지 못한다. 발생 조건은 장애가 아니라 **readiness 가 잠시 `STALE` 이었다가 복구되는 정상 경로**다.

- **선언은 도메인** — `NeutralFanoutSteps.signalConsumersOf(producer)` 가 producer `{SENDER_WLF, RECEIVER_WLF}` → consumer `{STR, ONGOING_RA}` 를 순수 도메인으로 선언한다. `STR` 은 양당사자 스크리닝 행을 읽는 **직접** 소비자이고, `ONGOING_RA` 는 그 STR 알럿 가중을 소비한다 — RA 를 명시 재무장하지 않으면 STR 재실행 안에서 RA 가 실패해도 `ONGOING_RA` step 이 **거짓 `SUCCEEDED`** 로 남아 본 절이 없애려던 은폐가 되살아난다. `CTR` 은 현금 채널·임계·영업일만 읽어 대상이 아니고, `TM` 은 step 계약("파이프라인 완료")이 이미 충족돼 있으며 두 consumer 의 재실행이 그 파이프라인을 다시 돌린다. `PII_VAULT` 는 하위가 아니라 WLF 의 **선행 입력**이라 vault 지연 성공 → WLF 재시도 성공 → 재무장으로 체인이 이미 이어진다.
- **전이는 `SUCCEEDED` 행만** — `FanoutJobStorePort.rearmSucceededSteps` 가 `SUCCEEDED → RETRYING`(attempt 0·due now·`last_error` NULL) 단일 전이를 수행한다. `RETRYING`/`IN_PROGRESS` 제외는 lease·attempt 상태 기계를 흔들지 않기 위함이며 같은 sweep 의 두 번째 producer 가 재무장을 중복시키지 않는 **자연 dedupe** 이다. `DEAD_LETTERED` 제외는 재시도 예산을 소진한 운영 결정을 신호 하나로 조용히 되살리지 않기 위함(부활은 replay 의 몫), `PENDING`/`NOT_APPLICABLE` 제외는 인라인 진행 중·비적용 보존이다.
- **발동은 워커 경로에만** — `FanoutStepExecutor.retry` 성공 직후. 인입 인라인 경로에는 두지 않는다(그 시점의 하위 단계는 아직 `PENDING` 이고 곧 신호를 보고 평가되므로 재무장 대상 자체가 없다).
- **멱등·순환 불가** — 재평가는 기존 자연키를 다시 만난다(TM 알럿 `(tenant, txn, scenario)`·보고 `(tenant, trigger)`·2차 RA 디바운스는 창 내 + 발동 계보 동일이면 skip) → **신호가 실제로 바뀐 경우에만** 새 알럿·점수가 생긴다. producer·consumer 집합이 서로소이고 consumer 는 아무 신호도 생산하지 않으므로 재무장 그래프는 **깊이 1 의 DAG** 이며 재시도 횟수·동시성과 무관하게 순환이 불가능하다.
- **계약 무변경** — claim SQL·backoff·`MAX_ATTEMPTS`·`DEAD_LETTERED`·replay 재개 계약 무변경, `SUCCEEDED → RETRYING` 은 V48 `status` CHECK(6종)가 이미 허용하는 값이라 **Flyway 신규 0**. 재평가마다 `MONITORED_TRANSACTIONS` 계상이 1건 늘어나는 성질은 본 절의 기존 성질이며 발생 조건이 "WLF 지연 성공"으로 넓어진 것이다(과금 정합은 계상 자체를 거래 참조 멱등으로 바꾸는 별건).
...
- **provider boundary(§5.4·연동 §14.1a, feature/aml-reports-amlc-migration §1.4-C 로 일부 대체)**: `ReportSubmissionPort`(FIU acked/failed outcome)는 여전히 aml-svc 가 기관 HTTP 를 **직접 소유하지 않고** ProviderSvc 로 위임하는 seam 이다(무변경). **`AmlcSubmissionPort`(AMLC 포털 lodgement receipt)는 더 이상 ProviderSvc 위임이 아니다** — 실 구현체 `PlaywrightAmlcSubmissionAdapter`가 테넌트별 저장 계정(신규 `aml_amlc_credentials`, DB §3.12b V57, `AmlcCredentialPort`)으로 aml-svc 프로세스 내부에서 브라우저 자동화(Playwright for Java, 신규 의존성 — §6.2 어댑터 레이아웃 콜아웃)를 통해 AMLC 포털에 **직접** 로그인·업로드·접수번호 파싱을 수행한다(구 "ProviderSvc AMLC adapter 위임" 서술 대체). non-prod/dev(`aegis.aml.report.submission.amlc.mode=mock`)는 `MockAmlcSubmissionAdapter` 가 결정적 receipt 를 유지하고, `ReportSubmissionPort`(FIU 아웃컴) 데모는 여전히 `MockRegulatorSubmissionAdapter` 로 submit→ACK/REJECT→reconcile 폐루프를 실 sandbox/실 포털 없이 검증한다(실 FIU sandbox 접속은 phase-2 BLOCKED, 실 AMLC 포털 셀렉터/DOM 대조는 go-live 전 별도 필요 — prod 셀렉터/도메인은 placeholder, §1.6-G).
...
-- P0-16 마이그레이션(V53): jurisdiction/base_currency/reporting_currency/timezone/policy_pack_version nullable additive 추가. 활성 binding의 timezone은 유효 IANA region ID 필수이며 legacy null/invalid/fixed-offset는 422 fail-closed.
...
CREATE TABLE aml_wlf_rescreen_jobs (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  paused_at TIMESTAMPTZ,                      -- V76 운영자 일시정지(claim 제외)
  paused_by VARCHAR(128),                     -- V76
...
- **WLF 스크리닝 NAME 한정 카브아웃(2026-08-22, 코드=truth)**: 위 P0-09 원칙의 예외는 `GET /aml/screenings/{id}`·`GET /admin/aml/screenings` 두 WLF 읽기 표면뿐이다. `ScreeningIdentityProjectionService`가 application 계층에서 `PiiVaultPort.findCiphertexts` 500건 단위 배치 조회와 `SecretCipherPort` 복호화를 수행해 `requestName`과 `matchedEntryNames`를 투영한다. `requestName`은 결과 insert 성공 뒤 `WLFREQ-{screeningId}` vault NAME으로 write-once 저장한 `nameTokens` 순서 보존 결합값이고, 과거 결과·누락 vault·개별 복호화 실패는 값만 생략한다. `aml:case:read` 보유 호출자만 이 두 NAME 값을 받고 `aml:screen:evaluate` 단독은 키 자체를 받지 않는다. bo-api는 값을 재계산·마스킹하지 않고 통과하며 이름이 실제 반환된 목록/상세 요청당 `RAW_DATA_ACCESS` 1건을 기록한다. 알림·근거거래·Subject360·evidence와 DOC/ACCOUNT/WALLET/NATIONALITY/GENDER/DOB reveal 계약은 불변이다.
...
> `domain/commerce`·관련 EventFamily/CaseType enum·DB 컬럼에 잔존하나 **hanpass-ph 운영 대상이 아니다**(§4·§8·§12). 송금/월렛 외 금융 도메인 확장 시 활성화하는 surface. (`domain/travelrule`·Travel Rule EventFamily/CaseType/ReportType·`aml_travel_rule_transfers`는 2026-07-09 Travel Rule 전면 제거로 삭제됨 — aegis-aml 84997e1, aml V31.) **2026-08-01(V61, refactor/remove-legacy-tm-scenarios) — `AdvancedDomainEvaluationService` 의 시나리오 평가 limb 제거(`TmScenario` enum 삭제와 lockstep)**: `AdvancedDomainPack.tmScenario`/`caseType` 필드·`openReviewCase` 메서드가 함께 제거되어, 본 pack 을 통한 검토 케이스(`TBML_REVIEW`·`B2B_INVOICE_REVIEW`·`ECOMMERCE_SETTLEMENT_REVIEW`·`INTERNAL_CONTROL_REVIEW` 등)의 **자동 개설 계약이 소멸**했다(EventFamily strict 게이트로 실인입 0 = 이미 휴면이었던 경로, hanpass-ph 비대상과 정합). **EDD 트리거 시그널 평가(`eddTrigger`)는 존치** — `AdvancedDomainEvaluationService` 는 시그널 판정만 수행하고 `TmScenarioStorePort`/`TmEvaluationEngine`/`CaseRepositoryPort` 의존은 이 클래스에서 제거됐다(`cddWorkflow` 의존은 유지). `CaseType` enum 값 자체는 무변경(기존 케이스 데이터 보존).
...
| 일자 | 변경 | 비고 |
|---|---|---|
| 2026-09-11 | **§24 항목 7 개정 — 당연고위험(HRR) 레지스트리 floor 조회 포트 단일화 `findForcedFloor`·도메인 정적 팩토리 `forcedFloorFrom`(코드=truth, 마이그레이션 없음, aegis-aml PLAN 20260911-aml-hrr-floor-consolidation, 사용자 지시로 F-115·F-116 해제).** 구 default 술어 `isKnownUnregistered`(exists 부재판정 fast path) + `find` 2단 조회를 포트 추상 단일 메서드 `findForcedFloor(tenantId, subjectRef)` 로 통합 — 어댑터는 `findByTenantIdAndSubjectRef`(인덱스 `ix_hrr_items_subject`, 비잠금) 로 아이템만 읽어 도메인 정적 `HighRiskRegistry.forcedFloorFrom(subjectRef, items)` 로 조립하고, 기존 인스턴스 `forcedFloorFor` 도 같은 정적 팩토리에 위임해 조립 규칙을 단일 정본화. 룰/판정 변경 없음(성능·구조 정리만). | system-architect. 코드 truth=`application/port/out/HighRiskRegistryStorePort#findForcedFloor`·`domain/registry/HighRiskRegistry#forcedFloorFrom`·`adapter/out/persistence/{HighRiskRegistryJpaAdapter,HighRiskRegistryReferenceItemJpaRepository}`·`application/usecase/OnboardingRaDerivationService`. DB §3.9·§3.20·PRD §12-A.4 BR-004 ① 동일 작업 단위, API §3.3 무변경. |
| 2026-09-10 | **§24 항목 7 신설 — 당연고위험(HRR) 레지스트리 조회 fast path·강제 floor 조립 단일 정본(코드=truth, 마이그레이션 신규 없음, aegis-aml PLAN 20260910-aml-hrr-floor-followups).** F-115 QA 로그 후속(M2·L1) — 온보딩 파생의 레지스트리 조회는 포트 `isKnownUnregistered(tenant, subjectRef)`(어댑터 exists 쿼리, 인덱스 `ix_hrr_items_subject`) 부재판정 fast path 후 등재 회원만 전량 로드하고, 등재 강제 재평가(`HighRiskRegistryService.triggerForcedReassessment`)의 floor·사유·근거 조립은 도메인 `HighRiskRegistry.forcedFloorFor` 단일 정본으로 위임(두 경로 공유, 중복 제거)함을 명문화. 룰/판정 변경 없음(성능·구조 정리만). | system-architect. 코드 truth=`application/port/out/HighRiskRegistryStorePort#isKnownUnregistered`·`domain/registry/HighRiskRegistry#forcedFloorFor`·`application/usecase/{OnboardingRaDerivationService,HighRiskRegistryService}`. API §3.3·DB §3.20·PRD §12-A.4 BR-004 ①·§12-B.6 BR-005 동일 작업 단위. |
| 2026-09-07 | **§10.5 재스크린 워커 인프라 타임아웃 서킷·잡 일시정지 + §17.3 DDL `paused_at/paused_by`(코드=truth, aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit).** 공유 스택 실측(2026-09-06): 후보 조회 8~10초·statement timeout 10s 에 1,3xx 타깃 재시도 핫루프로 DB 포화 → 워커가 증폭기가 되지 않도록 attempt 불변·서킷·틱 예산·pause 도입. 후보 조회 SQL 자체는 별건. | 코드 truth=`RescreenWorker`·`RescreenCircuitBreaker`·`RescreenFailureClassifier`·`WatchlistRescreenJobAdminController`·V76 |
| 2026-08-22 | **WLF 요청 이름 스냅샷·NAME 인라인 노출(P0-09 국지 카브아웃, 코드=truth).** `WlfScreeningService`는 신규 insert 승자의 persisted `screeningId`에만 `WLFREQ-{screeningId}` vault NAME 스냅샷을 저장해 멱등 replay/동시 경합이 기존 값을 덮지 못하게 한다. `ScreeningIdentityProjectionService`는 tenant-scoped batch vault 조회 2회(요청명·entry명)로 read projection을 만들며, `aml:case:read`만 `requestName`·`matchedEntryNames`를 받고 `aml:screen:evaluate` 단독은 생략된다. 명단 이름은 안정 entry id의 현재 게시본이라 재sync 표기 변경을 따른다. | API §1.6/§2.4/§3.2·DB §3.21·기능정의서 AML-WLF-001/002/003 동기화. |
| 2026-08-19 | **§5.5 다통화(법인별 자국통화) REST 계약 확장 역전파(코드=truth, PLAN 20260818-currency-profile-bo-setup U1/U18).** `PATCH .../policy-binding` 거래성 이력 보유 테넌트 `baseCurrency` 변경 fail-closed(422 `AML.TENANT_CURRENCY_HISTORY_LOCKED`), `report_cutoff_time` 전용 `PUT .../policy-binding/report-cutoff-time` 서브리소스 분리(PATCH cutoff 무접촉), raw `GET .../policy-binding`(12키 read-back) 신설, 엔진 CTR 임계 read 2종(`GET .../ctr-thresholds[/{currency}]`) 신설 — bo-api 기준통화 프로파일 apply(API §3.16a) `STEP CTR_THRESHOLD` 판정 정본. | 코드=truth. 근거=aml-svc `adapter/in/rest/{TenantPolicyBindingAdminController,CtrThresholdAdminController}`. API §2.7 동기화. 신규 마이그레이션 0. |
| 2026-08-13 | **신호 점수 비례 스케일링(§11.3a 신설) + `UnknownOutcome` 3값화 + `functions` 전량 보존 역전파(코드=truth, aegis-aml `feature/pep-name-risk-score-scaling` `206b7558`. 전부 additive — 엔드포인트·DTO 필드 삭제 0·Flyway 신규 0).** (1) **§11.3a 신설** — 2차 상시 RA 기여도에 세 번째 항 `× m(nameScore)` 도입. 종전 산식은 이름만 닮은 신호와 확정 매칭을 같은 `STR_PEP` 가중으로 취급해 이름 점수 0.65 와 0.99 가 **똑같은 만점 가중**을 받았다. 방향은 **깎기**이며 배수 상한 `1.0` 고정이라 **어떤 입력에서도 기존 가중을 초과하지 않는다**(사용자 결정). 스케일 입력은 `matchScore`(=overall)가 아니라 **이름 하위점수 `nameScore`**(evidence additive 운반, `matchScore` 무변경). **확정 매칭·KYC 플래그 대표 알럿은 언제나 배수 1.0** — 동행한 약한 수취인 신호로 확정 PEP 고객의 RA 를 깎으면 미탐이다. 선택키 + 항등원 폴백(부재 = 현행 동작 바이트 동일), `STR_SANCTION` 스케일 금지 fail-fast, 읽기·**쓰기**(검증기 허용키) 두 경로 검증, `factorBreakdown.signalScaling` 근거·`severityDriver`, 4-eyes 시뮬레이션 표본의 동일 역파싱 parity, 되돌리기 3중을 명문화. (2) **§10.3b `UnknownOutcome` 3값화** — `RISK_SIGNAL` 신설(억제하되 계량 가능한 신호로 표식). status `NO_MATCH`·차단 계약 무변경이고 사유 코드(`PEP_CORROBORATION_UNKNOWN_RISK_SIGNAL`)와 표식 2키(`riskSignal`·`signalOrigin`)만 갈린다. **STR 발동 게이트 무연결**(F-064 계약 보존 — `carries`/`presentIn`/`strongest` 무수정, 새 술어로 분리)이며 **엔진 하류 소비는 아직 없다**(정책 결정 대기)는 사실을 그대로 기록. 기본값 `SUPPRESS` 유지(WLF-C19 ③ 문자 고정·append-only). 설정 표·판정 우선순위 ③·설명가능성 "두 갈래"→"세 갈래" 정정. (3) **§10.3b `wideMultiValueFields` 범위 정정** — 그 키는 **코로보레이터 2필드 전용**으로 남고 `functions` 광폭은 **별도 키 `wideFunctionFields`** 다(연동 §7.4). 두 신호는 뜻이 다르다(코로보레이터 광폭 = 미탐 위험, `functions` 광폭 = 정상·census 최대 60). (4) **§10.3b 유효 정책 노출** — `GET .../wlf-engine-config` 응답에 읽기 전용 `pepAxis` 블록 8필드 추가(건별 근거는 후보가 있었던 행에만 붙어 테넌트 유효값을 읽을 수 없었다). 4-eyes 편집 승격은 범위 밖. | 코드=truth. 근거=aml-svc `domain/risk/{SignalScaling,AlertSignalMark,OngoingRaParameters,OngoingRaFactorDeriver,RiskModelDefinitionValidator}`·`application/usecase/{OngoingRaService,StrEvaluationService}`(`nameSubScore`)·`domain/tm/AlertEvidence.WatchlistMatch.nameScore`·`domain/screening/match/{PepAxisPolicy,WlfMatchVerdict}`·`domain/screening/PepNameRiskSignal`(메서드 추가만)·`global/config/WlfPepAxisConfig`·`adapter/in/rest/WlfEngineConfigurationController`·`adapter/out/persistence/RiskModelSimulationSampleJpaAdapter`·`adapter/out/feed/DilisenseJsonlParser`. 검증=`RaOngoingSignalScalingIntegrationTest`·`RaSignalScalingSimulationIntegrationTest`·`OngoingRaFactorDeriverSignalScalingTest`·`AlertSignalMarkTest`·`RiskModelDefinitionValidatorTest`·`WlfPepAxisUnknownRiskSignalScreeningTest`·`PepAxisUnknownRiskSignalTest`·`DilisenseFunctionsTruncationRegressionTest`. API §2.3·§2.4·§3.2·§3.3 · DB §3.9 · 연동 §7.4 · 기능정의서 §6.1 AML-RA-002·§12-B.8 BR-007 동일 작업 단위. Flyway 신규 0. |
| 2026-08-13 | **코로보레이터 배열 상한 제거 + 지연 WLF 성공 시 신호 소비 단계 재무장 + `:simulate` 회수 옵션 단일화 역전파 — 재리뷰 REJECT 높음 3건 해소분(코드=truth, aegis-aml main `3d3f8bc0`. 전부 additive — 엔드포인트·DTO 스키마·Flyway 무변경).** (1) **§10.3b 「정규화(입력 손실 제거)」 사실 정정 註** — 직전 판이 등재한 "배열 전체 보존" 은 **등재 시점에 거짓**이었다(파서는 앞 5개로 절단, 같은 날 연동 §7.4 는 `각 최대 5개` 라고 적어 두 정본이 어긋나 있었다). 절단분만 남으면 매처가 `UNKNOWN_*` 이 아니라 **`MISMATCH`** 로 읽고 우선순위상 억제되므로 **억제 결과와 억제 사유가 함께 거짓**이 되며 `customer.unknown-outcome=MATCH` 로도 구제되지 않는다. 실 피드 census(선두 228,339 레코드 26.5% + delta 27,969 전수 계수) 기반으로 상한을 제거했고(6개 이상 DOB 35건 실재·최대 13·국적 초과 0·증가분 80값/1,120 B·인덱스 영향 0), 대신 길이 20 초과를 `wideMultiValueFields` 로 **계수만** 한다. 이 문장이 참이 된 시점은 2026-08-13 이다. (2) **§15.9 「신호 소비 단계 재무장」 신설** — durable retry 가 각 step 을 독립 복구하는데 **WLF 신호를 읽는 하위 단계는 독립이 아니다**. WLF 만 실패한 인입의 STR·2차 상시 RA 는 **신호 없이 `SUCCEEDED`** 로 끝나고 워커가 WLF 만 되살려도 이를 깨우는 경로가 없어 위험 신호가 영원히 소비되지 않았다(readiness `STALE`→복구라는 정상 경로에서 발생, 송금인 축에서는 `STR_SANCTION` 미발동으로 번진다). `signalConsumersOf`(producer `{SENDER_WLF, RECEIVER_WLF}` → consumer `{STR, ONGOING_RA}`)·`rearmSucceededSteps`(`SUCCEEDED → RETRYING` 단일 전이, `DEAD_LETTERED`/`IN_PROGRESS`/`PENDING` 미대상)·워커 경로 한정 발동·멱등(자연키·디바운스 계보)·깊이 1 DAG 순환 불가 근거를 명문화. V48 CHECK 가 이미 허용하는 전이라 Flyway 신규 0. (3) **§10.3b 「회수 옵션 단일화」 신설** — 직전 판의 "판정 단일화" 는 **판정 함수·정책 인스턴스 축만** 닫았고 후보 회수 축은 갈려 있었다(`CandidateOptions.defaults()` vs `aegis.aml.wlf.candidate.*`). 같은 판정 함수를 태워도 후보 집합이 다르면 결과가 반대로 갈린다(운영 cap 500 테넌트의 축 rank 201 진성 제재 후보 → 실운영 매치 / 시뮬레이션 `NO_MATCH`, cap 하향 시 반대 방향). 실운영이 `candidateRecallOptions()` 로 자기 회수 계약을 공표하고 시뮬레이션이 읽어 가도록 바꿔 **조립 지점을 1개로** 만들고, 어떤 옵션·절단 상태로 돌았는지를 `scoreBreakdown.candidateRecall`(API §3.2)로 공표한다(종전에는 `capHit` 를 버려 **잘린 후보 집합 위의 판정**임을 알 수 없었다). | 코드=truth. 근거=aml-svc `adapter/out/feed/DilisenseJsonlParser`(`wideAware`·`WIDE_MULTIVALUE_THRESHOLD`)·`domain/fanout/NeutralFanoutSteps`·`application/port/out/FanoutJobStorePort`·`adapter/out/persistence/FanoutJobJpaAdapter`·`application/usecase/fanout/FanoutStepExecutor`·`application/port/in/ScreenSubjectUseCase`·`application/usecase/{WlfScreeningService,ScreeningOperationsService}`. 검증=`DilisenseMultiValueTruncationRegressionTest`·`WlfLateRetryRearmIntegrationTest`(Testcontainers)·`WlfSimulateCandidateOptionsParityTest`(8건 red 실증)·`FanoutDomainTest`·`FanoutStepExecutorTest`. API §2.4·§3.2 · 연동 §3.1b-1·§7.4 · 기능정의서 §12-B.8 BR-007 동일 작업 단위. DB 스키마·Flyway 무변경(신규 마이그레이션 0). |
| 2026-08-12 | **PEP 축 분리 정책 v2 + 후보 회수 축 분리 역전파 — 독립 리뷰 REJECT 높음 4건 해소분(코드=truth, aegis-aml main `d9520f0b`. 전부 additive — 엔드포인트·DTO 스키마·Flyway 무변경).** (1) **§10.2b·§10.2b-1 신설 — 명단축별 후보 cap(`RecallAxis` `SANCTIONS_LAW`/`PEP`/`OTHER`)**: 전역 `LIMIT cap` 을 `row_number() OVER (PARTITION BY axis …)` 축별 랭킹으로 대체해 **제재·법집행이 PEP 물량에 잘리지 않음**을 계약으로 세운다(구 전역 cap 에서는 동일 이름 PEP 201건이 SANCTIONS 1건을 후보 집합 밖으로 밀어내 정책 평가에 도달조차 못 하고 `NO_MATCH`→ALLOW 였다). 진단 `candidateCapScope`(`PER_LIST_TYPE_AXIS`)·`axisCounts`·`truncatedAxes[]` 가산, `candidateStrategyVersion` **`wlf-cand-v1`→`wlf-cand-v2`** 회전, 회수는 strictly additive(상한 `cap`→`3 × cap`). (2) **§10.3b 정책 축 `wlf-pep-axis-v1`→`wlf-pep-axis-v2`** — 코로보레이터 판정을 boolean 에서 **4상태**(`MATCH`/`MISMATCH`/`UNKNOWN_LIST_DATA`/`UNKNOWN_TARGET_DATA`)로 분리, 사유 코드 3종화(**`PEP_CORROBORATION_UNKNOWN`** 신설), 근거 키 `corroboration`·`unknownOutcome` 가산, 설정 `customer.unknown-outcome`(기본 `SUPPRESS`·오타 부팅 거부), `BirthDate`(`yyyy`·`yyyy-MM-dd`·`dd/MM/yyyy`·`yyyyMMdd`)·`CountryIso`(ISO2/ISO3/국가명 배열 교집합) 정규화로 입력 손실 제거. (3) **§10.3b 수취인 위험 신호 소비 신설(`PepNameRiskSignal`)** — 강등 행이 STR PEP 신호로 집계되고 STR 계보에 `origin=PEP_NAME_RISK_SIGNAL` 로 실려 알럿 가중을 통해 **송금인의 2차 상시 RA `TRANSACTION_BEHAVIOR` 로 가산**된다(차단은 무발생 — `sanctionHit`·status·HOLD 무변경). (4) **§10.3b 판정 단일화 `WlfMatchVerdict` + `targetType` 신뢰 경계 `ScreeningAxisResolver`** — 실운영과 `:simulate` 가 같은 순수함수·같은 정책 빈을 쓰고(구: simulate 가 정책·§10.3a 이름 상향을 건너뛰어 반대 결과), 등록 회원의 COUNTERPARTY 선언은 더 엄격한 CUSTOMER 축으로 정정(`WLF_TARGET_ROLE_MISMATCH`·`declaredAxis`·`axisMismatch`). (5) **§10.3b 제재 회귀 방지 불변식 2층화** — v1 서술의 "후보 선택 이전 판정" 은 cap 통과분 내부에서만 참이었으므로 회수 층(축별 cap)을 앞에 세웠고, `effectiveBandRank` 표기를 실제 코드 이름 `WlfMatchVerdict.selectionRank` 로 정정. | 코드=truth. 근거=aml-svc `domain/screening/match/{PepAxisPolicy,WlfMatchVerdict,MatchScore,FuzzyMatchEngine}`·`domain/screening/PepNameRiskSignal`·`domain/watchlist/{BirthDate,CountryIso}`·`domain/tm/AlertEvidence`·`application/service/ScreeningAxisResolver`·`application/port/out/WatchlistEntryStorePort.RecallAxis`·`adapter/out/persistence/WatchlistEntryJpaAdapter`·`application/usecase/{WlfScreeningService,ScreeningOperationsService,StrEvaluationService,TransactionReportSideEffectRunner}`·`global/config/WlfPepAxisConfig`. API §3.2·§2.4·§3.4a · 기능정의서 §12-B.8 BR-007·§3.1·§3.3 동일 작업 단위. DB 스키마·Flyway 무변경(신규 마이그레이션 0). |
| 2026-08-12 | **PEP 축 분리·점수 정밀도·아웃박스 상태기계·dilisense 실 PEP 연동 역전파(코드=truth, aegis-aml `staging/round15` `e7d53836`·main `7e00b79d`·`beba9987`·`ac3b4faa`).** (1) **§10.3b 신설 — PEP 축 분리 정책(`PepAxisPolicy`, 도입 시 `wlf-pep-axis-v1` → 같은 날 `d9520f0b` 에서 **`wlf-pep-axis-v2`** 로 회전, 위 행)**: `list_type=PEP` 한정(제재 무변경), CUSTOMER 축 코로보레이터(국적·생년) 요구, COUNTERPARTY 축 위험 신호 강등, **후보 선택 이전 판정 = 제재 회귀 방지 불변식**(`effectiveBandRank=0`), `pepAxis` 감사 블록, 설정 축·부팅 시 오설정 거부, 실측 6/9→2/9. (2) **§10.3c 신설 — 저장 스케일(`numeric(8,4)`)과 비교 스케일(`MatchScore.raw()`)의 분리**: 임계·밴드·이름 floor·후보 순위는 반올림 전 값으로만 판정(반올림이 임계를 내려 매치를 떨어뜨리던 미탐), 반올림은 값객체 경계 1회·모드 `HALF_UP`(DB 일치), 불일치 시 `scorePrecision` 진단. (3) **§10.4 `NO_MATCH` 행** — PEP 축 억제 결과의 착지 상태임을 명기. (4) **§15.8 아웃박스 註** — status **5종**(`DEAD_LETTERED`), 재시도 상한 단일 정의(`OutboxRetryPolicy`, 릴레이·claim 술어 양쪽 주입), `claim_token` CAS, 전이 계약 `void→boolean`. (5) **§10.5 ④ phase-2 경계 부분 해제** — `DILISENSE_CONSOLIDATED` 가 실 PEP 공급자로 연동·임포트되며 mock 은 미등록 소스 폴백으로만 남음(필수 source 정책의 waiver 회수 여부는 운영 결정·미확정). | 코드=truth. 근거=aml-svc `domain/screening/match/{PepAxisPolicy,MatchScore}`·`application/usecase/{WlfScreeningService,ScreeningOperationsService}`·`domain/outbox/OutboxRetryPolicy`·`domain/enums/OutboxStatus`·`adapter/out/feed/{WatchlistFeedRouter,DilisenseConsolidatedFeedAdapter}`. DB §3.15·§5.17·§7 V64~V67 · API §3.2 · integration §7.4·§8.1 · 기능정의서 §12-B.8 BR-007 동일 작업 단위 |
...
| 2026-07-21 | **AML 보고 제출 KoFIU→AMLC(GoTRACS) 실 브라우저 자동화 전환(feature/aml-reports-amlc-migration §1.4-C).** §15.10 provider boundary 절 정정 — `AmlcSubmissionPort`(AMLC 포털 lodgement receipt)가 더 이상 ProviderSvc 위임이 아니라 실 구현체 `PlaywrightAmlcSubmissionAdapter`가 테넌트별 저장 계정(신규 `aml_amlc_credentials`, DB §3.12b V57)으로 aml-svc 프로세스 내에서 브라우저 자동화(Playwright for Java, 신규 버전 카탈로그 의존성)를 통해 AMLC 포털에 **직접** lodge 함을 명시(`ReportSubmissionPort`(FIU 아웃컴)는 ProviderSvc 위임 무변경). §6.2 패키지 레이아웃에 `domain/report.AmlcCredential`·`application/port/out.{AmlcSubmissionPort,AmlcCredentialPort}`·`adapter/out/submission.{MockAmlcSubmissionAdapter,PlaywrightAmlcSubmissionAdapter}` 반영. 신규 admin REST `GET/PUT /api/v1/admin/aml/amlc-credential`(4-eyes 미적용) + bo-web `/aml/reports/amlc-account` 화면. `aml_regulatory_reports.amlc_submission_ref`(DB §3.12, V58) 컬럼 신설로 이중 lodge 방지 판단을 report 자신의 상태로 즉시 수행. | system-architect. 코드 truth=aml-svc `application/port/out/{AmlcSubmissionPort,AmlcCredentialPort}`·`adapter/out/submission/PlaywrightAmlcSubmissionAdapter`·`application/usecase/{AmlcCredentialAdminService,RegulatoryReportService}`·`domain/report/{AmlcCredential,RegulatoryReport}`·Flyway `V57`/`V58`. DB §3.12/§3.12b/§7·API §2.7·연동 §3.4/§9.4 동기화. `ReportSubmissionPort` 무변경 |
...
1. CDD 인입은 canonical event/고객 projection 이후 ONBOARDING RA를 즉시 실행한다. 필수 명단 적용본 부재나 모델 문제는 인입 성공을 유지한 채 durable job으로 보류한다. freshness/refresh 실패는 보류하지 않고 마지막 적용본으로 평가한다.
...
7. **(2026-09-11, 코드=truth, aegis-aml PLAN 20260911-aml-hrr-floor-consolidation — 사용자 지시로 F-115·F-116 해제)** 당연고위험(HRR) 레지스트리 병합(§12-B.6 BR-010) 의 조회·조립은 포트 단일 메서드 `HighRiskRegistryStorePort.findForcedFloor(tenantId, subjectRef)`(추상, default 술어 없음 — 구 `isKnownUnregistered` exists 부재판정 fast path 는 삭제)로 통합됐다. 어댑터는 `HighRiskRegistryReferenceItemJpaRepository.findByTenantIdAndSubjectRef`(인덱스 `ix_hrr_items_subject`, 비잠금·`@Transactional(readOnly = true)`)로 subject 의 레지스트리 아이템만 읽어 도메인 정적 팩토리 `HighRiskRegistry.forcedFloorFrom(subjectRef, items)` 로 조립한다. 기존 인스턴스 `forcedFloorFor(subjectRef)`(등재 강제 재평가 `HighRiskRegistryService.triggerForcedReassessment` 경로)도 같은 정적 팩토리에 위임해, 온보딩 파생(`OnboardingRaDerivationService`)의 인덱스 조회 경로와 강제 재평가의 전량 로드 경로가 floor/사유/근거 조립 규칙 **단 하나**를 공유한다(룰/판정 변경 없음, 성능·구조 정리만).
