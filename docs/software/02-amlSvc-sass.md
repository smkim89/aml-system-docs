...
정본 기준 `com.hanpass.aml` 하위로 배치한다(참조 구현 `hanpass-ph/services/fds-svc`의 `com.hanpass.fds` 패턴과 동일). 도메인 불변식은 `domain`, 유스케이스·트랜잭션 경계는 `application`, I/O는 `adapter`.
...
최상위 event family는 **15종으로 종결**한다(본 §8.1이 family 카운트 정본). 연동 §3.1이 '14종'으로 표기한 것은 `vendor.*` 미반영 오기이며, 본 표 15종을 단일 진실로 삼아 정합한다(연동 측 차기 갱신 대상).
...
`aml_regulatory_reports.status`의 정본 상태 enum·전이는 다음과 같다(DB §5.11·연동 §9.1과 1:1). 제출 콜백으로 `SUBMITTED`/`REJECTED`가 확정된다.
...
|---|---|
| `DRAFT` | 보고 초안 생성 |
| `UNDER_REVIEW` | 검토·보완 진행 |
| `APPROVED` | 4-eyes 승인 완료(제출 대기) |
| `SUBMITTED` | 외부 보고기관 제출 완료(제출 식별자 evidence 저장) |
| `REJECTED` | 검토·제출 반려 |
| `CANCELLED` | 보고 취소 |
...
| 항목 | 정본 |
|---|---|
| 물리 테이블 | `aml_outbox`(DB 지원 인프라). 핵심 컬럼: `tenant_id`·`outbox_id`·`data_scope`·`aggregate_type`(허용값 5종: `REGULATORY_REPORT`/`CASE`/`SCREENING`/`FDS_FEEDBACK`/`WEBHOOK`)·`aggregate_ref`·`event_type`·`payload`(JSONB)·`payload_hash`·`status`·`attempt`·`next_attempt_at`·`trace_id`·`created_at`·`created_by`·`published_at`(공통 감사 컬럼 `created_at`·`created_by` 포함, DB §3.15·연동 §8.1 정본) |
...
#### 16.0c status (3종, 운영 생명주기) — DB §5.28b 정본

`aml_tenants.status`. SaaS 고객사 **운영** 생명주기다. 배포 프로비저닝 진행 단계(`onboarding_status`, §16.0b)와 직교하며, onboarding이 `ACTIVE`/`REGISTERED`에 도달하면 `status=ACTIVE`로 전환한다.

| 코드값 | 표시값(권고) | 의미 |
|---|---|---|
| `ACTIVE` | 운영중 | 정상 운영(온보딩 완료) |
| `SUSPENDED` | 일시정지 | 계약·미납·보안사유 일시 정지 |
| `OFFBOARDING` | 해지진행 | 해지·데이터 반출/파기 진행 |
...
| `aml_tenants.status` | `ACTIVE` / `SUSPENDED` / `OFFBOARDING` | 고객사 운영 생명주기(§16.0c) |
...
> 아래 status·model 컬럼의 허용값(enum)은 DB 설계서가 정본이다: `aml_tenants.deployment_model` ∈ {`MANAGED_DEDICATED`,`SELF_HOSTED`,`SHARED`}(§16.0a, 구 `isolation_mode` 폐기), `aml_tenants.onboarding_status` ∈ {`REQUESTED`,`PROVISIONING`,`DEPLOYED`,`VERIFIED`,`ACTIVE`,`PACKAGE_ISSUED`,`CUSTOMER_DEPLOYED`,`REGISTERED`}(§16.0b), `aml_tenants.status` ∈ {`ACTIVE`,`SUSPENDED`,`OFFBOARDING`}, `aml_source_systems.status` ∈ {`ACTIVE`,`DISABLED`}, `ingest_mode` ∈ {`REST_PUSH`,`QUEUE`,`POLLING`,`CDC`,`SNAPSHOT`,`VENDOR_BRIDGE`}(DB §5.14). 전 테이블은 `data_scope` 격리 컬럼과 RLS를 갖는다(§16.2.1).
...
CREATE TABLE aml_tenants (
  ...
  status VARCHAR(32) NOT NULL DEFAULT 'ACTIVE',          -- §16.0c 운영 생명주기 ACTIVE | SUSPENDED | OFFBOARDING
