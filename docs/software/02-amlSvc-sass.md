...
| `CUSTOMER` — 확인 불가 갈래 | 요건 미달 원인이 불일치가 아니라 **확인 불가**(명단·고객 어느 쪽에 값이 없거나 읽히지 않는다)일 때. 귀결은 `customer.unknown-outcome` 노브가 정한다(기본 `SUPPRESS` = 억제) | `PEP_CORROBORATION_UNKNOWN` |
...
CUSTOMER 축 판정 우선순위는 **MATCH > MISMATCH > UNKNOWN** 이다 — ① 요구 개수만큼 `MATCH` 면 정책 무개입(매치 승격), ② 부족하지만 `MISMATCH` 가 하나라도 있으면 `CUSTOMER_CORROBORATION_MISSING`, ③ 부족하고 전부 `UNKNOWN` 이면 `CUSTOMER_CORROBORATION_UNKNOWN`. ②가 ③보다 앞서는 이유는 불일치가 **있는 증거**이고 결측은 **없는 증거**이기 때문이다(라이브 실측 탈락 4건이 전부 국적 불일치였다). 판정 enum 이름 `CUSTOMER_CORROBORATION_MISSING` 은 v1 값을 그대로 유지한다 — 엔진 케이스 카탈로그 WLF-C19 가 이 문자열을 기대값으로 고정하고 카탈로그는 append-only 다.
...
| `aegis.aml.wlf.pep-axis.customer.unknown-outcome` | `SUPPRESS` | 전부 확인 불가일 때의 귀결(`SUPPRESS`\|`MATCH`) |
