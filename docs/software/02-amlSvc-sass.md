...
**V71 runtime RLS 경계.** `%>`와 expression overlap은 leakproof가 아니어서 `aegis_app_runtime` RLS barrier 아래에서 V64 GIN을 잃었다. 후보 CTE를 고정 3열 SECURITY DEFINER 함수로 옮기고, NOLOGIN/non-super/BYPASSRLS 전용 owner에 watchlist 두 테이블 SELECT만 부여한다. 함수는 tenant GUC 일치 또는 elevated만 허용하며 missing/mismatch는 42501이다. 직접 테이블 RLS 정책 2종, candidate 의미·점수·응답은 불변이다. V71은 같은 release bootJar의 privileged runner로만 적용하고, runner는 Java V38+선택 currency repeatable의 `version/type/script/checksum` inventory를 고정한 뒤 `aegis_aml_app` non-super login을 exact runtime membership/history-only ACL로 provision한다. 선존 recall/app role의 잘못된 attribute·membership·AML ownership·direct/default ACL이나 app password probe 실패는 V71 mutation 전에 중단하며 credential을 교정·승격하지 않는다. 함수 planner GUC를 영구 강제하지 않는다.

배포 검증은 privileged init과 ordinary app을 같은 image ID로 고정한 bounded non-super session에서 수행한다. Dilisense는 upstream key를 host streaming proxy에만 두고 AML에는 session inner key를 전달하며, REST sync가 실제 소비한 동일 JSONL을 0600 tee+version/SHA-256으로 결속한다. active-version join 기준 Dilisense PEP 90만·전체 150만 미만이면 성능 증적은 무효다. live plan runner는 recall-owner transaction에서만 `statement_timeout=10000`·`pg_trgm.word_similarity_threshold=0.30`을 LOCAL 적용하고 canonical `EXPLAIN (ANALYZE, BUFFERS)`의 V64 GIN 3종·entries Seq Scan 0·3초 이하를 판정한다(57014=`PLAN_TIMEOUT`). `aml-response-performance-v1`은 WLF 3회 protocol 성공과 median≤3초, CDD→RA≤3초, 국내 TM≤4초, 해외 TM≤6초 및 partial/NOT_RUN 6건을 기록하는 운영 증적이며 엔진 케이스 카탈로그를 대체하지 않는다.
