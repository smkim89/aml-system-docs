...
배포 검증은 privileged init과 ordinary app을 같은 mandatory image `.Id`로 고정한 tenant_demo/PHP bounded non-super session에서 수행한다(RepoDigests가 있으면 exact, local empty 허용). Launcher의 canonical state는 `up → active-version-counts → capture-evidence → explain → down`이다. Dilisense proxy는 current-UID 0700 trusted root에서 symlink/hardlink/outside-root를 거부하고 동일 upstream stream을 capture/import 독립 digest로 누적한다. 저장 activeVersion은 `dilisense-<rawVersion>`이며 tenant_demo active-version PEP 90만·전체 150만, WLF-C21 same-byte/checksum, GIN3/no Seq/3초 plan JSON이 모두 있어야 gate-ready다. live plan은 recall-owner transaction에서만 timeout 10초·pg_trgm 0.30을 LOCAL 적용하고 57014를 `PLAN_TIMEOUT`으로 기록한다. `aml-response-performance-v1`은 WLF 3회 protocol 성공+median≤3초, CDD→RA≤3초, 국내 TM≤4초, 해외 TM≤6초와 실패 wall/trace latency·partial/NOT_RUN 6건을 보존하며 엔진 카탈로그를 대체하지 않는다.
