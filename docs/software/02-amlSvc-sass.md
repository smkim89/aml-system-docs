...
7. **(2026-09-10, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-followups)** 당연고위험(HRR) 레지스트리 병합(§12-B.6 BR-010) 에서 온보딩 파생의 레지스트리 조회는 포트 `isKnownUnregistered(tenant, subjectRef)`(어댑터 exists 쿼리, 인덱스 `ix_hrr_items_subject`) 부재판정 fast path 후 등재 회원만 전량 로드한다. 등재 강제 재평가(`HighRiskRegistryService.triggerForcedReassessment`)의 floor/사유/근거 조립은 도메인 `HighRiskRegistry.forcedFloorFor` 단일 정본으로 위임한다(온보딩 파생과 강제 재평가 두 경로 모두 이 도메인 메서드를 공유).
