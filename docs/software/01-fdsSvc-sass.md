...
승인 범위(scope) 원칙: 각 승인은 단일 `payload_hash`에 바인딩되며 `subjectKind`(정본 DB `fds_approval_requests.subject_kind` / API `ApprovalRequestDto.subjectKind` **11종**(r16 이격 3 교정): `ACTION`/`RULE`/`MAPPING`/`SECRET`/`GROUP`/`EXPORT`/`MERCHANT_NORMALIZE`/`CASE_CLOSE`/`POLICY_PACK`/`RULE_PARAM`/`TENANT_REGULATORY_CURRENCY`), `approvalLine`, `expiresAt`, `maxExecutions`로 범위가 한정된다. FE 결재 필터·라벨도 `RULE_PARAM`·`TENANT_REGULATORY_CURRENCY`를 포함한 이 11종을 사용한다. 이는 감사의 변경 resource `targetKind`(`RULE`/`SOURCE_SYSTEM`/`CONNECTOR`/`NOTIFY_CHANNEL` 등)와 별도 enum이다. 승인 후 payload가 바뀌면 결재를 무효화(`FDS-APPROVAL-PAYLOAD-CHANGED`)한다.
...
| `TENANT_REGULATORY_CURRENCY` | 테넌트 규제통화 전환(`POST /api/v1/admin/fds/tenants/{tenantId}/compliance/regulatory-currency:change`, `subjectRef=tenant_id`, 다통화 PLAN 20260818 U17, DB V32) | `COMPLIANCE_MANAGER` |
