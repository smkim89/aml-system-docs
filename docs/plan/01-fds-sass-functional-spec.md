...
| **작성일** | 2026-06-08 |
| **작성자** | SM Kim |
| **상태** | 정합화(설계서 v1.5 + DB v1.3 + API v1.7 정본 — **격리(isolation_mode)→배포 모델(deployment_model) 재설계** 반영) + 시나리오 흐름 연결 보강(목록→상세→액션→케이스→결재→규제 보고 딥링크 전수, 짝 PPT v4.3 동기화) |
...
| **입력 설계서** | `docs/software/01-fdsSvc-sass.md` v1.5 |
| **파생 정합** | DB `docs/design/db/01-fds-db.md` v1.3 · API `docs/design/api/01-fds-api.md` v1.5 · Integration `docs/design/integration/01-fds-integration.md` v1.1 · Tasks `docs/tasks/fds/00-overview.md` |
| **짝 문서(PPT)** | `docs/plan/BO-FDS-SASS-Planning_v4.6.pptx` (도형 기반 · 멀티탭 상세 탭 연속 전개: TNT-002 5탭·RULE-002 5탭·CASE-002 4탭 + **드릴다운 진입 트리거 배너**(TNT/CONN/MAP/RULE/DEC/ACT/CASE -002 상단 ↩ 진입 경로), 45슬라이드·기능 ID 31 불변) |
...
본 문서는 **SaaS FDS Platform**(설계서 `docs/software/01-fdsSvc-sass.md` v1.1)의 **백오피스(Admin Console) 관리·운영** 기능에 대한 기능정의서입니다. 본 플랫폼은 특정 월렛·송금 서비스 하나가 아니라, **여러 금융회사·핀테크 사업자(고객사)** 가 자기 시스템의 거래·회원·계좌·기기·정산·감사 이벤트를 연동하면 실시간 또는 비동기로 이상거래를 탐지·조치하는 **멀티고객사·멀티도메인 위험 탐지 플랫폼**입니다.
...
>
> **운영자 집계 API 소유 경계(정본)**: **대시보드(SFDS-DASH-001/002)·고객사 관리(SFDS-TNT-001/002/003)·감사 조회(SFDS-AUDIT-001)** 는 **`bo-api`가 소유·집약·인증**하는 운영자 집계 엔드포인트입니다. `fds-svc`/`aml-svc`는 저수준 데이터 API만 제공하며, 엔진 API 명세(`docs/design/api`)에는 운영자 집계 엔드포인트(대시보드/고객사/감사)를 두지 않습니다. 이들 화면의 호출 대상은 **bo-api 소유 경로 `/api/v1/bo/fds/*`**(대시보드/고객사/감사)이며, 과거 엔진 직접 경로 `/api/v1/admin/fds/dashboard|tenants|audit`는 **폐기**되었습니다(API §11.2·§12). 나머지 화면의 API 경로는 fds-svc 엔진 엔드포인트(`/api/v1/fds/*`, `/api/v1/admin/fds/*`)이며 bo-web은 항상 bo-api를 통해 호출합니다(§16.1 소유 열 참조).

따라서 본 백오피스는 단일 회사 내부 운영 화면(참조 구현 Hanpass PH FdsSvc)과 달리, 다음 두 종류의 사용자를 함께 고려합니다.

- **플랫폼 운영자** — SaaS 사업자 측. 전체 고객사의 ingest·decision·action·case 건전성, 커넥터 lag, SLA, 데이터 품질을 모니터링·운영합니다.
- **고객사 관리자** — 고객사 측. 자기 고객사(workspace) 범위 안에서 룰·그룹·케이스·규제 보고를 운영합니다.

백오피스는 아래 6가지 운영 책무를 화면으로 제공합니다.

1. **플랫폼 모니터링** — 전체 고객사·고객사별 ingest/decision/action/case 현황, 커넥터 지연, 스키마 검증 실패, SLA
2. **고객사·연동 관리** — 고객사 등록·설정(격리·리전·보존·마스킹·Policy Pack), 소스 시스템·커넥터(REST Push/Queue/Polling/Snapshot/CDC), 스키마·필드 매핑·PII 정책
3. **룰 운영** — 멀티도메인 feature catalog 기반 룰 정의(문장형 빌더/DSL 토글), 기준값 빠른 변경, 버전·4-eyes 결재·활성·롤백·중지(canary)
4. **명단 운영** — 차단/허용/감시/뮬 네트워크 그룹(회원·계좌·수단·단말기·가맹점·셀러 등) 정의·등록·해제·연장
...
| **Action Router** | 고객사 시스템 Capability별 차단/보류/취소/해제/케이스 조치 전달 | 액션 아웃박스(SFDS-ACT) |
...
단일 회사 BO 권한이 아니라 **고객사·서비스·데이터스코프 기반** 권한으로 설계합니다(`§13.2`, `§13.4`). 동일 기능이라도 **조회는 `:READ`, 작성·운영은 `:AUTHOR`/`:OPERATE`, 결재는 `:APPROVE`, 정의·마스터는 `:ADMIN`** 으로 분리하며, 모든 권한은 사용자에게 부여된 **고객사 스코프(전체 / 특정 고객사 / 없음)** 안에서만 작동합니다.

| 기능 영역 | 필요 권한 | Role 매핑 | 설명 |
|-----------|----------|-----------|------|
| 플랫폼 대시보드 (전체 고객사) | `SFDS_PLATFORM:READ` | `SFDS_PLATFORM_OPS` | 전 고객사 ingest/decision/action/case·커넥터 건전성 |
| 고객사별 대시보드 / 전 화면 조회 | `SFDS:READ` | `SFDS_VIEWER` 이상 | 자신의 고객사 스코프 내 조회 |
| 고객사 목록/상세 조회 | `SFDS_TENANT:READ` | `SFDS_PLATFORM_OPS` / `TENANT_ADMIN` | 고객사 설정 조회 |
| 고객사 등록/설정 변경 | `SFDS_TENANT:ADMIN` | `SFDS_PLATFORM_ADMIN` | 격리·리전·보존·마스킹·Policy Pack (플랫폼 운영자 전용) |
...
> **데이터스코프**: 고객사 관리자/분석가/CS는 자신에게 부여된 고객사 범위의 데이터만 조회·운영합니다. 플랫폼 운영자만 전체 고객사 횡단 조회가 가능하며, 고객사 데이터 원문 접근은 별도 통제(break-glass + 감사)를 따릅니다(`§13.4`).
...
|--------|------|------------|
| `fds_tenants` | SaaS 고객사 마스터(`tenant_status`·`deployment_model`·`onboarding_status`·리전·`infra_ref`) | `tenant_id (PK)` |
...
룰(`Rule`, `rule_status` DB §4.13)과 개별 버전(`RuleVersion`, `rule_version_status`)은 별도 상태 머신을 가지며 고객사·서비스 스코프 안에서 운영됩니다. 활성화·롤백은 결재 게이트(`fds_approval_requests`, `subject_kind=RULE`)를 통과합니다.
...
> 종결 상태 3종(`CLOSED_CONFIRMED`/`CLOSED_FALSE_POSITIVE`/`CLOSED_REPORTED`)은 화면에서 "사기 확정 종결 / 오탐 종결 / 보고 후 종결"로 표시합니다. `CLOSED_FALSE_POSITIVE`는 false positive feedback(`/feedback`)에 누적되어 룰 튜닝 근거가 됩니다.
...
SaaS 제품이지만 AML/FDS는 고객 PII·규제·내부보안 요건상 **고객사별 전용 배포가 기본**입니다(공유 SaaS DB가 기본이 아님 — 정본 `target-architecture §4.1`, 설계서 §13.0). 멀티테넌시 키 3종(`tenant_id` / `workspace_id` / `data_scope`)의 의미를 전용 배포 기준으로 재정의합니다.

| 키 | 의미(재정의) | 예시 | BO 반영 |
|------|------|------|---------|
| **`tenant_id`** | **배포의 고객사**. 전용 배포(매니지드 전용/자체 인프라 설치형)에서는 **배포=고객사이므로 단일 값**. 고객사 간 격리는 배포 경계가 보장 | 은행 A, PG B, 거래소 C | 전용 배포는 배포 엔드포인트 단위 라우팅(헤더 상수). SHARED만 `Tenant-Id` 헤더 행 라우팅. 화면 상단 고객사 선택/배지 |
| **`workspace_id`** | 그 고객사의 **서비스/환경** | `retail`/`corporate`, `prod`/`sandbox` | 룰셋·커넥터·케이스 큐·결재 라인을 서비스 단위 분리. 미지정 시 `default`. `Workspace-Id` 헤더 |
...
|------|------|------|
| **매니지드 전용**(`MANAGED_DEDICATED`, 기본) | 플랫폼 클라우드에 고객사별 전용 DB·스택, 온보딩 IaC(Terraform) 자동 프로비저닝 | 배포=고객사 단일 |
| **자체 인프라 설치형**(`SELF_HOSTED`) | 고객 자체 인프라에 설치형 패키지(Helm/Docker). 플랫폼은 산출물·가이드·라이선스만 제공 | 배포=고객사 단일(고객 인프라) |
| **소규모 공유**(`SHARED`) | 소규모/체험용 공유 DB + tenant 행 격리 | 공유 DB 위 행 격리 키 |

| 원칙 | 내용 | BO 반영 |
|------|------|---------|
| **고객사 격리** | 전용 배포(매니지드 전용/자체 인프라 설치형)는 **배포 경계가 1차 격리 경계**, SHARED만 행 격리. 격리는 온보딩 프로비저닝의 산출(D-01) | 고객사 상세·등록에 **배포 유형 표시·온보딩 상태(읽기)** |
| **샌드박스** | `sandbox` 서비스는 별도 격리·**shadow-only**(실제 action 미발행, `fds-actions`/`fds-aml-handoff`/`fds-webhook` 미발행) | 서비스 선택 시 샌드박스 배지·"shadow only" 표시 |
| **권한 분리** | 플랫폼 운영자 ↔ 고객사 관리자 ↔ 내부 support 권한 분리 | data-scope 기반 화면 노출 |
| **원문 접근 제한** | raw payload·식별자 원문 접근은 원칙적 미제공, 필요 시 break-glass + 감사 | 마스킹 응답, 복호화 UI 없음 |
| **데이터 레지던시** | 한국 고객은 한국 리전 저장·처리 원칙, 해외 리전은 고객사별 별도 계약 | 고객사 설정에 리전 표시 |
| **로그 보존** | 금융권 감사 요구에 맞춘 장기 보존 정책 고객사별 설정 | 고객사 설정에 보존 기간 |
| **PII 원칙** | raw 계좌·카드·주민번호 미저장, 토큰/keyed hash만 저장 | 화면 표시는 BE 마스킹 |

화면 상단에는 사용자 권한에 따라 **고객사 선택 드롭다운 + 서비스 선택**(플랫폼 운영자) 또는 **고객사·서비스 스코프 배지**(고객사 관리자, 자기 스코프 고정)를 노출합니다. cross-workspace 접근은 명시적 권한이 있어야 하며, 없으면 `FDS-AUTHZ-003`으로 차단합니다.

#### 1.7.1 온보딩 상태 머신 (`onboarding_status`, 배포 모델별)

고객사 등록은 '격리 토글'이 아니라 **배포 유형 선택 + 온보딩 신청**이며, 이후 프로비저닝 진행 상태를 `onboarding_status`(8종)로 읽기 표시합니다. `onboarding_status`는 운영 생명주기인 `tenant_status`(온보딩/운영중/정지/해지)와 **직교**합니다.
...
### 2.1 SFDS-DASH-001 · 플랫폼 운영 대시보드 (전체 고객사)
...
| **목적** | 전체 고객사의 ingest/decision/action/case 건전성과 커넥터 lag·SLA를 단일 화면에서 모니터링 (플랫폼 운영자 전용) |

#### 화면 레이아웃

```
┌──────────────────────────────────────────────────────────────────────────┐
│ 플랫폼 운영 대시보드  (전체 고객사)        기간: [최근 24시간 ▼]          │
├──────────────────────────────────────────────────────────────────────────┤
│ ┌─ 수신 이벤트 ───────┐ ┌─ 결정 ────────────┐ ┌─ 조치·케이스 ────────┐    │
...
│  • 규제 보고 제출 기한 임박(전 고객사) 6건                    [규제 큐]  │
│                                                                            │
│ ┌─ 고객사별 건전성 ────────────────────────────────────────────────────┐ │
│ │ 고객사     │ 수신     │ 검증실패 │ 커넥터lag │ 액션실패 │ 케이스SLA │ │
│ │ ───────────┼──────────┼──────────┼───────────┼──────────┼───────────│ │
...
| 고객사별 건전성 | 고객사, 수신, 검증실패, 커넥터lag, 액션실패, 케이스SLA | 고객사별 집계 |

#### 비즈니스 규칙

- **BR-001**: 기간 필터는 `최근 1h / 24h / 7d / 30d` 프리셋 + 커스텀 범위.
- **BR-002**: 플랫폼 알림의 각 항목은 해당 상세 화면(고객사 대시보드·커넥터·매핑·아웃박스·규제 큐)으로 딥링크. 대시보드는 read-only.
- **BR-003**: 고객사별 건전성 행 클릭 → 고객사별 상세 대시보드(SFDS-DASH-002). 임계 초과 셀은 ⚠ + 색상 강조.
- **BR-004**: 집계는 30~60초 캐시 허용(실시간 정합성보다 가용성 우선), 마지막 갱신 시각 표기.
- **BR-005**: 본 화면은 **플랫폼 운영자 전용**. 고객사 관리자는 자기 고객사만 보는 SFDS-DASH-002로 진입.

### 2.2 SFDS-DASH-002 · 고객사별 상세 대시보드

| 항목 | 내용 |
|------|------|
| **기능 ID** | SFDS-DASH-002 |
| **권한** | `SFDS:READ` (자기 고객사 스코프) |
...
| **목적** | 선택한 고객사의 ingest 상태·스키마 실패·결정 추이·액션 실패 큐·케이스 SLA·룰 hit rate·오탐을 모니터링 |

#### 화면 레이아웃

```
┌──────────────────────────────────────────────────────────────────────────┐
│ 고객사 대시보드 [고객사: 은행 A ▼] [서비스: retail ▼] 기간: [최근 24시간 ▼]│
├──────────────────────────────────────────────────────────────────────────┤
│ ┌─ 수신 상태 ─────────┐ ┌─ 결정 추이 ───────┐ ┌─ 케이스 SLA ─────────┐   │
...
- **BR-001**: 플랫폼 운영자는 상단 [고객사] 드롭다운으로 고객사 전환. 고객사 관리자는 자기 고객사로 고정(드롭다운 비활성, 스코프 배지 표시).
- **BR-001a (필터 축)**: 상단 필터는 `고객사 / 서비스(workspace, retail·corporate·prod·sandbox) / 기간` 3축이다. `서비스` 드롭다운은 멀티 workspace 고객사에서 workspace_id 단위로 집계 범위를 좁히며(`§1.7` 격리키), 단일 workspace 고객사는 `전체`로 고정 표시한다. `sandbox` 선택 시 shadow-only 배지를 표시한다.
...
고객사는 SaaS 운영의 마스터입니다. 플랫폼 운영자가 고객사를 **등록(마스터 생성)** 하면서 **배포 유형을 선택하고 온보딩을 신청**하며, 리전·보존·마스킹·알림·Capability·Policy Pack 을 설정합니다.

> **용어·계층(준법감시실 친화 표기)**: 화면 표시 용어는 **고객사**(내부 코드 `tenant_id`)이며, 한 고객사는 여러 **서비스**(내부 코드 `workspace_id`, 예: 리테일/기업, 운영/샌드박스)를 가질 수 있습니다 — **1 고객사 : N 서비스**. 서비스 단위로 룰셋·커넥터·케이스 큐·결재가 각각 분리됩니다.
> **배포 모델(`deployment_model`)**: AML/FDS는 고객 PII·규제·내부보안 요건상 **고객사별 전용 배포가 기본**입니다. 고객사별로 ① 기본 **매니지드 전용**(`MANAGED_DEDICATED`, 플랫폼 클라우드에 고객사별 전용 DB·스택, 온보딩 IaC 자동 프로비저닝) ② **자체 인프라 설치형**(`SELF_HOSTED`, 고객 인프라에 Helm/Docker 설치형 패키지, 플랫폼은 산출물·가이드·라이선스만) ③ **소규모 공유**(`SHARED`, 소규모/체험용 공유 DB + tenant 행 격리) 중 **온보딩 프로비저닝으로 결정**합니다. 격리는 화면 라디오 즉석 선택이 아니라 온보딩 프로비저닝의 산출이며, 진행 상태는 `onboarding_status`(8종)로 읽기 표시합니다(`§1.7.1`). 호출 대상은 모두 bo-api(`/api/v1/bo/fds/tenants/**` + `/onboarding/**`)이며 bo-web→bo-api 경유만 허용(엔진 직접호출 금지).

### 3.1 SFDS-TNT-001 · 고객사 목록

| 항목 | 내용 |
|------|------|
| **기능 ID** | SFDS-TNT-001 |
| **권한** | `SFDS_TENANT:READ` (목록 조회) / `SFDS_TENANT:ADMIN` (신규 버튼) |
| **API** | `GET /api/v1/bo/fds/tenants?status=&region=&deploymentModel=` (**bo-api 소유·집약·인증**. 고객사 관리 운영자 엔드포인트는 bo-api가 소유. 폐기 경로 `/api/v1/admin/fds/tenants` 대체 — API §11.2) |
...
| 컬럼(표시) | 설명 (괄호=내부 코드) |
|------|------|
| 고객사 ID | 시스템 식별자(`tenant_id`), 변경 불가 |
| 표시명 | 사람이 읽는 고객사명(`display_name`) |
...
- **BR-001**: 필터 `상태 / 리전 / 배포 유형` 3축 + `고객사명` 텍스트 검색.
- **BR-002**: 본 화면은 **플랫폼 운영자 전용**. 고객사 관리자는 자기 고객사 1건만(상세로 직행).
- **BR-003**: 행 클릭 → 고객사 상세(SFDS-TNT-002, **5탭: ① 기본 정보 / ② 배포·온보딩 / ③ 마스킹·보안 / ④ Policy Pack / ⑤ 알림·소스**). `[+ 새 고객사]` 는 `SFDS_TENANT:ADMIN` 만 노출(진입 시 별도 생성 화면 SFDS-TNT-003으로 이동, 상세 5탭과 분리).
- **BR-004**: 조회 결과 1만 건 초과 시 팝업 노출 — 기간/조건 좁히기 안내 후 재조회 요청.

### 3.2 SFDS-TNT-002 · 고객사 상세 / 설정 (5탭 연속 전개)

| 항목 | 내용 |
|------|------|
| **기능 ID** | SFDS-TNT-002 |
| **권한** | 조회 `SFDS_TENANT:READ` / 설정 변경 `SFDS_TENANT:ADMIN` |
| **API** | `GET /api/v1/bo/fds/tenants/{id}` · `PUT /api/v1/bo/fds/tenants/{id}` · `GET /api/v1/bo/fds/tenants/{id}/onboarding` · `GET /api/v1/admin/fds/source-systems` · `GET /api/v1/admin/fds/connectors/notify-channels` (**bo-api 소유·집약·인증** — API §11.2) |
...
> **SFDS-TNT-003(고객사 등록)은 별도 생성 화면**입니다. 상세 5탭과 탭 바를 공유하지 않으며, 고객사 목록(SFDS-TNT-001)의 `[+ 새 고객사]` 버튼에서 진입합니다(§3.3 참조).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 고객사 상세  >  은행 A (tenant_bank_a)                                    │
├─ [① 기본 정보] [② 배포·온보딩] [③ 마스킹·보안] [④ Policy Pack] [⑤ 알림·소스] ┤
│                                                                            │
│  고객사 ID    tenant_bank_a          (변경 불가)                          │
...
| 항목 (괄호=내부 코드) | 설명 |
|------|------|
| 고객사 ID (`tenant_id`) | 시스템 식별자, 변경 불가(immutable) |
| 표시명 (`display_name`) | 사람이 읽는 고객사명. `SFDS_TENANT:ADMIN`만 편집 가능 |
...
| 조치 권한 (`capabilities`, SourceSystemDto API §5.17) | 이 고객사 시스템에서 허용된 조치 집합 — 차단·자금 보류·해제·취소·케이스 전용 등. 조회 전용(변경 경로: SFDS-ACT-002 Capability 매트릭스, 4-eyes 결재) |
...
│  ─ 적용 규제 팩 ──────────────────────────────────────────────────────── │
│  팩 이름               적용    트리거·보고 양식                            │
│  한국 기본팩            ●      STR/CTR 기본 양식, 금융정보분석원 포맷     │
│  전자금융거래법          ●      전자금융거래 이상행위 보고                 │
│  특금법 (AML/CFT)        ●      의심거래·고액현금 보고(STR/CTR)            │
│  개인정보보호법          ●      개인정보 침해 대응 보고                   │
│  내부통제기준            ●      내부 감사 케이스 생성                     │
│  Travel Rule / PCI       ○      가상자산·카드 비즈니스 추가 시 활성화     │
│                                                                            │
│  ─ 변경 영향 안내 ─────────────────────────────────────────────────────  │
│  팩 변경 시 영향받는 보고 유형: STR 6건 · CTR 2건                         │
│  (팩 추가/제거 전 영향 범위 확인 후 진행 권장)                            │
│                                                                            │
│  [Policy Pack 변경]  ※ 변경은 SFDS_TENANT:ADMIN + 4-eyes 결재            │
│                                                          [←③ 이전] [다음 ⑤→]│
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 항목 (괄호=내부 코드) | 설명 |
|------|------|
| 적용 규제 팩 (`compliance_policy`) | 활성화된 관할별 규제 팩 목록 — 한국 기본·전자금융·특금법·개인정보·내부통제·Travel Rule / PCI 등 |
| 트리거·보고 양식 | 각 팩이 생성하는 보고 유형 및 양식 포맷(읽기) |
| 변경 영향 안내 | 팩 추가/제거 시 영향받는 STR/CTR 보고 건수 미리보기 |

**비즈니스 규칙**

- **BR-001**: Policy Pack 변경은 `SFDS_TENANT:ADMIN` 전용. 변경 전 영향받는 보고 유형·건수를 안내 모달로 표시.
- **BR-002**: 팩 변경은 규제 보고 큐(SFDS-REG) 양식·트리거에 즉시 영향 — 변경 전 현재 진행 중인 보고 후보 건수 확인 후 진행 권장.
- **BR-003**: 한국 기본팩은 비활성화 불가(최소 규제 요건). Travel Rule / PCI는 해당 비즈니스 도메인 계약 후 활성화.
- **BR-004**: Policy Pack 변경 이력은 감사 로그(SFDS-AUDIT-001)에 기록.
...
- **BR-001**: 알림 채널 변경은 `SFDS_TENANT:ADMIN` 전용. 변경 내역은 감사 로그에 기록.
...
- **BR-공통-003**: `SFDS-TNT-003(고객사 등록)`은 본 상세 탭과 **별도 화면**(탭 바 미공유). 목록(SFDS-TNT-001)의 `[+ 새 고객사]` 버튼에서만 진입(`§3.3`).
...
### 3.3 SFDS-TNT-003 · 고객사 등록 (마스터 생성)
...
고객사 **마스터(틀)를 생성**하는 화면입니다. **SFDS-TNT-002(고객사 상세 5탭)와 탭 바를 공유하지 않는 별도 생성 화면**으로, 고객사 목록(SFDS-TNT-001)의 `[+ 새 고객사]` 버튼에서만 진입합니다. 소스 시스템·커넥터·룰·그룹은 등록 후 각 화면에서 추가합니다.
...
│ 고객사 등록 / 수정                                                        │
├──────────────────────────────────────────────────────────────────────────┤
│ 고객사 ID *     [tenant_bank_a____]  영문 소문자·숫자·_ (생성 후 변경 불가)│
...
│ Policy Pack *   [✓] 한국 기본팩   추가: [ ] Travel Rule  [ ] PCI 최소화   │
...
| 필드 | 설명 |
|------|------|
| `tenantId` | 고객사 식별자(immutable). 생성 후 변경 불가 |
| `displayName` | 고객사 표시명 |
...
| `compliancePolicyPack` | 한국 기본 + 관할별 추가 팩 |
...
- **BR-005**: 데이터 보존·감사 보존 최소값(감사 7년)은 하향 불가. 고객사 생성·수정 이력은 감사 로그에 7년 보존.

---

## 4. 소스 시스템·커넥터 관리

고객사는 자기 시스템 이벤트를 소스 시스템·커넥터로 연동합니다. 커넥터는 REST Push / Queue / Polling / Snapshot / CDC 유형을 지원하며(`§12`), 백오피스는 ingest 상태·lag·오류·replay 를 운영합니다.
...
| **API** | `GET /api/v1/admin/fds/connectors/{id}` · `POST .../{id}/replay` · `/pause` · `/resume` (API §4.8 정본) |

#### 화면 레이아웃 (연동 방식별 운영 패널)

```
┌──────────────────────────────────────────────────────────────────────────┐
│ audit-log 커넥터 (폴링)  [고객사: 은행 A]            상태: 지연  [일시중지] │
├──────────────────────────────────────────────────────────────────────────┤
...
| 재처리/운영 | 재처리 범위(기간/오프셋)·커서 이동 = **오프셋 replay**(`POST /connectors/{id}/replay`), 일시중지/재개(`/pause`·`/resume`), 설정 변경(`PUT /source-systems/{id}` 4-eyes) |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 필드 매핑 / PII 정책 — core-banking.v2  [고객사: 은행 A]      [변경 결재] │
├──────────────────────────────────────────────────────────────────────────┤
...
룰 관리는 FDS 운영의 핵심 화면입니다. SaaS 멀티도메인 특성상 룰 빌더는 **도메인/채널 선택**과 **멀티도메인 feature catalog**(`§10.1`)를 반영하며, **목록 → 상세 → 빌더 → 결재 → 활성화** 라이프사이클을 4-eyes 로 통제합니다. 모든 룰은 고객사 스코프 안에서 운영됩니다.
...
| **기능 ID** | SFDS-RULE-003 |
| **권한** | `SFDS_RULE:AUTHOR` |
| **API** | `[임시저장(DRAFT)]`=`POST /api/v1/admin/fds/rules` (초안 DRAFT) · `PUT /api/v1/admin/fds/rules/{ruleId}` (수정) · `GET /api/v1/admin/fds/feature-catalog`. `[시뮬레이션 후 결재 상신]`=활성화 결재 게이트(SFDS-RULE-005) `POST /api/v1/admin/fds/rules/{ruleId}/activate` 🔒(4-eyes)로 이관(딥링크) — API §4.6 |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 룰 빌더 (업무 용어 기반)  [고객사: 은행 A]   모드: [쉬운 구성] [DSL(JSON)] │
├──────────────────────────────────────────────────────────────────────────┤
...
- **BR-001**: `ruleId`/`ruleSetId` 는 immutable·unique(고객사·서비스 내). ARCHIVED 룰 식별자도 재할당 금지 → `FDS-VALIDATION-001`(중복 룰 식별자). 표시명 변경은 `name`(displayName) 사용.
...
```
┌─ 기준값 빠른 변경 — [CRYPTO_RISK]  [고객사: 은행 A] ──────────┐
...
차단/허용/감시/뮬 네트워크 명단은 회원 외에도 계좌·수단·단말기·IP·상대방·가맹점·셀러 식별자를 차단/허용/감시합니다(`§10.1` Group feature). 탐지 시 대상이 자동 등록되며, 운영자가 수동 등록·해제·연장합니다.
...
| 종류(표시 분류) | 회원 / 계좌 / 수단 / 단말기 / IP / 이메일 / 상대방 / 가맹점 / 셀러 — **화면 표시 분류 9종**. 저장 멤버 종류(`member_kind`)는 **3종 `SUBJECT/INSTRUMENT/COUNTERPARTY`**(DB §5·API §5.10 정본)로 환원되며, 계좌·수단은 `INSTRUMENT`, 회원·단말기·가맹점·셀러는 `SUBJECT`, IP·이메일·상대방은 `COUNTERPARTY`로 매핑(`§7.3` 주석) |
...
│ 그룹 종류 *     [계좌 ▼] (회원/계좌/수단/단말기/IP/이메일/상대방/가맹점/셀러)│
│ 용도 *          (●) 차단  ( ) 허용  ( ) 감시  ( ) 뮬 네트워크             │
│ 등록 방식 *     (●) 수동 등록           ( ) 자동(룰 hit 시)              │
│ 룰 hit 자동등록 [✓] 룰 탐지 시 대상을 이 그룹에 자동 추가                 │
│ 기본 만료(일)   [90]   비우면 영구(자동 해제 안 함)                       │
│ 설명            [대포통장 의심 계좌 차단 그룹_____________]                │
│                                                       [취소] [등록]      │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| `groupType`(종류, 화면 표시 분류) | 회원/계좌/수단/단말기/IP/이메일/상대방/가맹점/셀러 — 화면 표시 분류 9종. 저장 멤버 종류 `member_kind`는 **3종**(`SUBJECT/INSTRUMENT/COUNTERPARTY`, DB §5·API §5.10)으로 환원(계좌·수단→`INSTRUMENT`, 회원·단말기·가맹점·셀러→`SUBJECT`, IP·이메일·상대방→`COUNTERPARTY`) (API §5.18·§5.19 정본) |
| `usage` | 차단(DENYLIST) / 허용(ALLOWLIST) / 감시(WATCHLIST) / 뮬 네트워크(MULE_NETWORK) — `risk_group_type` 정본(DB §4.14)에서 화면 노출 4종으로 사용. `RiskGroupType` API §10 OpenAPI enum은 6종(`BLACKLIST/WHITELIST` 포함)이나, **`BLACKLIST/WHITELIST`는 레거시 호환 enum으로 신규 화면 미노출**(그룹 코드 접두로만 사용) |
| `source` | 수동(MANUAL) / 자동(AUTO) |
| `autoEnrollOnHit` | 룰 탐지 시 대상 자동 추가 여부 |
| `defaultExpiryDays` | 등록 멤버 기본 만료 일수(비우면 영구) |
| `description` | 그룹 용도 설명 |

#### 비즈니스 규칙

- **BR-001**: `groupId` 는 immutable·unique(고객사 내). 생성 후 변경·중복 차단 → `FDS-VALIDATION-001`(중복 그룹 ID). 비활성은 멤버 0 확인 후 가능.
- **BR-002**: `groupType`(종류) 은 생성 후 변경 불가(멤버 식별자 체계 상이). 변경 필요 시 새 그룹 생성.
...
> 결과 = 거래가 어떻게 처리됐는지(통과/차단/추가인증/보류). 동작 = 그 룰의 조치 유형. 한 거래가 여러 룰에 걸리면 여러 행으로 표시(거래 1건 ≠ 결정 1건).

#### 데이터 항목

| 컬럼(표시) | 설명 |
|------|------|
| 처리시각 | 결정 생성 시각(`created_at`) |
| 룰 번호 | 결정을 만든 룰(`matched_rules`) |
...
| 영역 | 항목 |
|------|------|
| 판정 | 추적 번호, 룰 번호 + 버전, 동작, 결과, 처리시각, 처리 시간, 평가 방식(즉시/사후) |
| 판정 근거 | 집계값·기준값·충족 조건(`matched_rules` 스냅샷), 모델 점수·설명(외부 모델 제공/내장 소관) |
| 대상 거래 | 원본 거래/이벤트 링크(SFDS-EVT), 소스 시스템, 거래 유형, 금액/통화, 거래 상태 |
| 후속 조치 | 자동 그룹등록·자금보류/동결·케이스 개설·규제 보고 생성 여부 + 링크 |

#### 비즈니스 규칙

- **BR-001**: 판정 근거로 "왜 차단/탐지되었는지"를 사람이 읽도록 설명(감사·고객대응 핵심). 기준값·집계값·조건 충족 결과 표시.
- **BR-002**: ML 점수는 외부 모델 제공/내장 소관이며, 본 화면은 수신된 점수·설명만 표시(`§19` D-05). 점수 산출 로직은 표시 안 함.
- **BR-003**: 추적 번호로 거래 시점 응답과 연결. 내부 정보(룰 번호·기준값)는 본 화면에서만 조회.

### 8.3 SFDS-DEC-003 · Subject(대상) 타임라인 (조사)

| 항목 | 내용 |
|------|------|
| **기능 ID** | SFDS-DEC-003 |
| **권한** | `SFDS_DECISION:READ` |
| **API** | `GET /api/v1/evidence/fds/cases/{caseId}/timeline` (대상 기준 조회는 decision/case 목록 필터 + timeline 머지) |

#### 화면

대상(Subject) 1명의 이벤트·결정·그룹등록·액션·케이스·규제 보고를 시간순으로 통합 표시(사기 조사·CS·컴플라이언스용).

```
subj_8f...  KYC 2단계 · 국내송금 회원                기간 [최근 30일 ▼]
06-06 11:02  [MULE_BANK] 거래차단 · 차단   계좌이체 송금요청 18,200,000원
06-06 11:02  명단 자동등록 (대포통장 의심, 만료 2026-09-04)
06-06 11:02  케이스 개설 (대포통장 조사, case_77f0)         [케이스로 이동]
06-04 09:14  [ATM_GEO] 추가인증 · 추가인증  ATM 출금 요청
06-02 13:20  단말기 변경 (기기 정보 변경 감지)
06-01 10:05  규제 보고 생성 (CTR, 영업일 누계 52만 페소)    [규제 보고로 이동]
```

#### 비즈니스 규칙

- **BR-001**: 이벤트 + 결정 + 그룹 등록/해제 + 액션 + 케이스 + 규제 보고를 단일 타임라인에 머지(시간 desc).
- **BR-002**: 각 항목은 상세 화면 딥링크. 조사 목적 조회는 감사 로그에 기록(누가 어떤 대상을 조회했는지).
- **BR-003**: 대상 식별자(`subjectRef`)는 토큰/해시 기반이며, 원문 회원정보는 고객사 소스 시스템 소관(본 화면은 FDS 판단 이력만).

---

## 9. 이벤트 조회

### 9.1 SFDS-EVT-001 · Canonical 이벤트 조회

| 항목 | 내용 |
|------|------|
| **기능 ID** | SFDS-EVT-001 |
| **권한** | `SFDS_DECISION:READ` |
| **API** | `GET /api/v1/fds/events/{eventId}` (목록은 decision/case 화면에서 역참조) |

#### 화면 레이아웃

```
┌──────────────────────────────────────────────────────────────────────────┐
│ Canonical 이벤트 조회  [고객사: 은행 A ▼]                                  │
├──────────────────────────────────────────────────────────────────────────┤
│ [소스 시스템 ▼] [이벤트 종류 ▼] 대상 [____] 거래 [____] 수단 [____]  🔍  │
├──────────────────────────────────────────────────────────────────────────┤
...
`fds_canonical_events` 정규화 스키마(`§8.2`, `§14.2`): tenantId, sourceSystem, schemaVersion, eventId, idempotencyKey, eventType, occurredAt, subjectRef/actorRef, transactionRef/instrumentRef/counterpartyRef, channelType/paymentRail, amount/currency/amountBase/baseCurrency, payloadHash, canonicalPayload(JSONB).

#### 비즈니스 규칙

- **BR-001**: 필터 `소스 시스템 / 이벤트 종류 / 대상 / 거래 / 수단`. 검색 결과 1만 건 초과 시 팝업 노출 — 기간/조건 좁히기 안내 후 재조회.
- **BR-002**: 이벤트 → 해당 이벤트로 생성된 결정 목록(SFDS-DEC) 역참조 링크.
- **BR-003**: PII(IP·계좌해시·이메일 등)는 BE 마스킹(앞3+뒤4). raw payload 는 원칙적 미저장(`§13.3`, `§16.1`) — 저장된 경우에도 원문은 break-glass + 감사 기반 조회.
- **BR-004**: 검증 실패(reject) 이벤트는 별도 필터로 조회 가능(스키마/서명 실패 원인 표시). 검증 실패 이벤트는 정규화되지 않으므로 payloadHash 와 실패 사유만 보존.
...
탐지 결정에 따른 외부 조치는 고객사 시스템의 **Capability** 에 맞춰 발행됩니다(`§5.3`, `§9.4`). 모든 시스템이 hold/release 를 지원하지 않으므로 "하고 싶은 조치"와 "그 시스템에서 가능한 조치"를 분리합니다.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 액션 아웃박스  [고객사: 은행 A ▼]      발행 1,204 성공 1,202 실패 2 DLQ 0 │
├──────────────────────────────────────────────────────────────────────────┤
...
| 조치 종류 | 승인 거부 / 거래 차단 / 자금 보류 / 보류 연장 / 보류 해제 / 거래 취소 / 거래 역행 / 계정 정지 / 수단 정지 / 정산 보류 / 셀러 지급중지 / 유보금 증액 / 증빙 요청 / 명단 추가 / 케이스 개설 / 알림 발송 (`§11.2`) |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ Capability 매트릭스  [고객사: 은행 A ▼]                       [변경 결재] │
├──────────────────────────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 케이스 목록  [고객사: 은행 A ▼]      진행 88  기한 임박 3  기한 초과 0     │
├──────────────────────────────────────────────────────────────────────────┤
...
| 대상 | subject 또는 actor(`subject_ref`, 내부 감사는 actor) |
...
| **API** | `GET /api/v1/fds/cases/{caseId}` · `/events` · `PATCH /api/v1/fds/cases/{caseId}` · `POST .../assign` · `.../close` 🔒(내부감사·규제) · `.../feedback` |
...
│                                                                            │
│ 담당자 [analyst.a ▼]  분류 [사기 확정 / 오탐 / 추가조사 ▼]               │
│ 종결 사유 [_______________________________]                              │
│         [배정] [코멘트] [규제 보고 전환] [종결 상신]  [종결 승인(2인)]    │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 코멘트 | 조사 코멘트 이력(작성자·시각) |
| 종결 | 담당자 배정, 분류(사기 확정/오탐/추가조사), 종결 사유 |

#### 비즈니스 규칙

- **BR-001**: 케이스 종결은 **`SFDS_CASE:APPROVE` 4-eyes** (작성자 ≠ 승인자, 내부 감사 case 종결 포함 `§11.4`). 종결 상신 → 다른 승인자 종결.
- **BR-002**: "오탐" 종결은 룰 hit rate 오탐 피드백(SFDS-DASH-002)에 반영 — 룰 튜닝 근거.
...
- **BR-004**: 모든 배정·코멘트·전환·종결 이력은 감사 로그 7년 보존.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 결재함 (maker-checker)  [고객사: 은행 A ▼]                  관리자 admin ▼ │
├──────────────────────────────────────────────────────────────────────────┤
│ [유형 ▼] [상태 ▼] [상신자 ▼]                              🔍 대상         │
├──────────────────────────────────────────────────────────────────────────┤
...
|------|------|
| 결재 종류 | 결재 대상 작업 유형 — 케이스 액션 / 룰 활성화·롤백 / 필드매핑 변경 / 자격증명 회전 / 명단 멤버 / 증적 export 최종본 / high-risk 가맹점 정상화 / 케이스 종결 (`subject_kind` **8종** `ACTION/RULE/MAPPING/SECRET/GROUP/EXPORT/MERCHANT_NORMALIZE/CASE_CLOSE`, DB §5.23·API §5.12) |
...
- **BR-001**: 필터 `유형(subject_kind 8종) / 상태(approval_status 8종) / 상신자` + `대상` 검색. 행 ▶ 펼침 시 결재 단건(`payload_hash`·결재 단계·만료 시각·최대 실행 횟수)을 조회.
...
- **BR-006**: 고객사·서비스 데이터스코프가 적용되어, 운영자는 자기 스코프의 결재 요청만 조회·결재할 수 있다.
...
한국 Policy Pack(`§16.2`)을 기본으로 STR/CTR/Travel Rule 후보를 자동 생성합니다. 고객사별 관할에 따라 양식·트리거가 plugin 으로 적용됩니다(`§16.4`).
...
> **PEP·제재대상(SANCTION) 책임 경계**: 정치적 주요인물(PEP)·제재대상 명단은 외부 명단 DB 대조가 필요한 항목으로 **외부 스크리닝(고객사 KYC/WLF) 소관**입니다. FDS 는 그 결과 reference 만 평가 입력으로 사용하며, STR 사유 카탈로그에는 포함하지 않습니다(`§5.2`).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 규제 보고 큐  [고객사: 은행 A ▼]    기한 임박 12  24h내 2  기한 초과 0     │
├──────────────────────────────────────────────────────────────────────────┤
...
- **BR-005**: 적용 보고 유형은 고객사 Policy Pack 에 따라 달라짐(한국 기본 CTR/STR + Travel Rule 옵션). 미적용 유형은 큐에 표시 안 함.
...
- **BR-006**: 제출 양식·기한은 고객사 관할(Policy Pack)별로 달라짐 — 한국 기본 외 관할은 plugin 으로 추가(`§16.4`).
...
|------|------|
| **기능 ID** | SFDS-EXP-001 |
| **권한** | 추출 요청 `SFDS_*:EXPORT` (스코프) / **최종본(제출용)은 4-eyes** `SFDS_REG:APPROVE`(컴플라이언스 책임자) 🔒 / 다운로드 `SFDS_*:EXPORT` |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ Evidence Export (검사대응)  [고객사: 은행 A ▼]              관리자 admin ▼ │
├──────────────────────────────────────────────────────────────────────────┤
...
- **BR-005**: [다운로드]는 감사 로그(SFDS-AUDIT-001)에 기록한다. 고객사·서비스 데이터스코프가 적용되어 자기 스코프 데이터만 추출할 수 있다.
...
- **BR-004**: 고객사 데이터스코프 적용 — 고객사 관리자는 자기 고객사 로그만, 플랫폼 운영자는 횡단 조회 가능.
- **BR-005**: 조회 결과 1만 건 초과 시 팝업 노출 — 기간/조건 좁히기 안내 후 재조회.
...
| SFDS-TNT-002 (탭⑤) | 고객사 상세 — 알림·소스 | `GET /api/v1/bo/fds/tenants/{id}` · `GET /api/v1/admin/fds/source-systems` (bo-api 소유) | bo-api | T-03·T-20 |
| SFDS-TNT-003 | 고객사 등록(별도 생성 화면, 상세 5탭과 분리) | `POST /api/v1/bo/fds/tenants` (bo-api 소유) | bo-api | T-03·P8 |
| SFDS-CONN-001/002 | 소스시스템·커넥터 목록·운영 | `GET /api/v1/admin/fds/source-systems`, `/connectors`, `POST .../connectors/{id}/replay` (API §4.8 정본) | fds-svc | T-03·T-20·T-21 |
...
| Role | 플랫폼 대시 | 고객사 대시 | 고객사 관리 | 커넥터 운영 | 매핑 결재 | 룰 작성 | 룰 결재 | 그룹 운영 | 그룹 생성 | 결정/케이스 | 액션 운영 | Capability 결재 | 규제 결재 | 감사 |
|------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| `SFDS_PLATFORM_OPS` | ✓ | ✓ | | | | | | | | ✓ | ✓ | | | ✓ |
| `SFDS_PLATFORM_ADMIN` | ✓ | ✓ | ✓ | ✓ | ✓ | | ✓ | | ✓ | ✓ | ✓ | ✓ | | ✓ |
| `SFDS_VIEWER` | | ✓ | | | | | | | | ✓(R) | | | | ✓ |
...
> **플랫폼 운영자**(`SFDS_PLATFORM_*`)만 전체 고객사 횡단 조회·고객사 관리. 나머지 Role 은 자신의 고객사 스코프 안에서만 작동. 고객사 등록/설정·격리·리전은 `SFDS_PLATFORM_ADMIN` 전용. 커넥터 secret·필드매핑·Capability·규제 보고·케이스 종결은 모두 4-eyes(작성자≠승인자).

### 16.3 백오피스 주요 에러 코드

| Code | HTTP | 화면 처리 |
|------|------|-----------|
| `FDS-VALIDATION-001`(중복 ID) | 400 | 고객사 ID 중복 — 다른 ID 입력 안내 |
| `FDS-NOT-FOUND` | 404 | 목록 새로고침 안내 |
| `FDS-STATE-CONFLICT` | 409 | 동시 수정 — 최신 버전 재조회 |
| `FDS-APPROVAL-REQUIRED` | 409 | 결재 미완 — 활성화 차단 |
| `FDS-VALIDATION-001`(중복 룰 식별자) | 400 | 룰 번호 중복(고객사 내) — 다른 번호 |
...
|---|------|------|--------------|
| D-01 | 배포 모델 (`deployment_model`: `MANAGED_DEDICATED`/`SELF_HOSTED`/`SHARED`) | 고객사별 전용 배포 기본(매니지드 전용), 소규모만 공유 | 고객사 등록(SFDS-TNT-003)에서 **배포 유형 선택** + 온보딩 상태(`onboarding_status` 8종, 읽기). 실제 격리는 화면 즉석 선택이 아니라 **온보딩 프로비저닝의 산출**(인프라 참조·리전 읽기 전용). 변경 시 재배포·마이그레이션 (`§1.7`·target-architecture §4.1) |
...
| D-06 | raw payload (미저장 / 암호화 / 고객사별) | **미저장 기본**(`payload_hash`만) | 이벤트 조회(SFDS-EVT)·매핑(SFDS-MAP) 마스킹·미저장 표시 |
...
| **D-14** | 실시간 판단 API 장애 정책 | domain별 tenant policy(`fail_policy`) | 커넥터/고객사 설정의 **장애정책**(`FAIL_CLOSED`/`FAIL_OPEN`/`CASE_ONLY`) 표시·결정 화면 상태 반영 |
...
결재함(SFDS-APPR-001)에서 일괄 관리. `fds_approval_requests.subject_kind` **8종**(`ACTION/RULE/MAPPING/SECRET/GROUP/EXPORT/MERCHANT_NORMALIZE/CASE_CLOSE`, `CASE_CLOSE`=case 종결 4-eyes(대상=`fds_cases.case_id`), DB §5.23·API §5.12 정본) · `approval_line` **6종**(`SELF_APPROVAL_DISABLED/MAKER_CHECKER/COMPLIANCE_MANAGER/RISK_MANAGER/SECURITY_ADMIN/EXECUTIVE_APPROVAL`, DB §4.12) · `approval_status` 8종(`DRAFT/SUBMITTED/APPROVED/REJECTED/CANCELLED/EXPIRED/EXECUTED/EXECUTION_FAILED`). payload는 `payload_hash`로 고정되며 승인 후 변경 시 무효(`FDS-APPROVAL-PAYLOAD-CHANGED`). 상신자(maker)≠승인자(checker), AI agent는 maker만 가능.
...
> 본 기능정의서(v3.1)는 설계서 `docs/software/01-fdsSvc-sass.md` v1.5를 정본으로 삼아 DB(`01-fds-db.md` v1.3)·API(`01-fds-api.md` v1.5)·integration(`01-fds-integration.md` v1.1)·tasks(`docs/tasks/fds/`)와 100% 동기화한 백오피스(Admin Console) 화면 정의서이다. 룰 엔진 내부 동작·DSL 평가 로직·DB DDL 상세·멀티테넌시 인프라는 설계서·DB 설계서를 정본으로 한다. 짝 문서 `docs/plan/BO-FDS-SASS-Planning_v4.6.pptx`(SFDS-TNT-002 5탭·SFDS-RULE-002 5탭·SFDS-CASE-002 4탭 탭 연속 전개 + 드릴다운 진입 트리거 배너, 45슬라이드)는 본 문서의 기능 ID·권한·BR·표시 용어를 1:1 기준으로 한다.
