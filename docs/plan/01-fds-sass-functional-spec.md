...
| 컬럼(표시) | 설명 |
|------|------|
| 거래번호·내역 | `transactionRef`(mono, 첫 컬럼) + 보조줄에 채널(hanpass-ph 5채널 — 월렛충전/국내송금/해외송금/월렛결제/ATM출금)·금액(`amount`/`currency`)·회랑(`send_country`→`receive_country`). cross-border는 base 통화 USD(`amount_base`)·AML 비교는 PHP 환산(`phpEquivalent`) 맥락. hanpass-ph 데이터 레이어 필드(DB §5.5) |
