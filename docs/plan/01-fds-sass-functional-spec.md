...
| 버전 | 일자 | 작성자 | 변경 내역 |
|------|------|--------|----------|
| **6.12** | **2026-07-08** | **SM Kim** | **§1.8 표시 라벨 정본 — bo-web 전 화면 다국어(i18n) 카탈로그 정본화(코드=truth, bo-web next-intl).** bo-web 이 전 화면 다국어(한국어(`ko`, 기본)·English(`en`))를 갖춤에 따라 **화면 표시 라벨의 정본 = bo-web i18n 카탈로그 키(ko/en)** 를 §1.8 로 신설: 로케일 base(`messages/{ko,en}.json`) + FDS 도메인 번들(`messages/bundles/{ko,en}/fds.json`, 네임스페이스 `fds`) 얕은 병합, ko↔en 키 집합·ICU placeholder 패리티 게이트(`catalog-parity.test.ts`). **하드코딩 금지·래칫(ratchet) 스캔 게이트** — 주석 제외 한글 리터럴 스캔(`hardcoded-scan.test.ts`) + 축소 전용 allowlist(`i18n-baseline.json`), 신규/회귀 하드코딩은 빌드 게이트 실패. 활성 로케일은 쿠키(`NEXT_LOCALE`) 단독·URL 불변, 언어 선택기 명세는 IAM 정의서(FS-BO-IAM-001) §6. 본 문서 "표시(한국어)" 값=ko 카탈로그 1:1·en 은 카탈로그 정본. | 근거=bo-web `i18n/{locales,request}.ts`·`messages/{ko,en}.json`·`messages/bundles/{ko,en}/fds.json`·`messages/{assemble.ts,catalog-parity.test.ts,scan-hardcoded.ts,hardcoded-scan.test.ts,i18n-baseline.json}`·`components/common/LanguageSwitcher.tsx`. 코드=truth. 화면 수·기능·PPT 불변. |
...
### 1.8 표시 라벨 정본 — bo-web 다국어(i18n) 카탈로그 (v6.12 신설)

백오피스 전 화면이 다국어(한국어(`ko`, 기본)·English(`en`))를 지원하며, 화면 표시 라벨(메뉴·컬럼 헤더·버튼·상태/enum 표시값·안내문·빈 상태 문구)의 **정본은 본 문서의 한국어 서술이 아니라 bo-web next-intl 메시지 카탈로그 키**입니다.

- **카탈로그 구성**: 로케일 base(`messages/{ko,en}.json` — 셸 네임스페이스 `app`·`common`·`nav`·`enum`·`error`·`header`·`language`·`login`·`shell` 등) + 도메인 번들 `messages/bundles/{ko,en}/fds.json`(최상위 네임스페이스 `fds`) 의 얕은 병합(shallow merge). 번들 네임스페이스는 base·형제 번들(`commonUi`/`fds`/`amlMon`/`amlCust`/`admin`)과 상호 배타여서 병합이 무손실입니다.
- **패리티 게이트**: `messages/catalog-parity.test.ts` 가 ko↔en **키 집합 동일**·키별 **ICU placeholder 일치**·**네임스페이스 배타**를 테스트 게이트로 강제합니다 — 한 로케일에만 존재하는 키는 빌드 게이트 실패.
- **활성 로케일(URL 불변)**: `[locale]` URL 세그먼트 없이 쿠키(`NEXT_LOCALE`, 기본 한국어(`ko`))를 서버가 매 요청 읽어 로케일을 결정합니다(`i18n/request.ts`). 헤더·로그인의 언어 선택기 명세는 IAM 정의서(FS-BO-IAM-001) §6 공통 셸 참조.
- **하드코딩 금지(래칫(ratchet) 스캔 게이트)**: 컴포넌트/화면 코드에 한국어 UI 리터럴 직접 기입을 금지합니다. `messages/hardcoded-scan.test.ts` 가 주석/JSDoc 제거 후 한글(`[가-힣]`) 리터럴을 전 소스에서 스캔하여, 잔여 미이관 파일 allowlist(`messages/i18n-baseline.json`)에 없는 **신규 하드코딩·이관 완료 파일의 회귀를 게이트 실패**로 차단합니다. baseline 은 **축소 전용(래칫)** — 이관이 끝나 깨끗해진 파일은 목록에서 제거해야 하며(잔존 시 게이트 실패), 신규 추가는 금지입니다.
- **본 문서와의 관계**: 본 문서 각 표의 "표시(한국어)" 값은 ko 카탈로그 값과 1:1 이며, 영어 표기는 en 카탈로그가 정본입니다(문서에 en 병기하지 않음). 표시 문구 변경은 카탈로그 키 값 변경(코드=truth) → 본 문서 역전파 순서를 따릅니다.
