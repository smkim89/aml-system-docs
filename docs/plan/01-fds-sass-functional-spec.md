...
| **6.26** | **2026-07-20** | **SM Kim** | **SFDS-GRP-002 멤버 add/remove/extend 4-eyes 폐지·즉시 반영 역전파(코드=truth, fix/fds-risk-group-member-immediate-apply — 사용자 확정 방향).** §7.2 API 셀에서 🔒 제거·"즉시 반영(200, `GroupMemberMutationResponse`)" 명시, 동작에 결재 없이 즉시 mutation·`GROUP_UPDATE` 감사 서술 추가. BR-001을 중복=409 → **upsert(만료/식별자 종류 갱신) 무충돌**로 정정, BR-003(self-service 방지)을 폐지 표기, BR-006을 레거시 in-flight 결재 실행 한정으로 조정. §16.1 매핑표 SFDS-GRP-002 행 🔒 제거, §16.5 4-eyes 표 명단 멤버 add/remove 행을 폐지 취소선 표기로 갱신. 그룹 master(rename/비활성, SFDS-GRP-003)·merchant 정상화 4-eyes는 무변경. | 근거=FDS API §4.7/§5.10/§8(v4.20), fds-svc `RiskGroupAdminService`(addMember/removeMember)·`RiskGroupAdminController`·`GroupMemberMutationResponse`. `docs/qa/engine-rule-cases.md` FDS-C33 참조. |
...
| **6.7** | **2026-06-27** | **SM Kim** | **회원번호·거래번호 식별자 일관 노출(코드 정합) — TM·FDS 필수 식별자.** §8.1 SFDS-DEC-001: 탐지 결정 목록 첫 컬럼을 **「거래번호 · 회원 · 내역」**으로 보강 — 거래번호(mono) + **회원(SubjectKey, 360 딥링크)** 서브라인 추가(`Decision.subjectRef`). §11.1 SFDS-CASE-001: 케이스 목록 대상 컬럼을 **「회원 · 거래번호」**로 보강 — 회원 SubjectKey + **거래번호** 서브라인(`FdsCase.transactionRef`). 전부 마스킹 토큰(subj_***/txn-***), 프론트 공통 `SubjectTxnCell` 재사용. 판정·케이스 처리·결재 무변경. | 근거=bo-web `FdsDecisionInvestigation`·`FdsCaseManagement`·`SubjectTxnCell`. |
| **6.6** | **2026-06-27** | **SM Kim** | **라이브 결정 → 케이스 자동 개설 폐루프(코드 정합) — 심각 판정이 케이스로 분류.** §8 결정 → §11 케이스: 비-prod 라이브 인입 결정 중 **`recommendedActions`에 `OPEN_CASE`가 추천된 판정(BLOCK/HOLD/REVIEW/REPORT)**이 케이스를 자동 개설해 SFDS-CASE-001 목록에 실시간 반영됨을 명문화(ALLOW·CHALLENGE·검증실패는 미개설). 케이스는 결정과 양방향 연결(`decision.caseId ↔ case.originDecisionId`), 상태머신 §1.6.1 `[*]→OPEN`(자동 개설은 항상 OPEN·미배정 진입), priority=BLOCK→CRITICAL·HOLD/REPORT→HIGH·REVIEW→MEDIUM. caseType는 발동 룰 기반 파생(정본 case_type DB §4.10 11종만): MULE_BANK→`MULE_ACCOUNT_REVIEW`·TRAVEL_RULE_MISSING→`REGULATORY_REPORT`·CRYPTO_RISK→`CRYPTO_TRAVEL_RULE`·그 외→`FRAUD_REVIEW`. 최근 일정 건수만 링버퍼 유지(시드 케이스 보존)·엔진 무변경·prod fail-closed. | 근거=bo-api `FdsDecisionCaseStubService.openLiveCase`·`caseTypeFor`. |
| **6.5** | **2026-06-27** | **SM Kim** | **탐지 결정 평가가 룰의 설정 임계(`thresholdParams.score`)로 구동(코드 정합) — 룰 관리 단일 정본.** §6 룰 관리: 비-prod 라이브 결정 평가기가 전역 상수(80) 대신 **각 룰에 설정된 `thresholdParams.score` 값**으로 발동을 판정하도록 명문화 — 룰 관리(SFDS-RULE-002 기준값 탭)에서 임계를 편집(PUT)하거나 룰을 비활성하면 **탐지 결정에 즉시 반영**(provider 패턴, 가변 룰 상태 노출). 채널의 ACTIVE 룰 중 이벤트 점수 ≥ 룰별 임계인 룰만 후보, 임계가 가장 높은(엄격한) 룰 발동, 미달 시 정상 통과(ALLOW). 데모 룰 카탈로그 기본 임계(MULE_BANK 70·ATM_GEO 75·CRYPTO_RISK 80·VELOCITY_24H 65·TRAVEL_RULE_MISSING 60·TBML_INV 85[DISABLED]) — RULE-002 빌더 예시 "80점 이상"은 기본 시드값. DISABLED 룰 미발동·발동 룰은 모두 /fds/rules 존재(불변). 엔진 무변경·비-prod stub·prod fail-closed. | 근거=bo-api `FdsActiveRuleProvider`·`FdsRuleCatalog.scoreThreshold`·`FdsDecisionCaseStubService.ingestLiveDecision`. |
...
본 문서는 **hanpass-ph FDS Platform**(설계서 `docs/software/01-fdsSvc-sass.md` v1.1)의 **백오피스(Admin Console) 관리·운영** 기능에 대한 기능정의서입니다. 대상 시스템은 **hanpass-ph**(KR→PH 송금 + 국내송금 + 월렛 플랫폼)의 FDS RegOps 백오피스로, hanpass-ph의 **회원·해외송금(KR→PH)·국내송금·월렛충전·월렛결제·ATM출금** 거래 이벤트(트랜잭션 마이크로서비스 `member-svc`·`remit-svc`·`domestic-svc`·`walletchg-svc`·`wallet-svc`·`tx-history-svc`·`inbound-svc`)를 연동하면 실시간 또는 비동기로 이상거래를 탐지·조치합니다.
>
...
1. **모니터링** — hanpass-ph ingest/decision/action/case 현황, 커넥터 지연, 스키마 검증 실패, SLA
2. **연동 관리** — 서비스 설정(격리·리전·보존·마스킹·Policy Pack), 소스 시스템·커넥터(REST Push/Queue/Polling/Snapshot/CDC), 스키마·필드 매핑·PII 정책
...
| `fds_business_documents` / `fds_commerce_orders` / `fds_settlements` | 상업 증빙·주문·정산(Phase 7 commerce/trade 스키마 — DB·엔티티는 존재하나 **hanpass-ph 운영 미사용**) | `tenant_id`, `*_ref` |
...
AML/FDS는 고객 PII·규제·내부보안 요건상 **서비스별 전용 배포가 기본**입니다(공유 SaaS DB가 기본이 아님 — 정본 `target-architecture §4.1`, 설계서 §13.0). 본 시스템의 운영 테넌트는 **hanpass-ph(`tenant_demo`) 단일**이나, 엔진은 멀티테넌시 키 3종을 그대로 보유하므로(코드 truth) 본 절은 그 구조를 유지·서술합니다.

**계층(정본)**: 시스템을 납품받은 **기관(institution)** [상위, 배포·계약 주체 — 여기서는 hanpass]이 운영·연동하는 **서비스(=테넌트, hanpass-ph)** 가 격리 경계이며, 그 서비스 내부의 세부 환경/구분은 **워크스페이스**(리테일/기업, 운영/샌드박스 등)로 나뉩니다 — **기관 → 서비스(테넌트) → 워크스페이스**. 화면 라벨은 "서비스"·"워크스페이스"이며, 내부 코드(`tenant_id`·`Tenant-Id`·RLS·scope 이름)는 의미만 "서비스"로 두고 그대로 유지합니다. 멀티테넌시 키 3종(`tenant_id` / `workspace_id` / `data_scope`)의 의미를 전용 배포 기준으로 정의합니다.

| 키 | 의미(재정의) | 예시 | BO 반영 |
|------|------|------|---------|
| **`tenant_id`** | **배포의 서비스(테넌트)**. 전용 배포(매니지드 전용/자체 인프라 설치형)에서는 **배포=서비스이므로 단일 값**. 운영 대상은 hanpass-ph(`tenant_demo`) 단일 | `tenant_demo`(hanpass-ph) | 전용 배포는 배포 엔드포인트 단위 라우팅(헤더 상수). SHARED만 `Tenant-Id` 헤더 행 라우팅. 화면 상단 서비스 배지 |
| **`workspace_id`** | 그 서비스의 **워크스페이스(세부 환경/구분)** | `retail`/`corporate`, `prod`/`sandbox`, 미지정 `default` | 룰셋·커넥터·케이스 큐·결재 라인을 워크스페이스 단위 분리. 미지정 시 `default`. `Workspace-Id` 헤더 |
| **`data_scope`** | 운영자 row-level 가시 범위(**권한 필터**) | `channel=DOMESTIC_REMIT`, `region=KR`, `corridor=KR-PH` | 저장 격리가 아니라 **조회·조치 권한 필터**. bo-api가 운영자 토큰의 data-scope 집합으로 fds-svc 조회를 강제 IN 필터링 |
...
### 2.1 SFDS-DASH-001 · 플랫폼 운영 대시보드 (hanpass-ph)
...
| **목적** | hanpass-ph 서비스(워크스페이스 횡단)의 ingest/decision/action/case 건전성과 커넥터 lag·SLA를 단일 화면에서 모니터링 (플랫폼 운영자 전용) |

#### 화면 레이아웃

```
┌──────────────────────────────────────────────────────────────────────────┐
│ 플랫폼 운영 대시보드  (hanpass-ph)         기간: [최근 24시간 ▼]          │
├──────────────────────────────────────────────────────────────────────────┤
│ ┌─ 수신 이벤트 ───────┐ ┌─ 결정 ────────────┐ ┌─ 조치·케이스 ────────┐    │
...
│                                                                            │
│ [ 플랫폼 알림 ]                                                            │
│  • [remit-svc] 커넥터 지연 312초 — 임계 초과                  [커넥터]  │
│  • [domestic-svc] 스키마 검증 실패 4,201건 — 매핑 점검 필요   [매핑]    │
│  • [walletchg-svc] 액션 발행 실패 큐 적체 58건                [아웃박스] │
│  • 규제 보고 제출 기한 임박 6건                               [규제 큐]  │
│                                                                            │
│ ┌─ 워크스페이스별 건전성 (hanpass-ph) ──────────────────────────────────┐ │
│ │ 워크스페이스│ 수신     │ 검증실패 │ 커넥터lag │ 액션실패 │ 케이스SLA │ │
│ │ ───────────┼──────────┼──────────┼───────────┼──────────┼───────────│ │
│ │ retail/prod│ 2.1M     │ 4,201 ⚠  │ 12초      │ 0        │ 정상      │ │
│ │ corp/prod  │ 1.4M     │ 0        │ 312초 ⚠   │ 2        │ 정상      │ │
│ │ sandbox    │ 0.6M     │ 0        │ 8초       │ 58 ⚠     │ 임박 3    │ │
│ │ ...        │          │          │           │          │           │ │
│ └──────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 워크스페이스별 건전성 | 워크스페이스(retail/corporate·prod/sandbox), 수신, 검증실패, 커넥터lag, 액션실패, 케이스SLA | 워크스페이스별 집계 |

#### 비즈니스 규칙

- **BR-001**: 기간 필터는 `최근 1h / 24h / 7d / 30d` 프리셋 + 커스텀 범위.
- **BR-002**: 플랫폼 알림의 각 항목은 해당 상세 화면(서비스 대시보드·커넥터·매핑·아웃박스·규제 큐)으로 딥링크. 대시보드는 read-only.
- **BR-003**: 워크스페이스별 건전성 행 클릭 → 서비스별 상세 대시보드(SFDS-DASH-002). 임계 초과 셀은 ⚠ + 색상 강조.
- **BR-004**: 집계는 30~60초 캐시 허용(실시간 정합성보다 가용성 우선), 마지막 갱신 시각 표기.
- **BR-005**: 본 화면은 **플랫폼 운영자 전용**. 서비스 관리자는 자기 워크스페이스만 보는 SFDS-DASH-002로 진입.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│서비스 대시보드 [서비스: hanpass-ph ▼][워크스페이스: retail ▼] 기간:[24시간 ▼]│
├──────────────────────────────────────────────────────────────────────────┤
│ ┌─ 수신 상태 ─────────┐ ┌─ 결정 추이 ───────┐ ┌─ 케이스 SLA ─────────┐   │
...
│                                                                            │
│ ┌─ 커넥터 상태 ───────────────────┐ ┌─ 액션 실패 큐 ──────────────────┐ │
│ │ remit-svc      정상  lag 12초   │ │ 발행 1,204 / 성공 1,202 / 실패 2│ │
│ │ domestic-svc   정상  lag 8초    │ │ 재시도 대기 2  미처리(DLQ) 0    │ │
│ │ walletchg-svc  지연  lag 312초⚠ │ │                                 │ │
│ └─────────────────────────────────┘ └─────────────────────────────────┘ │
│                                                                            │
│ ┌─ 룰 hit rate / 오탐 ──────────────────────────────────────────────────┐ │
│ │ 룰 이름          │ 동작     │ 평가    │ 탐지 │ hit율 │ 오탐(피드백) │ │
│ │ ─────────────────┼──────────┼─────────┼──────┼───────┼──────────────│ │
│ │ PH 월렛충전 고액 차단│ 차단  │ 88,402  │ 142  │ 0.16% │ 12           │ │
│ │ PH ATM출금 추가인증 │ 추가인증│ 12,004  │ 31   │ 0.26% │ 5            │ │
│ └──────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│ ──────────────┼────────────┼────────────────┼───────────┼──────┼────────┼│
│ tenant_demo   │ hanpass-ph │ 매니지드 전용  │ 활성      │ 한국 │ 운영중 │▶│
├──────────────────────────────────────────────────────────────────────────┤
│ ◀ 1 ▶                                                   총 1건 (운영중 1) │
└──────────────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 서비스 상세  >  hanpass-ph (tenant_demo)                                    │
├─ [① 기본 정보] [② 배포·온보딩] [③ 마스킹·보안] [④ Policy Pack] [⑤ 알림·소스] ┤
│                                                                            │
│  서비스 ID    tenant_demo          (변경 불가)                          │
│  표시명       [hanpass-ph_________]  [편집]                                │
...
│  ─ 알림 채널 ──────────────────────────────────────────────────────────── │
│  유형           대상·주소                     이벤트                      │
│  Slack          #risk-hanpass-ph              케이스 신규·SLA 임박        │
│  이메일         ops@hanpass.example.com        정기 리포트·장애 알림       │
│  웹훅(Webhook)  https://hanpass.example/hook   FdsDecisionCreated          │
│                                                                            │
...
│ 서비스 등록 / 수정                                                        │
├──────────────────────────────────────────────────────────────────────────┤
│ 서비스 ID *     [tenant_demo____]  영문 소문자·숫자·_ (생성 후 변경 불가)│
│ 표시명 *        [hanpass-ph__________________]                            │
...
- **hanpass-ph 운영 채널 5종**: `CASH_IN`(월렛충전)·`DOMESTIC_REMIT`(국내송금)·`CROSS_BORDER_REMIT`(해외송금)·`WALLET_PAYMENT`(월렛결제)·`WALLET_WITHDRAWAL`(ATM출금). 화면 필터·룰 채널 스코프는 이 5종만 노출한다(bo-web `CHANNEL_OPTIONS` 정본). 정본 `channel_type` enum 자체(DB §4.4)는 21종 closed CHECK로 보존되나, 나머지(카드·PG·가상자산·무역·정산·B2B 등)는 Phase 7 미운영 채널이다. cross-border(`remit-svc`/`inbound-svc`)는 corridor(출발국/도착국·송금/수취 통화)와 base 통화 **USD**(remit `usd_amount`/`report_amount` 산출)를 정규 이벤트에 명시하며, hanpass AML 기준액 비교는 **PHP 환산값**(`transaction.phpEquivalent`, USD×56)으로 평가한다(DB §5.5·연동 §7.2).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 소스 시스템·커넥터  [서비스: hanpass-ph ▼]                       [+ 새 커넥터] │
├──────────────────────────────────────────────────────────────────────────┤
│ [연동 방식 ▼] [사용 ▼] [상태 ▼]                          🔍 소스시스템    │
├──────────────────────────────────────────────────────────────────────────┤
│ 소스 시스템   │ 연동 방식  │ 사용 │ 스키마 버전     │ 지연  │ 최근 오류 │ │
│ ──────────────┼────────────┼──────┼─────────────────┼───────┼───────────┼│
│ remit-svc     │ REST Push  │ ○    │ remit-svc.v1    │ 8초   │ —         │▶│
│ walletchg-svc │ REST Push  │ ○    │ walletchg-svc.v1│ 6초   │ —         │▶│
│ domestic-svc  │ REST Push  │ ○    │ domestic-svc.v1 │ 9초   │ —         │▶│
│ member-svc    │ REST Push  │ ○    │ member-svc.v1   │ 12초  │ —         │▶│
│ wallet-svc    │ REST Push  │ ○    │ wallet-svc.v1   │ 7초   │ —         │▶│
│ tx-history-svc│ REST Push  │ ○    │ tx-history-svc.v1│ 10초 │ —         │▶│
│ inbound-svc   │ REST Push  │ ○    │ inbound-svc.v1  │ 14초  │ —         │▶│
├──────────────────────────────────────────────────────────────────────────┤
│ ◀ 1 ▶                                                          총 7 (사용 7)│
└──────────────────────────────────────────────────────────────────────────┘
```
...
- **BR-006 (v6.16)**: 목록 상단 REST 인입 요약은 source system별 행이 아니라 단일 `POST /api/v1/fds/events` API 한 행으로 표시한다. `fds_canonical_events` accepted 거래 row(`transaction_ref IS NOT NULL`)의 `received_at` 실측으로 전체 24h 건수·마지막 수신·source별 60초 TPS 합계를 표시하고, metrics 응답 성공 여부를 API 상태로 분리한다. 멱등 replay/duplicate는 비가산하며 30초 자동 갱신한다.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ walletchg-svc 커넥터 (REST Push)  [서비스: hanpass-ph]   상태: 지연  [일시중지] │
├──────────────────────────────────────────────────────────────────────────┤
│ 연동 방식  REST Push       스키마 버전  walletchg-svc.v1                   │
│ 마지막 수신 2026-06-06T09:58:00Z   마지막 성공  2026-06-06 09:58           │
│ 지연       312초 ⚠   최근 오류  수신 지연 (서명 검증 후 재처리 대기)       │
├─ 처리 현황 (24시간) ───────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 커넥터 등록 / 수정  [서비스: hanpass-ph]                                       │
├──────────────────────────────────────────────────────────────────────────┤
│ 소스 시스템 *   [walletchg-svc______]  (생성 후 변경 불가)                │
│ 연동 방식 *     (●) REST Push  ( ) 큐  ( ) 폴링  ( ) CDC  ( ) 스냅샷       │
│ 스키마 버전 *   [walletchg-svc.v1 ▼]  (스키마 레지스트리 SFDS-MAP-001)    │
│ 사용            [✓] 활성                                                   │
│ ── REST Push 설정 ────────────────────────────────────────────────────────│
│ 엔드포인트      [/api/v1/fds/events]  인증 [API Key + HMAC]                │
│ ── 서명/시크릿 (변경은 2인 승인) ──────────────────────────────────────── │
...
- **구성**: 탭 `① 수신 API 카탈로그`/`② 인입 라이브 모니터링`.
  ...
  - **② 인입 라이브 모니터링**: 현행 지원 방식 REST 전용. `POST /api/v1/fds/events` 단일 논리 API 한 행에 API 상태·최근 24h accepted 고유 거래 전체 건수·전체 마지막 수신(n초/분 전)·source별 최근 60초 TPS 합계를 표시한다. source system은 동일 API 호출 주체이므로 API 행으로 노출하지 않는다. 집계는 `fds_canonical_events.received_at` + `transaction_ref IS NOT NULL`이고 tenant/workspace 범위를 강제한다. API 상태는 fds-svc metrics 응답 성공 여부이며 거래 미수신 freshness와 분리한다. queue/DLQ/backfill은 REST 실측 항목이 아니므로 운영 지표로 노출하지 않는다. 단일 API 행은 read-only이며 실제 connector 목록 행만 SFDS-CONN-002로 이동한다.
...
- **BR-005 (v6.16)**: bo-web query cache와 bo-api 45초 TTL cache는 `(tenantId, workspaceId)`를 cache identity에 포함한다. 플랫폼/서비스/워크스페이스 전환 시 이전 범위의 0건 또는 집계 snapshot을 재사용하지 않는다.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 스키마 레지스트리  [서비스: hanpass-ph ▼]                                      │
├──────────────────────────────────────────────────────────────────────────┤
│ [소스 시스템 ▼] [상태 ▼]                                  🔍 스키마 버전   │
├──────────────────────────────────────────────────────────────────────────┤
│ 스키마 버전        │ 소스 시스템  │ 상태   │ 검증 실패(24h)│ 적용 시작    │ │
│ ───────────────────┼──────────────┼────────┼───────────────┼──────────────┼│
│ remit-svc.v1       │ remit-svc    │ 운영중 │ 0             │ 2026-05-01   │▶│
│ domestic-svc.v1    │ domestic-svc │ 운영중 │ 0             │ 2026-01-01   │▶│
│ walletchg-svc.v1   │ walletchg-svc│ 운영중 │ 0             │ 2026-01-01   │▶│
│ member-svc.v1      │ member-svc   │ 운영중 │ 21 ⚠          │ 2026-01-01   │▶│
└──────────────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 필드 매핑 / PII 정책 — remit-svc.v1  [서비스: hanpass-ph]         [변경 결재] │
├──────────────────────────────────────────────────────────────────────────┤
│ 외부 필드(payload)        │ 정규화 항목         │ PII 처리        │ 필수 │ │
│ ──────────────────────────┼─────────────────────┼─────────────────┼──────┼│
│ sender.idNumber           │ (저장 안 함)        │ 수신 즉시 폐기  │ —    │ │
│ receiver.account          │ 수취 식별자         │ keyed hash      │ ○    │ │
│ receiver.name             │ (저장 안 함)        │ 토큰화 후 폐기  │ —    │ │
│ remit.usdAmount           │ 거래 금액(USD)      │ 그대로          │ ○    │ │
│ remit.receiveCurrency     │ 수취 통화           │ 그대로          │ ○    │ │
│ corridor.receiveCountry   │ 도착국(ISO)         │ 그대로          │ ○    │ │
├─ PII allowlist (저장 허용 컬럼) ───────────────────────────────────────────┤
│  허용: amount_base · receive_currency · channelType · send/receive_country │
│  금지: idNumber · receiver name · raw account · raw address · phone (원문) │
│                                                       [취소] [변경 결재]  │
└──────────────────────────────────────────────────────────────────────────┘
```
...
룰 관리는 FDS 운영의 핵심 화면입니다. 룰 빌더는 **hanpass-ph 5채널 선택**(월렛충전·국내송금·해외송금·월렛결제·ATM출금)과 **feature catalog**(`§10.1`)를 반영하며, **목록 → 상세 → 빌더 → 결재 → 활성화** 라이프사이클을 4-eyes 로 통제합니다. 모든 룰은 서비스(`tenant_demo`)·워크스페이스 스코프 안에서 운영됩니다.
...
│ 이름                       │ 채널    │ 동작    │ 평가 │ 상태 │           │
│ ───────────────────────────┼─────────┼─────────┼──────┼──────┼───────────│
│ PH 월렛충전 고액 차단       │ 월렛충전│ 차단    │ 즉시 │운영중│           │
│ PH 해외송금 고액 검토       │ 해외송금│ 검토    │ 즉시 │운영중│           │
│ PH 국내송금 고액 검토       │ 국내송금│ 검토    │ 즉시 │운영중│           │
│ PH 국내송금 분할입금 검토   │ 국내송금│ 검토    │ 즉시 │운영중│           │
│ PH 월렛결제 이상 고액 검토  │ 월렛결제│ 검토    │ 즉시 │운영중│           │
│ PH ATM출금 추가인증         │ ATM출금 │ 추가인증│ 즉시 │운영중│           │
├──────────────────────────────────────────────────────────────────────────┤
│ ◀ 1 ▶                                                    총 6건 (운영중 6)│
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 컬럼(표시) | 설명 (괄호=내부 코드) |
|------|------|
| 이름 | 사람이 읽는 룰명(예: `PH 월렛충전 고액 차단`) — bo-web 목록은 룰 이름을 1차 컬럼으로 표시 |
...
- **BR-001**: 필터 `채널(hanpass-ph 5종) / 상태 / 동작 / 평가` + `룰 이름` 텍스트 검색. 통합 검색어는 두지 않음(정확 식별 우선).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ PH 국내송금 분할입금 검토                         상태: ACTIVE  [비활성]  │
├──────────────────────────────────────────────────────────────────────────┤
│ 채널: 국내송금(DOMESTIC_REMIT) · 검토 · 즉시(실시간) 평가                  │
│ 적용 시작 2026-05-01   현재버전 v1                                        │
├─ 탭: [현재버전 조건] [버전 히스토리] [기준값] [최근 Hit] [결재 로그] ────┤
│                                                                            │
│ [탐지 조건 요약]                                                          │
│  국내송금에서, 같은 수취처로 24시간 내 입금 건수가 5건 이상이면 검토        │
│                                                                            │
│ 단계별 동작                                                               │
│  탐지 기준  │ 대상=수취처(상대방) / 측정=24시간 입금 건수                 │
│  현재 거래  │ 같은 수취처로 국내송금 입금할 때                            │
│  탐지 시 동작│ 검토(REVIEW) + 케이스 자동 개설(분할입금 조사)             │
│                                                                            │
...
운영자는 변수명·필드명을 직접 입력하지 않고 **엔진 측정항목 카탈로그(feature catalog `§10.1`)와 닫힌 선택지에서만 조건을 구성**합니다. **측정항목(카탈로그) select 는 velocity 계열을 base(집계×차원×distinct 대상×scope)당 1항목으로 그룹핑 노출**한다(PLAN 20260719-fds-rule-builder-window-dedupe U7 — 사용자 지시로 F-008·F-006 잠금 경로 append-only 해제) — 동일 base 가 시간창별 별개 행으로 나열되어 중복 노출되던 결함을 해소하며, base 라벨은 프론트 i18n 정본(`enum.ruleFeatureBase.*` ko/en 15종, 미등재 base 는 대표 행 라벨에서 윈도우 문구 제거 → base 키 원문 순으로 폴백)이다. 그룹 옵션 선택 값(featureKey)은 그 base 의 **최단 등재 윈도우 대표 키**이며, 이후 시간창 셀렉터에서 임의 윈도우로 자유 재조합한다(아래 시간창 셀렉터 계약과 동일 — 카탈로그 미등재 조합 키도 저작·평가 정상, 완화 아님). 비-velocity 측정항목은 그룹핑 없이 기존과 동일하게 노출된다. 폼은 ① 기본 정보(룰 번호·이름·룰셋·평가 방식) ② **고객 국적 차원** — `전 국적(ALL)`(무조건) 또는 특정 국적(ISO 3166-1 alpha-2, hanpass-ph 주요 회원 국적 후보 VN·PH·KR·TH·ID·MM·KH·NP) ③ **채널 범위** — `전 채널`(= `channelScope` NULL, 전채널 통합 평가) 또는 특정 채널 ④ **탐지 조건** — 측정항목 기반 조건 행을 **모두 만족(AND) / 하나라도(OR)** 로 결합 ⑤ 탐지 시 동작(decision)으로 구성됩니다. 각 조건 행은 세 종류입니다 — **측정항목 비교(cmp)**: 측정항목(카탈로그 select) + 연산자(수치형 `=`/`≠`/`>`/`≥`/`<`/`≤`, 문자형 `=`/`≠`/`포함(IN)`) + 값 / **시간창 집계(velocity)**: 집계(건수 `count`·금액합계 `sum`·서로 다른 값 개수 `distinct_count`) + distinct 대상(수취국가·이용 채널·수취처(상대방)·**단말기(deviceRef)·접속 IP·가맹점/ATM 단말(merchantRef)** — `distinct_count` 에서만, PLAN 20260717-fds-legacy-rule-overhaul U-F1 로 3종 확장) + 차원(회원 `subject`·수취처(상대방) `counterparty`·가맹점 `merchant` 등 6종 — `count`/`sum` 에서 선택 가능) + **시간창 셀렉터**(`1m`/`5m`/`10m`/`30m`/`1h`/`6h`/`24h`/`7d`/`30d`/**`1y`(1년) 10종**, PLAN 20260719-fds-rule-window-1y-value-hints U6/U7 — 사용자 지시로 F-006 잠금 경로 additive 해제. velocity 계열 측정항목 선택 시에만 조건부 노출되는 **가변 드롭다운** — 피처키 말미 윈도우 토큰만 재조합해 즉시 변경 가능, 비-velocity 측정항목에는 미노출) + 선택적 **채널 범위 한정 토글**(`동일 채널만`, `scope=sameChannel`, A9) + 연산자 + **기준값**(입력란에 측정항목별 실제 입력 가능 값의 **예시를 placeholder 로 표시** — 예: 건수 집계 "예: 3 (건수)", 금액합계 집계 "예: 1000000 (PHP 기준통화)", 국적 "예: PH (ISO 3166-1 alpha-2)" 등. ko/en 두 로케일 모두 지원(카탈로그 정본은 bo-web i18n 메시지, DB 스키마 변경 없음), U6/U7) / **리스크그룹 소속(in_group, 신규 — PLAN 20260717 U-W2)**: 대상 피처(카탈로그 select) + 리스크그룹(select, `/admin/fds/risk-groups` 등재분) + `그룹에 포함됨/포함되지 않음`(negated) 토글 — 블랙리스트 IP·단말·사기신고계좌·환금성 MCC 그룹 등을 저작. 화면 상태는 `buildRuleJson()` 순수 함수가 **canonical DSL(cmp/velocity/in_group + and/or)로 컴파일**하며(단일 정본), `DSL(JSON)` 토글은 컴파일 결과를 **읽기 전용**으로 확인하는 뷰입니다(`§19` D-03).
...
| 시간창 집계 항목 | `FeatureComputeAdapter`가 사전계산한 catalog key(`velocity.count.subject.6h`, `velocity.sum.subject.24h`, `velocity.distinct_count.receiveCountry.subject.24h`, `velocity.distinct_count.channelType.subject.6h`, `velocity.count.counterparty.24h`(동일 수취처 유입)·`velocity.sum.merchant.24h`(가맹점 결제 합계)·`velocity.distinct_count.counterpartyRef.subject.24h`(주체 distinct 수취처), **`velocity.distinct_count.{deviceRef,ip,merchantRef}.subject.<window>`(단말·IP·ATM단말 distinct 수)·`velocity.count.subject.sameChannel.7d`(동일 채널 한정, U-F1)·`velocity.*.1y`(1년 대표 키 6종, PLAN 20260719 U9)** 등)를 조건 첫 선택으로 직접 고른다. count/sum 차원은 subject/instrument/counterparty/actor/merchant/device 6종·distinct_count field 화이트리스트는 `{receiveCountry,channelType,counterpartyRef,deviceRef,ip,merchantRef}` 6종(V28, API §5.17). **시간창은 `1m/5m/10m/30m/1h/6h/24h/7d/30d/1y` 10종**(U-F1 로 5종, PLAN 20260719-fds-rule-window-1y-value-hints U1/U6 로 `1y`(1년) 1종 추가 — 사용자 지시로 F-006 잠금 경로 additive 해제). velocity 측정항목 선택 시 시간창은 **가변 셀렉터**로 노출되며(피처키 말미 윈도우 토큰 재조합, 카탈로그 미등재 조합 키도 저작·평가 정상), 카탈로그에는 윈도우별 대표 키만 등재된다(전 매트릭스 미등재). **측정항목 select 자체는 base(집계×차원×distinct 대상×scope)당 1항목으로 그룹핑 노출되고(중복 시간창 행 dedupe, PLAN 20260719-fds-rule-builder-window-dedupe U7), 시간창 셀렉터는 그룹핑과 무관하게 전 10종 그대로 유지된다(축소 없음 — 그룹 선택 시 base 최단 등재 윈도우 대표 키에서 시작해 셀렉터로 자유 재조합, 미등재 조합 키도 저작·평가 정상 계약은 완화·축소되지 않는다).** legacy/DSL `velocity` 노드는 호환 유지 |
| 리스크그룹 소속 조건(in_group, 신규) | 대상 피처 select(20260906 부터 `subject.phoneHash`·`counterparty.phoneHash`·`counterparty.accountNoHash` 식별자 해시 3키 포함 — 운영자가 블랙리스트 룰 BL-03~05 동형을 화면에서 저작 가능) + 리스크그룹 select(`/admin/fds/risk-groups` REST 등재분 — 블랙리스트 IP·단말·사기신고계좌·환금성 MCC·회원 블랙리스트·전화번호 블랙리스트 그룹 등, A10) + 포함/미포함(negated) 토글. 저장 DSL=`{"type":"in_group","feature":"<카탈로그 featureKey>","group":"<groupId>","negated":true\|false}`(`RuleDslParser.parseInGroup` 1:1, PLAN 20260717 U-W2) |
...
- **BR-003**: 시간창 집계는 엔진 사전계산 윈도우 **`1m`/`5m`/`10m`/`30m`/`1h`/`6h`/`24h`/`7d`/`30d`/`1y` 닫힌 집합 10종**(PLAN 20260717 U-F1 로 `1m`/`5m`/`30m`/`7d`/`30d` 5종 확장, 기존 `10m`/`1h`/`6h`/`24h` 4종 무회귀 + PLAN 20260719-fds-rule-window-1y-value-hints U1 로 `1y`(365일 고정, 윤년 미보정) 1종 추가 — 사용자 지시로 F-006 잠금 경로 additive 해제, `365d` 등 그 외 표기는 파싱 거부)의 catalog feature로만 선택한다(자유 기간/field 입력 없음 — feature 프리컴퓨트 키와 1:1). 룰 빌더는 velocity 계열 측정항목 선택 시 시간창을 **드롭다운으로 자유 변경**할 수 있게 하고(U6/U7), 기준값 입력란에는 측정항목별 **예시 값 placeholder**(ko/en i18n)를 표시한다. 시간창 집계에는 선택적 **채널 범위 한정(`동일 채널만`, 엔진 `scope="sameChannel"`)** 토글이 있다 — 켜면 조건 평가 시점 거래와 동일 채널(`channelType`)의 이벤트로만 집계를 좁힌다(가정 A9, 미선택=기존 전채널 집계 무회귀). **측정항목(카탈로그) select 드롭다운은 velocity 계열을 base(집계×차원×distinct 대상×scope)당 1항목으로 그룹핑 노출한다**(PLAN 20260719-fds-rule-builder-window-dedupe U7 — 사용자 지시로 F-008·F-006 잠금 경로 append-only 해제, 그룹 라벨은 `enum.ruleFeatureBase.*` ko/en 15종 정본, 옵션 value=base 내 최단 등재 윈도우 대표 키) — 이는 **드롭다운 표시 항목 수만 줄이는 것**이며, **시간창 가변 셀렉터의 전 10종 선택지·카탈로그 미등재 조합 키도 저작·평가 정상인 계약은 그대로 유지된다(완화·축소 아님)**.
...
```
┌─ 기준값 빠른 변경 — PH 해외송금 고액 검토 [서비스: hanpass-ph] ┐
│ 현재 기준값 280,000 PHP → 새 기준값 [250000] PHP            │
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 룰 시뮬레이션 (백테스트)  [서비스: hanpass-ph]                            │
├─ 시뮬레이션 설정 ─────────────────────────────────────────────────────────┤
│ 대상 룰* (●) 신규 빌더 룰(미저장 ruleJson) ( ) 기존 룰 [PH 해외송금 고액 검토 ▼]│
│ 과거 데이터 기간*  [2026-05-01 00:00]  ~  [2026-05-31 23:59]             │
│             빠른선택: [최근 7일] [최근 30일] [최근 90일]                  │
│ 평가 범위   채널 [해외송금 ▼]  ☑ 현재 운영 룰과 비교                      │
│                                                  [시뮬레이션 실행]        │
├─ 결과 ────────────────────────────────────────────────────────────────────┤
│ 탐지(Hit) 1,284건 │ 차단 시 영향 902건 │ 추정 오탐 6.4% │ 평가 412,900건 │
│ [샘플 Hit] 일시 · 대상(식별자) · 측정값 · 점수 · 시 동작                  │
│ [권고] 임계치 280,000→250,000 PHP이면 Hit +14% · 추정 오탐 +2.1%p         │
...
| 평가 범위 | 채널(hanpass-ph 5종) 필터 · 현재 운영 룰과 비교 여부 |
...
| 컬럼(표시) | 설명 |
|------|------|
| 거래번호·내역 | `transactionRef`(mono, 첫 컬럼) + 보조줄에 채널(hanpass-ph 5채널 — 월렛충전/국내송금/해외송금/월렛결제/ATM출금)·금액·회랑(`send_country`→`receive_country`). **금액 표기(코드=truth, 2026-08-05 F-2)**: `amount`/`currency` 를 우선 표기하되, **해외송금·월렛 등 flat 금액이 없는 canonical 이벤트**(송금·수취 leg 만 운반 — `amount`/`currency` null 이 엔진 진실)는 **서버 파생 규제통화 환산액 `amountBase`/`baseCurrency`(테넌트 기준통화, tenant_demo=PHP)로 폴백**한다. 둘 다 없으면 금액을 만들지 않고 회랑만 표기한다(공란 허용). 목록은 연결 canonical 이벤트를 조인하지 않으므로(N+1 방지) **leg 원통화(`sendCurrency`/`receiveCurrency`)는 상세(§8.2)에서만 표기**된다 — 엔진 `DecisionResponse` 는 leg 금액만 운반하고 leg 통화는 이벤트 조회로 해소한다. hanpass-ph 데이터 레이어 필드(DB §5.5) |
...
- **BR-005 (데모 데이터 정직화 — 실 payload+실 룰 판정, 코드=truth, feature/fds-demo-data-honesty · AML §7.1 BR-DEMO-HONESTY[기능정의서(AML) v9.27] 의 FDS 확장)**: 데모(비-prod stub) FDS 판정은 **실 인입 payload 기반**이다 — 라이브 인입(`FdsDecisionCaseStubService.ingestLiveDecision`)이 **`transactionRef` 을 payload 원문 그대로** 쓴다(조작 폐기 — 기존 `txn-live-{seq}` 합성 참조 제거). 시뮬레이터가 **동일 nested `transaction` 객체**(memberRef·transactionRef·channel·amount·currency·corridor·receiverName/Country·senderHolderName)를 FDS·AML 양쪽에 전송하므로, **한 거래가 FDS 판정과 AML 알림에서 동일 `transactionRef`** 로 상호 참조된다(기존 FdsIngestTestController.TestEvent 는 4필드만 소비해 나머지를 버렸으나 이제 payload 를 그대로 소비). 판정은 `FdsRuleCatalog` **ACTIVE 룰을 실 거래 속성**(금액밴드·채널·corridor·FDS 신호)으로 실평가해 ALLOW/REVIEW/BLOCK·발동 룰·점수를 산출한다 — **hash 파생(seq→score/channel/amount/corridor) 전면 폐기**. 룰 드릴다운(BR-006 발동 룰 카드 참조)은 실근거만 노출하고 근거가 없으면 "상세 근거 미제공"으로 정직 표기(조작 금지). **seed 픽스처(2 판정+2 케이스) 폐기** — 인입 0 이면 결정 목록이 비어 있다("합성 데이터를 표시하지 않습니다 — 실 인입 대기" 정직 문구). 케이스 auto-open(§11, BR-006)·엔진 위임·prod fail-closed 불변.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 액션 아웃박스  [서비스: hanpass-ph ▼]      발행 1,204 성공 1,202 실패 2 DLQ 0 │
├──────────────────────────────────────────────────────────────────────────┤
│ [조치 종류 ▼] [상태 ▼] [대상 시스템 ▼]                    🔍 추적 번호    │
├──────────────────────────────────────────────────────────────────────────┤
│ 발행시각        │ 조치 종류    │ 대상 시스템  │ 상태    │ 오류          │ │
│ ────────────────┼──────────────┼──────────────┼─────────┼───────────────┼│
│ 06-06 11:02:18  │ 거래 차단    │ walletchg-svc│ 발행    │ —             │▶│
│ 06-06 11:00:05  │ 자금 보류    │ remit-svc    │ 승인됨🔒│ —             │▶│
│ 06-06 10:59:40  │ 추가 인증    │ wallet-svc   │ 대기    │ 일시적 응답없음│▶│
│ 06-06 10:55:20  │ 케이스 개설  │ domestic-svc │ 실패 ⚠  │ 권한 없음(404)│▶│
├──────────────────────────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ Capability 매트릭스  [서비스: hanpass-ph ▼]                       [변경 결재] │
├──────────────────────────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 케이스 목록  [서비스: hanpass-ph ▼]      진행 88  기한 임박 3  기한 초과 0     │
├──────────────────────────────────────────────────────────────────────────┤
...
│ case_77f2   │ 이상거래 조사 │ subj_7c..│ 중간 │ analyst.b │ 종결 상신│ — │▶│
│ case_77f3   │ 규제 보고     │ subj_55..│ 높음 │ audit.c   │ 조사중   │8h │▶│
└──────────────────────────────────────────────────────────────────────────┘
```
...
- **BR-004**: 조회 결과 1만 건 초과 시 팝업 노출 — 기간/조건 좁히기 안내 후 재조회.
- **BR-005 (데모 데이터 정직화 — 실 판정 기반 케이스만, 코드=truth, feature/fds-demo-data-honesty · AML BR-DEMO-HONESTY[기능정의서(AML) v9.27] 확장)**: 데모(비-prod) FDS 케이스는 **실 라이브 판정에서 auto-open 된 것만** 존재한다(§8.2 BR-005·§8→§11 폐루프). **seed 케이스 픽스처(2건)는 폐기**되어, 인입 0 이면 케이스 목록이 비어 있다("합성 데이터를 표시하지 않습니다 — 실 인입 대기" 정직 문구). auto-open 은 실 판정(`recommendedActions` 의 `OPEN_CASE`)·발동 룰 기반 caseType 파생으로 이뤄지고 4-eyes(종결·배정)·엔진 위임·prod fail-closed 는 불변이다. 케이스↔결정은 실 판정의 동일 `transactionRef`(§8.2 BR-005, payload 원문)로 연결된다.
...
│                                                                            │
│ [개요·증적]                                                               │
│  트리거 룰 PH 국내송금 분할입금 검토   연결 결정 1건   연결 거래 1건       │
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 결재함 (maker-checker)  [서비스: hanpass-ph ▼]                  관리자 admin ▼ │
├──────────────────────────────────────────────────────────────────────────┤
│ [유형 ▼] [상태 ▼] [상신자 ▼]                              🔍 대상         │
├──────────────────────────────────────────────────────────────────────────┤
│ 결재 종류     │ 대상           │ 상신자   │ 결재 라인          │ 상태 │     │
│ ──────────────┼────────────────┼──────────┼────────────────────┼──────┼─────│
│ RULE 활성화      │ PH 해외송금 고액 검토 v2│ rule.eng │ COMPLIANCE_MANAGER │ 상신 │▶│
│ ACTION 자금보류  │ case_77f0      │ analyst.a│ MAKER_CHECKER      │ 상신 │▶│
│ MAPPING 변경     │ remit-svc.v2   │ ops.a    │ MAKER_CHECKER      │ 상신 │▶│
│ SECRET 회전      │ walletchg-svc key│ ops.a   │ SECURITY_ADMIN     │ 상신 │▶│
...
│ 규제 팩 변경     │ tenant_demo  │ admin.a  │ COMPLIANCE_MANAGER │ 상신 │▶│
├──────────────────────────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 규제 보고 큐  [서비스: hanpass-ph ▼]    기한 임박 12  24h내 2  기한 초과 0     │
├──────────────────────────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ Evidence Export (검사대응)  [서비스: hanpass-ph ▼]              관리자 admin ▼ │
├──────────────────────────────────────────────────────────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ 운영 변경 감사 로그  [서비스: hanpass-ph ▼]                                    │
├──────────────────────────────────────────────────────────────────────────┤
│ [대상 ▼] [작업자 ▼] 기간 [from]~[to]  대상 ID [______]            🔍     │
├──────────────────────────────────────────────────────────────────────────┤
│ 시각              │ 대상       │ 작업자    │ 작업           │ 대상 항목   │ │
│ ──────────────────┼────────────┼───────────┼────────────────┼─────────────┼│
│ 06-06 11:10       │ 룰         │ rule.eng  │ 새 버전 활성   │ PH 해외송금 고액 검토│▶│
│ 06-06 10:55       │ 커넥터     │ ops.a     │ 시크릿 변경    │ walletchg-svc│▶│
│ 06-06 10:40       │ 필드매핑   │ ops.a     │ PII 정책 변경  │ remit-svc.v1│▶│
│ 06-06 10:20       │ Capability │ approver.b│ 자금보류 허용  │ wallet-svc  │▶│
...
초기 배포(PHP 통화 프로파일) 시 디시전트리 목록에는 PH baseline 5종(해외송금·국내송금/인바운드·월렛충전·월렛결제·월렛출금)이 version 1 등록 상태로 노출되며, 채널별 적용(활성화)은 시뮬레이션·4-eyes 결재로만 이뤄진다([계약 §5](../design/fds-decision-trees.md)).
