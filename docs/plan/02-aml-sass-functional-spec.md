...
- **구성**: 탭 `① 수신 API 카탈로그`/`② 인입 라이브 모니터링`.
  ...
  - **② 인입 라이브 모니터링**: 소스 시스템 라이브 상태. **REST 전용 운영(2026-06-26, §1.11 ① 참조)** — 현재 노출 소스는 모두 `REST_PUSH`(마지막 수신(n초 전)·TPS·● 신호). KPI 4카드(수신중/지연/중단/큐 적체 합) + 큐 메트릭(`aml-ingest`·`.fifo`·`-dlq` depth·lag·DLQ — REST 푸시 이벤트의 내부 SQS 인입 파이프라인, **유지**). 행 `▶` → AML-AUD-001 ③(소스 시스템 관리·장애 정책). 비-REST 5종은 §1.11 표준 정의로만 유지(미노출·예정).
...
- **구성**: bo-api가 내려준 `ScenarioDefinition{family, severity, fields[]}`를 단일 공통 폼으로 렌더하는 **스키마 주도 빌더**. THRESHOLD 계열은 윈도우·건수·금액·국가·방향 등 수치/선택 필드, SIGNAL 계열은 시그널 토글을 표시한다. `HIGH_RISK_CORRIDOR`는 방향·고위험 국가·회랑 윈도우·회랑 건수·회랑 합산금액 임계를 직접 확인·수정한다. 자연어 미리보기.
- **BR-001**: 시나리오 변경 적용 = 4-eyes(`TM_SCENARIO`·준법감시 책임자). 적용 전 시뮬레이션(과거기간 백테스트) 권장(결재 불필요).
- **BR-002**: 내부 scenario DSL 비노출(업무 용어 필드). 편집값은 `fields → parameters`로 평탄화되고 bo-api가 엔진 DSL로 컴파일한다. payload_hash 고정·변경 시 무효화·자기 승인 차단.
...
- **구성**: 탭 `① STR 보고 현황 통계`/`② 룰 효과성`. ① 기간별 보고 퍼널(추출→검토→결재→제출) KPI 카드 + **지연 보고 일수 분포(법정 SLA 대비)** + 미보고(기각·취소) 사유 코드 분포 표. ② 시나리오(룰)별 효과성 표 — 알림 건수(A)·케이스 전환(a)·**STR 전환(B, `reportCount`)**·**보고 전환율(B/A=`conversionRate`)**·**케이스 전환율(a/A=`caseConversionRate`)**·**관련 거래건수(`relatedTxnCount`)·관련 거래총액(`relatedTxnAmount`)**(BR-004 read-only 집계 파생값 — TM alert evidence `aggregationSummary` 출처)·전월 대비 변동·튜닝 권고 배지 ⚠. 행 `▶` → **AML-TM-002(`scenarioCode` 컨텍스트, 시나리오 튜닝)**. tipping-off 보호(① STR 통계 전담 한정)는 본 ② 효과성 표 확장과 무관하게 불변.
