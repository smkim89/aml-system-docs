...
| 버전 | 일자 | 작성자 | 변경 내역 |
|------|------|--------|----------|
| **9.112** | **2026-09-11** | **Hanpass Global Team** | **RA 상세 요인 탭 강제 상향 사유 블록 단일화(코드=truth, aegis-aml PLAN 20260911-aml-ra-detail-evidence-tidy).** **§12-A.4 BR-004 ①** — v9.111 이 명문화한 강제 상향 사유·레지스트리 근거(tier 배지·참조 토큰 인라인 병기) 및 국가위험/HRR 설명 주석을, 요인 분해 카드 아래 중복 Callout(`FactorsTab`)이 아니라 **요인 탭 최상단 입력 원장 카드 내 대조 블록("강제 상향 사유 대조") 한 곳에만** 표기하도록 정정했다 — 이 블록은 CDD 입력 원장 조회 실패·미존재 분기에서도 렌더되어(로딩 중만 예외) 설명책임이 소실되지 않는다. 신규 엔드포인트·필드·엔진 판정 없음(bo-web DOM 배치 정리, 술어·데이터 계약 무변경). | 코드=truth. 근거=aegis-aml bo-web `components/aml/AmlRiskDetail.tsx`(`FloorReasonCompareBlock` 로컬 컴포넌트 추출), `messages/bundles/{ko,en}/aml-customer.json`(`amlCust.riskDetail.floorReasonTitle` 삭제). API/DB/integration 설계서 무변경(확인만). |
| **9.111** | **2026-09-11** | **Hanpass Global Team** | **당연고위험 레지스트리 floor 포트 단일화 + 사유·근거 단일 블록화(코드=truth, aegis-aml PLAN 20260911-aml-hrr-floor-consolidation, 사용자 지시로 F-115·F-116 해제).** **§12-A.4 BR-004 ①** — RA 상세 강제상향 근거 패널의 "레지스트리 등재 근거" **별도 소절을 폐기**하고, 강제 상향 사유 블록(`MandatoryFloorReasonList`)의 사유 행에 **tier 배지·회원 키(참조 토큰)를 인라인 병기**하도록 개정했다 — 사유 행의 `listType` 과 일치하는 레지스트리 근거를 찾아 그 행에 이어 붙인다(사유·근거 중복 노출 제거). 명단 매치 근거 패널(`WatchlistMatchEvidencePanel`)은 **이름 스크리닝 매치 전용**으로 복원해 레지스트리 등재 원소는 내부에서 제외한다(딥링크 계약 불변 — entryId 부재 원소는 여전히 미노출). 성능·구조 후속(문서 비영향 요약) — aml-svc `HighRiskRegistryStorePort` 의 default 술어 `isKnownUnregistered`(exists 2단 조회)를 삭제하고 추상 단일 메서드 `findForcedFloor(tenantId, subjectRef)`(인덱스 `ix_hrr_items_subject` 조회, 비잠금·readOnly)로 통합했다 — JPA 어댑터가 subject 의 아이템만 읽어 도메인 정적 팩토리 `HighRiskRegistry#forcedFloorFrom(subjectRef, items)` 로 조립하고, 기존 인스턴스 `forcedFloorFor` 도 같은 정적 팩토리에 위임한다(조립 규칙 단일 정본화, 룰/판정 무변경). | 코드=truth. 근거=aegis-aml aml-svc `application/port/out/HighRiskRegistryStorePort#findForcedFloor`·`domain/registry/HighRiskRegistry#forcedFloorFrom`·`adapter/out/persistence/{HighRiskRegistryJpaAdapter,HighRiskRegistryReferenceItemJpaRepository}`·`application/usecase/OnboardingRaDerivationService`, bo-web `components/common/{MandatoryFloorReasonList,WatchlistMatchEvidencePanel}.tsx`·`components/aml/AmlRiskDetail.tsx`. 설계서 §24·DB §3.9·§3.20 동일 작업 단위, API §3.3 무변경(확인만). |
| **9.110** | **2026-09-10** | **Hanpass Global Team** | **당연고위험 레지스트리 floor 병합(F-115) 후속 4건 — 조회 fast path·forcedFloorFor 위임·근거 미러 6키·레지스트리 근거 소절(코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-followups).** F-115 QA 로그 후속 4건을 코드 반영 후 문서 정합. ① **§12-B.6 BR-005** — 자동상신 멱등 no-op 의 '이미 등재' 판정은 `RA_HIGH_RISK_CUSTOMERS` 리스트 기준이며, `PEP_INDIVIDUALS`·`HIGH_NET_WORTH` 등 비RA 리스트 등재 회원의 당연고위험 분류는 BR-010 병합 경로로 계속 `HRR_REGISTRATION` 자동상신 대상임을 명시(코드 무변경 — D1 결정). ② **§12-A.4 BR-004 ①** — RA 상세 강제상향 근거 패널에 "레지스트리 등재 근거" 별도 소절(리스트 라벨+tier 배지+`subjectRef` 참조 토큰, 딥링크 없음, 판별 술어: `tier` 존재 또는 `entryId`·`screeningId` 부재)을 명문화. ③ 성능·구조 후속(문서 비영향 요약) — aml-svc `OnboardingRaDerivationService`가 `HighRiskRegistryStorePort.isKnownUnregistered`(인덱스 `ix_hrr_items_subject` exists fast path, 보수적 default false) 로 미등재 회원의 레지스트리 전량 로드를 생략하고, `HighRiskRegistryService.triggerForcedReassessment` 의 tier·reasons·evidence·floor 조립을 `HighRiskRegistry.forcedFloorFor` 단일 위임으로 정리했다(룰/판정 변경 없음). bo-api `RaDtos.ForcedFloorEvidence` 를 4키→6키(`subjectRef`,`tier` additive)로 확장해 explicit 매퍼(`AmlRaService#forcedFloorEvidence`)·breakdown 폴백 매퍼(`#forcedFloorEvidenceMarkers`) 양쪽에서 레지스트리 원소의 `subjectRef`/`tier` 소실을 해소했다(API §3.3 동일 작업 단위 정정). | 코드=truth. 근거=aegis-aml aml-svc `application/port/out/HighRiskRegistryStorePort#isKnownUnregistered`·`adapter/out/persistence/{HighRiskRegistryJpaAdapter,HighRiskRegistryReferenceItemJpaRepository}`·`application/usecase/{OnboardingRaDerivationService,HighRiskRegistryService}`, bo-api `aml/ra/{dto/RaDtos,service/AmlRaService}`, bo-web `lib/aml-risk.ts`·`components/common/WatchlistMatchEvidencePanel.tsx`·`components/aml/AmlRiskDetail.tsx`. API §3.3·DB §3.20 동일 작업 단위. |
| **9.109** | **2026-09-10** | **Hanpass Global Team** | **v9.108 QA 명세 리뷰 중간·낮음 이격 정정 — §12-B.6 BR-010 재파생 경로 정확화·§12-A.4 BR-010 `ALREADY_EVALUATED` 전제 병기(코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd).** ① **§12-B.6 BR-010** — "BR-003 참조 리스트 변경이 트리거하는 국가정책 재평가"라는 구 서술을 실제 병합 경로 3종(① 최초 CDD·재이행 re-CDD 인입, ② 국가위험 정책 활성화 재평가 `CountryRiskOnboardingReassessmentService`, ③ 등재 해제 재파생 `OnboardingSnapshotReassessmentSupport.reassessAfterDeregistration`)으로 정정하고, **BR-003 참조 리스트 변경 트리거는 이 병합 경로가 아니라 `HighRiskRegistryService.triggerForcedReassessment`→`RiskAssessmentService.evaluate` 직행 별도 경로**임을 명시(그 경로 자체가 등재 시점 floor 를 적용). ② **§12-A.4 BR-010** — "재CDD 는 신규 스코어를 append" 문장에 전제 "`occurredAt` 이 직전 평가시각 이후일 때 — 이전이면 `ALREADY_EVALUATED` 로 직전 스코어를 재사용하며 신규 스코어를 만들지 않는다" 병기(`OnboardingRaDerivationService.deriveInternal`). ③ **API §3.3 `forcedFloorEvidence`** — "명단 매치 floor 전용" 문구 폐기, 레지스트리 등재 근거 원소 `{listType,subjectRef,tier}`(스크리닝 원소 `{listType,screeningId,entryId,label}`와 같은 배열에 병합) 명시 + bo-api 미러(`AmlRaService.forcedFloorEvidence()`) 4키 고정으로 `subjectRef`/`tier` 소실·`screeningId`/`entryId`=`null` 손실 병기. 신규 필드·엔드포인트 없음(문구 정정만). | 코드=truth. 근거=aml-svc `application/usecase/{OnboardingRaDerivationService,CountryRiskOnboardingReassessmentService}`·`application/support/OnboardingSnapshotReassessmentSupport`·`application/usecase/HighRiskRegistryService#triggerForcedReassessment`·`application/usecase/RiskAssessmentService`, bo-api `aml/ra/service/AmlRaService#forcedFloorEvidence`(약 2295~2316행). API `02-aml-api.md` §3.3 동일 작업 단위. |
| **9.108** | **2026-09-10** | **Hanpass Global Team** | **§5(RA 콜아웃) (d)·§5.1 AML-RA-001 BR-013·§12-A.4 AML-RA-003 BR-010·§12-B.6 AML-HRR-001 BR-010 — 당연고위험 등재 회원의 재CDD/재파생 레지스트리 floor 유지(코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd, 사용자 지시로 F-006·F-100 해제).** 사용자 관측 — 명단(dilisense)에서 빠진 뒤 재CDD 한 당연고위험 등재 회원이 `mandatoryHighRisk=false`·LOW/APPROVE 로 내려가고 RA 상세 흐름도·직접 확정 버튼이 사라졌다. 온보딩 파생(`OnboardingRaDerivationService`)이 대상 등재 여부를 조회하지 않던 결함을 해소 — §12-B.6 **BR-001** "당연고위험 해당 대상은 등급 강제 상향" 이 등재 시점 1회성이 아니라 등재가 유지되는 동안의 **상태 속성**임을 BR-010 으로 명문화하고, 파생 직후 레지스트리 floor(tier `HIGH→HIGH`·`VERY_HIGH→PROHIBITED`)를 스크리닝/국가위험 floor 와 병합(등급 max·사유/근거 합집합)한다. 최초 CDD·재이행 re-CDD·`forcedReassessment` 국가정책 재평가 공통 적용, 미등재 회원은 diff 0. 재CDD 는 신규 ONBOARDING 스코어를 append 하며(직전 스코어 재사용 아님), 스크리닝 매치 없는 등재 회원의 재CDD 는 결정 `EDD_REQUIRED`(사유 `REQUIRED_ACTION_EDD`)·`HIGH_RA_SCORE` EDD_REVIEW 케이스 1건 멱등 개설(`SANCTION`/`PEP` 수동검토 스킵 미해당), tier `VERY_HIGH` 는 `REJECT`(`RISK_GRADE_PROHIBITED`). HRR 등재 자동상신은 재CDD 로 중복 상신되지 않는다(멱등). 해소는 §12-B.6 BR-009 등재 해제 결재만(명단 매치 소멸로 자동 해소 아님). 엔진 케이스 카탈로그 **RA-C30** 신설(사용자 지시로 F-006·F-100 해제). | 근거=aegis-aml aml-svc `application/usecase/OnboardingRaDerivationService`·`domain/registry/HighRiskRegistry#forcedFloorFor`, `docs/qa/engine-rule-cases.md` RA-C30. API §3.3(`mandatoryHighRiskReasons` 값집합)·DB §3.9 동일 작업 단위. |
| **9.107** | **2026-09-10** | **Hanpass Global Team** | **§12-A.10 AML-MBR-001 회원관리 이력 표에 "AML 심사결과(스냅샷)" 열 신설 — CDD 행마다 인입 판정 병기(코드=truth, aegis-aml PLAN 20260910-aml-member-history-decision-column).** 사용자 관측 — CDD/EDD/심사결과 히스토리 표는 CDD 행(`CDD_INITIAL`/`CDD_REVIEW`)마다 그 인입이 어떤 AML 판정(승인/거절/EDD 필요·사유·조치)을 받았는지 보여주지 않았다(`details`는 `kycLevel`뿐 — 판정 스냅샷은 `aml_cdd_onboarding_decisions`, F-076 불변, 이력 `source_event_id`로 조인 가능). **BR-005 신설** — CDD 이력 행마다 그 인입의 온보딩 판정 스냅샷을 이력 조회 시점에 `event_id` 조인으로 병기(이력 행·스냅샷 모두 무수정, 읽기 전용 read-back). **구성 ②**: 이력 표에 "AML 심사결과(스냅샷)" 열 신설 — CDD 행은 인입 판정 스냅샷(`OnboardingDecision`·사유·조치) 배지, `심사결과` 탭의 `MEMBER_DECISION` 행은 확정 결정·출처 배지(v9.106이 "탭 행 병기"로만 서술한 부분을 본 열로 이동해 정정, 복제 아님), 그 외 유형·스냅샷 없는 CDD 는 `-`. API `GET .../members/{memberRef}/cdd-history` 응답 행에 `ingestDecision{decision, reason, riskGrade, requiredAction, scoreId, createdAt}|null` additive(엔진 1:1, bo-api 미러). 스키마 변경 없음(DB §3.22f 에 읽기 조인 1문장 추가) — RA/WLF/STR/CTR/FDS 엔진 판정 로직 무변경. | 근거=aegis-aml aml-svc `application/usecase/MemberLedgerService`·`application/port/{in/QueryMemberLedgerUseCase,out/CddOnboardingDecisionStorePort}`·`adapter/{in/rest/MemberLedgerController,out/persistence/CddOnboardingDecisionJpa*}`, bo-api `aml/memberledger/{dto/MemberLedgerDtos,service/AmlMemberLedgerService}`, bo-web `components/aml/AmlMemberLedger.tsx`·`lib/aml-member-ledger.ts`. API §2.x(cdd-history)·DB §3.22f 동일 작업 단위. |
| **9.106** | **2026-09-10** | **Hanpass Global Team** | **§12-A.10 AML-MBR-001 회원관리 — 「AML 심사결과」 원장 요약 타일 + 회원 결정 이력(`MEMBER_DECISION`, 코드=truth, aegis-aml PLAN 20260910-aml-member-ledger-decision-history).** 사용자 관측 — RA 상세 회원 심사결과 이력은 승인→EDD 필요→거절(관리자 직접 확정)로 진행됐는데 회원관리 "KYC 상태"는 여전히 검증완료로 남아 있었다. 근본 원인은 KYC 상태(`aml_customers.kyc_status`)와 AML 심사결과(`aml_member_decisions`)가 **서로 다른 축**이라는 점이다 — `kyc_status`는 계정계가 CDD 인입으로 제공·소유하는 검증 상태이고, AML 은 이를 변경하지 않는다(내부 변경 경로 `CddProfile` 전이표가 `VERIFIED→REJECTED` 자체를 금지). 정답은 KYC 상태를 덮어쓰는 것이 아니라 **AML 심사결과를 나란히 노출**하는 것이다. ① **구성**에 원장 요약 「AML 심사결과」 타일 신설(현재 결정·출처·확정 시각·확정자, 결정 없음/조회 실패는 `-`) + 기존 "KYC 상태" 라벨을 **「KYC 상태(계정계)」**로 명확화. ② **BR-001 개정** — 이력 유형(`history_type`) **6종→7종**: 회원 심사결과가 확정되는 3경로(`EDD_CLOSE`·`RELATIONSHIP_REJECT`·`ADMIN_DIRECT`)에서 같은 트랜잭션 안에 `MEMBER_DECISION` 1행을 append 한다(`details={decision, source, decisionId, approvalId?, caseId?}`, `kyc_status`/`risk_grade`는 확정 시점 스냅샷). 적재 지점 **3종→4종**((d) 회원 심사결과 확정 추가). 히스토리 탭 2종→**3종**(`CDD 히스토리`/`EDD 히스토리`/`심사결과`). ③ **BR-004 신설** — "KYC 상태는 계정계가 CDD 인입으로 제공하는 검증 상태(`aml_customers.kyc_status`, `docs/aml-data.md` §12.2 필수 필드)이며 AML 심사결과(`aml_member_decisions`)와 별개 축이다 — AML 은 KYC 상태를 변경하지 않는다(최종 회원승인 판단은 채널 몫, 내부 변경 경로는 `CddProfile` 전이표가 `VERIFIED→REJECTED`를 금지)." **Flyway V78**(`aml_member_cdd_history_type_check` 7종 재생성 + 기존 `aml_member_decisions` 전 행 backfill, 멱등)만 추가 — RA/WLF/STR/CTR/FDS 엔진 판정 로직·산식 무변경. **(2026-09-10 QA 명세 리뷰 정정, 코드=truth, 문서 정정 패스 20260910-aml-member-ledger-decision-history-docfix.)** ⓐ **구성 ①**: 「AML 심사결과」 타일은 `GET .../customers/{ref}/decision` **병합 뷰**를 그대로 렌더하므로 출처는 확정 3종(`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`) **+ 인입 스냅샷 `INGEST`**(결정 `EDD_REQUIRED` 가능) **4종**이다 — 이력 `MEMBER_DECISION`(BR-001) 의 확정 3출처 한정과는 구분되는 별개 축임을 명시. ⓑ **BR-001**: `risk_grade` 스냅샷 규칙을 "**확정 결정의 등급 우선, 없으면 확정 시점 원장 스냅샷**"(DB §3.22f 와 동일 표현)으로 정정 — `kyc_status`만 단순 "확정 시점 원장 스냅샷"이다. ⓒ **API 행·화면 인벤토리 행**에 `GET /api/v1/bo/aml/customers/{ref}/decision`(scope `aml:case:read`, API §2.3) 을 원장 요약 API 로 추가 병기(누락 보완, 신규 엔드포인트 아님). API §2.x cdd-history 행에도 `details.decision`(`APPROVED\|REJECTED\|REPORTED`) 값이 **확정 행(`aml_member_decisions`) 한정**이며 인입 스냅샷의 `EDD_REQUIRED`는 `MEMBER_DECISION` 이력 대상이 아니라는 전제를 명시했다(API 문서 동일 작업 단위). | 근거=aegis-aml aml-svc `domain/enums/CddHistoryType`·`application/usecase/{MemberLedgerService,MemberDecisionService}`·`V78__member_cdd_history_member_decision.sql`, bo-api `aml/memberledger/dto/MemberLedgerDtos`, bo-web `components/aml/AmlMemberLedger.tsx`·`lib/aml-member-ledger.ts`. API §2.x(회원원장 read)·DB §3.22f·§5.36·§7(V78) 동일 작업 단위. |
| **9.105** | **2026-09-09** | **Hanpass Global Team** | **§12-A.4 AML-RA-003 BR-009 신설 — 관리자 액션 패널 「회원 심사 직접 승인/거절」(코드=truth, aegis-aml PLAN 20260909-aml-ra-direct-member-decision).** 당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 **EXECUTED** 된 회원에 한해 추가 4-eyes 없이 회원 심사결과를 바로 확정하는 [회원 심사 승인]/[회원 심사 거절] 두 버튼을 관리자 액션 패널(BR-005)에 신설 — 노출 조건(등재 완료·대기 결재 없음·scope `aml:admin:approval`)·모달(사유 필수)·엔진 게이트(ⓐ~ⓖ, 400/404/409 `AML.STATE_CONFLICT`)·`aml_member_decisions.source=ADMIN_DIRECT`(결재·케이스 없음)·웹훅 `AmlMemberDecisionResolved` 동일 배관 재사용을 명문화. 권한 표에 직접 확정 scope 병기. | 근거=aegis-aml aml-svc `domain/decision/MemberDecision#fromAdminDirect`·`application/usecase/DirectMemberDecisionService`·`adapter/in/rest/MemberDecisionAdminController`, bo-api `aml/ra/{controller/AmlRiskReadController,service/AmlRaService,dto/RaDtos}`, bo-web `components/aml/AmlRaAdminActions.tsx`. API §2.1·§2.3·§8.1, DB §3.22g·§7(V77) 동일 작업 단위. |
| **9.104** | **2026-09-08** | **Hanpass Global Team** | **§5.1 AML-RA-001 1차 RA 내역 기본 정렬 = 인입시각·인입시각 컬럼·정렬 기준 셀렉트·BR-012 신설(코드=truth, aegis-aml PLAN 20260908-aml-ra-initial-list-ingest-sort).** 엔진 `risk-scores` 목록 `sort` 파라미터(`INGESTED_AT_DESC`\|`EVALUATED_AT_DESC`)·응답 `ingestedAt` additive·기본 정렬 2차 키(`scoreId` 문자열 desc). |
| **9.103** | **2026-09-06** | **Hanpass Global Team** | **§7.1 AML-TM-001 BR-014 보강 — ③ 최근 거래·알럿 상세 근거거래에 FDS 차단 판정 배지(코드=truth, aegis-aml PLAN 20260906-fds-block-aml-propagation).** 계정계가 FDS·AML 에 동일 `transactionRef` 로 보낸 거래 중 FDS 가 `BLOCK` 판정한 건을 AML 화면에서 식별 — `FDS 차단` 배지 + 차단 룰명(최대 3·`+n`), 통지 없음 `-`, fail-safe 룰 0건 안내. 원천은 FDS 판정 시점 통지(`fds.decision.blocked` → `aml_fds_transaction_decisions`), 알럿·케이스·2차 RA 무변경(additive). | 근거=aegis-aml bo-web `components/common/FdsDecisionBadge.tsx`·`components/aml/{AmlTmTransactions,AmlTmAlertDetailSections}.tsx`·`lib/aml-tm.ts`, bo-api `AmlTransactionDtos.FdsDecisionView`, aml-svc `FdsDecisionView`. API §2.4·§3.10a, DB §3.22h V75, integration §3.2 v4.17 동일 작업 단위. |
| **9.102** | **2026-09-04** | **Hanpass Global Team** | **§12-B.6 AML-HRR-001 자동 등재 회원 4-eyes 등재 해제 경로 신설(BR-009, 코드=truth, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095).** **BR-009** 신설 — `RA_HIGH_RISK_CUSTOMERS` 자동 등재 회원 중 오등재분을 해제하는 결재(`HRR_REGISTRATION` 재사용 + `DEREGISTER\|<customerRef>` 접두, 승인선 `EXECUTIVE_APPROVAL`), 상신 처분표(ⓐ 미등재/ⓑ 해제 pending → NOOP, ⓒ 재파생 사전조건 미충족/ⓓ 독립 당연고위험 사유 보유 → 400), 승인 시 잠금 스냅샷 재검증 실패 시 409 `AML.HRR_DEREGISTRATION_BLOCKED`, EXECUTED 시 항목 제거·version+1·온보딩 CDD 스냅샷 재파생(비-mandatory 정정)·2차(ONGOING) 점수 보유 회원 best-effort 재산출(다음 2차 평가까지 캐시-operative 괴리 수용)·콜백 `AmlHighRiskRegistrationRevoked`(반려 미발행)·기존 OPEN EDD 미회수를 명시. 화면 노출 2곳(RA 상세 등재 패널·본 화면 ② 행 액션, scope `aml:admin:high-risk-registry`), 해제 pending 중 등재 (재)상신은 NOOP 유지, 범위는 `RA_HIGH_RISK_CUSTOMERS` 한정(`PEP_INDIVIDUALS` 해제는 별건). 아울러 **BR-005·BR-008** 에 RA 상세 수동 등재 상신의 화면 노출 scope 가 종전 `aml:case:update` 가 아니라 **`aml:admin:high-risk-registry`**(엔진·bo-api 게이트 실측 일치, 노출 축소)여야 하고 위임 본문 키도 `memberRef`(종전 `customerRef` 오기 — 정정 전 상시 400)여야 한다는 정정 문장을 추가했다(§0-7 이격 정정). §12-A.4 AML-RA-003 흐름(BR-006)에 등재 해제 액션 1문장을 병기했다. **(QA 정정)** API §HRR admin surface 위임 노트의 감사 `HRR_DEREGISTRATION_SUBMITTED` 카테고리를 `POLICY_CHANGE`(엔진 감사 카테고리)→`DATA`로 정정하고, 엔진 `:deregister` 계약에 재파생 보류 시 **409 `AML.STATE_CONFLICT`**(잠금 스냅샷 재검증 실패의 409 `AML.HRR_DEREGISTRATION_BLOCKED` 와 코드로 구분)와 재검증 시점 이미 미등재인 경우의 **EXECUTED(no-op)** 계약을 BR-009 에도 동일하게 추가했다. BR-006 미상신 상태의 수동 등재 상신 모달 scope 를 `aml:case:update`→`aml:admin:high-risk-registry`로 정정하고, BR-006 등재 해제 액션 노출 조건을 처분표 **ⓐ 등재·ⓑ 해제 대기 없음 충족 시 노출**, **ⓒ 재파생 사전조건·ⓓ 독립 사유는 상신 시 400 안내**로 세분했다. **(QA 재검 2차 정정)** API §HRR admin surface `:deregister` NOOP `200` 본문을 `approvalId:null`만이 아니라 **202 와 동형인 6필드 본문**(`action:"DEREGISTER"` 포함)임을 명시하고, §12-A.4 BR-006·§12-B.6 BR-009 에 **RA 상세 흐름도의 ③ 경영진 승인/반려 액션·결재 이력은 해제 결재(`DEREGISTER|`)도 포괄하되, 흐름도 단계 판정(④ 등재·⑤ floor)은 등재 결재·read-back `registered` 만 사용**한다는 정정 문장을 각각 추가했다(코드=truth, bo-web `deriveHrrFlow` 는 `latestApproval`(등재 결재)만 입력). | 근거=aegis-aml aml-svc `HighRiskCustomerRegistrationService`·`HighRiskRegistry#removeFromReferenceList`·`OnboardingSnapshotReassessmentSupport`·`ApprovalDispatchService`·`WebhookOutboxEmitter`, bo-api `AmlHighRiskRegistryController`·`AmlHighRiskRegistryService`, bo-web `HrrRegistrationSection`·`AmlHighRiskRegistry.tsx`. API §HRR admin surface·§8.1 콜백·DB §3.20·§5.16·integration §3.4 동일 작업 단위. |
| **9.101** | **2026-09-04** | **Hanpass Global Team** | **§12-B.6 AML-HRR-001 헤더 행 잠금 직렬화(BR-003)·BR-004 조회 권한 정정(코드=truth, PLAN 20260904-aml-hrr-registry-row-lock, aegis-aml F-094).** **BR-003** 에 참조 리스트 변경·`HRR_REGISTRATION`·`PEP_APPROVAL` 세 결재 유형이 레지스트리 헤더 행 잠금(DB §3.19)으로 직렬화되어 동시 EXECUTED 에도 version 을 정확히 +1 하고 상대 변경을 소실하지 않는다는 문장을 추가. **BR-004** 권한 조회를 코드 실측과 상이하던 `aml:case:read`(read-only) 에서 **`aml:admin:approval`(결재함 API §8 `GET /approvals` 게이트와 동일, `BO_SUPER_ADMIN` 포함) — 미보유 세션은 탭 본문에 사전 안내(BR-008 ②, F-092)** 로 정정(코드=truth, bo-api `AmlApprovalController` `@PreAuthorize`). **(QA 폴리시 보강)** DB §3.19 잠금 주석에 컨트롤러 GET 의 최초 seed INSERT 예외(별건 advisory lock)와 잠금 보유 시간 실측 근거(2026-09-04, 등재 90건·동시 4건 승인당 3.2~13.0s, bo-api 위임 timeout 30s 이내, READ COMMITTED 전제)를 추가하고, 표 **권한** 행에 ③ 승인 히스토리 탭 권한(`aml:admin:approval`, BR-004)을 병기해 조회/변경 권한과 구분했다. | 근거=aegis-aml aml-svc `HighRiskRegistryStorePort#findForUpdate`·`HighRiskRegistryJpaRepository#findByTenantIdForUpdate`·`HighRiskRegistryConcurrencyIntegrationTest`, bo-api `AmlApprovalController`. API §HRR admin surface·DB §3.19 동일 작업 단위. |
| **9.100** | **2026-09-04** | **Hanpass Global Team** | **§12-B.6 AML-HRR-001 참조 리스트 변경 결재 EXECUTED 적용 시 자동 등재 2종 보존·version 규칙·자동 등재 listType 상신 거부 정본 반영(코드=truth, PLAN 20260904-aml-hrr-registry-apply-staged, aegis-aml F-093).** **BR-002** 에 자동 등재 2종(`PEP_INDIVIDUALS`·`RA_HIGH_RISK_CUSTOMERS`) listType 을 대상으로 `PUT .../reference-lists/{listType}` 상신 시 400 `AML.VALIDATION_ERROR` 로 거부됨을 명시. **BR-003** 에 결재 EXECUTED 적용은 상신 스냅샷 전체 교체가 아니라 **대상 listType 만 교체**하며 그 사이 자동 등재된 항목은 보존된다는 문장을 추가(종전 전 5종 교체·상신 시점 목표 version 절대값 기록은 결함이었다). **BR-004** digest 문구에 `v<version>`(subjectRef 파생)은 **상신 시점 예상값**이며 적용 시 자동 등재가 끼어들면 실제 version 이 더 클 수 있다는 주석을 추가(변경 전 상태 부재 각주와 동일 한계). | 근거=aegis-aml aml-svc `HighRiskRegistryService#applyStaged`·`ReferenceListType#isAutoRegistered`. API §HRR admin surface·DB §3.19 동일 작업 단위. |
| **9.99** | **2026-09-04** | **Hanpass Global Team** | **§12-B.6 AML-HRR-001 BR-008 보강 — 등재 상신 위임 본문 필드명 정정·승인 히스토리 탭 결재 scope 사전 안내·읽기 전용 템플릿 다운로드(코드=truth, PLAN 20260904-aml-hrr-registration-followups).** ① bo-api 수동 등재 상신(`POST .../high-risk-registry/registrations`)의 엔진 위임 본문 필드명 `memberRef`→`customerRef` 정정(엔진 계약 1:1, bo-api 공개 요청 DTO `memberRef`는 무변경) — 정정 전에는 이 경로가 항상 400 이었다(엔진 자동상신은 영향 없음). ② 승인 히스토리 탭(③)은 3탭 구성을 그대로 유지하되 결재 scope(`aml:admin:approval`) 없는 세션에는 사전 안내를 표시한다(결재 목록 API 권한과 BR-004 조회 권한의 이격은 별건, 사용자 지목 지시 필요). ③ 참조 리스트 관리 3종(PRODUCT/VASP/HIGH_NET_WORTH)의 템플릿 다운로드는 `aml:admin:high-risk-registry` 변경 권한과 무관하게 읽기 전용 세션에도 항상 노출한다(클라이언트 생성 CSV, 서버 호출 없음). | 근거=aegis-aml bo-api `AmlHighRiskRegistryService`, bo-web `components/aml/AmlHighRiskRegistry.tsx`. API §HRR admin surface 동일 작업 단위. |
| **9.98** | **2026-09-04** | **Hanpass Global Team** | **§12-B.6 AML-HRR-001 조회 권한 이격 정정·참조 리스트 검색 BR-008 신설(코드=truth, PLAN 20260904-aml-hrr-registry-search).** ① **권한 행 정정** — 코드(bo-api)가 조회까지 `aml:admin:high-risk-registry` 전용 scope 로 축소했던 것을 PRD 정본(조회 **`aml:case:read`(또는 `aml:admin:high-risk-registry`)**)대로 복귀하고, 변경은 **`aml:admin:high-risk-registry`(🔒 준법감시 책임자)**로 유지해 재정합화(2026-09-04 라이브 실측 — `AML_COMPLIANCE`(`aml:case:read`) 세션이 조회에서 403). ② **BR-008 신설** — 참조 리스트 관리 탭에 회원 ref·라벨 부분일치 검색(클라이언트 필터, 5개 리스트 동시, `일치 n / 전체 m` 표기)을 추가하고, 변경 액션(업로드·상신)은 `aml:admin:high-risk-registry` 보유 세션에만 노출·그 외 읽기 전용 안내를 표시하며, bo-api 403(`BO-FORBIDDEN`)은 `commonUi.errors.bo.forbidden`로 해석해 원문 노출을 없앤다. 자동 등재 항목(라벨 없음)은 라벨 열 `-`. 엔진(aml-svc) 무접촉 — 변경 권한 확대 없음(mutation plane 전용 scope 불변). | 근거=aegis-aml bo-api `AmlHighRiskRegistryController`·`AmlStage4RbacWebMvcTest`, bo-web `components/aml/AmlHighRiskRegistry.tsx`·`lib/aml-hrr.ts`·`lib/error-messages.ts`. API §HRR admin surface 동일 작업 단위. |
| **9.97** | **2026-09-04** | **Hanpass Global Team** | **§8.1 AML-CASE-001 담당자 지정 팝업·배정 가능 계정 디렉토리·목록 탭 딥링크 반영(코드=truth, PLAN 20260904-aml-case-assignee-picker).** **BR-001** 담당자 필터를 자유 텍스트에서 배정 가능 계정 디렉토리 드롭다운(`GET /bo/aml/cdd/case-assignees`)으로 정정. **BR-002** 에 담당 지정·변경·생성 시 담당자를 팝업(본인 최상단 고정 + 배정 가능 로그인 계정 목록·검색)에서 선택하는 규칙과 `조사 시작(담당 지정)`(기본 선택 본인, 다른 계정 선택 가능·상태 `INVESTIGATING` 전환 동일) + 목록 탭 `?tab=mine\|all\|due-soon\|closed` 딥링크(헤더 케이스함 → 내 케이스)를 추가. **BR-008** 의 `[+ 케이스]` 모달 담당자 입력·목록 필터 표기를 텍스트에서 디렉토리/팝업 선택으로 정정. 화면 골격 액션행에 `[조사 시작(담당 지정 팝업)]` 병기. 헤더 우측 결재함 왼쪽에 케이스함 메뉴(`CaseInboxMenu`) 신설은 `03-bo-iam-approval-functional-spec.md` §6 참조. 엔진(aml-svc) `assignedTo` 문자열 계약·`Case.assignTo` 무변경. | 근거=aegis-aml bo-api `AmlCaseAssigneeController`, bo-web `CaseAssigneePickerModal`·`CaseAssigneeField`·`CaseInboxMenu`·`AmlCaseList`. API §13 CaseDto `assignedTo` 동기화. |
| **9.96** | **2026-09-03** | **Hanpass Global Team** | **§12-A.4 AML-RA-003 BR-006 대기결재 표기 통일(코드=truth, aegis-aml refactor/bo-web-common-listrow-intl-pending).** RA 상세 등재 패널(`HrrRegistrationSection`)의 대기중 결재 안내를 문장형 `Callout(warning)` 에서 F-088 공통 `PendingApprovalNotice`(tone=warning — 결재 유형·상태·상신자·상신 시각 + 안내 문장 슬롯 `note` + 결재함 딥링크)로 치환해 같은 화면의 회원 심사결과 카드와 정보 항목을 맞춘다. 안내 문장 3종·i18n 키·승인/반려 액션·scope·4-eyes 계약 무변경. 함께 bo-web 행 박스 공통 `ListRow`(11곳 치환 — 10곳 className 문자열 동일·1곳 토큰 집합 동일(CSS 동등))·테스트 헬퍼 `test-utils/render-with-intl`(잠금 외 59 테스트 치환) 정리 — 화면 계약 영향 없음. | 근거=aegis-aml bo-web `components/aml/{HrrRegistrationSection,PendingApprovalNotice}`·`components/common/ListRow`·`test-utils/render-with-intl`, 회귀 테스트 `HrrRegistrationSection.pending.test.tsx`·`PendingApprovalNotice.note.test.tsx`·`ListRow.test.tsx`. 코드=truth. |
...
| **9.89** | **2026-08-19** | **Hanpass Global Team** | **통화 프로파일 spec 리뷰 중간·낮음 이격 보완(코드=truth, U13 후속 정합).** ① **§13.4 구성·BR-003 — 결재함 귀속 분리 서술(M2)**: CTR 임계(`CTR_THRESHOLD`, 승인선 `REPORTING_OFFICER`)=**AML 결재함(AML-APR-001)** / FDS 룰 금액(`RULE_PARAM`)·FDS 규제통화 전환(`TENANT_REGULATORY_CURRENCY`)=**FDS 결재함(SFDS-APPR-001)** 로 상신 귀속을 명시(한 결재함 승인으로 오독 차단). BR-003 의 '결재함(AML-APR-001) 딥링크' 서술을 **현행 구현(approvalId 텍스트 노출·딥링크 없음)** 으로 정정하고 딥링크는 후속 요건 후보 ⓖ 신설로 이동. ② **§13.4 BR-002(L4)** — 2-pass 중간 창 안내 '배지로 안내' → '**Callout 목록으로 안내**'(구현 실측 — apply warnings·현황 warnings 모두 Callout 렌더). §13.4 구성의 STEP 나열도 실제 방출 순서(`FDS_REGULATORY_CURRENCY`→`BINDING`→`CTR_THRESHOLD`→`REPORT_RULES`→`FDS_RULES`)로 정렬. ③ **§12-B.3 AML-STAT-001 BR-010 신설(M5)** — CTR 임계 섹션 3-상태 렌더 규칙: `ctr_threshold` 조건행 부재→섹션 미노출(룰 상세)/기존 메타 유지(시나리오 상세), 조건행 존재·`unit=null`→미바인딩 안내(편집 폼 미노출·기본 통화 합성 금지), `unit` 값→그 통화로 편집 폼(API §3.6a `RuleConditionView.unit` nullable 계약과 짝). | 코드=truth. 근거=bo-web `components/aml/{AmlTenantCurrencyProfile,AmlReportRuleDetail,AmlScenarioStatsDetail}.tsx`·`lib/currency-profile.ts`, bo-api `CurrencyProfileApplyService`(FDS 상신 subjectKind `RULE_PARAM`/`TENANT_REGULATORY_CURRENCY`)·`AmlReportRuleParamService#conditionViews`. API `02-aml-api.md` §3.16a·§3.6a·§5 동일 작업 단위. |
...
| **설정** | 탐지·심사 정책 | TM 시나리오 관리(AML-TM-002) · CDD 체크리스트 정책(AML-CDD-001) · 국가위험 관리(AML-CTRY-001) · **WLF 엔진 조절(AML-WLF-005)** · **STR·룰 효과성 통계(AML-STAT-001)** · **CTR·룰 효과성 통계(AML-STAT-001)** · RA 모델 관리(AML-RA-002) · **위험 점수 가산 관리(AML-RA-002 ⑤ 탭 딥링크)** |
...
| 5 | RA 모델 관리·시뮬레이션·등급 조정·이름 위험 가산 (AML-RA-002, 5탭) | T-11 | `ra-models/{code}/simulate`(③ 시뮬레이션), `ra-models/versions/{v}:activate` 🔒, `risk-scores/{id}/override` 🔒 |
...
> **WLF/제재 스크리닝 소스(hanpass-ph 정합)**: 실시간 제재·PEP 스크리닝 신호 소스는 `member-svc zoloz_aml_screening`이며 점수 분해·상태·매칭 후보는 이를 정규화한 결과다. 명단군·provider 도메인은 불변이다. 48h 초과와 refresh FAILED/IMPORTING은 명단관리 운영 경고이며, `activeVersion`이 있으면 마지막 적용 엔트리로 계속 판정한다. 필수 명단 적용본 자체가 없을 때만 fail-closed한다(DB §3.6·API §3.2). 매칭 가중치·negative penalty·검토/고신뢰 임계는 Policy Pack 파라미터가 유일 정본이다.
...
| WLF NAME 인라인(2026-08-22) | **요청명**(`requestName`)은 이 스크리닝 요청의 `nameTokens` 순서 보존 결합 스냅샷, **명단 기재명(현재 게시본)**(`matchedEntryNames` 및 후보 카드 `listName`)은 안정 entry id가 가리키는 현재 feed 표기다. WLF 검토/처리 이력의 스크리닝 행은 `aml:case:read`만으로 두 NAME 값을 클릭·사유·`aml:pii:reveal` 없이 바로 표시하며, 값이 없으면 대체·마스킹·회원 마스터 폴백 없이 `—`을 표시한다. 목록/상세 응답에 이름이 실제 있으면 bo-api가 요청당 `RAW_DATA_ACCESS` 1건을 남긴다. **② 상위승인 결재 큐는 approval API 행이라 이 인라인 범위에서 제외**하고, NAME 이외의 기존 `subjectIdentity` reveal 계약은 불변이다. |

#### 비즈니스 규칙

- **BR-001**: 탭 바는 **[검토 필요 ●] [상위승인] [처리 이력]** — 이 화면에서 `검토 필요` 탭이 `active`. 필터는 `명단군 / 대상 유형 / 점수 / 기간` + `거래번호·대상 검색` 텍스트. 검색은 현재 조회 응답 범위에서 거래번호(`transactionRef`)·대상키(`targetRef`)·스크리닝ID(`screeningId`)를 대소문자 무시 부분일치로 거른 뒤 거래번호 그룹핑한다(`screeningMatchesQuery` 공용). bo-api 목록 API는 `q`를 받지 않으므로 미지원 `q`를 서버로 전송하지 않는다. `status=POSSIBLE_MATCH` 건만 표시.
...
- **BR-007a (WLF NAME 인라인 카브아웃, 2026-08-22)**: BR-007의 reveal 게이트는 **NAME 이외** 필드와 WLF 이외 화면에 그대로 적용한다. 다만 AML-WLF-001 검토·AML-WLF-003 처리 이력의 스크리닝 응답은 요청명과 매칭 명단 기재명 NAME 2종을 `aml:case:read`만으로 인라인 표시한다. 원천 시스템 `aml:screen:evaluate` 단독에는 두 필드가 없고, `counterpartyName`·TM 알림·근거거래·Subject360은 P0-09 마스킹 계약을 유지한다. 과거 결과의 요청명 누락은 회원 마스터/마스킹 값으로 추정하지 않고 `—`이며, 명단 이름은 재sync 후 현재 게시본으로 바뀔 수 있다.
...
- **BR-011 (hanpass-ph 거래당 sender·receiver 그룹 — 코드 정본 `AmlWlfTransactionGroups`)**: 해외송금 건은 검토 큐를 **거래번호(`transactionRef`) 단위 그룹**으로 묶어 표시한다 — 그룹 헤더(거래번호) 아래 **송금인(`CUSTOMER`)·수취인(`COUNTERPARTY`) 역할별 최신 각 1건**(거래당 2회 스크리닝)을 나열하고, 각 행에서 상세 열람·`[오탐 면제]`(FP 화이트리스트, 4-eyes `FP_WHITELIST`)를 수행한다. 동일 거래·역할의 과거 결과가 남아 있어도 현재 그룹에는 중복 노출하지 않는다. 그룹 간 순서는 각 그룹에 표시되는 스크리닝 중 최신 `createdAt` **DESC**이며, 동률은 `screeningId` DESC·`transactionRef` ASC로 결정해 upstream 응답의 최초 등장 순서에 의존하지 않는다. 유효한 `createdAt`이 없는 그룹은 유효 시각 그룹 뒤에 둔다. `transactionRef` 없는 건(국내송금·월렛 단건)은 평면 행으로 폴백한다. 역할 라벨은 송금인/수취인(`wlfRoleLabel`), 대상유형 미정의 시 일반 대상유형 라벨로 폴백. 상태 배지는 `amlMon.enum.screeningStatus.*` 번역을 해석해 raw enum/message key를 노출하지 않는다.
...
> - **(d) 당연고위험(HRR) 레지스트리 floor 병합(코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)** — 위 (a)~(c) 파생 직후 **당연고위험 레지스트리**(§12-B.6 AML-HRR-001)에 대상 회원(`customerRef`)이 등재돼 있으면(`registry.classify` 존재) 그 등재 floor 를 스크리닝·국가위험 floor 와 **병합**한다: `forcedFloorGrade = max(위 (a)·(b′) floor, registryFloor)`(레지스트리 tier `VERY_HIGH→PROHIBITED`·그 외 `HIGH`), `mandatoryHighRiskReasons` = 기존 사유 ∪ 매치된 참조 리스트 이름(예 `RA_HIGH_RISK_CUSTOMERS`·`PEP_INDIVIDUALS`·`HIGH_NET_WORTH`, 순서 보존·중복 제거), `forcedFloorEvidence` = 기존 근거 ∪ `{listType, subjectRef, tier}`(명단 매치 근거와 동일 포맷). **이 병합은 최초 CDD 뿐 아니라 등재 이후의 모든 재파생(재이행 re-CDD·국가정책 재평가 등 `forcedReassessment` 경로 포함)에서 매회 적용된다** — 명단(WLF) 매치가 그사이 사라진 회원의 재CDD 도 레지스트리 등재가 유지되는 한 `mandatoryHighRisk=true` 로 유지된다(해소 경로는 §12-B.6 BR-009 등재 해제 결재뿐이며, 명단 매치 소멸로 자동 해소되지 않는다). 등재가 없으면(레지스트리 미존재) 기존 (a)~(c) 산출과 **완전히 동일**(diff 0). 엔진 케이스 RA-C30(`docs/qa/engine-rule-cases.md`) 검증.
> - **적용본 부재 = 평가 보류 / refresh 이상 = 마지막 적용본 평가**: 필수 워치리스트의 `activeVersion`이 없으면 1차 RA를 보류하고 CDD 인입 자체는 성공한다. 반면 stale(48h 초과)·refresh FAILED/IMPORTING이어도 적용본이 있으면 WLF와 RA를 즉시 산출한다. freshness 상태·마지막 성공시각은 명단관리에서 별도 확인하며 가짜 점수나 가짜 명단을 합성하지 않는다.
...
│                                     │ 30일      1,240                │    │
│                                     └────────────────────────────────┘    │
│ [평가 보류·EDD 대상] 대상 │ 업무결정 │ 보류사유 │ 재시도 상태/횟수 │ 다음 재시도 │ 수신시각 │
│   member-…901          │ EDD 필요 │ RA 보류  │ 처리 중 / 2회    │ 06-25 10:30 │ 06-25 09:00 │
│ [1차 RA 내역] 대상 │ 단계 │ 점수·등급 │ 1차 주요 요인 │ 인입시각 │ 평가시각 │ 재심사일 │
│   cust_…501        │ 1차  │ 62 중간    │ country, screening │ 06-24 │ 06-24 │ 12-24 │▶
└──────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────┐
...
│ └─────────────────────────────────┘                                      │
│ [2차 RA 내역] 대상 │ 단계 │ 점수·등급 │ 2차 주요 요인 │ 인입시각 │ 평가시각 │ 재심사일 │
...
| 인입시각 | 회원 최초 인입(CDD 최초 수신) 시각 (`ingestedAt` = `aml_customers.created_at`, v9.104). 1차 RA 내역 기본 정렬 키. 인입 회원 타일(`coalesce(onboarding_at, created_at)`)과 달리 목록·필터와 같은 `created_at` 단독 기준 |
| 정렬 기준 | 1차 RA 내역 셀렉트 — `인입시각 최신순`(기본, `sort=INGESTED_AT_DESC`) / `평가시각 최신순`(`sort=EVALUATED_AT_DESC`). 변경 시 1페이지로 리셋. 2차 RA 내역은 평가시각 최신순 고정(v9.104). 인입시각 **컬럼**은 1·2차 공용 표라 2차 내역에도 표시되며 기본 정렬만 1차 전용 |
...
| 평가 보류·EDD 대상 | **실제 점수가 아직 없는 현재 1차 RA HELD 업무**. 별도 DTO로 `targetRef`·`decision=EDD_REQUIRED`·`reason=ONBOARDING_RA_HELD`·`retryStatus(PENDING|PROCESSING)`·`retryAttempts`·`nextRetryAt`·`receivedAt`을 표시한다. 기본 최근 30일 ON(toggle 해제 시 전체), targetRef 부분검색, server page. scoreId/점수/등급/권고조치를 합성하지 않는다. |
| 1차 RA 내역 | **최근 30일 인입(온보딩) 회원의 1차(ONBOARDING) RA — 회원별 최신 1건, 전 등급, 2차(ONGOING) 진입 회원 제외**(`risk-scores?riskGrade=LOW,MEDIUM,HIGH,PROHIBITED&scenario=ONBOARDING&latestPerTarget=true&registeredWithinDays=30&excludeOngoingTargets=true&sort=INGESTED_AT_DESC` — 인입 필터는 기본 ON 체크박스, 해제 시 전체 기간; `excludeOngoingTargets=true` 는 항상 동봉되는 고정 조건, BR-010) + 1차 주요 요인(KYC·국적/국가위험·이름 스크리닝·이용 서비스/채널). 서버 페이징(`page/size`, `ListPager` 총건수 노출) **정렬(v9.104)**: 기본 `sort=INGESTED_AT_DESC`(회원 최초 인입 시각 내림차순 — 일괄 재평가로 평가시각이 갱신돼도 최근 인입 회원이 상단), 셀렉트로 `EVALUATED_AT_DESC`(평가시각 최신순) 전환 가능. |
...
- **BR-012 (v9.104 — 1차 RA 내역 기본 정렬 = 인입시각, 코드=truth, aegis-aml PLAN 20260908-aml-ra-initial-list-ingest-sort)**: `1차 RA 내역`은 **회원 최초 인입 시각(`ingestedAt`) 내림차순**이 기본이며 동률·인입시각 없음은 평가시각 desc·`scoreId` 문자열 desc 로 결정적이다. 당연고위험 등재 결재·모델 활성화 등 **일괄 재평가로 `evaluatedAt` 이 갱신돼도 최근 인입 회원이 상단을 유지**해야 한다(운영자가 신규 회원 1차 RA 를 순서대로 처리하는 화면 목적). 운영자는 셀렉트로 `평가시각 최신순`으로 전환할 수 있고, `2차 RA 내역`·`고위험·EDD`·`평가 보류` 표의 정렬은 기존과 같다. 기본 정렬(`EVALUATED_AT_DESC`)도 `scoreId` 문자열 desc 2차 키로 페이지 경계가 흔들리지 않는다.
- **BR-011 (v9.94 — 점수 없는 HELD 업무 가시화)**: 1차 RA 탭은 score 목록 위에 별도 `평가 보류·EDD 대상` 표를 둔다. 현재 행 정의는 decision `EDD_REQUIRED` + reason `ONBOARDING_RA_HELD` + score-null + retry job `PENDING|PROCESSING`; tenant를 두 원천에 동일 적용한다. `COMPLETED`와 score-backed decision은 제외하며, retry 성공 시 immutable CDD decision을 바꾸지 않고 job이 `COMPLETED`로 수렴해 보류표 0건·실제 ONBOARDING 점수표 1건이 된다. exact replay는 최초 HELD 응답을 유지하되 업무표를 재등장시키거나 점수를 중복 생성하지 않는다(RA-C21). 별도 표 행은 점수 상세로 이동하지 않고 targetRef의 Subject360 업무참조 링크만 제공한다.
...
- **BR-013 (2026-09-10 — 당연고위험 등재 회원의 재CDD floor 유지, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)**: `고위험·EDD` 목록·`1차/2차 RA 내역`에 노출되는 등급·`당연고위험` 배지(BR-006)는 **등재 회원의 재CDD(재이행)에서도 유지**된다 — 온보딩 파생이 레지스트리 floor 를 스크리닝/국가위험 floor 와 병합하므로(§5(RA 콜아웃) (d)), 명단(WLF) 매치가 소멸한 재CDD 도 등재가 살아있는 한 `HIGH`(또는 `VERY_HIGH` tier 는 `PROHIBITED`) 등급·`당연고위험` 배지가 그대로 노출된다. 재CDD 는 신규 ONBOARDING 스코어를 append 하므로 `1차 RA 내역`에는 새 행이, 상세(§12-A.4 BR-006)에는 갱신된 흐름도가 나타난다.
...
| **탭** | ① 버전 목록 / ② factor 편집 / ③ **시뮬레이션** / ④ 등급 조정 이력 / **⑤ 이름 위험 가산**(2026-08-13 신설, 딥링크 `?tab=signal-scaling`) (모델 저작 흐름: 셋업→편집→검증→활성화) |
| **메뉴 진입점** | 설정 › 탐지·심사 정책에 **`RA 모델 관리`**(`/aml/ra-models`)와 **`위험 점수 가산 관리`**(`/aml/ra-models?tab=signal-scaling`, en `Risk Signal Scaling`) **2개**. 후자는 같은 화면의 ⑤ 탭 딥링크이며 별도 라우트가 아니다 — 별도 화면을 만들면 같은 `parameters` JSONB 를 두 쓰기 경로가 편집해 4-eyes 버전 이력이 쪼개진다. `pathname` 에 쿼리가 없으므로 활성 표시·권한 도출은 쿼리 없는 경로(`/aml/ra-models`)로 매칭한다(그러지 않으면 항목이 영원히 비활성이고 nav 파생 권한 요구에서도 빠진다). AML 메뉴 **22 → 23** 은 2026-08-13 사용자 지시로 승인된 IA 변경이다 |
...
| ⑨ 이름 위험 가산(⑤ 탭) | 신호 origin별 대상 룰·커브·하한/상한 점수·최소/최대 배수 | 2차 상시 RA 의 **이름 위험 신호 점수 비례 배수** 정의(`parameters.signalScaling`). ONGOING 정의의 한 키이므로 ② factor 편집과 **같은 초안**을 편집하고 ③ 시뮬레이션·활성화 상신 동선을 그대로 쓴다. |

#### 화면 레이아웃 — ⑤ 이름 위험 가산 (신호 점수 비례 배수 · 2026-08-13)

```
┌──────────────────────────────────────────────────────────────────────────┐
│ ↩ 진입: 좌측 메뉴 `위험 점수 가산 관리` 딥링크(?tab=signal-scaling)         │
│         또는 RA 상세(AML-RA-003)의 배수 근거 패널 `[정의 보기 ▶]`          │
├─ 신호 origin 카드: PEP_NAME_RISK_SIGNAL ────────────────────────────────┤
│  대상 룰        ☑ STR_PEP   ☐ STR_STRUCTURED …                          │
│                 (STR_SANCTION 은 선택 불가·비활성 + 사유 안내)            │
│  커브           ( ) 계단(현행)   (•) 비례                                 │
│  하한 점수 [0.65]  상한 점수 [0.92]   최소/최대 배수 [0.25] / [1.00]      │
│  ── 미리보기 ──  0.60→0.25 · 0.65→0.25 · 0.78→0.48 · 0.92→1.00 · 0.99→1.00│
│                  (= STR_PEP 기여 가중 100 이 각각 25/25/48/100/100 로 적용)│
│  [초안 저장]  [시뮬레이션 → ③]  [활성화 상신 🔒 4-eyes]  ← 기존 동선 재사용 │
└──────────────────────────────────────────────────────────────────────────┘
```
...
- **BR-008 (⑤ 이름 위험 가산 — 신호 점수 비례 배수 관리, 2026-08-13 사용자 지시 "점수관리하는 메뉴도 필요 … 그 점수대로 엔진에서 작동", 코드=truth)**: 2차(ONGOING) 정의의 선택키 `parameters.signalScaling` 을 운영자가 편집한다.
  - **신규 화면이 아니라 이 화면의 ⑤ 탭이다.** 이 파라미터는 ② factor 편집이 다루는 것과 **물리적으로 같은 `parameters` JSONB** 이므로 별도 라우트·별도 저장 API 를 만들면 같은 정본을 두 쓰기 경로가 편집하게 되고 4-eyes 버전 이력이 두 갈래로 쪼개진다. 편집 동선은 **기존 `versions:copy` → 초안 저장 → ③ 시뮬레이션 → 활성화 상신 🔒(`RA_MODEL`)** 를 그대로 재사용한다 — **새 승인 유형·새 결재선·새 권한 없음**(`aml:admin:policy`, §12-A.8 승인 유형 표 무변경).
  - **좌측 메뉴 진입점 1개**(`위험 점수 가산 관리` → `?tab=signal-scaling` 딥링크)를 둔다. 사용자 요구의 "메뉴" 를 탭으로만 이행하지 않되 라우트는 늘리지 않는 절충이며, AML 메뉴 22 → 23 은 승인된 IA 변경이다.
  - **편집 항목·검증은 엔진 불변식과 동일**하다: 대상 룰(명시 allowlist, **`STR_SANCTION` 은 선택 불가·비활성**), 커브(계단/비례), `0 ≤ 하한 < 상한 ≤ 1`, `0 < 최소 배수 ≤ 최대 배수 ≤ 1`. 프론트 인라인 검증과 백엔드 저장 검증이 **같은 경계값**을 쓴다 — 두 검증식이 갈리면 화면이 통과시킨 정의가 저장에서 400 이 되거나 그 반대가 된다.
  - **하한/상한 점수의 기본값은 WLF 문턱을 승계한다(임의 상수 금지).** 하한은 **검토 후보로도 올리지 않는 점수**(WLF 검토 임계), 상한은 **WLF 자신이 고신뢰로 보는 점수**(고신뢰 임계)와 같은 값을 쓴다 — 배수 곡선의 두 끝이 명단 판정 밴드와 정렬되므로 운영자가 "이 점수대는 WLF 가 어떻게 보는가" 와 "그래서 RA 가중이 얼마인가" 를 같은 축으로 읽는다. 서비스가 WLF 문턱을 튜닝하면 이 대역도 같은 값으로 옮긴다. 반면 **최소 배수는 정책 선택**이다(0 이면 알럿이 발동했는데 RA 기여가 사라져 조사자가 근거를 못 보고, 1 이면 기능이 무의미하다) — 라이브 점수 분포가 쌓이면 4-eyes 로 재튜닝한다.
  - **미리보기가 필수 표시 요소다.** 점수 → 배수 → "STR_PEP 기여 가중 100 이 얼마로 적용되는가" 를 같은 줄에서 보여 준다. 배수 상한이 `1.0` 이므로 **어떤 설정으로도 가중이 늘지 않는다**(약한 신호만 깎인다) — 화면 문구는 이 방향을 "가산" 이라는 낱말로 상향처럼 읽히게 쓰지 않는다.
  - **토글을 켜는 행위 자체로 탐지가 약해지지 않는다** — 신규 정의 시드는 최소·최대 배수가 모두 `1.0`(전 대역 항등원)이며, 감산은 운영자가 최소 배수를 **명시적으로** 낮출 때만 생긴다.
  - **미지 키 거부 갱신 필수** — 화면의 ONGOING 파서가 `parameters` 미지 키를 오류로 처리하므로, 이 키를 허용 목록에 넣지 않으면 새 키를 가진 ACTIVE 모델을 **화면이 invalid 로 거부**한다(신규 화면을 만들었어도 이 수정은 필요했다).
  - **4-eyes 시뮬레이션이 변화를 보여야 한다** — ③ 시뮬레이션 표본이 알럿의 신호 표식을 함께 싣지 않으면 `signalScaling` **만** 바꾼 초안이 언제나 등급 이동 0 으로 보이고 checker 가 "영향 없음" 으로 승인한다. 표본 복원은 엔진과 **같은 역파싱 계약**을 쓴다.
  - **i18n**: 탭·항목·미리보기·오류 문구 전부 ko/en 카탈로그 동시 등록(코드 문자열 화면 노출 금지).
...
- **BR-001 (검색조건 보강)**: 탭은 `알림 적체` / `시나리오 관리` / `최근 거래`(BR-014). 알림 적체 필터는 `시나리오 / 발생 출처 / 심각도 / 상태 / 기간 / 채널 / corridor` + `대상 식별자(customerRef)` 텍스트. **발생 출처(AML 모니터링 / FDS 에스컬레이션)는 목록 컬럼·필터로 제공**하여 FDS 위임 알림을 즉시 식별(`source_origin`, DB §5.20). 채널(충전/국내/해외/인바운드)·corridor(remit cross-border)는 hanpass-ph 거래 데이터 필드(integration §4.2).
...
- **BR-014 (③ 최근 거래 탭 신설, 코드=truth, PLAN 20260819-aml-tm-recent-transactions)**: **③ 최근 거래** 탭(`/aml/tm?tab=transactions`)은 **알럿 발생 여부와 무관하게 거래성 family 4종**(`transaction|remit|domestic|wallet`)의 canonical 거래를 최신순으로 노출한다 — 알럿 0건 PASS 거래도 포함(구 ① 알림 적체 탭은 알럿 존재 건만 노출해 중립 거래(예 202 ACCEPTED·알럿 0건)가 화면 어디에도 보이지 않던 공백을 해소). 데이터 원천은 `GET /api/v1/bo/aml/transactions`(API §2.5a, aml-svc `GET /api/v1/aml/transactions` §2.4 verbatim 위임). **검색 3종**(정확일치, select 1개 + 검색어): 거래번호(`transactionRef`, 기본)·이벤트ID(`eventId`)·회원키(`memberRef`) — 부분 일치(LIKE) 미지원. 부가 필터: 상품(`product`)·기간(`from`/`to`). 행은 거래번호·이벤트ID·회원키·상품·금액/통화(부호 그대로 표기 — reversal/refund netting 음수 절대값 변환 없음)·발생시각·평가결과를 표시한다. **결측 표기**: 파생 필드(거래번호·상품·금액/통화·채널·방향 등)는 전건 null 허용이며 결측 값은 `-` 로 표기하고 행을 목록에서 제외하지 않는다(API §2.4 결측 허용 규칙과 짝 — 직접 인입(`POST /api/v1/aml/events`) 행도 동일). **평가결과**: 알럿 0건은 "PASS(알럿 없음)" 정적 배지, 알럿 ≥1건은 알럿 건수+발동 룰 코드(최대 5건) 배지를 클릭 가능하게 노출하고 클릭 시 **① 알림 적체 탭으로 전환 + 대상 식별자(targetRef=회원키) 필터 프리필**(신규 알럿 목록 필터·거래번호 조인 없음 — 스코프 밖). **회수(RETIRED) 알럿은 알럿 건수·발동 룰 코드 집계에서 제외**(F-034 정합, ①과 동일 원칙). 서버 페이지네이션(기본 20·상한 200). PII 노출 없음(참조 토큰·enum·금액/통화/시각만, §19.2). **FDS 판정 컬럼(aegis-aml PLAN 20260906-fds-block-aml-propagation)**: 평가결과 뒤에 `FDS 판정` 컬럼 — 같은 거래참조로 FDS 가 차단(`BLOCK`)한 거래는 `FDS 차단` 배지 + 차단 룰명(최대 3, 초과 `+n`, title 에 전체 룰명·판정 시각)을 표시하고, 통지 없는 거래는 `-`, fail-safe(룰 0건)는 `룰 정보 없음(fail-safe)` 을 표시한다(공통 `FdsDecisionBadge`, 원천 `fdsDecision` = API §2.4 additive 필드, 알럿 상세 근거거래 표에도 동일 배지 병기). AML TM 알럿·2차 RA 산출과 무관한 additive 표기이며 FDS 측 알럿·케이스를 열지 않는다. | 근거=aml-svc `TransactionQueryController`(`adapter/in/rest`)·`TransactionEventBrowseService`/`TransactionEventBrowsePort`(`application`)·`TransactionEventBrowseAdapter`(`adapter/out/persistence`), bo-api `aml/transactions/{controller,service,dto}` package(`AmlTransactionController`/`Service`/`Dtos`), bo-web `components/aml/{AmlTmAlerts,AmlTmTransactions}.tsx`·`hooks/useAmlTm.ts`·`lib/aml-tm.ts`. DB §7 V70(additive 인덱스)·API §2.4/§2.5a 동일 작업 단위.
...
│   [메모/증빙 추가]  [상태·담당·우선 변경]  [조사 시작(담당 지정 팝업)]      │
...
| 트리거(목록 열, v9.86 신설) | 목록 표에 `eddTrigger` 라벨을 별도 열로 노출(없으면 `-`) — 상세 진입 없이 발단 유형을 한눈에 구분(PLAN 20260902-aml-case-workbench U11) |
...
- **BR-001**: 탭은 `내 케이스` / `전체` / `기한 임박` / `종결`. 필터는 `케이스 타입 / 상태 / 우선순위 / 담당자`(배정 가능 계정 디렉토리 드롭다운 — `GET /bo/aml/cdd/case-assignees`) + `대상 식별자`. 기한 임박/초과는 ⚠.
- **BR-002**: 생성(`POST cases`)·상태/담당/우선 변경(`PATCH`)·메모/증빙(`/timeline`)은 결재 불필요(`aml:case:update`). 메모·증빙은 timeline append, 수정 불가(증적 무결성). **담당 지정·변경·생성 시 담당자는 팝업(본인 최상단 고정 + 배정 가능 로그인 계정 목록·검색)에서 선택하며, `조사 시작(담당 지정)`은 기본 선택 본인·다른 계정 선택 가능(상태 `INVESTIGATING` 전환 동일). 목록 탭은 `?tab=mine|all|due-soon|closed` 딥링크를 지원한다(헤더 케이스함 → 내 케이스).** 담당자 **비우기(미배정)** 는 `[+ 케이스]` 생성 모달에 한정된다 — 담당·우선순위·기한 변경(PATCH)은 담당자 필드 미전송=무변경(`null`)이라 UI 해제 경로가 없으며, 해제 계약(빈값/`null` 구분) 신설은 엔진 계약 변경이 필요한 별건이다.
...
- **BR-002a (조사관 작업대 재구성 — 발단 계보·증적·처분, 코드=truth alert-triage-disposition)**: 케이스 상세는 조사관 작업대로서 ① **발단 계보 패널** — `originAlertId`(TM 알림에서 개설된 케이스) 요약(발동 룰코드·심각도·evidence 요지)을 노출하고 알림 상세 `/aml/tm/alerts/{alertId}` 로 딥링크한다(`originScreeningId` 는 WLF 발단, 기존 계보 필드 재사용). ② **조사 증적** — `POST .../cdd/cases/{caseId}/timeline` append(코멘트·판단 근거 유형, 수정 불가·BR-002 무결성) UI. ③ **처분** — 케이스 종결(`:close`, EDD_CLOSE 4-eyes 202 → PENDING 배지)은 알림 오탐 종결과 동일한 공통 `DispositionReasonModal`(사유 코드 select + 메모)로 사유를 받고, STR 권고로 개설된 STR 케이스는 규제 보고 흐름(AML-REP-001)으로 링크한다. **알림에서 전환된 STR_REVIEW·SAR_REVIEW 등 조사 케이스도 EDD_REVIEW 와 동일하게 `EDD_CLOSE` 결재로 종결**한다(케이스 유형 무관 — BR-003·코드=truth, STR_REVIEW `:close` 400 결함 해소). ④ **감사 표시** — timeline 행에 행위자·시각·전이 이력을 노출한다. **(v9.86, PLAN 20260902-aml-case-workbench U3) 생성·배정·상태변경 감사 타임라인** — 케이스 개설(알럿 hand-off·FDS 위임·1차/2차 RA EDD 자동 개시·수동 생성 전부)마다 `CREATED` 1행이 발단 evidenceRefs(알럿/거래/점수/스크리닝 토큰)와 함께 자동 append되고, 담당자·우선순위·기한·상태 변경은 각각 `ASSIGNED`/`PRIORITY_CHANGED`/`DUE_CHANGED`/`STATUS_CHANGE` 로 실제 변경분만 append된다(동일값 재요청은 신규 행 0) — 종전에는 2차 RA 자동 개시 케이스(147/181건 실측)가 생성 근거를 전혀 남기지 못했던 결함을 해소한다. 발단 식별자가 소급 불가한 기존 EDD 케이스는 인접 RA 점수 창(−60s~+5s) 매칭으로 소급 backfill 하고 '추정(backfill)' 배지를 단다(§12-A.7 BR-004). ⑤ **판단 재료 패널(코드=truth, edd-judgment-data)** — 공통 `CddSnapshotPanel`(CDD 온보딩 신고 정보: 직업(occupation)·자금출처(sourceOfFunds)·KYC 레벨·거주국·**신고소득 구간(declaredIncomeBand)**·실사 완료 시점(kycVerifiedAt). 내부 projection `UNKNOWN`은 ko `미상`/en `Unknown`, 지원 KYC 차원 0개는 기존 empty callout으로 표시)과 공통 `ActivitySummaryPanel`(최근 30일 건수·합계, **최근 3개월 평균** 거래액(= 최근 90일 Σ / min(3, 관측월수), API §monthlyAvgPhp) + **최근 30일 거래 리스트 임베드**(대상 360° 거래 피드 동일 소스, 최신순 **전량 표시** — 엔진 피드 cap 50 초과분만 360° 딥링크 안내) — 신고소득 대비 배율(incomeMultiple)은 API 파생만 유지·`UNKNOWN`이면 null이며 화면 미노출)을 배치한다. 두 패널은 RA 상세(AML-RA)·회원원장(AML-MEM)·고객 프로필(CDD-002)과 공유한다. (알림→케이스 발단 계보와 처분 UX 는 §7.1 BR-002a 트리아지 상태기계와 한 폐루프를 이룬다.)
  - **`UNKNOWN` 배율 wire 해석**: 위 `incomeMultiple=null`은 BO Java/read-model 의미다. 기존 `NON_NULL`
    producer JSON에서는 key가 생략되고, bo-web consumer는 생략/explicit null을 모두 배율 산출 불가로
    처리한다. activity 자체는 non-degraded required fields를 유지한다.
...
- **BR-008 (v9.86, PLAN 20260902-aml-case-workbench U11; 담당자 디렉토리·팝업 정정 PLAN 20260904-aml-case-assignee-picker)**: `[+ 케이스]` 버튼(`aml:case:update` 스코프)이 수동 생성 모달을 연다 — 입력: 케이스 타입(운영 6종 우선 노출)·대상 식별자(`targetRef`)·우선순위·담당자(팝업 선택, `CaseAssigneeField`)·기한·사유(필수) → `POST .../cdd/cases` 성공 시 상세로 이동. 목록 서버 필터는 `케이스 타입/상태/우선순위/담당자`(디렉토리 드롭다운)/`대상 식별자` 외 **`기한 임박`** 탭이 서버 `dueSoon=true`(72시간 이내 + 비종결) 파라미터로 위임된다(API §2.7). 페이지네이션은 엔진 total 미제공으로 범위 밖 — 캡션에 "최대 50건" 명시. 입력한 사유는 케이스 `CREATED` 타임라인 note 에 `MANUAL;REASON=<사유>`(알림 전환은 `ALERT_HANDOFF=…;ACTION=…;REASON=<사유>`)로 영속되어 조사관이 개설 근거를 타임라인에서 확인한다.
...
- **BR-008 (2026-09-03 — "회원 심사결과" 카드, 코드=truth, PLAN 20260903-aml-decision-status-webhooks, 사용자 지시로 F-084 부분 해제)**: SubjectPanel 아래(관리자 액션 패널 BR-005 와 별개) `MemberDecisionCard`(bo-web `components/aml/MemberDecisionCard.tsx`) 카드를 신설한다 — `GET /api/v1/bo/aml/customers/{customerRef}/decision`(엔진 `MemberDecisionView` 1:1 미러, bo-api §API 2.3) 을 조회해 계정계가 "결재를 통해 승인/거절이 완료된 시점" 의 상태값을 화면에서도 확인할 수 있게 한다.
  - **표시 항목**: 결정 배지(`APPROVED`/`REJECTED`/`EDD_REQUIRED`/`REPORTED`, ko/en 카탈로그 `enum.memberDecision.*`)·출처(`source`: `INGEST`/`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`(2026-09-09 BR-009, 관리자 직접 확정))·근거(`reason`)·확정 시각/결재자(`decidedAt`/`decidedBy`)·연결 케이스(`caseId` → `[케이스 상세 ▶ → AML-CASE-002]` 딥링크)·대기 결재(`pendingApproval` — 상태 배지 + `[결재 대기함 ▶ → /aml/approvals]` 딥링크)·인입 스냅샷(`ingestDecision` — CDD 인입 원문 결정·근거·시각)·이력(`history[]`, 최신순 상한 10 표시).
...
- **BR-009 (2026-09-09 — 관리자 액션 패널 「회원 심사 직접 승인/거절」, 코드=truth, PLAN 20260909-aml-ra-direct-member-decision)**: 관리자 액션 패널(BR-005)에 **당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 이미 **EXECUTED** 된 회원**에 한해 회원 심사결과(BR-008 `MemberDecisionCard`)를 **추가 4-eyes 결재 없이 즉시** 확정하는 두 버튼 [회원 심사 승인]/[회원 심사 거절]을 노출한다 — 2차 CDD 재이행(`CDD_REVIEW`)이 회원을 다시 `EDD_REQUIRED` 로 되돌려도 별도 결재 경로가 없던 결함을 해소한다.
  - **노출 조건**: 당연고위험 대상(`mandatoryHighRisk===true ∧ targetType===CUSTOMER`) ∧ 등재 상태 read-back(`GET .../high-risk-registry/registrations/{customerRef}`) `registered===true` ∧ 회원 심사결과 조회 성공(`GET .../decision`) ∧ 대기 중인 `EDD_CLOSE`/`RELATIONSHIP_REJECT` 결재(`pendingApproval`) 없음 ∧ 세션 scope `aml:admin:approval`. [회원 심사 승인]은 현재 결정이 `APPROVED` 가 아닐 때, [회원 심사 거절]은 `REJECTED` 가 아닐 때만 노출한다(동일 값 재확정은 버튼 자체가 없음).
  - **액션**: 각 버튼 → 공용 `ChangeRequestModal`(대상 회원·현재 결정 배지·**승인·거절 모두 사유 필수**·"4-eyes 결재를 거치지 않고 즉시 확정되며 계정계에 통지된다"는 경고 문구) → `POST /api/v1/bo/aml/customers/{customerRef}/decision`🔒(body `{decision: APPROVED|REJECTED, reason}`, scope `aml:admin:approval`) → 엔진이 게이트(ⓐ 회원 미존재 404 · ⓑ decision 값 오류 400 · ⓒ `reason` 검증 — 공백은 `REJECTED` 한정 400, 256자 초과는 decision 무관 공통 400 · ⓓ HRR 등재 미완료 409 · ⓔ 대기 결재 존재 409(HRR 등재 **해제** 결재 대기 중이어도 `registered=true`면 직접 확정 가능 — 등재/해제 판정 분리, F-095 경계) · ⓕ 현재 결정과 동일 409 · ⓖ actor 미검증 400)를 통과하면 `aml_member_decisions`(DB §3.22g)에 `source=ADMIN_DIRECT`(결재·케이스 없음, `approvalId`/`caseId`/`caseType`/`finalStatus` 모두 `null`) 행을 append 하고, 같은 트랜잭션에서 웹훅 `AmlMemberDecisionResolved`(§API §8.1, `docs/aml-data.md` §12.1a)를 기존 배관·목적지 규칙 그대로 발행한다.
  - **화면 반영**: 성공 시 안내(notice) + 회원 심사결과 카드(BR-008)·RA 재조회 무효화로 새 결정이 즉시 보인다. 출처 라벨은 `enum.memberDecisionSource.ADMIN_DIRECT`("관리자 직접 확정")로 표기해 결재 경유(`EDD_CLOSE`/`RELATIONSHIP_REJECT`)와 구분한다.
  - **1차 RA 검토(`onboardingReview`)와 무관**하다 — 직접 확정은 회원 관계 결정이며 RA 점수·등급 산정과 독립(BR-005 ①·② 액션은 그대로 유지).

- **BR-010 (2026-09-10 — 등재 회원 재CDD floor 유지, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)**: `RA_HIGH_RISK_CUSTOMERS`(또는 그 외 참조 리스트) 등재가 완료(EXECUTED)된 회원의 **재CDD(재이행)** 는 온보딩 파생이 레지스트리 floor 를 병합하므로 `mandatoryHighRisk=true`·등재 패널(BR-006) 흐름도·직접 확정 버튼(BR-009, `eligible = risk.mandatoryHighRisk`)이 **그대로 유지**된다 — 스크리닝(WLF) 매치가 그사이 사라져도 등재 자체가 해소되지 않는 한(§12-B.6 BR-009 해제 결재) 흐름도는 사라지지 않는다. 재CDD 는 신규 스코어를 append 하며(`occurredAt` 이 직전 평가시각 이후일 때 — 이전이면 `ALREADY_EVALUATED` 로 직전 스코어를 재사용하며 신규 스코어를 만들지 않는다) 신규 스코어의 `mandatoryHighRiskReasons` 에 매치된 참조 리스트 이름이 포함된다(§5(RA 콜아웃) (d)). tier `VERY_HIGH` 등재 회원의 재CDD 는 온보딩 결정이 `EDD_REQUIRED` 가 아니라 `REJECT`(`RISK_GRADE_PROHIBITED`)다.
...
- **BR-004 (조사관 작업대 재구성, 코드=truth, PLAN 20260902-aml-case-workbench)**: 상세 화면은 `GET .../cases/{caseId}/workbench`(API §2.7) 단일 응답으로 아래 요소를 함께 렌더한다. 발단 패널의 계보는 V72 추정 소급(`LINEAGE=BACKFILL_INFERRED`)에만 '추정' 배지를 달고, V73 확정 소급(`LINEAGE=BACKFILL`, origin 컬럼 확정값)은 정상 계보로 표기한다. 워크벤치 조회 실패는 빈 데이터로 표기하지 않고 상단 경고 배너 + 하위 패널 '미조회' 로 구분한다(정직한 빈 상태).
  - **"이 케이스가 열린 이유" 발단 패널** — `eddTrigger` 사람 문장 + 발단 카드(있는 것만): 알럿(룰코드·심각도·상태·발동 설명·거래참조·`/aml/tm/alerts/{id}` 딥링크) / 점수(등급·점수·1차/2차 구분·평가시각·트리거 알럿 칩·`/aml/ra/{targetRef}` 딥링크) / 스크리닝(상태·점수·매치 엔트리·`/aml/wlf?tab=history&screeningRef=` 딥링크). **V72 backfill 로 채워진 계보는 '추정(backfill)' 배지**를 달아 사람 4-eyes 확인분과 구분한다(휴리스틱 −60s~+5s 창 매칭). 발단이 전혀 없으면 정직한 빈 상태 안내(수동 연결 유도).
  - **관련 의심 거래·알럿 표** — 대상의 관련 알럿(회수 알럿 제외, 최대 50건, 최신순) + 발단 배지.
  - **회원 위험 스트립** — 정본 위험 등급·점수·1차/2차 구분·재실사 예정일·당연고위험 배지 + 이 회원의 다른 케이스 건수(전체/열린).
  - **결재 대기 카드**(`PENDING_APPROVAL`) — 결재 유형·요청 처분·maker·상신 시각 + `/aml/approvals` 딥링크.
  - **연결 보고 카드** — 연결 STR/CTR 상태·`/aml/reports/{id}` 딥링크.
  - **액션 게이팅** — `aml:case:update` 스코프 없으면 조치 버튼 전부 숨김(읽기 전용). terminal 상태는 조치 없음. `PENDING_APPROVAL`은 결재 대기 카드만(직접 조치 없음). `REPORTED` 종결 옵션은 케이스 유형에 맞는 연결 보고가 `SUBMITTED`/`ACKNOWLEDGED`일 때만 활성.
  - **타임라인 세로 시각화** — 날짜 그룹·카테고리(조사/처분/시스템) 필터 + 행위자·시각·증빙 칩 딥링크. 감사 타임라인은 생성(`CREATED`)·배정(`ASSIGNED`)·우선순위(`PRIORITY_CHANGED`)·기한(`DUE_CHANGED`)·상태(`STATUS_CHANGE`) 변경마다 append(BR-002a 참조). 시스템 note(`KEY=VALUE;…` 고정 어휘 — CREATED·STATUS_CHANGE·ASSIGNED·PRIORITY_CHANGED·DUE_CHANGED·CHECKLIST_ITEM)는 원문 대신 키별 도메인 라벨 칩(EDD 트리거·룰코드·알럿 상태·FDS 결정·사유·소급(확정/추정))으로 표시하고, 파싱 불가·자유문구(MEMO 등)는 원문 그대로 둔다.
  - **CDD/EDD 체크리스트 실행** — 항목별 상태(대기/완료/해당없음)·메모·증빙 입력 + 진행률(필수 완료 n/m). `OPEN`/`INVESTIGATING` 에서만 편집 가능(그 외 읽기 전용 + 안내).
  - **관계·UBO 표** — 유형·방향·상대·지분·UBO 배지(양방향 에지). 유효기간 열은 도메인 미매핑으로 **범위 밖**.
  - **증빙 카드** — 타임라인·체크리스트의 증빙참조를 유형별(알럿/거래/스크리닝/점수/보고/결재/manifest) 카드로 딥링크.
...
### 12-A.10 AML-MBR-001 · 회원관리 — 회원원장·CDD/EDD/심사결과 히스토리 (조회, 문서 미정의 → 코드 truth로 신설)
...
| **API** | (bo-api 위임) `GET /api/v1/bo/aml/members/{memberRef}/ledger`(원장 요약) · `GET /api/v1/bo/aml/members/{memberRef}/cdd-history?types=&page=&size=`(이력 페이지) — 엔진 `GET /api/v1/admin/aml/members/{memberRef}/{ledger\|cdd-history}` 위임(API §2.x·DB §3.22f) · `GET /api/v1/bo/aml/customers/{ref}/decision`(「AML 심사결과」 타일 소스, scope `aml:case:read`, 2026-09-10 QA 정정 — API §2.3) |

- **구성**: **회원번호(memberRef = `originator.partyReference` = `aml_customers.customer_ref` 단일 키; `nationalIdentityKey`는 PII fallback 토큰) 검색** → ① **원장 요약**(현재 **「KYC 상태(계정계)」**(v9.106 명명 — 계정계 CDD 인입이 제공·소유하는 검증 상태, BR-004)·위험 등급·재실사 예정일 + **「AML 심사결과」 타일**(v9.106 신설, **2026-09-10 QA 정정** — `GET .../customers/{ref}/decision` **병합 뷰**를 렌더하므로 현재 결정 배지·출처는 확정 3종(`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`) **+ 인입 스냅샷 `INGEST`**(결정 `EDD_REQUIRED` 가능) **4종**(이력 `MEMBER_DECISION`(BR-001) 의 확정 3출처 제한과 구분)·확정 시각·확정자, 결정 없음/조회 실패는 `-` 로 정직 강등·에러 콜아웃 없음, AML-CDD-002 프로필 원장 재사용·마스킹) + **공통 신원 요약 카드**(`IdentitySummaryCard`, v9.54 신설 — 원장 요약 아래·CDD 스냅샷 위 배치, 문서 미정의 지점 → 가정: 표시명/유형/국적/KYC 상태/등록일/온보딩일 + PERSON 분기 신원확인·문서 hash·생년(`birthYearMasked`, `YYYY-**-**`)·성별(`genderMasked`, 고정 토큰)·거주국(`country`) + ENTITY 분기 설립국·대표자, RA-003·CDD-002 와 동일 컴포넌트 공유) + ② **CDD/EDD/심사결과 히스토리** 유형별 목록(탭 `CDD 히스토리`/`EDD 히스토리`/**`심사결과`**(v9.106 신설), react-query·서버 페이징 20건, 최신순). 이력 행 = 이력 유형·**AML 심사결과(스냅샷)**(v9.107 신설, BR-005)·KYC 상태·위험 등급 스냅샷·인입 이벤트/trace·수행 주체·발생 시각(masked 스냅샷, raw PII 미노출 §19.2). **AML 심사결과(스냅샷) 열**은 CDD 행(`CDD_INITIAL`/`CDD_REVIEW`)이면 그 인입의 온보딩 판정 스냅샷(`OnboardingDecision`·사유·조치, `aml_cdd_onboarding_decisions.event_id` 조인 읽기 병기 — BR-005·DB §3.22f)을 배지로, `심사결과` 탭의 `MEMBER_DECISION` 행이면 확정 결정 배지(`details.decision`(결정)·`details.source`(출처) — **v9.106 은 이를 단순 "탭 행 병기"로만 서술했으나 v9.107 부터 본 열로 이동 렌더**)를 표시하며, 그 외 유형(`EDD_*`·재이행 접수)이나 판정 스냅샷 없는 CDD 행은 `-`. 유형 필터 `types`(반복 파라미터, 예 `types=CDD_INITIAL&types=EDD_OPENED`).
- **BR-001 (v9.106 개정 — 6종→7종)**: 이력 유형(`history_type`, DB §5.36 `cdd_history_type`) **7종** — `CDD_INITIAL`(최초 CDD 이행 — 원장 미존재 시 첫 `customer.cdd.completed` 인입)·`CDD_REVIEW`(재이행 CDD — 기존 회원 후속 재인입)·`EDD_OPENED`(EDD 착수)·`EDD_CLOSED`(EDD 종료, 4-eyes 승인 실행)·`CDD_REISSUE_REQUESTED`·`EDD_REISSUE_REQUESTED`(CDD/EDD 즉시 재이행 접수 — AML-RA-003 ⑤ 관리자 액션 BR-005, 계정계 지시)·**`MEMBER_DECISION`**(v9.106 신설 — 회원 심사결과 확정. 확정 경로 3종(§12-A.4 BR-005/BR-009) `EDD_CLOSE`(EDD 케이스 종료 4-eyes)·`RELATIONSHIP_REJECT`(관계거절 4-eyes)·`ADMIN_DIRECT`(RA 상세 관리자 직접 확정)가 같은 트랜잭션 안에 1행 append, `kyc_status` 는 확정 시점 원장 스냅샷·**`risk_grade` 는 확정 결정의 등급 우선, 없으면 확정 시점 원장 스냅샷**(2026-09-10 QA 정정, DB §3.22f 와 동일 표현), `details={decision(APPROVED\|REJECTED\|REPORTED), source(EDD_CLOSE\|RELATIONSHIP_REJECT\|ADMIN_DIRECT), decisionId, approvalId?, caseId?}` — null 값 키 제외, 사유 원문 미기록). 적재 지점 **4종**: (a) `customer.cdd.completed` 인입, (b) EDD 착수/종료, (c) 즉시 재이행 접수, **(d) 회원 심사결과 확정**(v9.106).
...
- **BR-004 (2026-09-10 신설, 코드=truth)**: **KYC 상태는 계정계가 CDD 인입으로 제공하는 검증 상태**(`aml_customers.kyc_status`, `docs/aml-data.md` §12.2 필수 필드)**이며 AML 심사결과(`aml_member_decisions`)와 별개 축이다** — **AML 은 KYC 상태를 변경하지 않는다**(최종 회원승인 판단은 채널 몫, 내부 변경 경로는 `CddProfile` 전이표가 `VERIFIED→REJECTED`를 금지). 두 축은 화면에 각각 「KYC 상태(계정계)」 타일과 「AML 심사결과」 타일로 병기하고, AML 심사결과의 확정 이력은 `MEMBER_DECISION`(BR-001) 으로 append-only 보존한다.
- **BR-005 (v9.107 신설, 코드=truth, aegis-aml PLAN 20260910-aml-member-history-decision-column)**: CDD 이력 행(`CDD_INITIAL`/`CDD_REVIEW`)마다 **그 인입의 온보딩 판정 스냅샷**(`aml_cdd_onboarding_decisions`, DB §3.22f, F-076 불변)을 **이력 조회 시점에 `source_event_id`=`aml_cdd_onboarding_decisions.event_id` 조인으로 병기**한다 — 이력 행 자체(append-only, BR-002)와 판정 스냅샷 모두 무수정(읽기 전용 read-back). API `GET .../members/{memberRef}/cdd-history` 응답 행에 `ingestDecision{decision, reason, riskGrade, requiredAction, scoreId, createdAt}|null` additive(CDD 유형·매칭 스냅샷 존재 시만 값, 그 외 유형·스냅샷 부재는 `null`, API §2.x). 스키마 변경 없음.
...
- **BR-007a (고정 룰 editable 전수 검증, 2026-08-23)**: 대표 `STR_VELOCITY_CASH.count_threshold` 1축 PASS를 전체 룰 PASS로 보지 않는다. CTR 공유 임계가 `CTR_SINGLE`·`CTR_DAILY` 양쪽을 반전시키는지, ACTIVE STR의 6개 editable 파라미터가 각자 엔진 판정을 반전시키는지 BO typed maker/checker 4-eyes로 전건 확인한다. 각 case는 exact alert/보고/ONGOING RA 계보·replay 신규 0·full snapshot 복원을 요구하고, read-only 이름 임계와 editable 0 ACTIVE STR 4종은 변경하지 않은 채 parity/reachability를 확인한다.
...
- **BR-010 (CTR 임계 섹션 3-상태 렌더 — 다통화, 2026-08-19 신설, 코드=truth)**: 룰 상세(BR-006, `/aml/stats/report-rules/{ruleCode}`)와 시나리오 효과성 상세(BR-009, `/aml/stats/scenarios/{scenarioCode}`)의 CTR 임계 섹션은 신규 훅 없이 이미 가진 `conditions[]` 의 `ctr_threshold` 조건행(`paramKey="ctr_threshold"`)에서 통화를 파생하며(F-042 잠금 훅 경계 보존) **3-상태**로 렌더한다 — ① **조건행 부재**: 섹션 미노출(룰 상세, fail-safe)/기존 메타 표현 유지(시나리오 상세). ② **조건행 존재·`unit=null`(미바인딩)**: 미바인딩 안내(Callout)만 표시하고 CTR 임계 편집 폼 미노출 — 기본 통화 합성 금지(PHP 폴백 없음, API §3.6a `RuleConditionView.unit` nullable 계약). ③ **`unit` 에 통화 코드**: 그 통화로 CTR 임계 편집 폼(`CtrThresholdEditSection`) 렌더. 편집 불가 세션은 조건행 유무와 무관하게 기존 읽기 전용 안내를 유지한다(룰 상세 분기 우선순위: 조건행 부재 → 미노출, 편집 불가 → 읽기 전용 안내, `unit=null` → 미바인딩 Callout, `unit` 값 → 폼 — BR-003 표시=read/편집=admin 분리 그대로).
...
| **권한** | 조회 `aml:case:read`(또는 `aml:admin:high-risk-registry`) / 변경 `aml:admin:high-risk-registry`(🔒 준법감시 책임자 — v7.0의 `aml:admin:policy`를 T2 전용 scope로 확정) / ③ 승인 히스토리 탭은 `aml:admin:approval`(BR-004) |
...
- **BR-002 (v7.0 — T2 확정)**: 분류 기준(criteria)은 read-only seed(가정 A2)이며, **참조 리스트 변경만** `🔒`(subjectType=`HIGH_RISK_REGISTRY`, 준법감시 책임자) + 전 이력 감사(`POLICY_CHANGE`). 국가 팩터는 AML-CTRY-001(COUNTRY_RISK)이 정본이며 본 화면은 파생 표시(이중 관리 금지). 참조 리스트 종류는 5종(`PRODUCT`·`VASP`·`HIGH_NET_WORTH` + **`PEP_INDIVIDUALS`**(V24) + **`RA_HIGH_RISK_CUSTOMERS`**(V28), DB §5.33). **`PEP_INDIVIDUALS`(정치적 주요인물) 리스트는 본 화면에서 직접 편집하지 않고 회원별 PEP 경영진 승인(AML-CDD-002 ③, subjectType=`PEP_APPROVAL`·승인선 `EXECUTIVE_APPROVAL`)이 EXECUTED될 때 자동 등재(tier HIGH)되고, `RA_HIGH_RISK_CUSTOMERS`(당연고위험 지정 고객) 리스트는 RA 당연고위험 등재 승인(BR-005, subjectType=`HRR_REGISTRATION`·승인선 `EXECUTIVE_APPROVAL`)이 EXECUTED될 때 자동 등재된다** — 본 화면 ② 는 승인 폐루프 자동 등재 2종을 read-only 로 노출하며, 템플릿 CSV 변경 상신 대상은 운영자 관리 3종(PRODUCT/VASP/HIGH_NET_WORTH)으로 한정한다(bo-web `AML_HRR_MANAGED_LIST_TYPES`). **(2026-09-04 — 자동 등재 listType 상신 거부, aegis-aml F-093)** 자동 등재 2종(`PEP_INDIVIDUALS`·`RA_HIGH_RISK_CUSTOMERS`)을 대상으로 `PUT .../reference-lists/{listType}` 을 상신하면 엔진이 **400 `AML.VALIDATION_ERROR`** 로 거부한다(운영자 편집 대상 아님·승인 폐루프 전용).
- **BR-003 (v7.0 — T2 확정)**: 참조 리스트 갱신은 결재 EXECUTED 시점에 적용되며, 일치 고객을 **즉시 동기 batch로 RA 강제 상향 재평가**(엔진 RA 유스케이스 연계, tier→forced floor: VERY_HIGH→PROHIBITED·HIGH→HIGH, 상향만 보장, 가정 A6·A7). 0건은 no-op. 고액자산가 추출 결과의 개별 확정·예외는 케이스(AML-CASE-001) 흐름 재사용. **(2026-09-04 — 적용 범위 정정, aegis-aml F-093)** 결재 EXECUTED 시점 적용은 상신 스냅샷 전체 교체가 아니라 **대상 listType 만 교체**하며, 그 사이 자동 등재된 항목은 보존된다. **(2026-09-04 — 직렬화, aegis-aml F-094)** 참조 리스트 변경·`HRR_REGISTRATION`·`PEP_APPROVAL` 세 결재 유형은 레지스트리 헤더 행 잠금(DB §3.19)으로 직렬화되어 동시 EXECUTED 에도 각 결재가 version 을 정확히 +1 하며 상대 변경을 소실하지 않는다.
- **BR-004 (v9.12 — 승인 히스토리 탭)**: ③ 승인 히스토리는 참조 리스트 변경의 4-eyes 결재 이력을 노출한다 — 상태(상신/승인/반려/반영)·변경 요약·상신자(`makerId`)·승인자(`checkerId`)·상신/결재 시점을 표(`DataTable`)로 최신순(submittedAt desc) 정렬해 표시하고, 행 상세는 모달에서 **변경 전→후 표**(공통 `ApprovalChangesTable`)로 노출한다. **공통 결재 조회 `GET /api/v1/bo/aml/approvals?subjectType=HIGH_RISK_REGISTRY`(API §8 결재함) 재사용으로 HRR 전용 히스토리 엔드포인트·백엔드 신규 없음** — subjectType 고정 래퍼(직접 fetch 금지)로 ② 참조 리스트 변경 상신(BR-002)의 결과를 같은 화면에서 폐루프 확인. 권한 조회 `aml:admin:approval`(결재함 API §8 `GET /approvals` 게이트와 동일, `BO_SUPER_ADMIN` 포함) — 미보유 세션은 탭 본문에 사전 안내(BR-008 ②, F-092). **(v9.72 — 감사 추적성)** 참조 리스트 변경 행은 대상이 엔진 내부 토큰 `UPDATE|<version>` 이고 변경 요약이 비어 있어 **무엇이 바뀐 결재인지 식별할 수 없던 결함**을 해소한다: ⓐ **대상** 열은 코드 토큰 대신 프론트 카탈로그 라벨(`참조 리스트 변경 (v10)` / `Reference list change (v10)`, ko·en)로 표기하고, ⓑ **변경 요약** 열은 엔진이 staged payload 에서 파생한 **적용 결과 스냅샷**(`v<version> · PRODUCT=n · VASP=n · HIGH_NET_WORTH=n · PEP_INDIVIDUALS=n · RA_HIGH_RISK_CUSTOMERS=n`, API §3.7 `detail`)을 **목록만으로** 노출한다(상세 재조회 없음). 결재 행에는 **변경 전 상태가 남지 않으므로** 추가/제거된 개별 `subjectRef` diff 는 산출할 수 없고 결과 스냅샷으로 대신하며, 화면은 그 사실을 표 하단 각주로 밝힌다(추측 금지). **(2026-09-04 주석, aegis-aml F-093)** digest 의 `v<version>`(subjectRef 파생)은 **상신 시점 예상값**이며, 적용 시 자동 등재(`HRR_REGISTRATION`·`PEP_APPROVAL`)가 사이에 끼어들면 실제 적용 version 이 이보다 더 클 수 있다(변경 전 상태 부재 각주와 동일 한계). 리스트가 0건이 된 경우도 보이도록 전 `listType` 을 항상 포함한다. **(v9.41)** RA 당연고위험 등재 승인(BR-005, `subjectType=HRR_REGISTRATION`) 건도 동일 탭에서 함께 조회한다 — 참조 리스트 변경(`HIGH_RISK_REGISTRY`)과 회원 등재(`HRR_REGISTRATION`) 두 subjectType 축을 병합 표시(공통 결재함 재사용, 전용 엔드포인트 미신설). **(병합 표시 후속 정합)** 두 축이 한 표에 섞이므로 **대상 컬럼**(`subjectLabel ?? subjectRef`)을 필수로 노출한다. **결재 시점**은 엔진 목록 요약 `ApprovalSummary.executedAt`을 그대로 표기하며 미실행 건만 `-`다.
- **BR-005 (v9.41 — RA 당연고위험 자동 등재 폐루프, 코드=truth)**: 회원관리 RA(1차 온보딩·2차 상시 위험평가)가 회원을 **당연고위험(`mandatoryHighRisk=true`, CUSTOMER 대상)으로 분류하면 엔진이 `HRR_REGISTRATION` 결재를 자동 상신**한다(maker=시스템 주체 `system:ra-engine`, tier=PROHIBITED→VERY_HIGH·그 외 HIGH, reason에 당연고위험 사유 코드 병기). **승인선은 `EXECUTIVE_APPROVAL`(고위경영진 수동승인)** — 자동 등재가 아니라 결재라인을 타고, **결재 EXECUTED 시에만** ② 참조 리스트의 `RA_HIGH_RISK_CUSTOMERS`(당연고위험 지정 고객) 리스트에 등재가 확정되고 RA 강제 상향 재평가가 트리거된다. 이미 등재(EXECUTED)/상신 대기(PENDING) 회원의 재상신은 **멱등 no-op**(승인→재평가→재상신 재진입 루프 종료 불변식) — 오류가 아니다. **(2026-09-10 명세, aegis-aml PLAN 20260910-aml-hrr-floor-followups)** 이 멱등 no-op 의 '이미 등재' 판정은 `RA_HIGH_RISK_CUSTOMERS` 리스트 기준이며, `PEP_INDIVIDUALS`·`HIGH_NET_WORTH` 등 다른 참조 리스트에만 등재된 회원의 당연고위험 분류는 BR-010 병합 → 본 BR-005 자동상신 대상이다(BR-006 시뮬레이터 ⓪′→⓪″ 체인의 전제). RA 상세(AML-RA-003) 화면의 등재 패널(bo-web `HrrRegistrationSection`)에서 **수동 상신·등재 결재 상태(미상신/대기/등재됨) 확인**이 가능하며(동일 `POST .../registrations` 진입점, v9.47 폐루프 흐름도로 확장 — §12-A.4 BR-006), 승인/반려는 공통 결재함(`:approve`/`:reject`)이 `HRR_REGISTRATION` 을 동일 라우팅한다. 등재 결과는 ③ 승인 히스토리(BR-004)와 ② 참조 리스트(read-only)에 폐루프로 반영된다. **(2026-09-04 정정, aegis-aml F-095)** RA 상세 수동 등재 상신(§12-A.4 BR-006 미상신 모달)의 화면 노출 scope 는 종전 상수 `aml:case:update` 가 아니라 **엔진·bo-api 게이트 실측과 일치하는 `aml:admin:high-risk-registry`**(BR-008 변경 권한과 동일)로 정정한다 — 정정 전에는 `aml:case:update`만 가진 세션에도 상신 버튼이 노출되던 이격이었고, 정정 후에는 노출이 축소된다. 위임 본문 키도 종전 `customerRef` 가 아니라 엔진 계약대로 **`memberRef`**로 정정한다(정정 전에는 이 경로가 항상 400 이었다).
...
- **BR-008 (2026-09-04, 코드=truth)**: 참조 리스트 관리 탭은 회원 ref·라벨 부분일치 검색(클라이언트 필터, 5개 리스트 동시, 리스트별 `일치 n / 전체 m` 표기)을 제공한다. 변경 액션(업로드·상신)은 `aml:admin:high-risk-registry` 보유 세션에만 노출되고 그 외는 읽기 전용 안내를 표시한다. bo-api 403(`BO-FORBIDDEN`)은 `commonUi.errors.bo.forbidden` 로 해석해 원문 노출 없음. 자동 등재 항목(라벨 없음)은 라벨 열 `-`. 조회 범위 = 참조 리스트 5종(자동 등재 `PEP_INDIVIDUALS`·`RA_HIGH_RISK_CUSTOMERS` 포함), 항목은 토큰(code)+tier(+label) 만 — raw PII 없음. ③ 승인 히스토리 탭은 3탭 구성을 유지하되 결재 scope(`aml:admin:approval`) 없는 세션에는 사전 안내를 표시한다(결재 목록 API 권한과의 이격은 별건); 관리 리스트 3종의 템플릿 다운로드는 읽기 전용 세션에도 허용한다(클라이언트 생성 CSV, 서버 호출 없음). **(2026-09-04 정정, aegis-aml F-095)** 본 변경 액션 권한(`aml:admin:high-risk-registry`)은 RA 상세(§12-A.4 BR-006)의 수동 등재 상신·**등재 해제 상신(BR-009)** 화면 노출에도 동일하게 적용된다(§0-7 이격 정정, BR-005 참조).

- **BR-009 (2026-09-04 신설 — 자동 등재 회원 등재 해제 경로, 코드=truth, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095)**: `RA_HIGH_RISK_CUSTOMERS`(당연고위험 지정 고객) 리스트에 등재된 회원 중 **오등재**(수동 등재 경로로 잘못 등재됐거나 독립 당연고위험 사유가 소멸한 회원)를 해제하는 4-eyes 결재 경로다(`PEP_INDIVIDUALS` 리스트 해제는 고객 `is_pep`/PEP 승인 역전·정책 재정의가 필요해 **별건**). 기존 등재 결재 유형 **`HRR_REGISTRATION`을 재사용**하되 `subjectRef`에 **`DEREGISTER|<customerRef>` 접두**를 붙여 등재 상신과 구분하고(등재 핸들러는 접두 행을 거부, 해제 핸들러는 접두 없는 행을 거부), **승인선은 등재와 동일한 `EXECUTIVE_APPROVAL`**(고위경영진 수동승인)이다. **상신 처분표**: ⓐ 미등재 회원 → **NOOP(200)**, ⓑ 이미 해제 상신 대기(pending) 중 → **NOOP(200)**, ⓒ 재파생 사전조건(온보딩 CDD 스냅샷·그 원 CDD 스크리닝 행) 미충족 → **400**, ⓓ 독립 당연고위험 사유(다른 참조 리스트 멤버십, 또는 WLF/국가위험 유래 강제 floor 사유) 보유 → **400**(그 리스트에서 먼저 제거하거나 사유를 먼저 해소해야 한다) — 결과적으로 해제 가능한 대상은 **`RA_HIGH_RISK_CUSTOMERS` 만이 유일한 당연고위험 사유인 회원**(주로 수동 등재 오등재분)으로 좁혀진다. **승인(EXECUTED) 시**: 레지스트리 헤더 행 잠금(DB §3.19) 재획득 스냅샷에서 ⓓ 사유를 **재검증**하고(상신↔승인 사이 타 리스트 등재 레이스 대비 fail-closed) 위반 시 **409 `AML.HRR_DEREGISTRATION_BLOCKED`**로 거부(결재는 SUBMITTED 로 잔류, 재시도 가능) — 통과분은 **참조 리스트 항목 제거 + `version+1`** 후 해당 회원의 **온보딩(ONBOARDING) CDD 스냅샷을 재파생**(당연고위험 재평가 로직이 아니라 CDD 팩터 기준 모델 재산정)해 `mandatoryHighRisk=false`·`forcedFloor` 마커 부재로 정정하되, **잠금 스냅샷 재검증 시점에 이미 미등재 상태면** 레지스트리 무변경·재파생·콜백 없이 그대로 **EXECUTED(no-op)** 처리하고 **재파생이 보류되면(워치리스트 stale·ACTIVE ONBOARDING 모델 부재) 409 `AML.STATE_CONFLICT`**로 실패한다(결재는 SUBMITTED 로 잔류, 재시도 가능 — 위 잠금 스냅샷 재검증 실패의 `409 AML.HRR_DEREGISTRATION_BLOCKED` 와 코드로 구분). **2차(ONGOING) 점수를 보유한 회원**은 재파생 직후 best-effort 로 2차 재산출을 함께 시도하되, 트리거 알럿 부재·디바운스 구간·모델 부재 시에는 재산출이 no-op 될 수 있다 — 이 경우 고객 캐시 등급(즉시 하향)과 화면에 표시되는 operative 등급(2차 재평가 전까지 HIGH 잔존)사이 일시적 괴리가 다음 2차 평가까지 존재할 수 있음을 **수용 사실**로 둔다(재실사 기한 `operativeNextReviewDueAt` 은 뒤로 밀리지 않는다 — min-clamp). 해제 EXECUTED 는 **`AmlHighRiskRegistrationRevoked`** 콜백을 발행(옵션 `callbackUrl`, API §8.1)하며 **반려는 콜백을 발행하지 않는다**(기존 정책과 동일). 해제로 등급이 내려가도 **기존 OPEN EDD 케이스·outbox 는 자동 회수되지 않는다**(조사관 종결 경로). **해제 pending 중에는 등재(재)상신이 `NOOP`으로 유지**된다(엔진이 모든 RA 평가 트랜잭션 내부에서 자동 상신을 시도하므로 예외로 막지 않는다) — read-back `deregistrationPending=true`로 세분한다. **화면 노출은 2곳**(scope **`aml:admin:high-risk-registry`**, BR-008 참조): RA 상세(AML-RA-003, §12-A.4 BR-006) 등재 패널의 「등재 해제 상신」 액션(사유 필수 모달, 해제 pending 이면 대기결재 알림)과, 본 화면 ② 참조 리스트 관리의 `RA_HIGH_RISK_CUSTOMERS` 행 액션(동일 모달). **범위는 `RA_HIGH_RISK_CUSTOMERS` 한정**이며 `PEP_INDIVIDUALS` 해제는 다루지 않는다(위 단서 참조). **(QA 재검, 코드=truth)** RA 상세 흐름도(§12-A.4 BR-006)의 ③ 경영진 승인/반려 액션·결재 이력은 해제 결재(`DEREGISTER|`)도 포괄하되, 흐름도 단계 판정(④ 등재·⑤ floor)은 등재 결재·read-back `registered` 만 사용한다.
- **BR-010 (2026-09-10 — 등재 회원 재파생 floor 유지, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)**: BR-001 의 "당연고위험 해당 대상은 등급 강제 상향" 은 **등재 시점 1회성 재평가가 아니라 등재가 유지되는 동안의 상태 속성**이다 — 등재 회원(`RA_HIGH_RISK_CUSTOMERS`·`PEP_INDIVIDUALS`·`HIGH_NET_WORTH` 등 참조 리스트 매치)은 이후 **온보딩 재파생 경로**(① 최초 CDD·재이행 re-CDD 인입, ② 국가위험 정책 활성화 재평가(`CountryRiskOnboardingReassessmentService`), ③ 등재 해제 재파생(`OnboardingSnapshotReassessmentSupport.reassessAfterDeregistration`, BR-009))에서 매회 레지스트리 floor(tier `HIGH→HIGH`·`VERY_HIGH→PROHIBITED`)가 스크리닝/국가위험 floor 와 **병합**(등급 max·사유/근거 합집합)되어 `mandatoryHighRisk` 가 유지된다 — **명단(WLF) 매치 소멸과 무관**하며, 해소 경로는 **BR-009 등재 해제 결재(4-eyes)뿐**이다. **(2026-09-10 QA 명세 리뷰 정정)** BR-003 참조 리스트 변경 트리거는 이 병합 경로가 **아니다** — `HighRiskRegistryService.triggerForcedReassessment` 가 `RiskAssessmentService.evaluate` 를 직행 호출하는 별도 경로로, 그 경로 자체가 등재 시점 floor 를 적용한다(위 ①~③ 은 온보딩(ONBOARDING) 파생 경로 한정). 스크리닝 매치가 없는 등재 회원의 재CDD 는 사유가 `SANCTION`/`PEP` 가 아니므로 수동검토 스킵이 걸리지 않고 결정 `EDD_REQUIRED`(사유 `REQUIRED_ACTION_EDD`)와 함께 `HIGH_RA_SCORE` EDD_REVIEW 케이스가 **대상당 1건 멱등**으로 개설된다(이미 OPEN 이면 재사용). tier `VERY_HIGH` 등재 회원의 재CDD 는 `REJECT`(`RISK_GRADE_PROHIBITED`)다. HRR 등재 자동상신(BR-005)은 재CDD 로도 **중복 상신되지 않는다**(멱등 가드). 엔진 케이스 RA-C30 검증.
...
- **BR-005 (CDD 소득구간 미입력 표시·평가 경계, v9.86)**: canonical input은 optional finite 4종이며 `UNKNOWN`을 송신하지 않는다. exact CDD completed omission/null/blank은 다른 지원 KYC 차원이 남는 경우 current-state에만 `UNKNOWN`으로 투영한다. 공통 `CddSnapshotPanel`의 4개 소비 화면은 이를 ko `미상`/en `Unknown`으로 표시한다. BO Java/read model은 `transactionActivity.incomeMultiple=null`을 유지하고 기존 `NON_NULL` producer JSON에서는 해당 key를 생략하며, bo-web consumer는 생략/explicit null을 모두 배율 산출 불가로 처리해 소득 배율을 표시하지 않는다. KYC block 또는 지원 차원 자체가 없으면 `UNKNOWN`만 합성하지 않고 기존 `-`/empty callout을 보존한다. `STR_KYC_INCOME_MISMATCH` 소득 조건만 미평가하며 무관한 AML 조건·1차 RA·비소득 FDS 평가는 계속된다.
- **BR-005a (숫자 신고소득 화면 권위, v9.87)**: public KYC의 숫자 4키 중 하나라도 있으면 그 경로가 화면의 최종 권위다. 완전한 tuple은 연산자·금액·통화·기간을 표시하고, 불완전 tuple은 존재하는 필드만 정직하게 표시하며 legacy band를 다시 적용하지 않는다. 네 키가 모두 없을 때만 BR-005의 legacy band/`UNKNOWN` 라벨을 사용한다. 표시 locale만 변환하고 금액·통화·기간을 환산하거나 합성하지 않는다.
...
- **BR-007 (PEP 축 분리 정책 — 2026-08-12 사용자 결정, 정책 축 `wlf-pep-axis-v2`)**: 제재(SANCTIONS)와 정치적주요인물(PEP)을 같은 매칭 축에서 판정하지 않는다. **대상은 `list_type=PEP` 뿐**이며 `SANCTIONS`·`LAW_ENFORCEMENT` 등 나머지 명단은 **무변경**이다 — 제재는 이 시스템의 유일한 차단 경로이므로 어떤 축에서도 완화하지 않는다.
  ...
  - **회원 축 — 왜 안 맞았는지를 구분한다(v2, 코로보레이터 4상태)**: 요건 미달의 원인은 **불일치**(등재 값과 실제로 다르다)와 **확인 불가**(명단·고객 어느 쪽에 값이 없거나 읽히지 않는다)로 나뉘며 **서로 다른 사유코드**를 남긴다 — 불일치는 **`PEP_CORROBORATION_REQUIRED`**, 확인 불가는 **`PEP_CORROBORATION_UNKNOWN`**. v1 은 둘을 같은 코드·같은 `NO_MATCH` 로 뭉갰고, 규제 관점에서 "다른 사람이다"와 "모른다"는 반대 상황이라 진짜 PEP 가 조용히 미탐이 될 수 있었다. 근거에는 추가 식별자별 판정 상태 4종(**일치(`MATCH`) / 불일치 — 등재 값과 다름(`MISMATCH`) / 확인 불가 — 명단 값 없음(`UNKNOWN_LIST_DATA`) / 확인 불가 — 고객 값 없음(`UNKNOWN_TARGET_DATA`)**)이 그대로 실려, 조치가 갈리는 지점(명단 보강 vs CDD 수집 보강)이 화면에 남는다. 판정 우선순위는 **일치 > 불일치 > 확인 불가**다 — 불일치는 있는 증거, 결측은 없는 증거이므로 국적이 명백히 다른 후보를 "생년을 몰라서 판단 유보"로 올리면 제거하려던 오탐이 그대로 돌아온다(라이브 탈락 4건이 전부 국적 불일치). 확인 불가 건의 귀결은 설정 `customer.unknown-outcome` 이 정하며 어느 쪽이든 사유는 불일치와 구분돼 남는다. **미탐 방향의 선택임을 명시적으로 감수하되, 그 미탐이 통계에서 사라지지 않게 한다.**
    - **귀결은 3값이다(2026-08-13 — 종전 2값 서술 정정)**: **억제(`SUPPRESS`, 기본)** / **강등하지 않음(`MATCH`)** / **위험 신호로 남김(`RISK_SIGNAL`, 신설)**. 억제의 문제는 "매치로 안 올린다" 가 아니라 **관측·계량 자체가 사라진다**는 것이었다 — 사유 코드는 갈렸어도 그 행을 하류가 소비하지 않아 탐지 결과가 정책 도입 전과 같았다. `RISK_SIGNAL` 은 매치로 올리지 않아 오탐 폭증 없이 미탐 방향만 닫는다: 그 행을 **계량 가능한 위험 신호로 표식**하며, 갈리는 것은 **사유코드(`PEP_CORROBORATION_UNKNOWN_RISK_SIGNAL`)와 근거의 표식 2키**뿐이고 상태(`NO_MATCH`)·차단 계약·검토 큐 계약은 무변경이다.
    - **STR 발동 조건은 무변경**이다 — 이 신호는 STR PEP 룰의 발동 게이트에 연결되지 않는다(STR 초안 볼륨 증가 없음). **또한 2026-08-13 시점 엔진에는 이 신호를 실제로 소비하는 하류 경로가 아직 없다**(RA 가중 반영은 정책 결정 대기). 현재 이 귀결이 바꾸는 것은 사유·표식·화면 필터의 해상도까지이며, 이 사실을 감추지 않는다.
    - **기본값은 `SUPPRESS` 유지** — 억제율을 전 테넌트에서 즉시 뒤집는 것은 라이브 오탐률 실측 없이는 되돌리기 비용이 큰 운영 결정이다. 전환은 설정 1개이고 오타·미지원 값은 부팅에서 거부된다.
  ...
  - **수취인 축 — 그 위험 신호는 실제로 가산된다(v2 정정)**: 사용자 결정의 "이름으로 위험 점수 가산"이 성립하려면 점수가 어딘가에서 소비돼야 한다. 강등된 행은 ① **STR PEP 룰의 이름 신호로 집계**되고, ② 그 발동 알럿의 가중을 통해 **송금인(회원)의 2차 상시 RA(거래행태)로 가산**된다. STR 근거의 명단 매칭 계보에는 확정 매칭(`WATCHLIST_MATCH`)과 구분되는 **`origin=PEP_NAME_RISK_SIGNAL`** 로 실린다(§10 BR-011·BR-013). 가산 귀속 대상이 송금인인 이유는 수취인이 CDD·1차 RA·재실사 주기를 갖지 않아 귀속시킬 RA 주체가 없고, "이 회원이 PEP 이름과 일치하는 수취인에게 송금한다"는 사실이 **회원의 거래행태 위험**이기 때문이다. 한 거래에 여러 신호가 있으면 **가장 강한 이름 일치 1건**으로 대표한다(합산은 같은 인물을 두 번 센다). **차단은 여전히 발생하지 않는다** — 제재 매칭 경로는 이 신호를 읽지 않으므로 송금 판정(ALLOW/HOLD)·`status` 는 무변경이며, 제재가 유일한 차단 경로라는 원칙은 그대로다. ※ 이 항목은 v1 서술("검토 큐·STR 이름 룰 계보·2차 RA 가중에 들어가지 않는다")을 **정정**한 것이다 — 기본 검토 큐(`POSSIBLE_MATCH`)에 뜨지 않는다는 사실만 유효하며, 강등 건은 §3.1·§3.3 의 배지·근거 카드·이력 탭 필터로 도달한다.
  - **수취인 축 — 그 가산은 「기존 가중 그대로」가 아니라 점수에 비례해 깎인다(v3 정정 — 2026-08-13 사용자 지시)**: 위 ②의 "기존 가중을 통해" 는 등재 당시 참이었으나, 종전 산식은 **이름만 닮은 신호와 확정 매칭을 같은 룰·같은 가중으로 취급**해 이름 점수 0.65 인 건과 0.99 인 건이 **똑같은 만점 가중**을 받았다. 이제 그 기여 가중에 **이름 점수에 비례하는 배수**가 곱해진다.
    - **방향은 「깎기」다.** 배수 상한은 `1.00` 이므로 **어떤 입력에서도 종전보다 높아지지 않는다** — 강한 이름 일치(점수 ≥ 상한)는 **종전과 똑같은** 가중을 받고, 약한 일치만 점수에 비례해 낮아진다. 성격은 가중 상향이 아니라 **오탐 억제**이며, 상한 `1.00` 은 "이름 유사도 신호가 확정 매칭보다 무거워질 수 없다" 는 구조 원칙(§BR-007 제재·확정매칭 초과 금지 취지)이 요구하는 값이라 완화하지 않는다.
    - **비례의 입력은 합성 점수가 아니라 이름 하위점수**다. 수취인 축은 이름·국가·전화만으로 채점해 합성 점수가 0.65 근방에 눌려 있어 "비례" 를 표현하지 못하지만, 이름 하위점수는 정확일치에서 1.0 까지 간다. 합성 점수 표기는 **무변경 보존**한다.
    - **확정 PEP 근거가 있는 알럿은 배수 1.00 이다.** 회원 자신이 확정 PEP(명단 계보 또는 KYC 확정 플래그)여서 발동한 알럿에 수취인 이름 신호가 **동행**했더라도 깎지 않는다 — 그 알럿의 발동 원인은 회원의 확정 PEP 이므로 "이번 송금 수취인 이름이 약하게 닮았다" 를 이유로 감산하면 확정 PEP 고객의 2차 RA 가 무관한 신호로 내려가는 **미탐**이 된다.
    - **기본은 현행 유지, 비례는 4-eyes 로 켠다.** 배수 정의는 RA 모델 파라미터(테넌트 정본)이고 변경 경로는 4-eyes 다 — 엔진 기본값을 비례로 바꾸면 **아무도 승인하지 않은 가중 변경이 전 테넌트에 즉시 적용**된다. 정의가 없는 테넌트의 동작은 위 ② 서술 그대로다. 관리 동선·되돌리기는 §6.1 AML-RA-002 **BR-008**(⑤ 이름 위험 가산 탭), 판정 규칙 정본은 설계서 §11.3a.
    - **왜 이 점수인가를 화면이 설명한다** — RA 상세(§12-A.4 AML-RA-003 ① 요인 탭)에 **배수 근거 패널**(적용 배수·입력 점수·커브·정의 파라미터·모델 버전·해당 알럿이 최고 기여였는지)이 붙고 정의 편집 화면으로 딥링크한다. 근거가 없으면(1차 RA·미적용 정의) 패널 자체가 렌더되지 않는다.
  ...
  - **설명가능성**: 억제된 결과에는 `scoreBreakdown.pepAxis` 감사 블록(`applied`·`policyVersion`·`axis`·`listType`·`decision`·`suppressedStatus`·`suppressedBand`·`riskScore`·`entryId`·`reviewThreshold`, 회원 축 **세 갈래**(불일치 / 확인 불가·억제 / 확인 불가·위험 신호)는 코로보레이터 3키 + **`corroboration`(식별자별 4상태)**·**`unknownOutcome`**, 역할 불일치 시 **`declaredAxis`·`axisMismatch`**, **위험 신호 귀결에만 표식 2키(`riskSignal`·`signalOrigin`)** 추가)이 붙어 운영자가 "왜 이 PEP 가 매치가 아닌가"와 "그럼에도 위험은 얼마인가"를 한 행에서 읽는다(API §3.2). 억제되지 않은 결과에는 블록 자체가 없고, 위험 신호 귀결이 아닌 판정의 근거 블록은 **바이트 동일**하다.
  - **엔진 유효 정책 자체의 노출(2026-08-13 신설)**: 위 근거는 **후보가 있었던** 스크리닝 행에만 붙으므로, 운영자가 "이 서비스의 확인 불가 귀결이 지금 무엇인가" 를 알려면 억제된 건을 하나 찾아 열어야 했다. WLF 엔진 조절 화면(AML-WLF-005) 상단에 **PEP 축 유효 정책 배지**(적용 여부·정책 버전·확인 불가 귀결·요구/인정 추가 식별자·생년 인정 하한·수취인 위험신호 전용 여부)를 두어 임계 튜닝과 축 정책을 한 화면에서 대조한다. 이 값은 정책팩이 아니라 엔진 런타임 설정이라 **읽기 전용**이며, 라벨은 ko/en 카탈로그에서 매핑한다(코드 문자열 노출 금지).
  - **BO 도달성(v2 신설)**: 강등 건은 `status=NO_MATCH` 라 기본 검토 큐에서 빠지므로, 목록 행에 **축 정책 배지**(사유코드 라벨)를, 상세에 **PEP 축 근거 카드**(축 판정·식별자별 4상태·확인 불가 귀결·역할 불일치)를, 처리 이력 탭에 **`PEP 축 정책` 필터**(전체 / 강등 건만 / 수취인 이름 위험 신호만 / **확인 불가로 강등된 건만** / **확인 불가·위험 신호로 남긴 건만**(2026-08-13 신설))를 둔다(§3.1·§3.3). "확인 불가로 강등된 건만" 은 **억제와 위험 신호 두 귀결을 모두 포섭**한다 — 원인은 같은 "몰라서" 이므로 한쪽만 잡으면 정책 노브를 바꾼 순간 잠재 미탐 모집단이 화면에서 반쪽이 된다. 시뮬레이션 화면도 같은 근거 카드를 재사용한다(§12-B.1). 라벨은 ko/en 카탈로그에 동시 등록하며 코드 문자열을 화면에 노출하지 않는다.
  ...
  - **실측 효과(v1 도입 시점 측정)**: 데모 회원 `POSSIBLE_MATCH` **6/9(66.7%) → 2/9(22.2%)**. 국가 불일치 동명이인 4건이 전건 탈락하고, 남는 2명은 이름+국적 동시 일치로 정당한 매치다. **제재 회귀 0**. v2 는 이 억제 여부 자체를 바꾸지 않고 **사유 해상도와 소비 경로**를 더한 판이므로 이 수치는 그대로 유효하다(불일치 4건은 v2 에서도 `PEP_CORROBORATION_REQUIRED`).
    - **단, 2026-08-13 이후 이 수치는 그대로 재현되지 않는다(데모 모집단 변경 — 엔진 회귀 아님)**: 데모 회원 1명의 합성 이름이 실 명단 인물과 **이름 정확일치 + 국적 일치**여서 축 정책이 개입하지 못하고 매치로 확정됐고, 해외송금 라이브 폐루프 검증(2건 모두 통과 요구)이 **엔진 결함이 아닌 이유로** 깨졌다. 사용자 지시로 그 회원 이름을 **어느 명단에도 없는 합성 이름**으로 교체했다(교체 후 라이브 실측: 전 명단 대상 매치 없음·최상위 점수 0.22 < 검토 임계 0.65). 이름 외 필드는 무변경이고 검증 기대값은 한 줄도 완화하지 않았다. 부수적으로 그 회원의 명단 floor → 당연고위험 자동상신 계보가 사라지는데, 이는 **데모가 우연한 실명 충돌에 의존하고 있었다는 사실의 해소**이지 탐지 후퇴가 아니다.
...
**구성**: 프로파일 선택(카탈로그 목록, 관할·통화·CTR 임계·보고 마감시각 미리보기) → 일괄 셋업 실행(`apply`) → STEP 별 결과 표시(`FDS_REGULATORY_CURRENCY`/`BINDING`/`CTR_THRESHOLD`/`REPORT_RULES`/`FDS_RULES`, 순서=`steps[]` 배열 정본 — 실제 방출 순서·미실행 STEP 미포함, API §3.16a). AML 바인딩은 즉시 반영, CTR 임계·FDS 룰 금액·FDS 규제통화는 **결재 대기(SUBMITTED)** 로 표시되며 각 상신이 귀속된 결재함 승인 후에야 EXECUTED 된다. **결재함 귀속 분리(2026-08-19 명시)** — CTR 임계(subjectType `CTR_THRESHOLD`, 승인선 `REPORTING_OFFICER`)는 **AML 결재함(AML-APR-001)**, FDS 룰 금액(subjectKind `RULE_PARAM`)·FDS 규제통화 전환(subjectKind `TENANT_REGULATORY_CURRENCY`)은 **FDS 결재함(SFDS-APPR-001)** 에 상신된다(한 결재함에서 전부 승인되지 않는다). **PROVISIONAL** complianceReview 프로파일은 배지로 경고 표시한다.
...
- **BR-002 (2026-08-19 구현 실측 정정)**: `STEP FDS_REGULATORY_CURRENCY` 가 EXECUTED 되었지만 `STEP BINDING` 이 아직 그 값으로 완결되지 않은 2-pass 전환 중간 창(§아래 참조)에서는 `warnings` 에 `FDS_CURRENCY_APPLIED_BINDING_PENDING` 이 노출되며, 화면은 이 구간 동안 FDS 금액 룰이 무발동함을 **Callout 목록으로 안내**한다(구 '배지로 안내' 서술 정정 — apply 결과 warnings 는 코드별 warning Callout 목록, 현황 카드 warnings 도 소스별 Callout 렌더).
- **BR-003 (2026-08-19 구현 실측 정정 — 결재함 귀속 분리·딥링크 없음)**: 결재 대기 STEP(`SUBMITTED`/`PENDING`) 은 `approvalId` 를 **텍스트로 노출**한다(현행 구현 — 결재함 딥링크 없음, 코드=truth). 각 상신의 결재함 귀속은 분리된다 — CTR 임계(`CTR_THRESHOLD`)=AML 결재함(AML-APR-001), FDS 룰 금액(`RULE_PARAM`)·FDS 규제통화 전환(`TENANT_REGULATORY_CURRENCY`)=FDS 결재함(SFDS-APPR-001). 구 '결재함(AML-APR-001) 딥링크로 이동' 서술은 구현과 이격이라 폐기하고, 딥링크는 후속 요건 후보 ⓖ 로 이동한다.
...
- ⓖ apply 결과 패널의 결재 대기 STEP(`SUBMITTED`/`PENDING`)에서 결재함 딥링크 이동(귀속별 — `CTR_THRESHOLD`→AML-APR-001, `RULE_PARAM`/`TENANT_REGULATORY_CURRENCY`→SFDS-APPR-001). 현행 구현은 `approvalId` 텍스트 노출뿐이다(BR-003 정정, 2026-08-19 이동 등재).
...
| AML-RA-002 | **설정 › 탐지·심사 정책** | 1차/2차 ACTIVE 상세·최근 CDD 입력 매칭·모델 편집·시뮬레이션·등급 조정·**이름 위험 가산(⑤, `?tab=signal-scaling` 딥링크·좌측 메뉴 진입점 1개)**(5탭) — **설정 영역** | `GET .../ra-models` · `GET .../ra-models/onboarding-input-catalog` · `POST .../ra-models/{code}/simulate` · `POST .../ra-models/{code}/versions/{v}:activate`🔒 · `POST .../risk-scores/{id}/override`🔒 |
...
| AML-MBR-001 | **운영 › 고객위험·심사** | 회원관리 — 회원원장·CDD/EDD/심사결과 히스토리(§12-A.10, 조회 · **v9.40 코드 truth 신설 · v9.106 「AML 심사결과」 타일·`MEMBER_DECISION` 이력 · v9.107 "AML 심사결과(스냅샷)" 열 신설**, 회원번호 검색 entry) | **bo-api** `GET /api/v1/bo/aml/members/{memberRef}/ledger`(원장 요약) · `GET /api/v1/bo/aml/members/{memberRef}/cdd-history?types=&page=&size=`(이력 페이지, 행별 `ingestDecision` additive — v9.107) → 엔진 `GET /api/v1/admin/aml/members/{memberRef}/{ledger\|cdd-history}` 위임(scope `aml:case:read`, API §2.x·DB §3.22f·§5.36) · `GET /api/v1/bo/aml/customers/{ref}/decision`(「AML 심사결과」 타일, scope `aml:case:read`, 2026-09-10 QA 정정 — API §2.3) |
