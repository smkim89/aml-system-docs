...
| 버전 | 일자 | 작성자 | 변경 내역 |
|------|------|--------|----------|
| **9.104** | **2026-09-08** | **Hanpass Global Team** | **§5.1 AML-RA-001 1차 RA 내역 기본 정렬 = 인입시각·인입시각 컬럼·정렬 기준 셀렉트·BR-011 신설(코드=truth, aegis-aml PLAN 20260908-aml-ra-initial-list-ingest-sort).** 엔진 `risk-scores` 목록 `sort` 파라미터(`INGESTED_AT_DESC`\|`EVALUATED_AT_DESC`)·응답 `ingestedAt` additive·기본 정렬 2차 키(`scoreId` 문자열 desc). |
...
| 정렬 기준 | 1차 RA 내역 셀렉트 — `인입시각 최신순`(기본, `sort=INGESTED_AT_DESC`) / `평가시각 최신순`(`sort=EVALUATED_AT_DESC`). 변경 시 1페이지로 리셋. 2차 RA 내역은 평가시각 최신순 고정(v9.104) |
