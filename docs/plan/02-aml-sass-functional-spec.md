...
### 12-B.2 AML-IRA-001 · 기관 위험평가(ML/TF) 지표 보고 (KR 정책팩 확장 plugin, 3탭) — 🗑️ 폐기(v9.58, 2026-07-22, 사용자 지시 메뉴 제거)

> **🗑️ 폐기(v9.58)**: 이 화면(NAV `/aml/ira`)은 사용자 지시로 bo-web 에서 제거되었다(fix/remove-aml-ira-edu-menus). 아래 스펙은 이력 보존용이며 현행 NAV·라우트에 존재하지 않는다. bo-api/aml-svc read 엔드포인트는 후속 정리 backlog.
...
### 12-B.4 AML-EDU-001 · 내부통제 교육·자격 관리 (2탭) — 🗑️ 폐기(v9.58, 2026-07-22, 사용자 지시 메뉴 제거)

> **🗑️ 폐기(v9.58)**: 이 화면(NAV `/aml/edu`)은 사용자 지시로 bo-web 에서 제거되었다(fix/remove-aml-ira-edu-menus). 아래 스펙은 이력 보존용이며 현행 NAV·라우트에 존재하지 않는다. bo-api/aml-svc read 엔드포인트는 후속 정리 backlog.
