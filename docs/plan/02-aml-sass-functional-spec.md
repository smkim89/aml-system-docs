...
| — | **당연고위험 레지스트리 (AML-HRR-001, 벤치마크 2차 보강 — §12-B.6)** | T-11 | (엔진) `GET .../high-risk-registry` · `PUT .../high-risk-registry/{category}` 🔒(subjectType=`HIGH_RISK_REGISTRY` 제안) **(제안 — 후속 API 정합, 부록 E v7.0)** |
...
| **기관 위험평가 지표 보고** | `aml:admin:policy` (제출 🔒 보고 책임자) | **AML-IRA-001(§12-B.2, KR 정책팩 확장 plugin)** |
...
| **당연고위험 레지스트리 관리** | `aml:admin:policy` (변경 🔒 준법감시 책임자) | **AML-HRR-001(§12-B.6, 벤치마크 2차 보강)** |
...
| **API(제안)** | `GET .../high-risk-registry` · `PUT .../high-risk-registry/{category}`🔒(subjectType=`HIGH_RISK_REGISTRY` 제안) — **후속 API 정합 필요(부록 E v7.0)** |
...
- **BR-002**: 분류 기준·참조 리스트 변경은 `🔒`(subjectType=`HIGH_RISK_REGISTRY` 제안, 준법감시 책임자) + 전 이력 감사. 국가 팩터는 AML-CTRY-001(COUNTRY_RISK)이 정본이며 본 화면은 파생 표시(이중 관리 금지).
- **BR-003**: 참조 리스트 갱신 시 영향 대상 재평가는 차기 정기 RA 배치에 반영(즉시 전체 재평가는 후속 오픈결정 — 부록 E v7.0). 고액자산가 추출 결과의 개별 확정·예외는 케이스(AML-CASE-001) 흐름 재사용.
...
| AML-TNT-002 ③ | 고객사 상세 — 소스 시스템 탭 | **bo-api** `GET .../source-systems` (`Tenant-Id` 헤더, API §1.1 — 연결 현황 조회, AML-AUD-001 연계) |
| AML-TNT-002 ④ | 고객사 상세 — 정책팩 탭 | **bo-api** `GET /api/v1/bo/aml/tenants/{tenantId}` (`policyPackCode` 파생 표시) · `POST /api/v1/admin/aml/policy-packs:change`🔒(POLICY_PACK, 2인, AML-PP-001 연계) |
...
| AML-RA-001 | RA 분포·고위험(목록·2탭 모니터링) | **bo-api** `GET /api/v1/bo/aml/dashboard`(분포 집계). 고위험 행 클릭 → AML-RA-003 (시뮬레이션은 AML-RA-002 ③ 소관) |
| AML-RA-002 | RA 모델 관리·factor 빌더·시뮬레이션·등급 조정(4탭) | `POST .../ra-models/{code}/simulate`(③ 시뮬레이션·결재 불필요) · `POST .../ra-models/{code}/versions/{v}:activate`🔒 · `POST .../risk-scores/{id}/override`🔒 |
...
| AML-REP-001 | 규제 보고(STR/CTR 후보 목록) | `GET/POST .../reports` · `POST .../reports/{reportId}:reject`🔒([기각])·`:cancel`🔒(사유 코드 필수, REPORTING_OFFICER 4-eyes). 행 클릭 → AML-REP-002 |
| AML-REP-002 | 보고 상세/제출(드릴다운) | `GET .../reports/{id}` · `POST .../reports/{id}:submit`🔒 · `:reject`🔒·`:cancel`🔒(사유 코드 필수, REPORTING_OFFICER 4-eyes) |
...
| AML-WLF-004 | 스크리닝 시뮬레이션·임의 수행(도구, §12-B.1) | (엔진·**제안**) `POST .../screenings:simulate`(단건·결재 불필요) · `POST .../screenings:bulk-run`(일괄·감사 기록) — **후속 API 정합 필요(부록 E v6.0)** |
| AML-IRA-001 | 기관 위험평가 지표 보고(KR plugin, §12-B.2) | (엔진·**제안**) `GET/POST .../ira/reports` · `PUT .../ira/reports/{id}/indicators` · `POST .../ira/reports/{id}:submit`🔒(IRA_SUBMIT 제안) — **후속 API 정합 필요** |
| AML-STAT-001 | STR·룰 효과성 통계(§12-B.3) | **bo-api(제안)** `GET /api/v1/bo/aml/stats/str` · `GET /api/v1/bo/aml/stats/scenarios`(집계 소유 — API §9 경계 준수) — **후속 API 정합 필요** |
| AML-EDU-001 | 내부통제 교육·자격 관리(§12-B.4) | **bo-api(제안)** `GET/POST /api/v1/bo/aml/training/courses` · `GET .../training/records` · `GET/POST .../certifications` — **후속 API 정합 필요** |
| AML-WL-003 | 내부 요주의 명단·오탐 면제 생명주기(§12-B.5) | (엔진·**제안**) `POST .../watchlist-sources/{code}/entries:draft`(수기 등록→diff 초안, 적용은 WL-002 `:apply`🔒 재사용) · `GET .../screenings/fp-whitelist` · `POST .../fp-whitelist/{id}:revoke`🔒 — **후속 API 정합 필요(부록 E v7.0)** |
| AML-HRR-001 | 당연고위험 레지스트리(§12-B.6) | (엔진·**제안**) `GET .../high-risk-registry` · `PUT .../high-risk-registry/{category}`🔒(subjectType=`HIGH_RISK_REGISTRY` 제안) — **후속 API 정합 필요(부록 E v7.0)** |
| AML-CDD-002 | 고객 CDD 프로필 원장(§12-B.7, 드릴다운) | (엔진·**제안**) `GET /aml/customers/{ref}/profile`(read-only 파생 — 진입: AML-RA-003 ①·AML-CASE-002 ①·AML-WLF-001 `[고객 프로필 ▶]`) — **후속 API 정합 필요(부록 E v7.0)** |
| AML-ING-001 | 수신 API 카탈로그·인입 라이브 모니터링(§12.2, v8.0) | **bo-api(제안)** `GET /api/v1/bo/aml/ingest/catalog` · `GET /api/v1/bo/aml/ingest/health`(집계 소유 — API §9 경계) — **후속 API 정합 필요(부록 E v8.0)**. 수신 API 자체 정본 = §1.11 ②(API §3.1~§3.4) |
...
> 2. **당연고위험 레지스트리 데이터 모델**(AML-HRR-001) — 분류 기준·참조 리스트(상품/VASP/고액자산가) 물리 테이블 또는 정책 store(versioned) 선택, subjectType `HIGH_RISK_REGISTRY` 신설, RA 평가 시 등급 강제 상향 결합 지점(엔진), 리스트 갱신 시 영향 대상 재평가 방식(차기 배치 vs 즉시) — 설계서·DB·API 확정 필요.
