...
│  매칭 이름(자동표시): Juan D**a C**z · 점수: 0.97 · 연계 WLF 판정: scr_…b2 ▶  │
│  ┌─ 식별정보 비교(참고) ────────────────────────────────────────────────┐ │
│  │ 항목    │ 원거래 수취인            │ 명단 엔트리                    │ │
...
│    · 명단 미보유 필드 "명단에 없음" · identity 부재 시 비교 블록 숨김         │
...
- **BR-011 (명단 매칭 근거 — WLF 동형 lineage, 코드=truth v9.22)**: **STR_PEP·STR_SANCTION** 알림(명단 기반 룰)은 상세에 **"명단 매칭 근거" 섹션**(`evidence.watchlistMatch`)을 표시한다 — WLF 판정 화면(§3.2)과 동형으로 **명단종류(`listType` PEP/SANCTIONS)·소스코드(`sourceCode`)·소스 제공처 이름(`provider`)·매칭 엔트리(`entryId` 마스킹 토큰)·매칭 이름(`entryName`, WLF 와 동일한 마스킹 규칙 — 전체 이름 공개는 기존 PII reveal 체계 범위, 본 스코프 아님)·매칭 점수(`matchScore`, 있으면)·연계 WLF 판정(`screeningRef`, 있으면 — WLF 판정 딥링크는 후속)**을 노출한다. 엔진은 대상의 **최신 WLF 스크리닝 매칭 엔트리 중 룰 `listType` 일치** 항목을 계보로 삼고(`origin=WATCHLIST_MATCH`), 데모 stub 은 데모 워치리스트(`DOW_JONES_PEP`·`OFAC_SDN` 등)에서 memberRef 결정적 매칭을 선정한다. **계보를 찾지 못하면 조작 없이 `origin=KYC_PEP_FLAG`**(엔트리·제공처 생략)로 "명단 매칭 계보 없음(KYC PEP 플래그 기반)"을 정직하게 표기한다. STR 보고 DRAFT payload 에도 동일 lineage(`watchlistMatches[]`)가 실려 알림-보고 근거가 일치한다. **비-명단 룰(CTR·STR_STRUCTURED 등) 알림·구 알림은 본 섹션 미표시**(evidence 무변경). 발동 로직 불변 — lineage 는 가산 정보이며 어떤 회원이 PEP/제재로 발동하는지(결정성)는 바꾸지 않는다. 스키마 변경 없음(evidence JSONB 확장).
...
- **BR-013 (송금 수취인 동시 명단 평가 — 당사자 구분, 코드=truth v9.25)**: **송금 채널**(국내송금 `DOMESTIC_REMIT`·해외송금 `CROSS_BORDER_REMIT`) 거래는 STR_PEP·STR_SANCTION 을 **회원(송금인)+수취인 당사자별로 동시 평가**한다. 수취인 신호는 **그 거래(transactionRef 그룹)의 수취인 COUNTERPARTY WLF 스크리닝 계보(TRUE_MATCH 우선·룰 `listType` 일치)만** 사용하며(합성 조작 금지), 계보 부재 시 수취인 평가를 **skip**(데이터 없으면 미발동이 정직). **비송금 채널은 회원만**(현행). 알림은 **(거래·룰)당 1건 유지**(기존 `ux_alert_tm` 멱등 불변)하고 `evidence.watchlistMatch` 에 **`matchedParty`(MEMBER/RECEIVER)·`partyRef`** 를 가산, 양당사자 동시 매칭은 **`additionalMatches[]`(동일 스키마)** 로 나머지 당사자를 수록한다(대표=회원 우선). bo-api 는 read-time 으로 **`partyIdentity {targetRef, fields[]}`**(매칭 당사자 식별정보 비교 참조 — `matchedParty=RECEIVER` 면 수취인)를 projection 한다. **STR_SANCTION RESTRICT(차단)·심각도 매핑은 수취인 매칭에도 동일** 적용. STR 보고 DRAFT payload `watchlistMatches[]` 에도 당사자 구분(matchedParty·partyRef)이 실린다. 인입 payload(`HanpassPhTransactionPayload`)에 비-PII 운영 수취인 식별자 `receiverRef`(nullable)가 가산된다(integration §). 상세 화면(①-b)은 당사자 배지("회원(송금인) 매칭"/"수취인 매칭")로 구분하고, RECEIVER 매칭이면 식별정보 비교 좌측 열이 원거래 수취인, 양당사자는 카드를 반복 노출한다.
