...
| 2차 RA 내역 | 재평가 대기/임박 RA 내역(`risk-scores?riskGrade=LOW,MEDIUM,HIGH,PROHIBITED&reviewDueSoon=true`) + 2차 주요 요인(거래행동·FDS 위임·TM 적중·관계/UBO·crypto/trade) |
...
- **BR-008 (내역 조회 기준 가시성)**: `1차 RA 내역`과 `2차 RA 내역` 테이블은 통계와 별도로 **조회 기준 패널**을 먼저 표시한다. 1차 기본 기준은 `기간=제한 없음`, `등급=전체`, `상태=최신 RA 평가 내역`, `건수=최대 50건`이다. 2차 기본 기준은 `기간=제한 없음`, `등급=전체`, `상태=재평가 대기/임박(reviewDueSoon=true)`, `건수=최대 50건`이다. 기간별 추이는 `periodHistogram` 통계로 확인하고, 개별 내역의 기간 검색은 후속 API 확장 전까지 제공하지 않는다.
...
- **BR-006 (RA 시나리오 구분, 코드 정합)**: RA 모델 정의는 **평가 시나리오(`scenario`) 축으로 구분**된다(DB §3.9 `aml_risk_models.scenario`, 도메인 `RaScenario` 2종·bo-api `RaDtos.RaScenario`·API §2.7/§3.3). ① **`ONBOARDING`(1차·온보딩)** — 회원가입 CDD 완료 시점 1회 baseline 위험평가(정본 활성 모델 `KR_DEFAULT_RA`, `is_default=true`). 목록·draft·응답의 각 모델/버전 행이 자기서술 배지로 이 값을 노출한다(버전은 소유 모델의 scenario 승계). draft 시 미지정이면 서버가 `ONBOARDING`으로 안전 기본 처리. ② **`ONGOING`(2차·상시)** — STR/CTR 발동 시 거래 가중 재평가→주기 단축→EDD 자동 개시를 의도한 시나리오. **본 단계는 활성화 흐름을 정의하지 않는다** — 코드상 `KR_ONGOING_RA` v1이 **DRAFT placeholder(`is_default=false`, V11 시드) 1행으로만 선반영**(화면 가시화·설계 선반영 목적)되며, 활성화·거래가중 재평가·주기 단축·EDD 자동 개시는 **다음 단계 예정**(§6.1 미정의 — 추측 금지). 1차 RA 운영 흐름(회원가입 시점 KYC factor)은 §5.1 AML-RA-001 콜아웃 정본.
- **BR-006 (RA 시나리오 구분 — 1차 온보딩·2차 상시, 코드=truth)**: RA 모델 정의(`aml_risk_models`)는 `scenario`(**ONBOARDING**=1차 온보딩 RA / **ONGOING**=2차 상시 RA, DB §3.9·API §2.7 draft `scenario?` 옵션·미지정 시 ONBOARDING)로 **어느 모델 정의가 어느 RA 흐름에 소비되는지 자기서술**한다. ① **버전 목록(탭 ①)·초안(POST ra-models)에 시나리오 배지 노출** — 기존 정본 `KR_DEFAULT_RA`(활성/작성중 버전)는 `ONBOARDING`(회원가입 CDD 완료 시점 1회 평가, §5.1·v9.18 RA 절 정본). ② **2차 상시 RA(`KR_ONGOING_RA`)는 현재 `DRAFT` placeholder(`is_default=false`) 1행만 시딩**되어 목록에 **"작성중(상시·예약)"** 로 표기된다 — **활성화·거래가중 재평가·주기 단축·EDD 자동 개시 흐름은 본 문서 §6.1 에 미정의(다음 단계 예정)**. 화면은 placeholder 를 **읽기 표시**로만 노출하고(활성화 상신 비활성), 상시 RA 저작 워크플로 확정 시 본 절을 갱신한다(현 단계 추측 금지).
