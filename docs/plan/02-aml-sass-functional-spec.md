...
- **BR-002-1 (v9.49 — 평가구분(단계) 표기, §5.1 BR-009 참조)**: 상단 대상 패널의 평가구분(단계)은 대상 최신 점수 행의 `scenario`(§API 3.3) 기준 풀 표기 `1차 (신규 온보딩)`/`2차 (활동 기반)` 로 노출한다 — AML-RA-001 ③ 고위험 목록의 축약 배지(`1차`/`2차`)와 **동일 단계 값**(1:1 매핑, §5.1 BR-009)이며, 목록·상세 어느 경로로 진입해도 같은 회원은 같은 단계로 보인다.
