...
| **태스크** | T-10 |
| **권한** | 조회 `aml:case:read` |
| **API** | `GET /api/v1/admin/aml/screenings?status=TRUE_MATCH,FALSE_POSITIVE,AUTO_DISCOUNTED` · `GET .../screenings/{screeningId}` — 다중 상태 응답은 **전역 createdAt 최신순**으로 정렬(상태그룹 순 아님, v9.50). |
