...
- **BR-010 (2026-09-10 — 등재 회원 재파생 floor 유지, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)**: BR-001 의 "당연고위험 해당 대상은 등급 강제 상향" 은 **등재 시점 1회성 재평가가 아니라 등재가 유지되는 동안의 상태 속성**이다 — 등재 회원(`RA_HIGH_RISK_CUSTOMERS`·`PEP_INDIVIDUALS`·`HIGH_NET_WORTH` 등 참조 리스트 매치)은 이후 **모든 온보딩 파생**(최초 CDD·재이행 re-CDD·BR-003 참조 리스트 변경이 트리거하는 국가정책 재평가)에서 레지스트리 floor(tier `HIGH→HIGH`·`VERY_HIGH→PROHIBITED`)가 스크리닝/국가위험 floor 와 **병합**(등급 max·사유/근거 합집합)되어 `mandatoryHighRisk` 가 유지된다 — **명단(WLF) 매치 소멸과 무관**하며, 해소 경로는 **BR-009 등재 해제 결재(4-eyes)뿐**이다. 스크리닝 매치가 없는 등재 회원의 재CDD 는 사유가 `SANCTION`/`PEP` 가 아니므로 수동검토 스킵이 걸리지 않고 결정 `EDD_REQUIRED`(사유 `REQUIRED_ACTION_EDD`)와 함께 `HIGH_RA_SCORE` EDD_REVIEW 케이스가 **대상당 1건 멱등**으로 개설된다(이미 OPEN 이면 재사용). tier `VERY_HIGH` 등재 회원의 재CDD 는 `REJECT`(`RISK_GRADE_PROHIBITED`)다. HRR 등재 자동상신(BR-005)은 재CDD 로도 **중복 상신되지 않는다**(멱등 가드). 엔진 케이스 RA-C30 검증.
