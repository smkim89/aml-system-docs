...
- **BR-009 (2026-09-09 — 관리자 액션 패널 「회원 심사 직접 승인/거절」, 코드=truth, PLAN 20260909-aml-ra-direct-member-decision)**: 관리자 액션 패널(BR-005)에 **당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 이미 **EXECUTED** 된 회원**에 한해 회원 심사결과(BR-008 `MemberDecisionCard`)를 **추가 4-eyes 결재 없이 즉시** 확정하는 두 버튼 [회원 심사 승인]/[회원 심사 거절]을 노출한다 — 2차 CDD 재이행(`CDD_REVIEW`)이 회원을 다시 `EDD_REQUIRED` 로 되돌려도 별도 결재 경로가 없던 결함을 해소한다.
  ...
  - **액션**: 각 버튼 → 공용 `ChangeRequestModal`(대상 회원·현재 결정 배지·**승인·거절 모두 사유 필수**·"4-eyes 결재를 거치지 않고 즉시 확정되며 계정계에 통지된다"는 경고 문구) → `POST /api/v1/bo/aml/customers/{customerRef}/decision`🔒(body `{decision: APPROVED|REJECTED, reason}`, scope `aml:admin:approval`) → 엔진이 게이트(ⓐ 회원 미존재 404 · ⓑ decision 값 오류 400 · ⓒ `REJECTED` 사유 공백/256자 초과 400 · ⓓ HRR 등재 미완료 409 · ⓔ 대기 결재 존재 409 · ⓕ 현재 결정과 동일 409 · ⓖ actor 미검증 400)를 통과하면 `aml_member_decisions`(DB §3.22g)에 `source=ADMIN_DIRECT`(결재·케이스 없음, `approvalId`/`caseId`/`caseType`/`finalStatus` 모두 `null`) 행을 append 하고, 같은 트랜잭션에서 웹훅 `AmlMemberDecisionResolved`(§API §8.1, `docs/aml-data.md` §12.1a)를 기존 배관·목적지 규칙 그대로 발행한다.
