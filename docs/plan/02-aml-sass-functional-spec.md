...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ RA 분포 · 고위험 현황   서비스 [은행 A ▼]   모델 [RA-KR v4 ▼]  admin ▼   │
├─ 탭: [점수 분포] [고위험 목록] [CDD·RA 처리 현황] ────────────────────────┤
│ ┌─ 등급 분포 ───────────────────────┐ ┌─ 다음 재심사 예정 ────────────┐ │
...
#### 화면 레이아웃 — [CDD·RA 처리 현황] 탭 (read-only 통계)

```
┌──────────────────────────────────────────────────────────────────────────┐
│ CDD·RA 처리 현황   서비스 [은행 A ▼]   기간 [최근 30일 ▼]        admin ▼   │
├─ 탭: [점수 분포] [고위험 목록] [CDD·RA 처리 현황] ────────────────────────┤
│ ┌─ KYC 상태 분포 ────────────────┐ ┌─ RA 처리 상태 ─────────────────┐    │
│ │ 인증완료(VERIFIED)  39,210      │ │ 평가완료     41,820            │    │
│ │ 진행중(PENDING)        980      │ │ 재평가 대기      88 ⚠          │    │
│ │ 미비(INCOMPLETE)       420      │ │ 미평가          292            │    │
│ │ 만료(EXPIRED)          210      │ └────────────────────────────────┘    │
...
- **BR-001**: 탭은 `점수 분포` / `고위험 목록`(`riskGrade=HIGH/PROHIBITED`) / `CDD·RA 처리 현황` **3탭(순수 모니터링·read-only)**. 모델 버전 필터로 분포 조회. **모델 초안 시뮬레이션은 AML-RA-002 ③ 시뮬레이션 탭으로 이동**(모델 저작 활동·권한 `aml:admin:policy`이 모니터링 `aml:case:read`과 분리). 본 화면은 분포·통계 조회만, 변경 동작 없음.
- **BR-007 (v9.6 CDD·RA 처리 현황 통계)**: `CDD·RA 처리 현황` 탭은 **read-only 통계**로 ① KYC 상태 분포(`kycStatusCounts`) ② RA 처리 상태(평가완료/재평가대기/미평가, `raProcessing`) ③ 인입 회원(24h/7d/30d, `registrationWindow`) ④ 기간별 처리량(일별 평가 건수 histogram, `periodHistogram`)을 표시한다. **API = bo-api 소유** `GET /api/v1/bo/aml/ra/pipeline-stats?histogramDays=`(엔진 위임 `GET /api/v1/admin/aml/customers/pipeline-stats`, scope `aml:case:read`, 응답 `CddRaPipeline`). **집계 출처 = `aml_customers`(`kyc_status`·`onboarding_at`) + `aml_risk_scores`(`evaluated_at`·`next_review_due_at`)**, **tenant 스코프·PII 집계만**(raw 식별자 미노출). **비-prod stub / prod fail-closed**(엔진 미가용 시 운영 환경은 빈/오류 반환). **tipping-off 무관**(케이스·STR 미연계 순수 처리량 지표). DB: aml-svc V21 — `aml_customers.onboarding_at` DEFAULT now() + 기존 null 백필(additive). 부록 H **KYC 통계 상세 backlog 해소**.
