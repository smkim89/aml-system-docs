...
| 버전 | 일자 | 작성자 | 변경 내역 |
|------|------|--------|----------|
| **9.42** | **2026-07-08** | **Hanpass Global Team** | **§1.12 표시 라벨 정본 — bo-web 전 화면 다국어(i18n) 카탈로그 정본화(코드=truth, bo-web next-intl).** bo-web 이 전 화면 다국어(한국어(`ko`, 기본)·English(`en`))를 갖춤에 따라 **화면 표시 라벨의 정본 = bo-web i18n 카탈로그 키(ko/en)** 를 §1.12 로 신설하고, §1.9 온보딩 상태 "표시 라벨 정본" 표 캡션을 §1.12 참조·"표시" 열=ko 카탈로그 값으로 개정: 로케일 base(`messages/{ko,en}.json`, `aml`·`enum` 등 셸 네임스페이스 포함) + AML 도메인 번들(`messages/bundles/{ko,en}/{aml-monitoring,aml-customer}.json`, 네임스페이스 `amlMon`·`amlCust`) 얕은 병합, ko↔en 키 집합·ICU placeholder 패리티 게이트(`catalog-parity.test.ts`). **하드코딩 금지·래칫(ratchet) 스캔 게이트** — 주석 제외 한글 리터럴 스캔(`hardcoded-scan.test.ts`) + 축소 전용 allowlist(`i18n-baseline.json`), 신규/회귀 하드코딩은 빌드 게이트 실패. 활성 로케일은 쿠키(`NEXT_LOCALE`) 단독·URL 불변, 언어 선택기 명세는 IAM 정의서(FS-BO-IAM-001) §6. | 근거=bo-web `i18n/{locales,request}.ts`·`messages/{ko,en}.json`·`messages/bundles/{ko,en}/{aml-monitoring,aml-customer}.json`·`messages/{assemble.ts,catalog-parity.test.ts,scan-hardcoded.ts,hardcoded-scan.test.ts,i18n-baseline.json}`·`components/common/LanguageSwitcher.tsx`. 코드=truth. 화면 수·기능·PPT 불변. |
...
| WLF/명단 | WLF 검토(WLF-001~003), WLF 시뮬레이션(WLF-004) | 명단 소스·임포트(WL-001/002), 내부 명단·오탐 면제(WL-003) |
...
**표시 라벨 정본** (bo-web i18n 카탈로그 키(ko/en)로 일원화 — §1.12, 오픈결정 반영. 아래 "표시" 열=ko 카탈로그 값·English 는 en 카탈로그 정본):
...
### 1.12 표시 라벨 정본 — bo-web 다국어(i18n) 카탈로그 (v9.42 신설)

백오피스 전 화면이 다국어(한국어(`ko`, 기본)·English(`en`))를 지원하며, 화면 표시 라벨(메뉴·컬럼 헤더·버튼·상태/enum 표시값·안내문·빈 상태 문구)의 **정본은 본 문서의 한국어 서술이 아니라 bo-web next-intl 메시지 카탈로그 키**입니다. §1.9 온보딩 상태·§1.10 케이스 타입 등 본 문서의 "표시 라벨 정본"/"표시(한국어)" 표는 모두 이 카탈로그의 인스턴스입니다.

- **카탈로그 구성**: 로케일 base(`messages/{ko,en}.json` — 셸 네임스페이스 `app`·`common`·`nav`·`enum`·`error`·`header`·`language`·`login`·`shell`·`aml`·`approvalInbox`) + AML 도메인 번들 `messages/bundles/{ko,en}/aml-monitoring.json`(네임스페이스 `amlMon`)·`aml-customer.json`(네임스페이스 `amlCust`) 의 얕은 병합(shallow merge). 번들 네임스페이스는 base·형제 번들(`commonUi`/`fds`/`amlMon`/`amlCust`/`admin`)과 상호 배타여서 병합이 무손실입니다.
...
> **WLF/제재 스크리닝 소스(hanpass-ph 정합)**: 실시간 제재·PEP 스크리닝 신호 소스는 `member-svc zoloz_aml_screening`(`decision`/`risk_level`/`total_hits`/`hit_results`)이며, 점수 분해·상태(§3.2)·매칭 후보는 이를 정규화한 결과다. 명단군(`source_type` SANCTIONS/PEP/RCA/ADVERSE_MEDIA/INTERNAL/LAW_ENFORCEMENT/VASP_RISK)·provider 도메인은 불변, 48h 신선도 초과 시 fail-closed(DB §3.6·API §3.2). **규제 임계(자동낮춤/검토/고신뢰 유사도)는 정책팩 파라미터로 불변** — PH 운영은 `PH_AMLC` 옵션(near 0.90 등)으로 1줄 병기만(BR-009).

> **WLF 스크리닝 대상 = 해외송금 sender + receiver 양방향(hanpass-ph 6 거래유형 기준, 코드 정합, v9.18)**: WLF/제재 스크리닝은 **해외송금(`remit-svc` cross-border)** 거래의 **송금인(sender = 회원 본인, `targetType=CUSTOMER`)** 과 **수취인(receiver = 상대방, `targetType=COUNTERPARTY`)** 을 **둘 다** 스크리닝한다(수취국 PH/VN/ID 제재 대상 = 진양성 시연). 이전에 sender만 보던 단일 주체 인상을 보강 — receiver 스크리닝은 워치리스트 receiver 엔트리(aml-svc Flyway `V26` PH/VN/ID 엔트리)와 매칭하며, sender·receiver 매칭 후보·식별정보(§3.1 reveal 게이트)는 동일 `subjectIdentity` 4필드(NAME/NATIONALITY/GENDER/DOB) 균일 구조로 노출된다(주체 무관, COUNTERPARTY 미보유 필드는 공백). 데모(비위임)에서도 bo-api 스텁 스크리닝이 sender + receiver를 모두 평가한다. 회원가입(member-svc)·국내송금·월렛 등 비-cross-border 거래는 sender(회원)만 스크리닝. **명단군·임계·reveal 게이트 불변** — 6유형 정렬은 데모/시뮬레이터/시드 한정(엔진 도메인 비변경).

> **WLF 거래당 sender·receiver 2회 스크리닝(hanpass-ph 해외송금 정본 — 코드 `AmlWlfTransactionGroups`·`lib/aml-screening`·`ScreeningController.transactionRef`)**: hanpass-ph 해외송금은 **거래번호(`transactionRef`)당 WLF 스크리닝이 2회** 발생한다 — **송금인(sender, `targetType=CUSTOMER`, 회원 UUID 키)** 과 **수취인(receiver, `targetType=COUNTERPARTY`, 이름+국가+전화로 합성한 거래간 안정 키)**. 검토 큐는 거래번호 단위로 송금인·수취인 2건을 한 그룹으로 묶어 표시한다(그룹 헤더 = 거래번호, `N건 (송금인·수취인)`). **FP 화이트리스트(오탐 면제)** 는 receiver 키 기준으로 거래간 유효하다 — 동일 키의 후속 거래 스크리닝에서 자동 낮춤(`FalsePositiveWhitelist`·`FpWhitelistStorePort`, 현 룰버전 일치 시에만, §13.5). 데모 진양성 명단: 송금인 매치(JUAN/MARIA, V19) + 수취국 PH/VN/ID receiver 매치(V26). 국내송금·월렛 거래는 회원(sender) 단건 스크리닝. 이 receiver COUNTERPARTY 스크리닝 계보는 TM STR_PEP·STR_SANCTION 수취인 명단 평가(§7.1 BR-013)에도 재사용된다.

> **데모(비위임) 스크리닝 = 엔진 매칭 알고리즘 미러(코드 정합, v9.16)**: localhost 데모(aml-svc 엔진 미가동·비위임)에서는 더 이상 스크리닝ID 해시 기반의 **고정 후보·고정 점수**(예 일률 81%)를 만들지 않는다. bo-api 스텁(`StubNameMatcher`)이 aml-svc 엔진의 결정적 매칭 1차요소(`TextNormalizer`·`NameSimilarity`·`FuzzyMatchEngine`·`MatchRuleSet`)를 **충실히 미러**하여 **회원 본인 실제 이름을 워치리스트 엔트리와 정규화·유사도 계산**하고, **검토 임계(0.66) 이상인 후보만** 매칭으로 부착한다. ① **정규화**: 유니코드 NFKD 분해 → 발음부호(결합문자) 제거 → 소문자 → 영숫자 외 문자=공백 → 토큰화 → 순서보존 중복제거(엔진 `TextNormalizer`와 동일). **한글 이름은 라틴 명단과 토큰 교집합 0**(한국 회원명은 라틴 제재명과 결코 퍼지 매칭되지 않음 — 의도적). ② **유사도**: exact(토큰집합 동일 1.0) / tokenSet(Jaccard) / editDistance(`1 − Levenshtein/max`), `nameScore = exact≥1 ? 1 : max(tokenSet, edit)`. ③ **가중합**: name 0.55·date 0.10·country 0.10·document 0.15·address 0.05·relationship 0.05의 가중치를 두되 **분모는 항상 전체 가중치 합(1.0)** — 스텁이 제공하지 않는 document/address/relationship 컴포넌트는 분자 기여 0이고 분모에는 그대로 남아 엔진(`FuzzyMatchEngine`이 `sumOfWeights()`로 나눔)과 **1:1** 일치한다(이름만 완전일치 = 0.55, 부풀린 0.733 아님). 임계 `possibleThreshold 0.66`·`trueThreshold 0.92`는 `MatchRuleSet.defaultRuleSet()` 미러다. **검증**: 김민준(한국)→0.0 NO_MATCH; IVANOV 완전일치+생년·국적 일치→0.75 POSSIBLE(`NAME_EXACT_MATCH`); IVANOV/Sergey(1글자 변형)+생년·국적→≈0.708 POSSIBLE(`SANCTIONS_NAME_SIMILARITY`); 생년·국적 미제공 name-only 완전일치→0.55 NO_MATCH(엔진과 동일하게 보강신호 없으면 검토 밴드 미만). 즉 **닮은 회원만 POSSIBLE_MATCH, 비유사(한국 이름 등)는 NO_MATCH**다. **BR-007 유지** — 미러 계산은 마스킹 토큰·파생 속성으로만 수행하며(원문 PII 보유·영속 없음), 응답은 마스킹 토큰·점수·reason code만 노출한다.

> **스크리닝 큐 = 실 인입 레코드(데이터 정직화, 코드=truth v9.27, BR-DEMO-HONESTY)**: 데모 스크리닝 큐/상세는 **더 이상 데모 멤버 집합을 즉석 생성하지 않는다**. 송금 거래 인입마다 sender(CUSTOMER, 회원 vault 이름)+receiver(COUNTERPARTY, payload 이름/국가) **2건의 실매칭 레코드**를 인메모리 스토어(상한·eviction·`transactionRef` 쌍 그룹)에 적재하고, 큐/상세는 그 레코드만 읽는다 — **인입 전이면 큐가 비어 있다**(합성 행 0, 정직 문구 노출). `screeningId` 은 인입 시 부여한 **랜덤 UUID** 이며(자기서술적 hash 인코딩 폐기), 레코드가 targetRef/targetType/매처 결과를 보관하므로 상세 복원에 hash 재유도가 없다. 밴딩은 엔진 동형(≥0.66 POSSIBLE_MATCH·<0.66 NO_MATCH·`TRUE_MATCH`=분석가 4-eyes만). **부분 식별(이름+국가) 수취인은 overall 상한 0.65(name 0.55 + country 0.10, 생년월일 미제공)로 자동 밴딩이 NO_MATCH — 엔진 `FuzzyMatchEngine`(전체 가중치 합 정규화) 동형이다.** 즉 수취인 COUNTERPARTY 행은 큐엔 뜨지만 자동 POSSIBLE 승격은 불가하고 확정은 분석가 4-eyes 로만 이뤄진다. **TM 명단 룰(nameScore 축 발동, §7.1 BR-011)과의 비대칭은 의도된 정직 동작**이다(제품 후속: 매처 재정규화 검토). **FP 화이트리스트 시드(3행)는 폐기**되어 등록 액션(4-eyes 폐루프)으로만 생성되고, **벌크런 결과 카운트는 업로드 행을 실매처로 대조한 실제 카운트**(transient·파일 미보존)다.
...
| 점수 분해 | factor별 기여도 (이름 유사·생년·국가·문서·주소·관계, `scoreBreakdown`, 가중 분모=전체 가중치 합 1.0). **데모 스텁(aml-svc 미가동·비위임)은 name/dob/country 서브셋만 산출**(엔진=6 컴포넌트, 의도적 단순화 — overall은 동일 분모로 엔진과 1:1, API §3.2) |
...
| 식별정보(reveal 게이트) | **회원 본인 식별정보**와 **워치리스트 엔트리 원문(매칭 후보)** 을 모두 **이름·국적·성별·생년월일(NAME/NATIONALITY/GENDER/DOB) 4필드 균일**로 표시(`subjectIdentity` = 마스킹 토큰 + reveal 가능 필드 키, API §3.2). **데이터가 없는 필드는 공백**으로 둔다(수취자=상대방(`COUNTERPARTY`)·비-customer 주체는 성별·생년월일을 보유하지 않을 수 있어 공백 — 예 내부 명단 엔트리 `wl-internal-002`). 기본은 마스킹 토큰만 노출하고, 권한 `aml:pii:reveal` 보유 시 각 필드의 `[원문 보기]` 버튼으로 사유 입력 후 `POST /internal/v1/aml/pii/reveal`(API §2.6, `field`∈ NAME/NATIONALITY/GENDER/DOB) 호출 → 이 요청 한정 cleartext 표시 + `RAW_DATA_ACCESS` 감사. reveal stub 은 인식된 데모 주체의 미보유 식별필드에 빈 값(`""`)을 반환한다(placeholder 아님). **scope 없으면 버튼 숨김**, 원문 부재 필드는 fail-closed(BR-007) |
...
- **BR-011 (hanpass-ph 거래당 sender·receiver 그룹 — 코드 정본 `AmlWlfTransactionGroups`)**: 해외송금 건은 검토 큐를 **거래번호(`transactionRef`) 단위 그룹**으로 묶어 표시한다 — 그룹 헤더(거래번호) 아래 **송금인(`CUSTOMER`)·수취인(`COUNTERPARTY`) 각 1건**(거래당 2회 스크리닝)을 나열하고, 각 행에서 상세 열람·`[오탐 면제]`(FP 화이트리스트, 4-eyes `FP_WHITELIST`)를 수행한다. `transactionRef` 없는 건(국내송금·월렛 단건)은 평면 행으로 폴백한다. 역할 라벨은 송금인/수취인(`wlfRoleLabel`), 대상유형 미정의 시 일반 대상유형 라벨로 폴백.
...
> - **(c) CUSTOMER = kyc_evidence 파생** — `CUSTOMER = (sofRisk[sourceOfFunds] + kycLevelRisk[kycLevel]) / 2`(시뮬레이터 매핑을 엔진 정본 이관, 가정 A3). occupation 은 모델 parameters 예약 슬롯(기여 0).
...
| 1차 RA 내역 | **최근 30일 인입(온보딩) 회원의 1차(ONBOARDING) RA — 회원별 최신 1건, 전 등급**(`risk-scores?riskGrade=LOW,MEDIUM,HIGH,PROHIBITED&scenario=ONBOARDING&latestPerTarget=true&registeredWithinDays=30` — 인입 필터는 기본 ON 체크박스, 해제 시 전체 기간) + 1차 주요 요인(KYC·국적/국가위험·이름 스크리닝·이용 서비스/채널). 서버 페이징(`page/size`, `ListPager` 총건수 노출) |
...
│  ※ 발동=실명 매칭(이름점수≥0.92, WLF 데모 매처) — 데모는 4-eyes 폐루프 압축 근사│
...
| 레거시 시나리오(설정 전용) | `TmScenario` enum 10종(DB §5.6)은 **알림 발동에서 폐기**(BR-010)되고 TM 시나리오 관리(AML-TM-002) 설정·시뮬레이션에만 남는다. hanpass-ph 설정 ACTIVE 6종(STRUCTURING·RAPID_MOVEMENT·MULE_NETWORK·HIGH_RISK_CORRIDOR·REFUND_LAUNDERING·ROUND_TRIPPING) + advanced-domain 4종(SHELL_MERCHANT·TRADE_MISPRICING·CRYPTO_OFF_RAMP·INTERNAL_OVERRIDE_ABUSE, 송금·월렛 미발화) — 아래 'phpEquivalent 임계' 표 참조 |
...
#### hanpass-ph 운영 시나리오 임계 (구현 정본 — phpEquivalent, tenant_demo)

`TmEvaluationService.buildSnapshot` 가 `transaction.phpEquivalent`(hanpass-ph PHP 환산액)를 subject 의 최신 transaction-bearing 캐논 payload(`payload->>'phpEquivalent'`)에서 노출하며, 금액 기반 시나리오는 PHP 임계로 발화한다(Flyway V19/V22/V26/V28, 부재 시 미발화 fail-safe). 아래는 tenant_demo ACTIVE 시나리오 6종의 임계·윈도우 정본이다.

| 시나리오(코드) | 활성 버전 | DSL 임계(코드 정본) | 심각도 |
|---|---|---|---|
...
> PHP 임계는 데모 동등 유지 환산(기존 USD 임계 × 56, V28). velocity count 는 항상 subject(=`targetRef`) 기준 윈도우 집계다. 거래상대 분산·환불 전용·순환 hop feature 는 미-materialize 상태로 가용 feature(velocity count/sum subject + phpEquivalent)로 근사하며, 미가용 차원은 `parameters`(가이드 폼 정본)로 표현한다(V22 주석). **advanced-domain 4종은 위 임계 표에 없으며 hanpass-ph 운영에서 미발화** — bo-api 데모 스텁도 `DEFAULT_DRAFT_SCENARIOS`(`REFUND_LAUNDERING`·`TRADE_MISPRICING`)·ACTIVE 시나리오만 알림을 발동한다(`AmlTmService.scenarioActive`).
...
| **권한** | 조회 `aml:case:read` / EDD 케이스 생성·주기 변경·즉시 재이행 접수 `aml:case:update` |
| **API** | `GET /aml/customers/{customerRef}/risk` · `POST .../cdd/cases`(EDD 착수) · `POST /api/v1/bo/aml/customers/{customerRef}/review-cycle:change`🔒(PERIODIC_REVIEW_CHANGE) · `POST /api/v1/bo/aml/members/{memberRef}/reissue:request`(즉시 재이행 접수, API §2.x) |
...
- **BR-002**: 점수·factor는 설명가능성 원칙(설계서 §5.2)으로 분해 표시한다. 화면에서는 동일 `factorBreakdown`을 **1차 고객 위험평가 요인**(KYC 원장·국적/국가위험·직업/고객속성·이름 스크리닝/제재·PEP·이용 서비스/채널)과 **2차 활동 기반 재평가 요인**(거래행동·FDS 위임/에스컬레이션·TM 반복 적중·관계/UBO·crypto/trade)으로 나누어 표시한다. factor breakdown 탭에는 **평가 입력 원장**을 함께 표시해 사용자가 점수 분해와 원천 입력을 대조할 수 있어야 한다 — CDD/KYC 공통 원장(고객유형·국적/설립국·KYC 상태·신원확인 상태), 개인/법인별 추가 원장(생년 마스킹, 대표자/UBO 요약), RA 결과 대조(`riskSummary` vs 현재 score), 2차 활동 입력(스크리닝·케이스·관계·STR 건수)을 영역 분리한다. raw PII 미노출(토큰/마스킹); STR 건수는 tipping-off guard 를 유지한다.
...
- **BR-007 (룰 임계·변수 편집 = 4-eyes `REPORT_RULE_PARAM` + TM 평가 반영 폐루프 — 코드=truth, feature/aml-report-rule-conditions-editing)**: 룰 상세 편집 섹션에서 카탈로그 `params[]`(키·라벨·카탈로그 기본값·resolved 현재값·단위·min/max·editable)를 편집해 `POST /api/v1/bo/aml/report-rules/{ruleCode}:update-params`(`aml:admin:policy`) 로 **룰 단위 전체 파라미터 셋을 원자 상신**한다(`band_lower`/`band_upper` 교차검증이 단건 승인으로 깨지는 것 방지, pending 중복 상신 거부·`pendingParamApprovalId` 뱃지). 서버는 카탈로그 스키마로 검증(스키마 외 키·읽기전용 키 거부, min/max 범위, `band_upper > band_lower`)하고 🔒 `REPORT_RULE_PARAM`(승인선 **COMPLIANCE** — 탐지정책, §03 §4.2) 결재를 생성한다(202). **승인 EXECUTED 시 반영 = submit-time 위임**: 엔진 연결 시 aml-svc admin `POST /api/v1/admin/aml/report-rules/{ruleCode}:update-params` 위임(`aml.aml_report_rule_params` V22 원자 upsert, 운영 미연결 fail-closed), 비운영 stub 은 `backoffice.aml_report_rule_params`(bo-api V9) 로컬 폐루프 — 이후 TM CTR/STR 평가(bo-api 라이브·aml-svc STR 평가 경로)가 오버라이드를 resolve 해 **수정값이 실제 발동 판정에 적용**된다(parity, 무-오버라이드 시 기존 동작 완전 보존). 편집 가능 파라미터: `income_multiplier`(STR_KYC_INCOME_MISMATCH)·`count_threshold`/`window_hours`(STR_VELOCITY_CASH)·`band_lower`/`band_upper`/`min_consecutive_days`(STR_STRUCTURED). **CTR 임계는 본 편집 밖** — `CTR_THRESHOLD` 결재(§9.1) 단일 정본 재사용(absent=미발동 fail-safe 불변). **`STR_PEP`/`STR_SANCTION` 의 `name_match_threshold`(실명 매칭 0.92)는 읽기전용 표시**(WLF 매칭 정본 결합 — 편집 후속). 감사 `REPORT_RULE_PARAM_CHANGE_SUBMITTED`(V9).
...
- **구성**: 탭 `① CDD 프로필(신원확인·검증)`/`② 위험·활동 요약`/`③ PEP(정치적 주요인물) 관리`(v9.13 신설). ① 고객 유형(개인/법인)별 KYC 수집 항목 원장(마스킹) — 공통(식별자·유형·국적·신원확인 증표 구분·검증 방법·대면 여부·고객 상태) + 개인(직업·업종·자금 원천·거래 목적·월평균 소득 구간) + 법인(법인 유형·업종·상장 여부·비영리 설립목적 검증·**실소유자(UBO) 확인 면제 여부**·대표자 요약). 이 원장은 **1차 고객 위험평가 입력 원장**으로 표시한다. ② 위험·활동 요약 — **1차 RA 결과**(RA 등급·위험 점수·재이행 예정일·당연고위험 사유)와 **2차 활동 기반 재평가 신호**(스크리닝 이력 건수·진행 케이스·STR 보고 건수[전담 한정]·거래 거절 이력·관계/UBO 요약)를 영역 분리해 표시한다(→ AML-RA-003 ②). ③ PEP 관리 — 현재 PEP 상태(`isPep` 배지: PEP 확정/비-PEP)·진행 중 결재 상태(`pepApprovalStatus`: 상신/승인/반려·미상신)·증거 링크(`pepApprovalId`) + `[PEP 경영진 승인 상신]` 액션 + PEP 결재 히스토리 표(상신/승인/반려·상신자·승인자·시점, 공통 결재함 재사용).
...
| AML-CTRY-001 | 설정 › 탐지·심사 정책 | 국가위험(고위험 국가) 관리 — FATF 일일 자동 수집 + 수동 4-eyes(3탭) | (엔진) `GET .../country-risk` · `GET .../country-risk/import-status` · `POST .../country-risk:import`(수동 수집 트리거) · `POST .../country-risk:change`🔒(COUNTRY_RISK) |
...
| AML-SUBJ-001 | 운영 › 고객위험·심사 | 대상 360 조회(검색 entry — customerRef/transactionRef/walletRef 입력→대상360°/CDD/RA 이동, v9.4 코드 정합) | **bo-api** `GET /api/v1/bo/aml/subjects/{customerRef}/360`(scope `aml:case:read`, DB §3.16·API §2.5a·§3.4b). 진입 후 대상 360° 통합 뷰 → AML-CDD-002(고객 프로필)·AML-RA-003(RA 상세) 드릴다운 |
