...
- **subject_type 정합(엔진)**: `aml_approvals` CHECK(16) → **19**로 확장(IRA_SUBMIT·HIGH_RISK_REGISTRY·**PEP_APPROVAL** 추가, Flyway — IRA/HRR=엔진 V13/V14, PEP=엔진 V24; 단 2026-06-30 consolidate 후 통합 baseline `V1__baseline.sql`에 19종 흡수, DB §7·§5.16 참조) 후 **V23에서 `CTR_THRESHOLD` 추가로 20종**, **V28에서 `HRR_REGISTRATION` 추가로 21종**이 되었다가, **V31(Travel Rule 전면 제거)에서 `TRAVEL_RULE_EXCEPTION` 제거로 20종**으로 정정됐다(현행 `ApprovalSubjectType` 20종). `REPORT_RULE`/`REPORT_RULE_PARAM`은 bo-api 계층 결재 경계를 유지한다.
