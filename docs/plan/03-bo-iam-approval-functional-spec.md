...
| 테이블 | 변경 |
|---|---|
| `bo_admin_users` | (유지). 데모 부트스트랩(V18)·데모 승인자 5명(V27) 시드. **bo-api V16__demo_executive_checker** — 데모 고위경영진 checker `choi.exec@aegis.example`(id=6, admin_type=EXECUTIVE) 활성화(bcrypt `test1234!`·`ACTIVE`)·`AML_APPROVER`/`AML_VIEWER`/`AML_POLICY_ADMIN` 3역할 자연키 바인딩(`bo_user_roles`, id 하드코딩 금지)·`tenant_demo` 바인딩(`bo_user_tenants`) — `HRR_REGISTRATION`(승인선 `EXECUTIVE_APPROVAL`) 승인 주체 부재 갭 해소(가정 A2, 라인-역할 강제는 미구현·scope 게이트만). 신규 감사 이벤트 없음(기존 `AML_APPROVAL_APPROVED/REJECTED` 재사용) |
