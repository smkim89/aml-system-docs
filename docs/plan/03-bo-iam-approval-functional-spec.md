...
>
> 변경 이력:
> - 2026-09-04 — **§2.4 케이스 담당자 디렉토리 권한 경계 정정·§6 헤더 케이스함 메뉴 신설(코드=truth, PLAN 20260904-aml-case-assignee-picker)**: §2.4 담당자 디렉토리 포함 조건에 플랫폼 컨텍스트·플랫폼 운영자(`BackofficeRole.isPlatformOperator`) 예외를 병기(현재 테넌트·워크스페이스 바인딩 계정 한정이 원칙이나 플랫폼 컨텍스트/운영자는 전 테넌트 대상). §6 헤더에 **케이스함 메뉴**(`CaseInboxMenu`, 결재함 왼쪽) 신설 — 본인 담당 진행 케이스 건수 배지 + `내 케이스`·`전체 케이스` 진입.
...
- **케이스 담당자 디렉토리**(`GET /api/v1/bo/aml/cdd/case-assignees`, scope `aml:case:read`, PLAN 20260904-aml-case-assignee-picker)는 위 계정관리 API 의 권한 경계를 **담당자 지정 목적의 최소 투영(이메일·이름·부서, 활성·`aml:case:update` 보유·현재 테넌트·워크스페이스 바인딩 계정(또는 플랫폼 컨텍스트·플랫폼 운영자 `BackofficeRole.isPlatformOperator`)만)** 으로 케이스 조회 권한자에게 확대한 별도 읽기 계약이다(역할·상태·로그인 이력 비노출).
...
추가: 헤더 **케이스함 메뉴**(`CaseInboxMenu`, 결재함 왼쪽, PLAN 20260904-aml-case-assignee-picker) — 본인 담당 진행(OPEN·INVESTIGATING) 케이스 건수 배지 + `내 케이스`(`/aml/cases?tab=mine`)·`전체 케이스` 진입. `/aml/cases` nav 접근 권한 보유 세션에만 노출. 사이드바 항목 추가 없음(AML nav 23 유지).
