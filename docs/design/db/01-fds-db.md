...
| V24 | `V24__settlement_payout_country.sql` | **정산 지급국가 컬럼 + A군 7종 카탈로그 라벨(F-002 input-slot 26종 커버리지, PLAN 20260716 A군)**. (1) `fds.fds_settlements` 에 `payout_country character varying(2)` `ADD COLUMN IF NOT EXISTS`(ISO-3166 alpha-2 소스 제공값·nullable additive·NOT NULL 1단계 금지 준수) — `settlement.payoutCountry` 피처 계산 근거. (2) `fds_feature_catalog` `_global` 공유행 A군 7종(`counterparty.newBeneficiary`·`vendor.onboardingAgeDays`·`invoice.amountDeviation`·`seller.salesSpike`·`seller.gmvSpike`·`settlement.payoutCountry`·`settlement.preSettlementAccountChange`) `display_label` 의 "(외부/후속)"→"(계산 지원)" UPDATE(V23 RLS 정책 비접촉·정책명 불변, 등재·enable 여부 불변) | additive(컬럼·라벨) |
