...
> 변경하지 않는다.
> **디시전트리 baseline 팩의 워크스페이스 bootstrap**: `tenant_demo` 가 아닌 프로파일(예: `aud`→`tenant_au`)의 트리 팩(`db/currency/<code>/R__baseline_decision_trees.sql`, §5.37 참조)은 위 테넌트 metadata 와 동일 값을 재사용해 `fds_workspaces`(§5.2)의 `(tenant_id, 'default')` 행도 `ON CONFLICT DO NOTHING` 으로 함께 bootstrap한다. `fds_decision_trees`(§5.37, V37)가 `(tenant_id, workspace_id) → fds_workspaces` FK 를 요구하는데, `fds_workspaces` 시드는 `tenant_demo/default`(`V2__seed.sql`) 1행뿐이고 repeatable Flyway 는 파일명 알파벳순으로 적용되어 `R__baseline_decision_trees.sql` 이 테넌트를 만드는 `R__currency_rule_set_configuration.sql` 보다 먼저 실행되기 때문이다(그대로 두면 FK 미충족으로 트리 0행 삽입에 실패하고, repeatable 은 체크섬 불변 시 재실행되지 않아 이후 부팅에도 0행이 고정된다). `tenant_demo` 프로파일(`php`)은 기존 워크스페이스를 그대로 쓰므로 이 prelude 가 없다. rule-set 팩(`R__currency_rule_set_configuration.sql`)도 기본 워크스페이스를 멱등 시드한다(`ON CONFLICT (tenant_id, workspace_id) DO NOTHING`) — 트리 팩 prelude 는 repeatable 알파벳 정렬상 rule-set 팩보다 먼저 실행되므로 실질적으로 선점하지만, 두 팩이 같은 행을 동일 조건으로 시드해 실행 순서가 바뀌어도 FK 가 깨지지 않는 안전망이다.
...
> **V35 식별자 해시 피처 시드(`V35__identifier_hash_feature_catalog.sql`, 20260906 — aml-data §11.7.9)**: `_global`/`default` 3행 upsert(`ON CONFLICT … DO UPDATE` 멱등, V31 동형) — `counterparty.accountNoHash`(STRING, '수취 계좌 해시' — canonical payload `counterparty.accountNoHash`)·`subject.phoneHash`(STRING, '회원 전화 해시' — `originator.phoneHash`, raw `originator.phone` 수용 즉시 `SHA-256(숫자만)` 파생 후 폐기)·`counterparty.phoneHash`(STRING, '수취인 전화 해시' — `counterparty.phoneHash`/raw `counterparty.phone` 파생). 운영자 블랙리스트 룰팩 BL-03/BL-04/BL-05 의 `in_group` 대상 피처이며 리스크그룹 멤버 해시와 동일 canonical 로 대조된다.
>
...
> **V33 V28 archive 감사 backfill.** V28이 DB migration으로 직접 `ARCHIVED` 처리한 §2.1 legacy rule UUID 21건은 application audit 경로를 지나지 않아 `RULE_ARCHIVE` 행이 없었다. V33은 rule 상태나 기존 감사 행을 수정하지 않고 `(tenant_demo, default)` 현재 chain tail 뒤에 UUID 정렬 순서로 정확히 21행을 append한다. 각 행은 actor/detail provenance `system:migration/V28`, 결정적 audit UUID, strictly-increasing microsecond `created_at`, V22 공식으로 계산한 non-null `prev_hash`/`row_hash`를 가진다. hash는 PostgreSQL core `sha256(convert_to(canonical,'UTF8'))`/`encode(...,'hex')`와 core `md5(text)`만 사용하며 `pgcrypto` 설치·`CREATE EXTENSION` 권한을 요구하지 않는다. 동일 target의 기존 `RULE_ARCHIVE`가 있으면 생성하지 않아 재실행 중복은 0이며, `FdsAuditHashChainVerificationService` 전체 chain 검증이 PASS해야 한다.
...
| 일자 | 버전 | 변경 내용 | 비고 |
|---|---|---|---|
| 2026-09-22 | v4.19 | **AU(AUD) 통화 프로파일 디시전트리 baseline 팩·워크스페이스 prelude 역전파(코드=truth, aegis-aml PLAN fix-20260922-au-profile-gaps U1·U6).** (1) §5.1 註에 비-`tenant_demo` 프로파일(`aud`→`tenant_au`) 트리 팩이 `fds_tenants`+`fds_workspaces` 를 멱등 prelude 로 bootstrap 한다는 사실(FK·repeatable 알파벳 정렬 사유) 추가. (2) §5.37 통화 팩 註를 "현재 php" → "php·aud" 로 갱신 + 워크스페이스 prelude 1문장 추가. 스키마·마이그레이션 변경 없음(팩 SQL 은 기존 V37 자산 테이블에 행만 추가) | 코드 truth=`scripts/generate_currency_packs.py`·`config/decision-trees/aud.json`·`services/fds-svc/.../db/currency/aud/R__baseline_decision_trees.sql`(aegis-aml 커밋 `4b9fb7b`) |
...
| 2026-08-24 | v4.16 | **V28 legacy archive 감사 hash-chain backfill(V33, F-006).** §5.32에 21개 `RULE_ARCHIVE` provenance·V22 chain append·재실행 중복 0·core hash 함수 사용/pgcrypto 불요 계약을 추가하고 §8 migration 표/헤더를 V33으로 갱신했다. | 코드 truth=`V33__legacy_rule_archive_audit_chain.sql`·`LegacyRuleArchiveAuditMigrationIntegrationTest`·`FdsAuditHashChainVerificationService`. |
...
통화 팩 `db/currency/<code>/R__baseline_decision_trees.sql`(생성 파일, 정본 `config/decision-trees/<code>.json`; 현재 `php`·`aud`)이 `fds_decision_trees`·`fds_decision_tree_versions`(version 1) 에 프로파일 baseline 5종을 `<프로파일 전용 테넌트>/default` 로 시드한다 — 포인터·simulation·approval 행은 만들지 않는다([계약 §5](../../fds-decision-trees.md)). `tenant_demo` 는 `V2__seed.sql`이 시드한 `tenant_demo/default` 워크스페이스를 그대로 쓰지만, 그 외 프로파일(예: `aud`→`tenant_au`)은 해당 워크스페이스 행이 없으므로 트리 팩이 스스로 테넌트·기본 워크스페이스를 멱등 prelude 로 먼저 만든다(§5.1 註 참조) — 그렇지 않으면 `fds_decision_trees`의 workspace FK 가 걸려 트리가 0행으로 남는다.
