...
| `report_cutoff_time` | TIME | Y | NULL | | **CTR/STR 법정 보고 마감시각**(관할, V69). 기한은 `+N 영업일 <마감시각>` 이며 이 값은 같은 행 `timezone` 으로 해석된다 — 둘은 항상 짝으로 읽힌다. NULL=미설정 → 엔진 기본값 17:00 폴백(기존 배포·PH 테넌트 무변경). 해석·폴백 정본 `JurisdictionCalendarPolicy` |
...
| `kyc_evidence` | JSONB | N | '{}' | | CDD current-state PII-safe projection. KYC 코드(`occupation`,`sourceOfFunds`,`declaredIncomeBand`,`kycLevel`,`residenceCountry`,`kycVerifiedAt`)와 ISO `nationality`, 소스 등록시각 `registeredAt`만 보존한다. canonical 입력의 `declaredIncomeBand`는 기존 유한 4종이지만, exact `customer.cdd.completed`에서 다른 지원 KYC 차원이 남고 band가 부재/null/blank이면 projection에만 내부 sentinel `UNKNOWN`을 저장한다. raw 이름/DOB/문서번호는 금지하며 안전한 masked birth-year 정본이 없으면 profile `birthYearMasked=null` |
...
| V69 | `V69__tenant_report_cutoff_time.sql` | **테넌트 관할 보고 마감시각**: `aml_tenants.report_cutoff_time TIME NULL`을 additive 추가하고 기존 PH 데모 지문 행만 `17:00`으로 backfill한다. NULL은 미확정/구 배포를 뜻하며 애플리케이션이 기존 기본값 17:00으로 폴백한다. 시간대는 V53 `timezone`과 한 snapshot으로 해소한다. | V68 |
...
| 2026-08-16 | **신고소득 숫자 직접 수신 + 보고기한 관할화 역전파(V69, 사용자 지시로 F-029·F-040·F-041 해제).** (1) **신고소득 인입 계약 개편** — 밴드 코드(`declaredIncomeBand`) 대신 `declaredIncomeOperator`+`declaredIncomeAmount`+`declaredIncomeCurrency`+`declaredIncomePeriod` 를 직접 받는다. 상한을 알 수 있는 연산자(`<=`·`<`·`=`)만 평가하고 하한만 아는 신고(`>=`·`>`)는 미평가한다 — 룰이 `거래금액 > 소득 × multiplier` 라 소득을 크게 잡을수록 덜 발동하므로, 임의 상한을 지어내면 오탐이 는다(구 `OVER_10M_PHP` skip 과 같은 판단). 밴드는 레거시 폴백으로 남아 공유 사전 계약(F-041)은 불변. **부수 효과**: 밴드가 PHP 액면이라 비PHP 법인의 소득 룰이 영구 미평가였던 제약이 해소됐다(F-077 이 별도 요건으로 남긴 범위) — 단위 정합 가드는 유지되어 신고 통화 ≠ 테넌트 통화면 여전히 미평가다. (2) **보고기한 관할화** — `BankingCalendar` 의 `Asia/Manila`·17:00 하드코딩을 주입 가능하게 바꾸고, 시간대는 `timezone`(V53)·마감시각은 신규 `report_cutoff_time`(V69)에서 읽는다. 미설정은 기본값 폴백이라 PH 동작 불변(F-029 "+5영업일 17:00 PHT"). 달력 생성 4곳이 `JurisdictionCalendarPolicy` 단일 정본을 공유한다. (3) 미상 sentinel 조건 보정 — 숫자로 신고한 회원이 밴드 키 부재를 이유로 `UNKNOWN` 으로 거짓 표기되던 결함(F-076 계약은 유지). | 코드 truth=`services/common-policy/.../DeclaredIncomePolicy`·`services/aml-svc/.../domain/report/{BankingCalendar,JurisdictionCalendarPolicy}`·`db/migration/V69__tenant_report_cutoff_time.sql`·`IdentityProjectionService`·`TransactionReportSideEffectRunner`. 검증=DeclaredIncomePolicyTest 10 · BankingCalendarJurisdictionTest 6 · NumericDeclaredIncomeStrTest 6 · IdentityProjectionNumericIncomeTest 4 신규 + aml-svc 2,524 · python 562 전건 PASS(잠금 F-076·F-032 포함). |
...
| 2026-08-05 | **STR 법정 보고 기한 DRAFT 개설 시 확정 역전파(코드=truth, fix/bo-str-draft-link-and-tm-rulecode).** §3.12 `aml_regulatory_reports.due_at` 은 CTR 전용이 아니라 **STR 에도 채워진다** — BR-011("STR 기한 = 의심확정 +5영업일", API §3.6 "의심확정(트리거)일 +5영업일")대로 `RegulatoryReport.strDraft()` 가 트리거(의심확정) 시각 +5영업일 17:00 PHT 를 DRAFT 개설 시점에 freeze 한다(`StrEvaluationService` 가 `BankingCalendarPort` 로 CTR 과 **동일한** `BankingCalendar.dueAt` 정본 경로 공유 — STR 전용 산식 없음). 종전에는 `strDraft()` 가 `due_at` 을 세팅하지 않아 제출·접수(ACKNOWLEDGED) 이후까지 NULL 이었다(CTR 만 정상인 비대칭). 사유 누적(`addStrReason`)은 기한을 이동시키지 않으며, 기한 배선 이전 legacy 행은 NULL 로 남는다. **스키마·마이그레이션 신규 없음**(기존 V4 `due_at` 컬럼 재사용). | 코드=truth. 근거=aml-svc `domain/report/RegulatoryReport#strDraft`·`application/usecase/StrEvaluationService#strDueAt`·`domain/report/BankingCalendar#dueAt`·`test/.../StrReportDueAtTest`. API §3.6 `reportDeadlineAt`·기능정의서 §9.1 BR-011 동기화. |
