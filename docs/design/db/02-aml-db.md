...
| `source` | VARCHAR(32) | N | — | CHECK 2종 | `MemberDecisionSource`(`EDD_CLOSE`/`RELATIONSHIP_REJECT`) — 영속 값만(`INGEST`는 read-back 파생 전용 가상값, 저장 안 됨) |
| `reason` | VARCHAR(256) | Y | NULL | | 결재 사유(`ApprovalRequest.reason`) |
| `approval_id` | UUID | Y | NULL | | 이 확정을 만든 4-eyes 결재 id(`aml_approvals`) |
| `case_id` | UUID | Y | NULL | | 근거 케이스(`aml_cases`) |
| `case_type` | VARCHAR(64) | Y | NULL | | 결재 실행 시점 케이스 유형(§5.8 `case_type`) |
| `final_status` | VARCHAR(32) | Y | NULL | | 결재 실행 시점 케이스 종결 상태(§5.9 `case_status`) |
...
PK: `(tenant_id, decision_id)` · 인덱스 `ix_aml_member_decisions_member (tenant_id, member_ref, decided_at DESC)`(회원별 최신순 조회) · UNIQUE 부분 인덱스 `ux_aml_member_decisions_approval (tenant_id, approval_id) WHERE approval_id IS NOT NULL`(결재당 1행 — replay 멱등, `ApprovalDispatchService` 의 `findByIdForUpdate` 직렬화와 결합해 경합 없음). RLS 2종(`aml_rls_tenant`/`aml_rls_owner`, §1.2·V57·V72 동형) ENABLE+FORCE. **append-only**(UPDATE/DELETE 없음). **raw PII 미적재** — 참조 토큰·enum 스냅샷만. 결정 append·웹훅 아웃박스 enqueue는 결재 실행(`markExecuted`) 트랜잭션 내부(`@Transactional` REQUIRED 조인)이며, **replay(동일 `approvalId`)는 선조회 skip 으로 무예외**이고 **그 외 실패는 결재 실행 tx 와 함께 롤백**된다(결정 없는 EXECUTED 결재를 남기지 않음 — PLAN §4 L1).
