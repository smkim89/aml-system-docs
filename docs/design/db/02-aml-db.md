...
| `policy_pack_version` | VARCHAR(32) | Y | NULL | | **활성 Policy Pack revision 핀**(P0-16, V53). `TenantPolicyResolver` 가 `(tenant, policy_pack_code, policy_pack_version)` 로 `aml_policy_packs`(§3.14) 활성 revision 을 조인·evidence 고정에 사용. 미존재/비-effective revision → fail-closed(422). tenant_demo backfill=현재 활성 revision(저장소 truth 서브쿼리, 활성 pack 부재 시 NULL) |
