...
| V57~V60 | *(미머지 위임)* | AMLC 이관(V57·V58)·HRR 콜백(V59)·제재 4소스 EU/UK/AU/JP(V60)는 각 소유 문서 브랜치가 서술한다 — 본 표는 행만 예약. | — |
| V61 | `V61__remove_legacy_tm_scenarios.sql` | **레거시 TM 시나리오 정의 10종 제거 + `scenario_code` 자유형 전환(F-025 실측 — 거래 인입 경로에서 미발동)**: `aml.aml_tm_scenarios` 의 레거시 정의 10종(SUPERSEDED 구버전 포함 13행 — `STRUCTURING`·`MULE_NETWORK`·`RAPID_MOVEMENT`·`ROUND_TRIPPING`·`SHELL_MERCHANT`·`TRADE_MISPRICING`·`REFUND_LAUNDERING`·`CRYPTO_OFF_RAMP`·`HIGH_RISK_CORRIDOR`·`INTERNAL_OVERRIDE_ABUSE`) DELETE 후 `aml_tm_scenarios_scenario_code_check` 를 닫힌 목록에서 형식 정규식(`^[A-Z][A-Z0-9_]{2,64}$`)으로 재정의해 신규 자유형 코드 저작을 허용한다. 테이블 DROP 없음(저작 경로 존치)·`aml_alerts.ck_aml_alerts_scenario_code`(레거시 값 합집합 보존, V7)는 무변경. additive·멱등(DELETE 0행=no-op, CHECK 는 DROP IF EXISTS 후 재생성). | — |
...
> **문서 드리프트 각주(범위 확대 금지)**: 저장소에는 본 표 미기재 `V57__amlc_credentials.sql`·`V58__regulatory_report_amlc_submission_ref.sql`·`V59__hrr_registration_callback.sql`·`V60__eu_uk_au_jp_sanctions_watchlist_sources.sql` 4개 마이그레이션이 존재한다 — 본 작업(V61) 이전에 발생한 기왕 문서 역전파 누락으로 **별건 보완 대상**이며, 본 U8 은 V61 1행만 추가한다(범위 확대 금지).
