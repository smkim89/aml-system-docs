...
- **V71/WLF 대용량 증명은 스키마를 우회하지 않는다**: one-shot privileged init 뒤 ordinary non-super app에서 active-version join 행수와 relation/index bytes만 read-only SQL로 측정하고, WLF-C21 값 검증은 서명된 admin REST browse/`entryIds`의 ordered `functions`/`refs`를 원본 JSONL `externalRef`와 결속한다. 진단은 checksum-bound production parser의 비영속 2회 결과가 소유하며 replay 단계의 임의 `diagnosticCountSame=true`를 금지한다. 이 증명은 `aml_watchlist_entries.attributes`나 V64/V71 인덱스·RLS 정책을 변경하지 않는다.
