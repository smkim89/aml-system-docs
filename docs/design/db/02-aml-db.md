...
PK: `(tenant_id, list_type, subject_ref)`. FK `(tenant_id)`→`aml_high_risk_registry` ON DELETE CASCADE. 인덱스 `ix_hrr_items_subject (tenant_id, subject_ref)`(RA 강제 상향 매칭 조회). tier→RA 강제 floor: `VERY_HIGH`→PROHIBITED, `HIGH`→HIGH(상향만 보장, 가정 A6). **(2026-09-10, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-followups)** 온보딩 RA 파생(`OnboardingRaDerivationService`)은 이 인덱스로 `EXISTS(tenant_id, subject_ref)` 부재판정(fast path, `HighRiskRegistryStorePort#isKnownUnregistered`)을 먼저 수행해 미등재 회원은 헤더+아이템 전량 로드(`find`)를 생략하고, 등재가 입증되지 않은 경우에만 전량 로드로 폴백한다(마이그레이션 신규 없음, 이 인덱스는 F-115 이전부터 실재). 부재판정은 비잠금 읽기이며 §3.19 `findForUpdate` 잠금 순서 불변식에 간선을 추가하지 않는다.
