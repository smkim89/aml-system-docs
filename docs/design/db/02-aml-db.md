...
| 버전 | 파일 | 내용 | 의존 |
|---|---|---|---|
| V71 | `V71__wlf_candidate_runtime_rls_recall.sql` | **WLF runtime RLS-safe 후보조회**: mutation 전 superuser를 요구하는 일회성 privileged init-job migration. 전용 NOLOGIN/non-super/BYPASSRLS owner와 최소 ACL을 만들고, tenant/elevated guard·22023 입력 검증·고정 S1~S4/grouped/ranked/capped를 가진 3열 SECURITY DEFINER 함수를 생성한다. 잘못된 선존 role/membership과 non-privileged 적용은 부분 변경 없이 실패한다. 신규 RLS policy·planner GUC·timeout/cap/floor/시드 0. | V70 |
