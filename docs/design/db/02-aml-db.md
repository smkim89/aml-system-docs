...
| 금액 | 다통화(AUD/JPY/KRW/PHP/USD…) 수용. `NUMERIC(24,8)`은 외화 소수 수용용, `*_amount_minor BIGINT`(통화 최소단위) 병행 컬럼 제공. TM/CTR 금액 임계는 **`baseEquivalent`(테넌트 기준통화 액면)** 정본(§5.6a) — 법인별 자국 통화로 해석되며 FX 환산은 없다. `phpEquivalent` 는 `base_currency='PHP'` 테넌트 전용 별칭(하위호환) | 스킬 §2 |
...
- **WLF 후보조회 bounded BYPASSRLS 예외(V71)**: pg_trgm `%>`·표현식 array-overlap은 leakproof가 아니므로 runtime RLS security barrier 아래에서 V64 GIN 대신 163만 행 tenant slice를 훑었다. 공개 범용 우회가 아니라 `aml.wlf_recall_candidates_runtime(varchar,jsonb,text,jsonb,text,boolean,jsonb,integer)` 한 함수만 `SECURITY DEFINER`로 두고, owner는 `NOLOGIN NOSUPERUSER BYPASSRLS` 전용 role `aegis_wlf_recall_owner`다. 함수는 입력 검증(22023) 뒤 세션 tenant 일치 또는 elevated만 허용(42501), 고정 S1~S4 CTE로 entry id/strategy/axis 3열만 반환한다. role은 `aml` USAGE + watchlist entries/sources SELECT + lower-array helper EXECUTE 외 권한·membership·LOGIN이 없고 recall 함수 ownership만 가진다. V71은 선존 role의 attribute·membership·current database owner/datacl·AML object ownership·direct/default ACL을 mutation 전에 거부한다. 성공 후 PUBLIC EXECUTE=false, runtime EXECUTE=true, exact owner/SECURITY DEFINER, proconfig=`search_path=pg_catalog, aml`+`row_security=on`, 직접 테이블 RLS 2정책과 최소 ACL을 재검증한다.
- **V71/WLF 대용량 증명은 스키마를 우회하지 않는다**: one-shot privileged init 뒤 ordinary non-super app에서 active-version join 행수와 relation/index bytes만 read-only SQL로 측정하고, WLF-C21 값 검증은 서명된 admin REST browse/`entryIds`의 ordered `functions`/`refs`를 원본 JSONL `externalRef`와 결속한다. Import/replay 시점 screen은 서로 다른 key의 동일 입력 snapshot이고, 진단은 checksum-bound production parser 비영속 2회 결과와 독립 raw-wide 전수계수를 함께 대조한다. replay 단계의 임의 `diagnosticCountSame=true`를 금지하며, C08 sync-delist/F-012와 C19 sanctions recall/F-063은 기존 잠긴 DB 통합 경로가 소유한다. 이 증명은 `aml_watchlist_entries.attributes`나 V64/V71 인덱스·RLS 정책을 변경하지 않는다.
...
| `base_currency` | VARCHAR(3) | Y | NULL | | **테넌트 기준통화 ISO 4217**(P0-16, V53). CTR/금액 TM 임계가 이 통화(native) 액면으로 해석된다 — 법인별 자국 통화(AUD/JPY/KRW/PHP…)가 그대로 금액 기준이며 **FX 환산은 없다**(인입이 `amounts.baseCurrency` 일치를 강제해 테넌트 윈도우가 단일 통화이므로 합계·평균이 그대로 성립). payload 에는 전 통화 테넌트에 통화중립 `baseEquivalent`+`baseCurrency` 가 실리고, `phpEquivalent` 는 `base_currency='PHP'` 일 때만 추가되는 하위호환 별칭이다. CTR 임계는 `aml_ctr_thresholds`(§3.x) 의 **동일 통화 행**으로만 조회된다(타 통화 임계 미적용). tenant_demo=`PHP`. NULL=미바인딩 → fail-closed |
...
| `report_cutoff_time` | TIME | Y | NULL | | **CTR/STR 법정 보고 마감시각**(관할, V69). 같은 행 `timezone`으로 해석한다. 현재 확정된 deadline 정책은 PH AMLC뿐이며 `PH`+`Asia/Manila`에서 NULL(레거시) 또는 정확히 `17:00`만 +5영업일 기한 계산에 사용한다. 다른 PH 값과 비PH 관할의 NULL/값은 임의 폴백하지 않고 보고 DRAFT의 `due_at=NULL` + 구조화 WARN으로 수렴한다. 영업일 달력 정본=`JurisdictionCalendarPolicy`, 법정 기한 허용 정본=`ReportDeadlinePolicy` |
...
| `kyc_evidence` | JSONB | N | '{}' | | CDD current-state PII-safe projection. KYC 코드(`occupation`,`sourceOfFunds`,`declaredIncomeOperator`,`declaredIncomeAmount`,`declaredIncomeCurrency`,`declaredIncomePeriod`,`declaredIncomeBand`,`kycLevel`,`residenceCountry`,`kycVerifiedAt`)와 ISO `nationality`, 소스 등록시각 `registeredAt`을 보존한다. 숫자 amount 키가 원문에 존재하면 내부 marker `__declaredIncomeNumericPresent="true"`도 보존해 malformed/null/blank가 레거시 band로 되살아나지 않게 한다(marker는 외부 KYC DTO/정본 입력 키가 아님). 숫자 키가 아예 없을 때만 finite band를 사용한다. 소득 형식이 모두 없고 다른 지원 KYC 차원이 남으면 projection에만 내부 sentinel `UNKNOWN`을 저장한다. raw 이름/DOB/문서번호는 금지하며 안전한 masked birth-year 정본이 없으면 profile `birthYearMasked=null` |
...
| `is_pep` | BOOLEAN | N | FALSE | | **PEP 여부**(V24). `PEP_APPROVAL EXECUTED` pair-only write 소유 컬럼으로 승인 확정 시 TRUE. CDD/re-CDD current-state upsert는 이 컬럼을 쓰지 않는다. PEP 등재 시 당연고위험 레지스트리(`PEP_INDIVIDUALS`) + RA 위험등급 HIGH 강제 상향(거래 허용+EDD). 기존 행 비-PEP |
| `pep_approval_id` | UUID | Y | NULL | | **PEP 경영진 승인 결재 링크**(V24). `PEP_APPROVAL EXECUTED` pair-only write 소유 컬럼이며 결재 row의 `approval_id`를 증거로 저장한다. CDD/re-CDD current-state upsert는 이 컬럼을 쓰지 않는다. 승인 EXECUTED 시 당연고위험 레지스트리 등재 + RA 강제 상향 폐루프(§5.16 PEP_APPROVAL). 비-PEP은 NULL |
...
> **CDD 소득구간 projection 경계(2026-08-14, 코드=truth).** `UNKNOWN`은 "이번 CDD에서 소득구간을 제공하지 않음"을 나타내는 `aml_customers.kyc_evidence` 현재상태 전용 값이며 canonical 입력 enum을 확장하지 않는다. blank 제거 뒤 `occupation`·`sourceOfFunds`·`kycLevel`·`residenceCountry`·`kycVerifiedAt` 중 하나 이상이 남을 때만 생성한다. `kyc` 부재/null/빈 객체, 지원 차원 0개, whitelist 밖 키만 있는 객체에는 sentinel-only JSONB를 합성하지 않는다. re-CDD의 non-empty KYC snapshot은 full replace이므로 band를 생략한 최신 이벤트가 과거 finite band를 `UNKNOWN`으로 교체한다. 각 `aml_canonical_events.payload`/`payload_hash`는 불변이고 exact replay는 원장·이력·RA를 다시 쓰지 않으며, 과거 행 backfill과 Flyway/DDL 변경은 없다.

> **CDD projection ↔ PEP 승인 컬럼 소유권(2026-08-14, 코드=truth).** CDD/re-CDD는 `(tenant_id, customer_ref)` 단일 원자 upsert로 등록 projection(`customer_type`·hash·country·KYC/risk current-state·source/data scope·next review)만 쓰며 `is_pep`·`pep_approval_id`·`onboarding_at`·`created_at`을 INSERT conflict update-set에 포함하지 않는다. 반대로 PEP 승인은 tenant-scoped pair-only UPDATE로 `is_pep=TRUE`·`pep_approval_id`만 쓰고 CDD current-state를 수정하지 않는다. 두 write가 stale read와 경합해도 pair UPDATE 직후 current row를 refresh한 뒤 같은 승인 트랜잭션의 registry RA 재평가가 그 row를 읽으므로, 최신 CDD snapshot과 승인 pair가 함께 보존된다. 스키마·Flyway 변경은 없다.
...
> **소스 행 생성 경로**: `aml_watchlist_sources` 행은 마이그레이션(`tenant_demo` 전용 — V8 공개 제재 2종·V60 공개 제재 4종·V62 dilisense) 또는 시뮬레이터 `demo_ingest.ensure_watchlists()` 의 REST 등록(`POST /api/v1/admin/aml/watchlist-sources`)으로만 생성된다. `tenant_demo` 가 아닌 스택(다통화 프로파일 등)은 위 마이그레이션 시드가 없으므로, `ensure_watchlists()` 가 공개 제재명단 소스 6종(필수 `OFAC_SDN`·`UN_CONSOLIDATED` + 비필수 `EU_CFSL`·`UK_OFSI`·`AU_DFAT`·`JP_MOF_FEFTA`, V60 이 `tenant_demo` 에 시드하는 것과 동일 소스 집합) 중 미등록분을 sync 전에 먼저 등록한다 — 필수 소스 등록 실패는 셋업 실패로 처리하고, 비필수 소스 등록 실패는 §3.6a 필수/비필수 등급 계약대로 흡수(경고만, 셋업 성공)한다. 통화 팩은 이 테이블을 시드하지 않는다(REST-only 원칙 유지).
...
| `created_at/updated_at` | TIMESTAMPTZ | N | now() | | |
| `paused_at` | timestamptz | Y | NULL | | **(V76)** 운영자 일시정지 시각 — 값이 있으면 `claimDueTargets` 가 이 잡의 타깃을 `NOT EXISTS` 로 제외한다(status·`aggregate`·`recomputeJobStatus`·`findOpenJobs` 무변경, aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit) |
| `paused_by` | varchar(128) | Y | NULL | | **(V76)** 일시정지 actor(`POST /api/v1/admin/aml/watchlist-rescreen-jobs/{jobId}:pause`) |
...
Runtime 후보회수는 V71 함수 경계를 사용한다. Java adapter는 RLS-enabled(default)에서 세션 GUC를 쓰지 않고 함수 guard에 맡긴다. 명시적 `aegis.aml.rls.enabled=false` test/tool profile에서만 기존 adapter 통합테스트 호환을 위해 동일 transaction에 tenant와 `elevated=off`를 local bootstrap한다. candidate cap/floor/10초 timeout, V64 GIN, 명단축별 ranking, score·판정·응답은 변경하지 않는다.
...
| `callback_url` | VARCHAR(2048) | Y | NULL | | **요청별 아웃바운드 웹훅 목적지 오버라이드**(V74, PLAN 20260903-aml-decision-status-webhooks — 사용자 지시로 F-084 부분 해제). `POST /aml/screen` 요청 `callbackUrl`(API §3.2)을 저장 — 있으면 `AmlScreeningResolved`/`AmlScreeningWhitelistChanged`(API §8.1) 목적지로 우선 사용, 없으면 테넌트 `aml_api_credentials.webhook_url` 폴백. V59 CDD `callback_url` 3컬럼과 동일 길이(2048) |
| `created_at/created_by/updated_at/updated_by/trace_id` | (공통) | | | | |
...
>
> **ONGOING `parameters.signalScaling` 선택키(2026-08-13, 코드=truth — Flyway 신규 0건).** `KR_ONGOING_RA` 의 `parameters` JSONB 에 최상위 **선택키** `signalScaling` 이 추가됐다: `{ "<신호 origin>": { "rules": ["STR_PEP", …], "curve": "LINEAR"|"STEP", "floorScore": 0..1, "ceilingScore": 0..1, "minMultiplier": (0,1], "maxMultiplier": (0,1] } }`. origin 키는 알럿 evidence 의 `watchlistMatch.origin` 값(현 운영값 `PEP_NAME_RISK_SIGNAL`)이며 패턴은 `[A-Z][A-Z0-9_]{0,63}`. 2차 상시 RA 기여도가 `weightFor(ruleCode) × recencyMultiplier × m(nameScore)` 로 계산되고 배수 상한이 `1.0` 이라 **가중이 늘지 않는다**(약한 이름 신호만 감산 — 설계서 §11.3a). 불변식 위반(`floorScore ≥ ceilingScore`·`minMultiplier > maxMultiplier`·범위 밖·빈/중복 `rules`·룰코드 패턴 위반(`(STR|CTR|FDS)_.+`)·**`STR_SANCTION` 포함**·미지 하위키)은 읽기(`OngoingRaParameters.from`)와 쓰기(`RiskModelDefinitionValidator` — 허용키 7 → 8) 두 경로에서 fail-fast 로 거부한다. **DDL·컬럼·인덱스·제약 전부 무변경이며 `services/aml-svc/src/main/resources/db/migration` 에 신규 파일이 없다**(실측) — 기존 JSONB 안의 키 추가이고, 키 부재 = 미적용(배수 항등원 1.0)이라 V12 시드를 포함한 기존 ACTIVE 정의는 **파싱·동작 모두 바이트 동일**하다. 데모/검증 스택의 활성화는 DB 시드가 아니라 시뮬레이터 셋업 스테이지의 **REST 4-eyes(`versions:copy` → draft 저장 → `simulate` → `:activate` → `:approve`, maker≠checker)** 로만 이뤄진다(REST-only 원칙). 마이그레이션이 필요 없다는 사실 자체를 남기지 않으면 이후 "ONGOING 파라미터 스키마가 늘었는데 V-파일이 없다" 를 누락으로 오독할 수 있어 여기 註로 고정한다.
>
...
>
> **당연고위험(HRR) 레지스트리 floor 를 온보딩 파생에 병합 — 재CDD/재파생 floor 유지(코드=truth, 마이그레이션 신규 없음, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd).** 위 국가위험 floor(V20)·명단 매치 floor(SCREENING) 파생 직후, 엔진(`OnboardingRaDerivationService`)은 **당연고위험 레지스트리**(§5.33 `aml_high_risk_registry_items`)에 대상 `target_ref` 가 등재돼 있는지(`HighRiskRegistry.classify`) 조회해 등재 floor(tier `HIGH→HIGH`·`VERY_HIGH→PROHIBITED`)를 **병합**한다 — `mandatory_high_risk_reasons` 에 매치된 참조 리스트 이름(`RA_HIGH_RISK_CUSTOMERS`/`PEP_INDIVIDUALS`/`HIGH_NET_WORTH`/`PRODUCT`/`VASP`, §5.33)을 추가하고 `factor_breakdown.forcedFloor.evidence` 에 `{listType, subjectRef, tier}` 원소를 추가한다(명단 매치 evidence 와 동일 포맷). **이 병합은 최초 CDD 뿐 아니라 등재 이후의 모든 온보딩 재파생(재이행 re-CDD·`forcedReassessment` 국가정책 재평가)에서 매회 수행된다** — 종전에는 이 조회가 없어 명단(WLF) 매치가 소멸한 등재 회원의 재CDD 가 `mandatory_high_risk=false`(LOW/APPROVE)로 착지하는 결함이 있었다. **(2026-09-11 정정, aegis-aml PLAN 20260911-aml-hrr-floor-consolidation)** 조회는 포트 단일 메서드 `HighRiskRegistryStorePort.findForcedFloor(tenantId, subjectRef)` 가 `findByTenantIdAndSubjectRef`(인덱스 `ix_hrr_items_subject`, 비잠금·readOnly) 로 subject 의 등재 아이템만 읽어 도메인 정적 `HighRiskRegistry.forcedFloorFrom` 으로 조립한다(구 부재판정 exists→전량 `find()` 2단 경로는 폐기) — `findForUpdate` 잠금 순서 불변식(§3.19)에 새 간선을 추가하지 않는 것은 동일. 등재 해제(EXECUTED, 위 §3.9 등재 해제 후주)로 항목이 제거되면 아이템이 없어 `forcedFloorFrom` 이 empty 를 반환해 그 이후 재파생부터 병합이 빠진다(해소는 등재 해제 결재뿐). **스키마·마이그레이션 신규 없음** — 기존 V13 컬럼·`factor_breakdown` JSONB 를 그대로 재사용한다. 엔진 케이스 RA-C30(`docs/qa/engine-rule-cases.md`) 검증. 정책 메타만(PII 없음).
...
>
> **비-`tenant_demo` 테넌트(통화 프로파일 팩) RA 모델 baseline v1 시드(코드=truth).** `tenant_demo` 가 아닌 프로파일(AUD·KRW·JPY 통화 팩, 예: `aud`→`tenant_au`)은 REST 최초 생성 경로(`versions:copy`)가 원본 버전을 전제해 RA 모델을 만들 수 없으므로, 각 팩의 `R__currency_reference_configuration.sql` 이 `aml_risk_models` 에 **`KR_DEFAULT_RA` v1**(`scenario='ONBOARDING'`, `status='APPROVED'`, `is_default=true`)·**`KR_ONGOING_RA` v1**(`scenario='ONGOING'`, `status='APPROVED'`)를 `ON CONFLICT (tenant_id, model_code, version) DO NOTHING` 으로 멱등 bootstrap 한다(팩 예외 경계, CLAUDE.md §데모 시드 절대 금지). 값 정본은 `config/ra-models/baseline.json`(통화·관할 중립)이며 `tenant_demo` 의 V2/V11/V12/V19/V20/V34/V35 최종 상태(`weights` 10팩터 레거시 형상 + `parameters` CDD 파생·국가 floor·SANCTION/PEP 점수 + ONGOING 룰 심각도 가중 9종+FDS 2종)를 그대로 재현한다 — php(`tenant_demo`) 팩은 무변경. 시드된 v1 이후의 버전 개정·재승인은 여느 테넌트와 동일하게 REST 4-eyes(`versions:copy → simulate → activate`)만 소유한다.
...
| `scenario_code` | VARCHAR(80) | Y | NULL | 식별자 CHECK | **TM_SCENARIO 알림의 발동 룰 코드**. built-in `AmlReportRuleCode` 10종 또는 V33 사용자 정의 안정 코드(`[A-Z][A-Z0-9_]{2,79}`)를 저장한다. 레거시 시나리오 10종 기존 행은 보존. **`ck_aml_alerts_scenario_code`(V7 합집합) 는 V61(§3.10a — 레거시 `aml_tm_scenarios` 정의 10종 제거) 이후에도 무변경**이다 — CHECK 는 정의 테이블이 아니라 알림 컬럼 자체에 걸려 있고 `scenario_code` 에 FK 가 없어 정의 DELETE 와 무관하며, 레거시 알럿 보유 환경에서 CHECK 축소는 기동 실패 위험이라 V61 은 명시적으로 CHECK 를 손대지 않는다. 부분 UNIQUE `ux_alert_tm(tenant_id, transaction_ref, scenario_code)` 로 (transactionRef, ruleCode) 멱등. API `ruleCode`(§3.4a) 정본 매핑 |
...
| `retired_at` | TIMESTAMPTZ | Y | NULL | CHECK | **설정형 룰 회수(retire) 시각(V63)**. 발동한 설정형 STR/CTR 룰이 `:retire` 4-eyes(API §2.4)로 회수될 때 기록. `ck_aml_alerts_retired_at` 이 **`status='RETIRED'` ⟺ `retired_at IS NOT NULL`** 을 양방향 강제한다. `disposition_*`(V30, 오탐 종결 전용)과 **별개 컬럼** — 오탐 종결은 분석가의 업무 판단이고 회수는 룰 자체의 무효화라 의미가 다르며, 회수해도 그때까지의 처분 필드는 보존된다. API `retiredAt`(§3.4a) 정본 매핑 |
| `retired_by` | VARCHAR(128) | Y | NULL | | **회수 실행자(V63)**. 회수 승인을 실행한 checker. `retired_at` 과 동일 불변식. raw PII 아님(운영 행위자 참조). API `retiredBy`(§3.4a) |
| `retire_reason` | VARCHAR(256) | Y | NULL | | **회수 사유(V63)**. 상신 body `reason` 이며 미지정 시 엔진이 `RULE_RETIRED:{ruleCode}` 로 채운다. CHECK 미부과(비-PII 자유 문자열). API `retireReason`(§3.4a) |
| `created_at/created_by/updated_at/updated_by/trace_id/data_scope` | (공통) | | | | |

PK: `(tenant_id, alert_id)`

> **발동 관련거래 창 동결(2026-08-17, JSONB additive).** 법정 CTR/STR 알럿은
> `evidence.evidenceWindow{dimension,windowStart?,asOf,startInclusive,cashOnly,label,timezone?}`를 발동
> 시점에 저장한다. related-transactions 조회는 이 snapshot을 현재 tenant binding보다 우선한다.
> `CTR_DAILY`·`STR_STRUCTURED`는 시작 포함, rolling 24h는 시작 제외이며 상한 `asOf`는 모두 포함한다.
> snapshot 없는 기존 알럿만 현재 정책으로 재해석한다. 새 컬럼·Flyway·소급 백필은 없다.

> **회수(RETIRED) 알림은 행을 지우지 않는다(V63).** 설정형 룰이 회수되면 그 룰이 만든 TM 알림을 운영 목록·룰 효과성 통계에서 제외하되 **행과 evidence 는 감사 목적으로 보존**한다(DELETE 없음). 조회 경로는 기본 제외이고 감사 조회만 `includeRetired=true` 로 명시한다(API §2.4). **케이스가 개설된 알림(CASE_OPENED/ESCALATED/STR_RECOMMENDED)은 회수 대상에서 제외**되어 status 가 그대로 유지된다 — `aml_cases.origin_alert_id` 계보와 STR draft 가 끊기면 규제 산출물이 사라지기 때문이다. 회수 대상 조회·기본 제외 목록용 인덱스 `ix_aml_alerts_scenario_code(tenant_id, scenario_code, status)`(V63). 데모 데이터 정리는 마이그레이션이 아니라 REST(`:retire`)로만 수행한다(REST-only 원칙). **`ix_alert_txn_ref (tenant_id, transaction_ref) WHERE transaction_ref IS NOT NULL`(V70)** 은 TM "최근 거래" 브라우즈(API §2.4 `GET /api/v1/aml/transactions`)의 `alertCount`/`firedRuleCodes`(RETIRED 제외) 상관 서브쿼리를 지지한다 — `ux_alert_tm`(V1, `(tenant_id, transaction_ref, scenario_code)` 부분 유니크)은 `alert_type='TM_SCENARIO' AND scenario_code IS NOT NULL` 한정이라 전 `alert_type` 매칭을 지지하지 못해 별도 신설했다.

### 3.10a `aml_tm_scenarios` — TM 시나리오 정의(룰 DSL) (설계서 §12.1, 구현 V5, **V61 자유형 전환**)

레거시 거래모니터링 시나리오 저작 store다. **v9.21 이후 실 알림 발동 정본은 CTR/STR 룰 카탈로그(`AmlReportRuleCatalog`)뿐**이며, 본 테이블은 거래 인입 경로에서 전혀 발동하지 않는 저작/백테스트 전용 표면이다(F-025 실측). **V61(2026-08-01)이 정의 10종(레거시 닫힌 enum `TmScenario`, 13행 — `STRUCTURING` v1/v2·`HIGH_RISK_CORRIDOR` v1/v2/v3 포함)을 전량 DELETE 하고 `scenario_code` CHECK를 닫힌 목록에서 형식 정규식으로 재정의**했다 — 신선 배포·V61 적용 후 본 테이블의 정상 상태는 **0행**이며, 저작 경로(`TmScenarioAdminController` REST `draft`/`simulate`/`:activate`)로 자유형 코드를 신규 등록할 수 있다(테이블·저작 UI 존치, 목록이 비는 것이 정상). 신규 사용자 정의 실평가 룰은 §3.10b가 정본이며 `TM_SCENARIO` 승인선을 공유한다.

| 컬럼 | 타입 | NULL | 기본값 | 제약 | 설명 |
|---|---|---|---|---|---|
| `tenant_id` | VARCHAR(64) | N | — | PK,FK→aml_tenants | |
| `scenario_code` | VARCHAR(80) | N | — | PK,CHECK 형식 정규식 | §5.6 tm_scenario(**V61 이후 자유형** — 닫힌 enum 아님). CHECK `^[A-Z][A-Z0-9_]{2,64}$`(대문자·숫자·언더스코어, 2~64자 몸통) — 예약 코드 `CUSTOM_RULE`(설정형 STR/CTR 룰 결재 subjectRef prefix 와 충돌 방지)는 유스케이스(`TmScenarioService.draft`) 단계에서 거부(DB CHECK 는 형식만 강제) |
...
| `dsl` | JSONB | N | '{}' | | 시나리오 룰 그래프(`cmp`/`velocity`/`and`/`or` 노드). 금액 leaf는 **`transaction.baseEquivalent`**(테넌트 기준통화 액면, 다통화 정본 §5.6a; `transaction.phpEquivalent` 는 PHP 테넌트 전용 별칭)·속도는 `velocity.<count\|sum>.subject.<window>` |
...
> **레거시 6종 ACTIVE 시드 — V61(2026-08-01)로 제거(과거 사실, 역사적 기록).** V19/V22/V26/V28 이 `tenant_demo` 한정 ACTIVE 로 시드했던 6종(**STRUCTURING**·**HIGH_RISK_CORRIDOR**·**RAPID_MOVEMENT**·**MULE_NETWORK**·**REFUND_LAUNDERING**·**ROUND_TRIPPING**, 나머지 4종 SHELL_MERCHANT·TRADE_MISPRICING·CRYPTO_OFF_RAMP·INTERNAL_OVERRIDE_ABUSE 는 advanced-domain 비-hanpass 잔존값)은 **거래 인입에서 전혀 발동하지 않는 것이 v9.21 이후 확정 사실**(TM 알림 발동 정본 = CTR/STR 룰 카탈로그)이라 관리 혼동을 없애기 위해 V61 이 정의 10종(13행) 전량을 DELETE 했다. 위 마이그레이션 번호는 과거 시드 이력이며 수정하지 않는다(V1/V2/V7 등 준용) — **현재 실효 상태는 §3.10a 상단 서술(0행·자유형 저작)이 정본**이다.
...
| `origin_screening_id` | VARCHAR(96) | Y | NULL | | **(V15)** RA→EDD 착수 발단 screening/RA 스코어 id(추적성, FK 아님 — 문자열 참조 토큰, `origin_fds_case_ref` 와 동형). `origin_alert_id`(TM 알림 발단)와 **별개 슬롯** — 이 컬럼 부재로 위임(엔진) 경로에서 케이스 상세 '발단'이 `null` 로 유실되던 결함(D5·D8) 해소. 코드=truth: `CaseJpaEntity.originScreeningId`·`Case.originScreeningId` 와 1:1. **(V72) A6 참조 토큰 접두 규약**: 값은 `risk-score:<uuid>`(RA 스코어 발단) 또는 `screening:<uuid>`(WLF 스크리닝 발단) 형식으로 저장한다 — 컬럼 신설 없이 기존 슬롯에 접두 토큰을 얹는 방식(PLAN 20260902-aml-case-workbench A1). **접두 없는 legacy 값은 `screening` 으로 하위호환 해석**한다(수동 생성 경로가 남긴 원문 screening/스코어 id). 파싱은 `CaseEvidenceRef.parseOrigin(String)`(도메인 순수 VO, `com.aegis.aml.domain.CaseEvidenceRef`)이 담당하며 `evidenceRefs` 목록과 같은 규약을 공유한다(kind 카탈로그: `alert`/`txn`/`screening`/`risk-score`/`report`/`approval`/`manifest`, 접두 없는 evidenceRefs 값은 `manifest` 로 해석 — origin 문맥과 기본값이 다름에 주의). |
...
### 3.11a `aml_case_checklist_items` — 케이스별 체크리스트 실행 상태 (PRD §12-A.4, V72)

케이스 조사관 작업대(PLAN 20260902-aml-case-workbench)의 CDD/EDD 체크리스트 실행 저장소. 템플릿(`aml_cdd_checklists`, 정적 정의)과 달리 이 테이블은 **케이스별 실행 상태**(항목 상태·메모·증빙·행위자)를 보관한다.

| 컬럼 | 타입 | NULL | 기본값 | 제약 | 설명 |
|---|---|---|---|---|---|
| `tenant_id` | VARCHAR(64) | N | — | PK, FK(→`aml_cases`) | |
| `case_id` | UUID | N | — | PK, FK(→`aml_cases`) | |
| `item_key` | VARCHAR(128) | N | — | PK | 템플릿 항목 키(`aml_cdd_checklists` 정의와 매칭) |
| `status` | VARCHAR(32) | N | `'PENDING'` | CHECK(PENDING/DONE/NOT_APPLICABLE) | 도메인 `ChecklistItemStatus` 1:1 |
| `note` | VARCHAR(2000) | Y | NULL | | 운영자 메모 |
| `evidence_ref` | VARCHAR(512) | Y | NULL | | A6 참조 토큰(`alert:`/`txn:`/`screening:`/`risk-score:`/`report:`/`approval:`/`manifest:`) |
| `actor` | VARCHAR(128) | N | `'system'` | | 최종 갱신 행위자 |
| `updated_at` / `created_at` | TIMESTAMPTZ | N | `now()` | | |

PK: `(tenant_id, case_id, item_key)`. FK `fk_aml_case_checklist_case (tenant_id, case_id)` → `aml_cases ON DELETE`(명시 없음, 기본 RESTRICT). RLS: V72 자기 마이그레이션이 V47 동형 정책 2종(`aml_rls_tenant`(runtime, `tenant_id` 일치 OR `app.elevated='on'`)·`aml_rls_owner`(owner 전량)) 을 신설과 함께 명시 ENABLE+FORCE(`RlsCoverageGuardIntegrationTest` 통과). 갱신은 `PUT /api/v1/admin/aml/cdd/cases/{caseId}/checklist/{itemKey}`(API §2.7, `aml:case:update`)로만 upsert되며, 케이스가 `OPEN`/`INVESTIGATING` 이 아니면(`Case.assertMutableForInvestigation()`) 409로 거부한다. 저장마다 `aml_cases`(§3.11) 타임라인에 `CHECKLIST_ITEM` 행이 append된다. **데이터 seed 없음 — REST-only.**
...
| `report_amount` | NUMERIC(20,2) | Y | NULL | | **CTR 합산 금액** — freeze 된 서버 파생 테넌트 기준통화 합계(엔진 재계산 금지, BR-501). 동일 관할 영업일 후속 현금거래 시 누적(CTR_DAILY 보완재, `accumulateCtr`)(V4). |
| `due_at` | TIMESTAMPTZ | Y | NULL | | **법정 보고 기한(CTR/STR 공용, nullable freeze)** — 현재 구현이 계산하는 확정 정책은 PH AMLC뿐이다. PH(`Asia/Manila`, cutoff NULL 레거시 또는 `17:00`)는 CTR 거래 시각/STR 의심 트리거 시각 +5영업일 17:00 PHT를 DRAFT 개설 때 동결한다. 다른 PH cutoff와 KR/AU/JP 등 비PH 관할은 탐지·DRAFT를 보존하되 `NULL`로 두고 `AML.REPORT_DEADLINE_POLICY_UNCONFIGURED` WARN을 남긴다. KR의 STR 승인+3영업일/CTR 거래+30일은 정확한 cutoff·anchor/lifecycle 결선 전에는 추정하지 않는다. |
...
> **규제 제출 durable boundary 관련 필드는 이 테이블에 추가하지 않는다(P0-11, 코드=truth) — 단 AMLC 포털 lodgement 접수번호는 예외(V58).** provider 접수 hash·provider message id·form schema 스냅샷은 `aml_regulatory_reports` 컬럼이 아니라 **전용 제출 job 테이블 `aml_report_submission_jobs`(§3.12a, V54)** 에 고정된다 — report 테이블의 나머지 컬럼·CHECK·인덱스는 **무변경**이다. `amlc_submission_ref` 만은 예외적으로 `aml_regulatory_reports`에도 직접 컬럼으로 존재한다(V58, feature/aml-reports-amlc-migration §1.4-C) — `RegulatoryReportService`가 이중 lodge 방지 판단(멱등 가드)에 report 자신의 상태만으로 즉시 접근해야 하기 때문이며, `aml_report_submission_jobs.amlc_submission_ref`(제출 job 회차 provenance 에코)와 값은 동일하되 저장 목적·조회 경로가 분리된 별도 컬럼이다. report 는 `submitted_ref`·`fiu_ack_ref`·`submission_error_code`·`evidence_hash`·`amlc_submission_ref`(V58)로 제출 계보를 유지하고, provider transmission 회차의 durable 재시도·receipt 대사는 제출 job 이 담당한다.
...
| `amlc_submission_ref` | VARCHAR(256) | Y | NULL | | eAMLA(AMLC) 포털 lodgement 접수번호 — `AmlcSubmissionPort` 실 구현체(`PlaywrightAmlcSubmissionAdapter`, 브라우저 자동화로 AMLC 포털에 직접 lodge, feature/aml-reports-amlc-migration §1.4-C)가 반환한 `AmlcReceipt.amlcSubmissionRef` 를 이 job 회차의 provenance 로 에코 기록(`aml_regulatory_reports.amlc_submission_ref`(§3.12, V58)와 값은 동일하되 별도 컬럼) |
...
### 3.12b `aml_amlc_credentials` — 테넌트별 AMLC 포털 로그인 계정 (feature/aml-reports-amlc-migration, V57)

| 컬럼 | 타입 | NULL | 기본값 | 제약 | 설명 |
|---|---|---|---|---|---|
| `tenant_id` | VARCHAR(64) | N | — | PK | 멀티테넌시 격리키. 테넌트당 계정 1쌍(§1.4-A, 회사 전체 단일 계정 아님) |
| `username` | VARCHAR(255) | N | — | | AMLC 포털 로그인 계정명(raw, 평문 저장 — 시크릿이 아닌 식별자) |
| `secret_ciphertext` | VARCHAR(512) | N | — | | 로그인 비밀번호 암호문 — `SecretCipherPort`(`AesGcmSecretCipher`, AES-256-GCM, `fds_api_credentials` 동형 재사용) 암호화. raw 평문 미저장(§19.2) |
| `enabled` | BOOLEAN | N | true | | 계정 활성 여부 |
| `created_at/updated_at/updated_by` | (공통) | | | | |

PK: `(tenant_id)`. RLS: V47 DO 루프 경과 후 신설 테이블이라 자기 마이그레이션(V57)에서 직접 V47 동형 정책 2종(`aml_rls_tenant`(runtime, tenant 일치 OR `app.elevated='on'`)·`aml_rls_owner`) 명시 ENABLE+FORCE. 저장은 `PUT /api/v1/admin/aml/amlc-credential`(`aml:case:update`, **4-eyes 미적용** — 결정 C, 같은 화면의 보고기관(RI) 헤더 편집과 동급 즉시반영)로만 적재하며, 저장 시 `AMLC_CREDENTIAL_SAVED` 감사 이벤트(평문 비밀번호 미기록)를 남긴다. 조회(`GET`, `aml:case:read`)는 `AmlcCredentialAdminService.MaskedView`(`configured`·`username`·`enabled`·`updatedAt`·`updatedBy`)만 반환하고 시크릿 필드는 어떤 형태로도(마스킹 포함) 포함하지 않는다. bo-api는 `/api/v1/bo/aml/amlc-credential`로 이 API를 위임 프록시(로컬 stub 없음, delegate 불가 시 fail closed)한다.
...
> **writer 직렬화 잠금 단위(2026-09-04).** 이 헤더 행은 결재 EXECUTED 실행 트랜잭션 내 `SELECT … FOR UPDATE`(비관적 행 잠금)로 잠가 참조 리스트 변경·`HRR_REGISTRATION`·`PEP_APPROVAL` 세 writer 간 lost-update 를 차단한다. 잠금 순서는 `aml_approvals` 행 → 본 헤더 행 → `aml_high_risk_registry_items` → `aml_customers`/`aml_risk_scores` 순으로 고정한다. 컨트롤러 GET 은 잠금 없이 조회만 한다(헤더 행 부재 시 최초 seed INSERT 는 예외 — 그 구간의 경합은 별건 advisory lock). 상신(`submitReferenceListChange`)·재평가 읽기도 동일하게 잠금 없이 조회만 한다. 잠금 보유 시간은 결재 실행 트랜잭션의 재평가 배치(등재 주체 수 N)에 비례 — 실측(2026-09-04, 등재 90건·동시 4건) 승인당 3.2~13.0s 로 bo-api 위임 timeout 30s 이내; 격리수준 READ COMMITTED 전제(items 는 헤더 잠금 후 별도 SELECT).
...
PK: `(tenant_id, list_type, subject_ref)`. FK `(tenant_id)`→`aml_high_risk_registry` ON DELETE CASCADE. 인덱스 `ix_hrr_items_subject (tenant_id, subject_ref)`(RA 강제 상향 매칭 조회). tier→RA 강제 floor: `VERY_HIGH`→PROHIBITED, `HIGH`→HIGH(상향만 보장, 가정 A6). **(2026-09-11 정정, 코드=truth, aegis-aml PLAN 20260911-aml-hrr-floor-consolidation, 사용자 지시로 F-115·F-116 해제)** 온보딩 RA 파생(`OnboardingRaDerivationService`)은 이 인덱스를 쓰는 **단일 조회** `HighRiskRegistryReferenceItemJpaRepository.findByTenantIdAndSubjectRef(tenantId, subjectRef)`(비잠금·`@Transactional(readOnly = true)`)로 subject 의 등재 아이템만 읽어 도메인 정적 팩토리 `HighRiskRegistry.forcedFloorFrom` 으로 조립한다 — 헤더+아이템 전량 로드(`find`)는 이 경로에서 더 이상 수행하지 않는다(구 `EXISTS` 부재판정 fast path→전량 `find()` 2단 경로는 폐기, 마이그레이션 신규 없음, 이 인덱스는 F-115 이전부터 실재). 단일 조회는 비잠금 읽기이며 §3.19 `findForUpdate` 잠금 순서 불변식에 간선을 추가하지 않는다.

> **등재 해제(2026-09-04, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095).** `RA_HIGH_RISK_CUSTOMERS` 등재 해제 결재(§5.16 `HRR_REGISTRATION`, `subject_ref=DEREGISTER|customerRef`) EXECUTED 시 해당 `(tenant_id, RA_HIGH_RISK_CUSTOMERS, subject_ref)` 항목을 **DELETE**하고 헤더(`aml_high_risk_registry.version`)를 **+1** 한다(항목 부재여도 version+1 — 참조 리스트 변경의 `replaceReferenceList` 계약과 동형). 해제된 회원은 같은 트랜잭션에서 온보딩 CDD 스냅샷을 재파생해 `aml_risk_scores`(§3.9)의 `mandatory_high_risk`·forcedFloor 마커를 정정한다(`02-aml-sass-functional-spec.md` §12-B.6 BR-009).
...
|---|---|---|---|---|---|
| `tenant_id` | VARCHAR(64) | N | — | PK,FK→aml_tenants | |
| `target_ref` | VARCHAR(128) | N | — | PK | tokenized 대상 식별자(원문 미저장, §19.2). **WLF 요청 이름 스냅샷 네임스페이스** `WLFREQ-{screeningId}`는 persisted `aml_screening_results.screening_id`별 NAME 암호문 1개를 가리키며, 새 insert 승자만 기록해 write-once 의미를 얻는다(스키마 변경 없음). |
...
> **WLF 이름 read 모델(2026-08-22).** 스크리닝 요청 이름은 결과의 persisted `screening_id`에서 파생한 `WLFREQ-{screeningId}` target_ref NAME 한 행으로만 보존하며, 이름·점수·근거를 `aml_screening_results` 또는 evidence JSONB에 중복 저장하지 않는다. 매칭 엔트리 이름은 `matched_entries`의 안정 entry id를 NAME vault로 해소한 **현재 게시본**이다. 제재 feed 재sync는 같은 entry id의 NAME 암호문을 최신 rawName으로 upsert하므로 과거 화면 표기가 바뀔 수 있으나, entry id·rule version·score breakdown은 결과 행에 보존된다. 스키마·Flyway 변경은 없다.
...
CTR/STR 영업일 경계·알럿 evidence window와, **정책이 확정된 관할의** 보고 기한 산정을 위한 테넌트별
영업일/공휴일 override store다(테이블명 `aml_ph_banking_calendar`는 PH 전용이던 시절의 레거시 명칭이며,
행은 `tenant_id`로 관할별 분리 — 통화 프로파일 팩이 그 관할 공휴일을 적재한다). 주말은
`BankingCalendar.isWeekend`에서 판정하고 이 테이블에는 공휴일과 근무 토요일 override만 둔다.
`BankingCalendar.plusBusinessDays`는 PH AMLC +5영업일 계산에도 쓰지만, 다른 관할은
`ReportDeadlinePolicy`가 허용하기 전 `due_at=NULL`이므로 캘린더 존재만으로 기한을 만들지 않는다.
...
### 3.22f `aml_member_cdd_history` — 회원원장 CDD/EDD append-only 이력 (Flyway V26·V27·V78, member-ledger-history)
...
| `history_type` | VARCHAR(32) | N | — | CHECK 7종(V78) | §5.36 `cdd_history_type`(`CDD_INITIAL`/`CDD_REVIEW`/`EDD_OPENED`/`EDD_CLOSED`/`CDD_REISSUE_REQUESTED`/`EDD_REISSUE_REQUESTED`/`MEMBER_DECISION`) — 엔진 `CddHistoryType` 와 1:1 |
...
PK: `(history_id)`. 인덱스 `ix_aml_member_cdd_history_member (tenant_id, member_ref, occurred_at DESC)`(회원별 최신순 페이지·요약 카운트)·`ix_aml_member_cdd_history_member_type (tenant_id, member_ref, history_type)`(유형 필터). 멀티테넌시 키 `(tenant_id, member_ref)` 선두. **append-only**(UPDATE/DELETE 없음, `append` 는 항상 신규 `history_id` INSERT). **raw PII 미적재** — `kyc_status`/`risk_grade` 는 enum 스냅샷, `details` 는 참조 토큰·사유 코드만(§19.2). 표시명·문서번호 미노출(프로필 마스킹 소관). 적재 지점 4종: (a) `customer.cdd.completed` 인입(`AmlEventIngestService`) — projection upsert **전** 원장 존재 여부로 `CDD_INITIAL`(최초)/`CDD_REVIEW`(재이행) 판정, (b) EDD 착수/종료(`CddEddService`) — `EDD_OPENED`/`EDD_CLOSED`(4-eyes 승인 실행), (c) CDD/EDD **즉시 재이행 접수**(`DueDiligenceReissueService`, V27) — `CDD_REISSUE_REQUESTED`/`EDD_REISSUE_REQUESTED`, 멱등 키 `source_event_id = 'reissue-req:' + requestId`(중복 요청 replay 판정, `ix_aml_member_cdd_history_source_event`). 실 재이행 수행은 **계정계 연동 예정**(`AccountSystemReissuePort` no-op 아답터, 코드 토큰 `TODO(계정계-연동)`) — 계정계가 재수행 후 `customer.cdd.completed` 재인입 시 (a) 경로 `CDD_REVIEW` 로 폐루프가 닫힌다. **(d) 회원 심사결과 확정**(`MemberDecisionService.confirm`/`DirectMemberDecisionService`, V78) — `EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT` 3출처가 `aml_member_decisions`(§3.22g) 확정과 같은 트랜잭션 안에 `MEMBER_DECISION` 1행을 append 한다(`kyc_status`/`risk_grade` = 확정 시점 원장 스냅샷 — `risk_grade` 는 결정 자체의 등급이 있으면 우선, 없으면 원장 스냅샷, `details={decision, source, decisionId, approvalId?, caseId?}`, `trace_id` 는 확정 컨텍스트 trace 폴백). **V78 backfill**: 마이그레이션 시점 `aml_member_decisions` 전 행을 같은 규칙으로 `aml_member_cdd_history` 에 INSERT(멱등 — `details->>'decisionId'` 가 이미 있는 회원·결정은 `WHERE NOT EXISTS` 로 제외). backfill 행의 `kyc_status` 는 **현재 원장 스냅샷**(확정 시점 스냅샷이 아님, LEFT JOIN 값 — 원장 미존재 시 NULL)이고 `trace_id` 는 기존 확정 행이 trace 를 저장한 적이 없어 **전부 NULL** 이다. 정책·스냅샷 메타만. **(2026-09-10 신설, aegis-aml PLAN 20260910-aml-member-history-decision-column)** CDD 유형 행(`CDD_INITIAL`/`CDD_REVIEW`)은 조회(read) 시점에 `source_event_id` = `aml_cdd_onboarding_decisions.event_id`(§3.15) 조인으로 그 인입의 온보딩 판정 스냅샷을 병기한다 — 이력 행·판정 스냅샷 모두 무수정(스키마 변경 없음), 회원원장 화면(AML-MBR-001) "AML 심사결과(스냅샷)" 열의 소스.

### 3.22g `aml_member_decisions` — 회원(고객 관계) 심사결과 확정(append-only) (Flyway V74/V77, PLAN 20260903-aml-decision-status-webhooks·PLAN 20260909-aml-ra-direct-member-decision)

`aml_cdd_onboarding_decisions`(§3.15, V42)는 CDD 인입 시점의 **불변 스냅샷**이며 이후 4-eyes 결재(`EDD_CLOSE`·`RELATIONSHIP_REJECT`)로 회원 관계가 확정돼도 갱신되지 않는다(F-076). 본 테이블은 계정계가 "결재를 통해 승인/거절이 완료된 시점"의 상태값을 읽을 수 있도록, 그런 결재가 **EXECUTED**될 때만(결재당 정확히 1행) append 하는 확정 결정 행이다. **(V77, 2026-09-09 추가)** 당연고위험(HRR) 등재가 경영진 결재로 이미 **EXECUTED**된 회원에 한해서는, 결재를 거치지 않는 RA 상세 관리자 **직접 확정**(`source=ADMIN_DIRECT`)도 같은 규칙으로(직접 확정 요청당 정확히 1행) append 한다. API `GET /api/v1/aml/customers/{customerRef}/decision`(설계 API §2.3)과 웹훅 `AmlMemberDecisionResolved`(API §8.1)의 정본. 회원의 **현재 결정**은 본 테이블 최신 행과 `aml_cdd_onboarding_decisions` 최신 스냅샷을 병합해 최신순 정렬한 첫 행이다(API §2.3 `history[0]`, `MemberDecisionService.view`).

| 컬럼 | 타입 | NULL | 기본값 | 제약 | 설명 |
|---|---|---|---|---|---|
| `tenant_id` | VARCHAR(64) | N | — | PK 선두(멀티테넌시) | 테넌트 격리 키 |
| `decision_id` | UUID | N | `gen_random_uuid()` | PK | 확정 행 식별자(append 마다 신규 발급) |
| `member_ref` | VARCHAR(256) | N | — | | 회원 키(= `aml_customers.customer_ref` = 엔진 `targetRef`) |
| `decision` | VARCHAR(32) | N | — | CHECK 4종 | `MemberDecisionKind`(`APPROVED`/`REJECTED`/`EDD_REQUIRED`/`REPORTED`) — DB CHECK 와 1:1 |
| `source` | VARCHAR(32) | N | — | CHECK 3종(V77) | `MemberDecisionSource`(`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`) — 영속 값만(`INGEST`는 read-back 파생 전용 가상값, 저장 안 됨). `ADMIN_DIRECT`(V77, 2026-09-09)는 HRR 등재 EXECUTED 회원에 대한 결재 없는 관리자 직접 확정 |
| `reason` | VARCHAR(256) | Y | NULL | | 결재 사유(`ApprovalRequest.reason`) |
| `approval_id` | UUID | Y | NULL | | 이 확정을 만든 4-eyes 결재 id(`aml_approvals`) — `source=ADMIN_DIRECT`는 결재가 없으므로 항상 `NULL` |
| `case_id` | UUID | Y | NULL | | 근거 케이스(`aml_cases`) — `source=ADMIN_DIRECT`는 항상 `NULL` |
| `case_type` | VARCHAR(64) | Y | NULL | | 결재 실행 시점 케이스 유형(§5.8 `case_type`) — `source=ADMIN_DIRECT`는 항상 `NULL` |
| `final_status` | VARCHAR(32) | Y | NULL | | 결재 실행 시점 케이스 종결 상태(§5.9 `case_status`) — `source=ADMIN_DIRECT`는 항상 `NULL` |
| `score_id` | UUID | Y | NULL | | 결재 실행 시점 operative RA 점수(`aml_risk_scores.score_id`) 스냅샷 |
| `risk_grade` | VARCHAR(16) | Y | NULL | | 위 점수의 위험등급(§5.2) 스냅샷 |
| `required_action` | VARCHAR(32) | Y | NULL | | 위 점수의 후속조치(§5.26) 스냅샷 |
| `checker_id` | VARCHAR(128) | Y | NULL | | 결재 승인자(checker) |
| `decided_at` | TIMESTAMPTZ | N | — | | 결재 실행(EXECUTED) 시각 — 최신순 정렬 키 |
| `created_at` | TIMESTAMPTZ | N | now() | | 적재 시각 |
| `trace_id` | VARCHAR(64) | Y | NULL | | 결재 실행 요청 trace id |

PK: `(tenant_id, decision_id)` · 인덱스 `ix_aml_member_decisions_member (tenant_id, member_ref, decided_at DESC)`(회원별 최신순 조회) · UNIQUE 부분 인덱스 `ux_aml_member_decisions_approval (tenant_id, approval_id) WHERE approval_id IS NOT NULL`(결재당 1행 — replay 멱등, `ApprovalDispatchService` 의 `findByIdForUpdate` 직렬화와 결합해 경합 없음. `source=ADMIN_DIRECT` 행은 `approval_id IS NULL` 이라 이 부분 UNIQUE 범위 밖 — 중복 방지는 게이트 ⓕ(현재 결정과 요청 decision 동일 시 409)가 담당). RLS 2종(`aml_rls_tenant`/`aml_rls_owner`, §1.2·V57·V72 동형) ENABLE+FORCE. **append-only**(UPDATE/DELETE 없음). **raw PII 미적재** — 참조 토큰·enum 스냅샷만.

**append 시점(2경로)**: ① `EDD_CLOSE`/`RELATIONSHIP_REJECT` 확정 — 결정 append·웹훅 아웃박스 enqueue는 결재 실행(`markExecuted`) 트랜잭션 내부(`@Transactional` REQUIRED 조인)이며, **replay(동일 `approvalId`)는 선조회 skip 으로 무예외**이고 **그 외 실패는 결재 실행 tx 와 함께 롤백**된다(결정 없는 EXECUTED 결재를 남기지 않음 — PLAN §4 L1). ② **(V77, 2026-09-09 추가)** `ADMIN_DIRECT` 확정 — 결재가 없으므로 결정 append·웹훅 아웃박스 enqueue·감사(`CASE_APPROVAL`)는 `POST /api/v1/admin/aml/customers/{customerRef}/decision`(API §2.3) 자체의 `@Transactional` 안에서 함께 커밋/롤백되는 **독립 트랜잭션**이다 — 게이트(ⓐ~ⓖ) 미통과 시 행 0(append 없음).

**`aml_cdd_onboarding_decisions`(V42)와의 관계**: 두 테이블은 서로 다른 생명주기를 갖는 별개 append 소스다 — 전자는 CDD 인입마다(재이행 포함) 새 스냅샷을 남기고, 본 테이블은 그 스냅샷이 `EDD_REQUIRED`인 상태에서 `EDD_CLOSE` 결재가 실행되거나(`caseType==EDD_REVIEW` **∧** 확정 전 현재 결정이 `EDD_REQUIRED`일 때만) `RELATIONSHIP_REJECT` 결재가 실행될 때만, 또는 **(V77, 2026-09-09 추가)** HRR 등재 EXECUTED 회원에 한해 결재 없는 관리자 **직접 확정**(`source=ADMIN_DIRECT`)이 게이트(ⓐ~ⓖ)를 통과할 때만 행을 남긴다. 재이행(re-CDD)으로 새 `EDD_REQUIRED` 스냅샷이 과거 `APPROVED`/`REJECTED` 확정 행보다 최신이면 회원은 다시 보류 상태로 읽히고(현재 결정 재계산), 그 EDD 케이스가 다시 종결되면 새 확정 행이 추가로 append된다(동일 승인당 1행 제약은 approval_id 단위이므로 재이행 발 신규 결재는 신규 행).

### 3.22h `aml_fds_transaction_decisions` — FDS 차단 판정의 거래 단위 사실(append-only) (Flyway V75, aegis-aml PLAN 20260906-fds-block-aml-propagation)

FDS 가 `BLOCK` 판정을 내린 거래(계정계 동일 `transactionRef`)를 AML 거래 read model 에 표기하기 위한 정본. `fds.decision.blocked` 통지(`POST /internal/v1/aml/fds-decisions` / SQS `aml-fds-decision`)의 착지 테이블이며 **알럿·케이스를 열지 않는다**(기존 `fds.case.escalated`/`fds.decision.applied` 와 의미 분리).

| 컬럼 | 타입 | NULL | 기본 | 설명 |
|---|---|---|---|---|
| `tenant_id` | varchar(64) | N | | 테넌트(RLS) |
| `decision_row_id` | uuid | N | `gen_random_uuid()` | PK 2번째 키 |
| `data_scope` | varchar(128) | Y | | FDS workspaceId(저장만 — 조인 키 아님) |
| `transaction_ref` | varchar(256) | N | | 계정계 거래참조 = `aml_canonical_events.payload->>'transactionRef'` |
| `fds_decision_id` | varchar(96) | N | | FDS `fds_decisions.decision_id` — `UNIQUE(tenant_id, fds_decision_id)` |
| `fds_event_id` | varchar(128) | N | | 통지 멱등키(`decision-blocked:{decisionId}`) |
| `fds_event_ref` | varchar(256) | Y | | FDS 인입 `eventId` |
| `target_ref` | varchar(256) | Y | | 회원 키(참조 토큰) |
| `outcome` | varchar(32) | N | | FDS `DecisionOutcome` raw(현재 `BLOCK`) |
| `phase` | varchar(32) | Y | | `INLINE`/`ASYNC` |
| `matched_rules` | jsonb | N | `'[]'` | `[{ruleId, ruleName, versionNo, outcome}]` — 발동 룰 전건 |
| `reason_codes` | jsonb | N | `'[]'` | FDS reason code |
| `decided_at` | timestamptz | N | | FDS 판정 시각 |
...
>
> **(2026-08-10) `WEBHOOK` 타입 행은 런타임 등재 대상이다** — 이 테이블의 다른 타입(`API_KEY`/`OAUTH2_CLIENT`/`MTLS`)은 인바운드 호출 자격증명이고 아래 provisioner 소관이지만, `WEBHOOK` 행은 **아웃바운드 전용**이며 admin REST `GET|PUT /api/v1/admin/aml/webhook-credential`(API §2.7a, 조회 `aml:case:read` / 쓰기 `aml:admin:policy`·즉시 반영·4-eyes 없음)로 **등재·교체된다**(종전 유일 수단이던 DB 직접 INSERT 대체 — REST-only 원칙 준수). **DDL 변경 0** — 필요한 컬럼(`webhook_url`·`secret_ciphertext`·`enabled`·`created_by`/`updated_by`)과 `credential_type` CHECK 의 `WEBHOOK` 이 이미 있어 엔티티 매핑만 추가됐다. 업서트는 테넌트당 **한 행을 제자리 갱신**한다(대상 정렬 `enabled` 우선 → `webhook_url` 존재 우선 → `credential_id`, `credential_id`·`created_at` 보존, 신규 생성 시 `credential_id='webhook-primary'`) — 중복 `WEBHOOK` 행이 쌓여 릴레이의 목적지 해석이 비결정적으로 갈리는 것을 막는다. `webhook_url` 은 **NULL 로도 등재 가능**(서명 시크릿만 보유 — 아웃박스 행이 `webhook_target_url` 로 목적지를 오버라이드하는 이벤트용, §3.15 `aml_outbox` V59). 저장 시 `POLICY_CHANGE` 감사(`detail.action=WEBHOOK_CREDENTIAL_SAVED`·`operation=CREATE|REPLACE`, 시크릿 미기록·목적지는 host 까지만)를 남긴다.
...
| `scopes` | JSONB | N | '[]' | | 부여 scope 배열. P0-04 FDS escalation row는 `aml:internal:fds-escalation:write` 하나만, BO row는 기존 union + `aml:pii:reveal` + `COMPLIANCE`. **`WEBHOOK` row는 admin REST 저장 시 빈 배열 `[]` 로 고정**(아웃바운드 서명 전용 — 인바운드 API 호출 권한 0, 2026-08-10) |
...
| `callback_url` | VARCHAR(2048) | Y | | **(V59, 2026-07-24)** `customer.cdd.completed` 요청이 실어 보낸 콜백 URL — HELD→비동기 재시도(`OnboardingRaRetryScheduler`) 경로에도 유실 없이 전달하기 위해 이벤트 자체에 durable 보존(API §8 `AmlHighRiskRegistrationApproved`) |
...
V70은 TM "최근 거래" 브라우즈(API §2.4 `GET /api/v1/aml/transactions`) 지지용 인덱스 3종을 추가한다 — `ix_ace_txn_ref (tenant_id, (payload->>'transactionRef'))`(거래번호 정확일치)·`ix_ace_txn_family_time (tenant_id, occurred_at DESC) WHERE split_part(event_type,'.',1) IN ('transaction','remit','domestic','wallet')`(무필터 최신순, 거래성 family 4종 partial)·`ix_ace_target_ref_time (tenant_id, (payload->>'targetRef'), occurred_at DESC)`(회원키 검색). 조회는 `TransactionEventBrowseAdapter`(read-only, `CanonicalEventWindowAdapter` 동형 네이티브 쿼리)가 소유하며, STR/CTR 평가 윈도우 계약(`CanonicalEventWindowPort`)과는 별개 아웃바운드 포트(`TransactionEventBrowsePort`)다(§7 V70).
...
| `callback_url` | VARCHAR(2048) | Y | | **(V59, 2026-07-24)** subject_type 무관 범용 컬럼 — `HRR_REGISTRATION` 상신 시 원 CDD 요청의 콜백 URL 결부, 승인(EXECUTED) 완료 시점에 `AmlHighRiskRegistrationApproved` webhook 발행에 사용(거절 시 미발행, API §8). 상신~승인 사이(수 시간~수일) 결부값 보존이 목적 — 재상신(멱등 no-op) 시 최초 값을 덮어쓰지 않는다 |
| `created_at/created_by/updated_at/updated_by/trace_id` | (공통) | | | |
...
| `status` | VARCHAR(32) | N | 'PENDING' | enum | §5.17 outbox_status **5종**(`PENDING`/`DISPATCHING`/`DISPATCHED`/`FAILED`/`DEAD_LETTERED`). `DEAD_LETTERED`(재시도 예산 소진 종료·DLQ)는 **V65**에서 CHECK `aml_outbox_status_check` 재생성으로 추가되었고 claim 대상 밖이다 |
| `attempt` | INTEGER | N | 0 | | 발행 시도 횟수. **재시도 상한(`OutboxRetryPolicy.maxAttempts`, 설정 `aegis.aml.outbox.max-attempts` 기본 5)이 claim 술어 자체에 들어 있다** — 상한 도달 행은 dispatchable 이 아니며 전이 시점 판정만으로는 재큐 경로가 상한을 우회한다(V65 배경) |
| `next_attempt_at` | TIMESTAMPTZ | Y | NULL | | 재시도 backoff 예정(poller `SELECT ... FOR UPDATE SKIP LOCKED`). `DISPATCHING` 동안에는 **클레임 리스 만료 시각**(SQS visibility-timeout 유사)으로 재해석되며, 초과 시 stale 회수 대상이 된다 |
| `dispatched_at` | TIMESTAMPTZ | Y | NULL | | 발행 완료 시각(`DISPATCHED` 전이 시 `now()`). **(2026-08-12 정정)** 종전 본 표는 `published_at` 으로 기재했으나 V1 baseline·`OutboxJpaAdapter` 의 실제 컬럼명은 `dispatched_at` 이다 |
| `claim_token` | UUID | Y | NULL | | **(V66)** 클레임 소유권 토큰 — claim 1회당 `gen_random_uuid()` 발급(`claimBatch`·stale 회수 클레임), 전이 3종(`markDispatched`·`markFailed`·`markDeadLettered`)이 `IS NOT DISTINCT FROM` 로 CAS 대조해 **자기 클레임일 때만** 적용, 전이 성립 시 NULL(클레임 해제). 전송 payload 아님 |
| `trace_id` | VARCHAR(64) | Y | NULL | | 관측성 traceId 전파 |
| `webhook_target_url` | VARCHAR(2048) | Y | NULL | | **(V59, 2026-07-24)** 행 단위 아웃바운드 webhook 목적지 오버라이드 — non-null 이면 릴레이(`HttpWebhookRelayAdapter`)가 테넌트 사전등록 `aml_api_credentials.webhook_url` 조회 대신 이 URL 로 직접 발송한다(서명 시크릿은 그대로 테넌트 소유, API §8 예외 문단). null 이면 기존처럼 테넌트 조회 폴백(하위호환, 기존 3종 webhook 이벤트 무회귀). SSRF 3단계 검증은 이 값에도 매 전송 직전 동일 적용 |
| `created_at/created_by` | (공통, append 중심) | | | | |
...
| aml_watchlist_entries | `gin_wle_tokens_lower` | GIN(`aml.wlf_lower_text_array(normalized_tokens)`) | **WLF 후보 블로킹 키(V64)** — S2 TOKEN_OVERLAP 소문자 토큰 중복(`&&`). 대용량 명단(약 1.6M 행)에서 전량 스캔을 비트맵 인덱스 스캔으로 전환(실측 4,490ms→73ms) |
| aml_watchlist_entries | `gin_wle_phonetic_lower` | GIN(`aml.wlf_lower_text_array(phonetic_codes)`) | **WLF 후보 블로킹 키(V64)** — S4 PHONETIC 소문자 double-metaphone 코드 중복(`&&`). 종전 `jsonb_exists_any(...)` 함수형은 어떤 인덱스도 타지 못했다 |
| aml_watchlist_entries | `ix_wle_active_version` | (tenant_id, source_code, version) WHERE status='ACTIVE' | **(V67)** unchanged 스킵의 로컬 무결성 판정 — "ACTIVE 행은 전부 `active_version` 에 있다" 불변식을 min/max 두 번으로 확인(깨져 있으면 같은 publisher version 이어도 재적재해 치유) |
...
### 5.6 tm_scenario (설계서 §12.1, **V61 이후 자유형 코드 — 닫힌 enum 폐기**)

코드(`aml_tm_scenarios.scenario_code` CHECK)는 **V61(2026-08-01) 이후 닫힌 목록이 아니라 형식 정규식**(`^[A-Z][A-Z0-9_]{2,64}$`)이다. 과거 닫힌 enum `TmScenario` 10종(`STRUCTURING`·`MULE_NETWORK`·`RAPID_MOVEMENT`·`ROUND_TRIPPING`·`SHELL_MERCHANT`·`TRADE_MISPRICING`·`REFUND_LAUNDERING`·`CRYPTO_OFF_RAMP`·`HIGH_RISK_CORRIDOR`·`INTERNAL_OVERRIDE_ABUSE`)는 **v9.21 확정 사실(거래 인입 경로에서 전혀 발동하지 않음, F-025 실측)에 따라 정의 행이 전량 제거**됐다 — 관리 대상 정의 목록의 정상 상태는 0행, 신규 시나리오는 REST 저작(`draft`)으로 자유형 코드를 등록한다. **예약 코드 `CUSTOM_RULE`은 거부**된다(설정형 STR/CTR 룰 결재 subjectRef prefix `CUSTOM_RULE|`와의 오라우팅 방지, `TmScenarioService.draft` 유스케이스 불변식). 옛 10종 코드값 자체를 재사용해 신규 저작하는 것은 형식상 허용되나(코드=truth, 카탈로그 라벨 없음 — `displayName`은 코드 원문), 자동 발동되지 않는다는 사실은 동일하다.

### 5.6a tm 금액 임계 feature — `baseEquivalent` (다통화 정본) / `phpEquivalent` (PHP 별칭)

금액 기반 TM 시나리오의 `cmp` leaf feature 정본은 **`transaction.baseEquivalent`** — **테넌트 기준통화(`aml_tenants.base_currency`) 액면 금액**이며 전 통화 테넌트에 노출된다. 임계도 같은 통화 액면으로 비교한다(호주 법인 A$, 일본 법인 ¥, 한국 법인 ₩).

**FX 환산은 하지 않는다.** 중립 인입 검증이 `amounts.baseCurrency == 테넌트 바인딩 base` 를 강제하므로(§3.1 V53) 한 테넌트의 canonical 윈도우에는 단일 통화 행만 존재하고, 따라서 합계·평균·일합계가 환산 없이 그대로 성립한다. 환율 테이블·FX 포트는 도입하지 않는다.

`transaction.phpEquivalent` 는 **`base_currency='PHP'` 테넌트에서만 노출되는 하위호환 별칭**이다(PHP 액면으로 작성된 기존 룰 보존). 비PHP 테넌트에는 노출되지 않으므로 PHP 액면 룰이 자국통화 금액과 비교되는 오탐이 구조적으로 불가능하다. `velocity`(건수)·채널(`transaction.channelType`) cmp는 금액 무관.

### 5.6b tm_scenario_status (`aml_tm_scenarios.status`, 도메인 `TmScenarioStatus`)
`DRAFT`(작성중) / `ACTIVE`(활성·scenario_code당 단일) / `SUPERSEDED`(대체됨)

### 5.7 alert_status (설계서 §12.2 → §13 case 인계, **V63 회수 축 `RETIRED` 추가**)
`DETECTED` → `TRIAGED` → `CASE_OPENED` → (`DISMISSED` | `ESCALATED` | `STR_RECOMMENDED`) / `RETIRED`(별도 축)

> **`RETIRED` 는 위 triage 상태기계의 일부가 아니다(별도 축, V63/2026-08-06).** 진입 경로는 설정형 STR/CTR 룰 회수(`:retire` 4-eyes 승인 실행, API §2.4) 하나뿐이며 운영자 triage 액션으로는 도달할 수 없다. **`DISMISSED` 재사용 금지** — 오탐 종결은 분석가의 업무 판단이고 회수는 룰 자체의 무효화라 의미가 다르며, 오탐 종결된 알림도 회수될 수 있고 그때 `disposition_*` 는 그대로 보존된다. 케이스가 개설된 알림은 회수되지 않는다(§3.10 회수 주석). 아래 6종 종결 서술은 **triage 축에 한해** 유효하며, 물리 CHECK 는 V63 이후 7종이다.
...
| `HRR_REGISTRATION` | 당연고위험 등재 승인 | RA 당연고위험 분류 회원의 `RA_HIGH_RISK_CUSTOMERS` 레지스트리 등재 상신(V28). 승인선 `EXECUTIVE_APPROVAL`(고위경영진 수동승인). RA 평가(1차 온보딩·2차 상시)가 `mandatoryHighRisk=true`(CUSTOMER 대상) 산출 시 엔진이 자동 상신(maker `system:ra-engine`, tier=PROHIBITED→VERY_HIGH·그 외 HIGH)하며 RA 상세 수동 상신(`POST .../high-risk-registry/registrations`, API §2)도 동일 subjectType. 이미 등재(EXECUTED)/상신 대기(PENDING)는 멱등 no-op(재평가 루프 종료 불변식). 결재 EXECUTED 시에만 `RA_HIGH_RISK_CUSTOMERS` 등재 + RA 강제 상향 재평가 확정. **(2026-09-04 신설, aegis-aml F-095)** 등재 해제 상신은 동일 subjectType 을 재사용하되 `subject_ref` 접두 **`DEREGISTER\|customerRef`**(선례 `FP_WHITELIST` `REVOKE\|`·`TM_SCENARIO` `CUSTOM_RULE_RETIRE\|`)로 등재 상신과 구분한다(등재 핸들러는 접두 행 거부, 해제 핸들러는 접두 없는 행 거부). 결재 EXECUTED 시 항목 제거(§3.20 주석) — CHECK 재생성 없음(subjectType 신규 값 아님) |
...
### 5.17 outbox_status (integration §8.1) — **5종(V65 `DEAD_LETTERED` 추가)**
`PENDING`(대기) / `DISPATCHING`(발행중) / `DISPATCHED`(발행완료) / `FAILED`(실패·재시도) / `DEAD_LETTERED`(재시도 예산 소진·종료)

> **상태 기계**: `PENDING → DISPATCHING → DISPATCHED`(종료) · `DISPATCHING → FAILED →` (백오프 재시도 | 상한 도달 시 `DEAD_LETTERED` 종료). 종료 상태는 `DISPATCHED`·`DEAD_LETTERED` 2종(도메인 `OutboxStatus.isTerminal()`)이며, `DEAD_LETTERED` 는 claim 술어 밖이라 어떤 스윕도 다시 집지 못한다. 도메인 `OutboxStatus` enum 과 1:1.
...
### 5.36 cdd_history_type (`aml_member_cdd_history.history_type`, §3.22f, V26·V27·V78)
`CDD_INITIAL`(최초 CDD 이행 — 원장 미존재 시 첫 `customer.cdd.completed` 인입) / `CDD_REVIEW`(재이행 CDD — 기존 회원 후속 재인입) / `EDD_OPENED`(EDD 착수 — 케이스 개시·트리거 병기) / `EDD_CLOSED`(EDD 종료 — 4-eyes 승인 실행·최종 상태 병기) / `CDD_REISSUE_REQUESTED`(CDD 즉시 재이행 접수 — RA 상세 관리자 액션, 계정계 지시, V27) / `EDD_REISSUE_REQUESTED`(EDD 즉시 재이행 접수 — 동일, V27) / `MEMBER_DECISION`(회원 심사결과 확정 — `EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT` 3출처, V78)

> DB가 물리 정본(CHECK **7종** — V26 4종 + V27 재이행 접수 2종 + V78 회원 심사결과 확정 1종). 도메인 enum `CddHistoryType` 및 bo-api `MemberLedgerDtos.HistoryType` 와 1:1. `CDD_*` 는 원장 존재 여부로 판정(§3.22f 적재 지점 a), `EDD_*` 는 EDD 착수/종료(`CddEddService`, 적재 지점 b), `*_REISSUE_REQUESTED` 는 즉시 재이행 접수(`DueDiligenceReissueService`, 적재 지점 c) — 실 재이행은 **계정계 연동 예정**(`TODO(계정계-연동)`), 계정계 재수행 결과의 `customer.cdd.completed` 재인입이 `CDD_REVIEW` 로 폐루프를 닫는다. `MEMBER_DECISION` 은 회원 심사결과 확정(`MemberDecisionService.confirm`/`DirectMemberDecisionService`, 적재 지점 d, V78)이며 `details.decision`/`details.source` 로 확정 값·출처를 구분한다(§3.22f).
...
스키마: `aml`. 파일 위치: `services/aml-svc/src/main/resources/db/migration/`. **본 표는 실제 구현 파일(V1~V23, V26~V38, V41~V78, fresh-DB Flyway 검증 통과)과 1:1 일치한다** — 구 누적 마이그레이션 체인(구 phase 단위 `V1~V25`)은 2026-06-30 `AML-ENGINE-FIX: consolidate Flyway baselines`(commit 9a3ac74)로 **검증된 최종 상태의 pg_dump 를 단일 `V1__baseline.sql`(schema-only DDL) + `V2__seed.sql`(data-only bootstrap/demo seed)로 통합(consolidate)**되었다. 이후 CTR/STR 모니터링 통합(feature/aml-ctr-str-monitoring, 2026-07-01)이 `V3`~`V6` 을, 이어 TM 룰코드(`V7`)·실 제재명단 수집(`V8`)·WLF 수취인 임계(`V9`)·워치리스트 브라우즈 인덱스(`V10`)·RA 모델 시나리오(`V11`)·2차 상시 RA(ONGOING) 모델 활성화(`V12`)·RA 당연고위험(HRR 강제 상향) 사유 영속(`V13`)·FP whitelist 등록 메타데이터 영속(`V14`)·케이스 발단(origin) 계보 `origin_screening_id`(`V15`)·국가위험 일일 웹 수집(`V16`)·국가위험 단일 ACTIVE 불변식(`V17`)·국가위험 수집 소스 제공자화(EU 집행위 기본·FATF 대안 provenance, `V18`)·1차 온보딩 RA 엔진 CDD 파생 파라미터 정본(`V19`)·1차 온보딩 RA 국가위험 등급 기반 강제 floor 파라미터(`V20`)·데모 국가위험 수동 기준선(`V21`)·CTR/STR 룰 튜너블 파라미터 오버라이드(`V22`)·CTR 임계 엔진 결재 subject 확장(`V23`)·회원원장 CDD/EDD append-only 이력 신설(`V26`)·CDD/EDD 즉시 재이행 요청 접수 이력 유형 확장(`V27`)·RA 당연고위험 등재 폐루프(`V28`)·데모 시드 비즈니스 데이터 제거(`V29`)·알림 처분 메타(`V30`)·Travel Rule 제거(`V31`)·FDS 프로필 outbox(`V32`)·사용자 정의 보고룰(`V33`)·RA retry/FDS 점수(`V34`)·RA 버전 설정 폐루프(`V35`)·최근 CDD 카탈로그 인덱스(`V36`)·Policy Pack 반려 고아 종결(`V37`)·WLF SANCTIONS/PEP profile 이력 canonical backfill(`V38`)·lifecycle invariant(`V41~V43`)·machine-auth credential/replay(`V44`)·운영 demo fingerprint 격리(`V45`)·admin 감사 trace 폭 확장(`V46`)·`SHARED` 배포 RLS tenant 격리(`V47`)·거래 fan-out 완전성·durable retry 추적(`V48`)·WLF 후보 recall trigram/phonetic 보강(`V49`)·WLF source readiness 상태기계(`V50`)·필수 watchlist source 정책(`V51`)·WLF 명단 갱신 후 durable rescreen 배치(`V52`)·tenant 관할·통화·Policy Pack revision 바인딩(`V53`)·규제 제출 durable worker job(`V54`)·불변 evidence artifact bytes·버전 핀(`V55`)·감사 hash chain append-only(`V56`)를 additive 로 얹었다. 이후 AMLC 계정·접수번호(`V57`·`V58`)·HRR 콜백(`V59`)·제재 4소스(`V60`)·TM 시나리오 레거시 제거(`V61`)·dilisense 소스(`V62`)를 additive 로 얹었다. 이어 TM 알림 회수 축(`V63`)·WLF 후보 조회 블로킹 인덱스(`V64`)·아웃박스 종료 상태 `DEAD_LETTERED`(`V65`)·아웃박스 클레임 소유권 토큰(`V66`)·제재 sync run 리스(`V67`)·데모 교차통화 CTR 임계 정리(`V68`)·테넌트 보고 마감시각(`V69`)·거래 브라우즈 조회 인덱스(`V70`)를 additive 로 얹었다. 이어 **케이스 조사관 작업대(체크리스트 실행 테이블 신설 + `origin_screening_id` NULL-only backfill, `V72`)** 를, 이어 레거시 케이스 `CREATED` 타임라인 소급(`V73`)을, 이어 **회원(고객 관계) 심사결과 확정 append-only 테이블 + 스크리닝 요청별 콜백 URL + CDD callbackUrl 조회 인덱스(`V74`, PLAN 20260903-aml-decision-status-webhooks)** 를 additive 로 얹었다(**본 표 1:1 파일 범위 V41~V78** — `V71` 은 WLF runtime RLS 후보조회 경계(`fix/aml-engine-response-performance`, 2026-09-07 main 착지)이며 이후 FDS 차단 판정 기록(`V75`)·재스크린 잡 일시정지(`V76`)·회원 심사결과 직접 확정 출처 `ADMIN_DIRECT`(`V77`, PLAN 20260909-aml-ra-direct-member-decision)·**회원 결정 이력 `MEMBER_DECISION`·기존 확정 backfill(`V78`, PLAN 20260910-aml-member-ledger-decision-history)** 가 additive 로 얹혔다). **V24·V25 및 V39·V40은 예약·공번이며 재사용하지 않는다.** 아래는 코드(truth) 기준.

| 버전 | 파일 | 내용 | 의존 |
|---|---|---|---|
| V71 | `V71__wlf_candidate_runtime_rls_recall.sql` | **WLF runtime RLS-safe 후보조회**: mutation 전 superuser를 요구하는 one-shot privileged migration. 전용 NOLOGIN/non-super/BYPASSRLS owner와 최소 ACL을 만들고 tenant/elevated guard·22023 입력 검증·고정 S1~S4/grouped/ranked/capped 3열 SECURITY DEFINER 함수를 생성한다. 선존 role의 attribute/membership, current-DB owner/datacl, `aml` ownership, unexpected direct/default ACL을 mutation 전에 거부하며 종료 직전 exact ownership·2 table SELECT·helper EXECUTE·schema USAGE·PUBLIC/runtime function ACL·2 proconfig·직접 RLS 2정책을 재검증한다. 동일 bootJar runner는 Java V38+PHP repeatable inventory를 적용한 뒤 app provision을 별도 JDBC transaction으로 수행하고, `aegis_aml_app`은 runtime membership+history-only direct ACL을 가진다. V71-only 중간상태는 재실행 가능하며 malformed partial app role은 거부한다. 신규 RLS policy·planner GUC·timeout/cap/floor/시드 0. | V70 |
| V72 | `V72__case_checklist_state_and_origin_backfill.sql` | **케이스 조사관 작업대 — 체크리스트 실행 테이블 신설 + 발단 계보 backfill(PLAN 20260902-aml-case-workbench U1, §3.11a·§3.11)**: (1) 신규 테이블 `aml_case_checklist_items`(§3.11a) — 케이스별 체크리스트 항목 실행 상태(상태/메모/증빙/행위자), PK `(tenant_id,case_id,item_key)`, FK→`aml_cases`, V47 동형 RLS 2종. **컬럼 신설 없음**(`aml_cases` 스키마 변경 0 — A1: 정본이 이미 `origin_screening_id` 를 RA 스코어 id 슬롯으로 정의). (2) **backfill**: `case_type='EDD_REVIEW' AND origin_alert_id IS NULL AND origin_screening_id IS NULL AND origin_fds_case_ref IS NULL` 대상 케이스 중 같은 `(tenant_id,target_ref)` 의 `aml_risk_scores` 에서 `created_at BETWEEN case.created_at - 60s AND case.created_at + 5s` 창 내 첫 매치(`model_code='KR_ONGOING_RA'` 우선)를 `SELECT DISTINCT ON`으로 골라 `origin_screening_id = 'risk-score:' || score_id` 로 UPDATE(NULL 슬롯만, 값 있는 행 무변경). 매칭 없으면 그대로 NULL(정직한 빈 상태). 동일 트랜잭션에서 backfill된 케이스마다 `aml_case_timeline` 에 `kind='CREATED', actor='system:migration', note='EDD_TRIGGER=…;LINEAGE=BACKFILL_INFERRED;window=-60s..+5s', evidence_refs=['risk-score:'||score_id]` 1행 INSERT(이미 `CREATED` 행이 있는 케이스는 제외). (3) `COMMENT ON COLUMN aml_cases.origin_screening_id` 를 A6 토큰 규약(`risk-score:<uuid>`/`screening:<uuid>`/접두 없음=screening 하위호환)으로 재기술(주석만, 타입·값 불변). 행 삭제·컬럼 변경 없음(V43 원칙). additive·멱등(재실행 시 이미 채워진 슬롯·이미 있는 `CREATED` 행은 재적용 없음). | V70 |
| V73 | `V73__case_created_timeline_backfill.sql` | 레거시 케이스 `CREATED` 타임라인 소급(INSERT 만) — CREATED 행이 없는 모든 케이스에 origin 컬럼의 **확정 값**으로 1행 추가: `origin_alert_id` 보유 → `ALERT_HANDOFF=<scenario_code>;ACTION=<alert.status>;LINEAGE=BACKFILL`(evidenceRefs `alert:`·`txn:`), `origin_fds_case_ref` → `FDS_DECISION=UNKNOWN;LINEAGE=BACKFILL`, `edd_trigger` → `EDD_TRIGGER=<trigger>;LINEAGE=BACKFILL`, `edd_trigger` 없이 `origin_screening_id` 만 → `SCREENING;LINEAGE=BACKFILL`, 그 외 `MANUAL;LINEAGE=BACKFILL`; `origin_screening_id` 는 A6 토큰(접두 없으면 `screening:`)으로 evidenceRefs 에 동봉, actor=`left(<created_by>,118)`+`:migration`(varchar(128) 상한 가드), created_at=케이스 생성 시각. 추정(`BACKFILL_INFERRED`)이 아니므로 화면 '추정' 배지 대상이 아니다. 케이스 행·컬럼 무변경, 재실행 멱등(CREATED 존재 시 제외). 게이트: `verify_bo_case_workbench_exposure.py` 불변식(timeline 에 CREATED 1행 이상) | V72 |
| V74 | `V74__member_decisions_and_screening_callback.sql` | **회원 심사결과 확정 append-only 테이블 + 스크리닝 요청별 콜백 URL + CDD callbackUrl 조회 인덱스(PLAN 20260903-aml-decision-status-webhooks, §2.3·§3.8·§3.22g, 사용자 지시로 F-084 부분 해제).** (1) 신규 테이블 `aml_member_decisions`(§3.22g) — `EDD_CLOSE`/`RELATIONSHIP_REJECT` 4-eyes 결재 EXECUTED 시점에만 append. PK `(tenant_id,decision_id)`, CHECK `decision`(4종)·`source`(2종), 인덱스 `ix_aml_member_decisions_member`, UNIQUE 부분 인덱스 `ux_aml_member_decisions_approval(tenant_id,approval_id) WHERE approval_id IS NOT NULL`(결재당 1행 멱등), V57/V72 동형 RLS 2종 ENABLE+FORCE. (2) `aml_screening_results`(§3.8)에 `callback_url varchar(2048)` 1컬럼 additive(nullable) — `POST /aml/screen` 요청별 웹훅 목적지 오버라이드, 없으면 테넌트 `webhook_url` 폴백. (3) 부분 인덱스 `ix_ace_cdd_customer_ref_time ON aml_canonical_events (tenant_id, (payload->>'customerRef'), occurred_at DESC) WHERE event_type='customer.cdd.completed'` — 회원 결정 웹훅 목적지(최신 CDD `callback_url`) 조회의 대용량 풀스캔 방지(V70 동형 비-CONCURRENTLY 인덱스 선례). 파괴적 변경 0(신규 테이블 CREATE + nullable 컬럼 ADD만). | V73 |
| V75 | `V75__fds_transaction_decisions.sql` | **FDS 차단 판정의 거래 단위 사실 기록 테이블(aegis-aml PLAN 20260906-fds-block-aml-propagation, §3.22h).** 신규 `aml_fds_transaction_decisions` — PK `(tenant_id, decision_row_id)`, `UNIQUE(tenant_id, fds_decision_id)`(FDS 판정 1건=1행 멱등), 인덱스 `(tenant_id, transaction_ref, decided_at DESC)`(거래 조회 LATERAL 최신 1건), `matched_rules`/`reason_codes` JSONB(기본 `'[]'`), RLS `aml_rls_tenant`/`aml_rls_owner` 2정책 ENABLE+FORCE(V74 동형). 알럿·케이스 테이블 무변경. 파괴적 변경 0 | V74 |
| V76 | `V76__rescreen_job_pause.sql` | **재스크린 잡 운영자 일시정지 컬럼(aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit, §3.6b).** `aml_wlf_rescreen_jobs` 에 `paused_at timestamptz NULL`·`paused_by varchar(128) NULL` additive. CHECK·인덱스·RLS·부분 인덱스 무변경, 파괴적 변경 0 | V75 |
| V77 | `V77__member_decisions_admin_direct_source.sql` | **회원 심사결과 직접 확정 출처 `ADMIN_DIRECT` 허용(aegis-aml PLAN 20260909-aml-ra-direct-member-decision, §3.22g).** `aml_member_decisions.source` CHECK 를 `('EDD_CLOSE','RELATIONSHIP_REJECT')`→`('EDD_CLOSE','RELATIONSHIP_REJECT','ADMIN_DIRECT')` 로 재정의(제약 DROP+재생성, 값 집합 확장만 — 기존 행 전부 새 CHECK 도 만족). 신규 테이블·컬럼·인덱스 없음, 파괴적 변경 0 | V76 |
| V78 | `V78__member_cdd_history_member_decision.sql` | **회원원장 이력 유형 `MEMBER_DECISION` 추가 + 기존 회원 결정 backfill(aegis-aml PLAN 20260910-aml-member-ledger-decision-history, §3.22f·§5.36).** `aml_member_cdd_history_type_check` 를 6종→**7종**(`MEMBER_DECISION` 추가, V27 동형 DROP+재생성, additive)으로 재정의 + `aml_member_decisions`(§3.22g) 전 행을 `WHERE NOT EXISTS(... details->>'decisionId' ...)` 멱등 조건으로 `aml_member_cdd_history` 에 `MEMBER_DECISION` 1행씩 INSERT(backfill). backfill 행 `kyc_status` 는 `aml_customers` LEFT JOIN 현재 스냅샷(확정 시점 스냅샷 아님), `risk_grade` 는 `aml_member_decisions.risk_grade`, `trace_id` 는 기존 확정 행에 저장된 적이 없어 전부 NULL. 신규 테이블 없음, 컬럼·인덱스 변경 없음(CHECK 재정의 + INSERT만), 파괴적 변경 0 | V77 **운영 런북(QA 리뷰 2026-09-10)**: backfill 은 마이그레이션 시점 1회 스냅샷이므로 다중 인스턴스 롤링 배포에서는 구버전 인스턴스가 V78 적용 후에 실행한 결재의 `MEMBER_DECISION` 이력이 빠질 수 있다 — 전 인스턴스 교체 완료 후 V78 의 INSERT 구간(멱등, `details->>'decisionId'` NOT EXISTS)을 1회 재실행한다. |
| V68 | `V68__drop_cross_currency_ctr_threshold.sql` | **데모 교차통화 CTR 임계 정리**: V2 immutable seed 지문이 정확히 일치하는 `tenant_demo`에만 남아 있던 비-PHP 임계 행을 제거한다. 동일 tenant id의 고객 구성을 건드리지 않도록 V45 동형 복합 지문을 사용하며, 통화 프로필 팩은 이후 자기 테넌트·자기 통화 임계 1행만 적재한다. | V67 |
| V69 | `V69__tenant_report_cutoff_time.sql` | **테넌트 관할 보고 마감시각**: `aml_tenants.report_cutoff_time TIME NULL`을 additive 추가하고 기존 PH 데모 지문 행만 `17:00`으로 backfill한다. 통화 profile repeatable pack은 이미 바인딩된 자기 tenant에도 소유 cutoff를 재적용한다. NULL은 미확정/구 배포를 뜻한다. PH+`Asia/Manila`만 legacy NULL을 17:00으로 해석하며 비PH/다른 PH 값은 법정 기한을 계산하지 않고 `due_at=NULL`로 둔다. 시간대는 V53 `timezone`과 한 snapshot으로 해소한다. | V68 |
| V70 | `V70__canonical_event_browse_indexes.sql` | **거래 브라우즈 조회 전용 인덱스(PLAN 20260819-aml-tm-recent-transactions AML-2, additive·동작 무변경)**: TM "최근 거래" 목록(API §2.4 `GET /api/v1/aml/transactions`) 지지용 btree 인덱스 4종만 추가 — 테이블·컬럼·CHECK 변경·시드 없음. ① `ix_ace_txn_ref (tenant_id, (payload->>'transactionRef'))` — 거래번호 정확일치 검색. ② `ix_ace_txn_family_time (tenant_id, occurred_at DESC) WHERE split_part(event_type,'.',1) IN ('transaction','remit','domestic','wallet')` — 무필터 최신순 목록(partial, 거래성 family 한정). ③ `ix_ace_target_ref_time (tenant_id, (payload->>'targetRef'), occurred_at DESC)` — 회원키 검색(기존 TM 윈도우 조회도 수혜). ④ `ix_alert_txn_ref (tenant_id, transaction_ref) WHERE transaction_ref IS NOT NULL`(on `aml_alerts`) — 알럿 동봉(RETIRED 제외 집계) 상관 서브쿼리 지지(`ux_alert_tm`(V1) 부분 유니크는 alert_type 한정이라 전 alert_type 매칭을 지지하지 못해 별도 신설). ②의 거래성 family 술어는 `ck_aml_events_family`(`V1__baseline.sql` 최초 선언(20 family, travel-rule 포함) → `V31__drop_travel_rule.sql` 재정의(현행 **19 family**, travel-rule 제거))와 `EventFamily.isTransactionBearing()`(TRANSACTION·REMIT·DOMESTIC·WALLET 4종)에 lock-step. `CREATE INDEX IF NOT EXISTS`(비-CONCURRENTLY, 로컬/데모 규모 기준). | V69 |
...
| V4 | `V4__ctr_report_idempotency.sql` | **CTR/STR 통합 P2 — CTR 멱등/일합산**: `aml_regulatory_reports`에 CTR 컬럼 4종 추가(`ADD COLUMN IF NOT EXISTS`, 전부 nullable — legacy/비-CTR 행 무영향): `subject_ref VARCHAR(256)`·`banking_day_key DATE`·`report_amount NUMERIC(20,2)`·`due_at TIMESTAMPTZ`(freeze 된 서버 파생 테넌트 기준통화 합계 + nullable 법정 기한). 부분 UNIQUE `ux_aml_ctr_draft (tenant_id, subject_ref, banking_day_key) WHERE report_type='CTR' AND status='DRAFT'` — (테넌트,주체,영업일)당 열린 CTR DRAFT 정확히 1건, 동일 영업일 후속 현금거래는 `report_amount` 누적(CTR_DAILY 보완재). 집계 SQL은 `[startOfDay,startOfNextDay)` 반개구간이다. `CtrEvaluationService` upsert 계약과 일치. additive. | V1~V3 |
...
| V57 | `V57__amlc_credentials.sql` | **테넌트별 AMLC(Anti-Money Laundering Council, 필리핀) 포털 로그인 계정 저장(feature/aml-reports-amlc-migration, §1.4-A)**: 신규 테이블 `aml_amlc_credentials`(PK `(tenant_id)`, `fds_api_credentials` 동형 컬럼 컨벤션 재사용) — `username varchar(255) NOT NULL`·`secret_ciphertext varchar(512) NOT NULL`(`SecretCipherPort`/`AesGcmSecretCipher` AES-256-GCM 암호화, raw 평문 미저장·§19.2)·`enabled boolean NOT NULL DEFAULT true`·`created_at`/`updated_at`/`updated_by`. RLS: V47 DO 루프 경과 후 신설 테이블이라 자기 마이그레이션에서 직접 V47 동형 정책 2종(`aml_rls_tenant`(runtime, tenant 일치 OR `app.elevated='on'`)·`aml_rls_owner`) 명시 ENABLE+FORCE. 저장은 `PUT /api/v1/admin/aml/amlc-credential`(4-eyes 미적용 — 결정 C, 기존 보고기관 헤더 편집과 동급 즉시반영) 로만 적재하며 조회 응답(`MaskedView`)은 시크릿 필드를 전혀 포함하지 않는다. additive. | V44,V47 |
| V58 | `V58__regulatory_report_amlc_submission_ref.sql` | **`aml_regulatory_reports`(§3.12)에 실 AMLC 포털 lodgement 접수번호 컬럼 추가(feature/aml-reports-amlc-migration, §1.4-C)**: `amlc_submission_ref VARCHAR(128)` 1컬럼 additive 추가(nullable). `PlaywrightAmlcSubmissionAdapter`(`AmlcSubmissionPort` 브라우저 자동화 실 구현체)가 AMLC 포털에 직접 lodge 후 반환한 `AmlcReceipt.amlcSubmissionRef` 를 `RegulatoryReportService`(usecase) 가 이 컬럼에 결선한다 — `submitted_ref`(제출 manifest 해시, `aml_report_submission_jobs.amlc_submission_ref` 와도 별개 값)와 값이 다르며 혼동 금지. 서비스는 `amlc_submission_ref IS NOT NULL` 이면 어댑터를 재호출하지 않아 이중 lodge 를 방지한다(멱등, STR-C11). 컬럼 추가만이라 RLS(§1.2 V47) 재적용 불요. additive. | V57 |
| V59 | `V59__hrr_registration_callback.sql` | **CDD 인입 요청별 콜백 URL + HRR_REGISTRATION 승인 완료 아웃바운드 콜백(feature/aml-cdd-hrr-approval-callback, API §8 4번째 webhook 이벤트 `AmlHighRiskRegistrationApproved`)**: 3개 테이블에 nullable 컬럼 각 1종 additive(전부 `ADD COLUMN`, 기존 행 NULL 하위호환). (1) `aml_canonical_events` — `callback_url varchar(2048)`(CDD 원본 이벤트 보존 — HELD→비동기 재시도(`OnboardingRaRetryScheduler`) 경로에도 콜백 URL 유실 없이 전달). (2) `aml_approvals`(§3.15) — `callback_url varchar(2048)`(subject_type 무관 범용 컬럼명 — HRR_REGISTRATION 상신~승인 사이 결부값 보존, 향후 다른 결재 유형도 재사용 가능). (3) `aml_outbox`(§3.15) — `webhook_target_url varchar(2048)`(행 단위 아웃바운드 웹훅 목적지 오버라이드 — null 이면 기존처럼 테넌트 `aml_api_credentials.webhook_url` 조회로 폴백, 하위호환·기존 3종 webhook 이벤트 무회귀). additive·멱등. | V56 |
...
| V61 | `V61__remove_legacy_tm_scenarios.sql` | **레거시 TM 시나리오 정의 10종 제거 + `scenario_code` 자유형 전환(2026-08-01, refactor/remove-legacy-tm-scenarios, §3.10a·§5.6)**: v9.21 확정 사실(TM 알림 발동 정본 = CTR/STR 룰 카탈로그, F-025 실측)에 따라 `aml_tm_scenarios` 가 거래 인입 경로에서 전혀 발동하지 않는 레거시 정의 10종(닫힌 enum `TmScenario`, SUPERSEDED 구버전 포함 실측 13행 — `STRUCTURING` v1/v2·`HIGH_RISK_CORRIDOR` v1/v2/v3 포함)을 관리 혼동 없이 제거한다. (1) `DELETE FROM aml.aml_tm_scenarios WHERE scenario_code IN (10종)` — 테이블 DROP 은 하지 않는다(저작 경로 `TmScenarioAdminController` 존치, 0행이 정상). (2) `aml_tm_scenarios_scenario_code_check` 를 닫힌 10종 목록에서 형식 정규식 `^[A-Z][A-Z0-9_]{2,64}$` 로 재정의(자유형 코드 저작 허용, 예약 코드 `CUSTOM_RULE` 거부는 유스케이스 계층). **`aml_alerts.ck_aml_alerts_scenario_code`(V7 합집합)는 명시적으로 무변경** — `scenario_code` 컬럼에 FK 가 없어 정의 DELETE 와 무관하고, 레거시 알럿 보유 환경에서 CHECK 축소는 기동 실패 위험이라 범위에서 제외(§3.10). `RAISE EXCEPTION` 등 fail-closed 가드도 두지 않음(로컬 무참조 확인은 사전 실측 게이트가 담당). 멱등(DELETE 0행 매치=no-op, CHECK DROP IF EXISTS 후 재생성). | V1~V60 |
| V62 | `V62__dilisense_watchlist_source.sql` | **dilisense AML Database 소스 카탈로그 등록(§7.4 상세 — integration 참조)**: `aml.aml_watchlist_sources` 에 `tenant_demo`/`DILISENSE_CONSOLIDATED`/`SANCTIONS`/`DISABLED` 1행을 V8·V60 과 동형 패턴(REST-only 데모 예외·V45 production-quarantine sweep 이후 실행이라 `ACTIVE` 직접 시드 금지)으로 삽입한다. `active_version`·`last_imported_at` NULL 시드(미적용 소스는 48h freshness fail-closed 게이트 범위 밖 — 최초 sync 가 승격). `source_code` 는 `WatchlistFeedRouter.DILISENSE_CONSOLIDATED` 상수와 정확히 일치해야 라우팅 실패(mock adapter 폴백)를 방지한다. mandatory 정책은 본 마이그레이션 범위 밖(REST `POST /api/v1/admin/aml/mandatory-sources` 별도). additive·멱등(`ON CONFLICT DO NOTHING`). **현재 상태(2026-08-10 라이브 실증, aegis-aml main `e29f9ab0`) — 미임포트**: 이 소스는 **한 번도 임포트에 성공한 적이 없어** `active_version`·`last_imported_at` 이 **V62 시드 그대로 `NULL`**(`status='DISABLED'`) 이고, 따라서 `aml_watchlist_entries` 에 `source_code='DILISENSE_CONSOLIDATED'` 행도 **0건**이다. 사유는 두 단계다 — ① 2026-08-10 이전에는 API 키 미보유로 `DilisenseConfigurationException`→`FAILED`, ② 키 확보 후에는 전량 파일이 종전 256 MiB 캡을 2.22배 초과(실측 597,038,659 B)해 `FAILED`, ③ 캡을 1 GiB 로 상향한 뒤의 라이브 sync 는 **HTTP 500 + `java.lang.OutOfMemoryError: Java heap space`**(84.7초, 컨테이너 생존, 메모리 1.479 GiB/2 GiB)로 실패했다. OOM 원인은 파서가 엔트리 **1,503,937개**(레코드 1,025,647 + 별칭 478,290)를 **힙 `List` 에 전량 적재**하는 F-024 계약이고 aml-svc 최대 힙이 약 1 GiB 이기 때문이며, **소스 단위 분할 임포트(`getConsolidatedFile?includes=<source_id>` 서버측 필터 동작은 실증됨)는 미구현·후속 재설계 대상**이다. `active_version IS NULL` 이므로 이 소스는 **48h freshness fail-closed 게이트 범위 밖**(미적용 소스)에 머물러 타 소스 스크리닝을 차단하지 않는다. 실측·파서 계약 상세는 integration §7.4 「실 피드 스키마 실측 정정」. | — |
| V63 | `V63__alert_retirement.sql` | **설정형 보고룰 회수에 따른 TM 알림 회수 축(2026-08-06, feature/aml-configurable-rule-retire, §3.10·§5.7)**: 설정형 STR/CTR 룰이 `:retire` 4-eyes 로 회수되면(API §2.4) 그 룰이 만든 TM 알림을 운영 목록·룰 효과성 통계에서 빼되 **행은 감사 목적으로 보존**해야 하므로 (1) `aml_alerts` 에 `retired_at TIMESTAMPTZ`·`retired_by VARCHAR(128)`·`retire_reason VARCHAR(256)` 3컬럼 **additive** 추가(전부 NULL 허용, 기존 행·비-RETIRED 는 NULL). `disposition_reason`/`disposition_actor`(V30)는 오탐 종결 전용 의미라 **재사용하지 않는다**. (2) status CHECK 2종(`aml_alerts_status_check`·`ck_aml_alerts_status` — V1 baseline 이 같은 값 집합으로 둘 다 건다)을 기존 6종 **∪ `RETIRED` 합집합**으로 재정의(기존 행 보존 — V7 `ck_aml_alerts_scenario_code` 선례와 동일 방향). (3) 회수 축 불변식 `ck_aml_alerts_retired_at`: `status='RETIRED'` ⟺ `retired_at IS NOT NULL`(양방향). (4) 회수 대상 조회·기본 제외 목록용 `ix_aml_alerts_scenario_code(tenant_id, scenario_code, status)`. **데모 데이터 DELETE/UPDATE 는 넣지 않는다** — 기존 시뮬레이터 룰 잔재(`AML_SIM_*`)의 실제 회수는 REST `:retire` 1회 실행이 담당한다(REST-only 원칙). 멱등(`ADD COLUMN IF NOT EXISTS`·CHECK `DROP IF EXISTS` 후 재생성·`CREATE INDEX IF NOT EXISTS`). V1 baseline 무수정(추가 마이그레이션 규약). | V1~V62 |
| V64 | `V64__wlf_candidate_blocking_indexes.sql` | **WLF 후보 조회(candidate recall) 인덱스 기반 블로킹(2026-08-11, fix/aml-scheduler-starvation-wlf-index)**: dilisense 임포트로 `aml_watchlist_entries` 가 약 1.6M 행이 되자 후보 회수 질의(`WatchlistEntryJpaAdapter.CANDIDATE_RECALL_SQL`)의 **S2 TOKEN_OVERLAP·S4 PHONETIC 이 인덱스를 전혀 타지 않았다**(라이브 EXPLAIN: S2 ACTIVE 슬라이스 전량 per-row jsonb 언네스트 cost 580,865 · S4 Parallel Seq Scan cost 157,978). statement timeout(10s) 반복 + 후보 상한(200) 상시 절단 = 미탐 벡터. (1) **IMMUTABLE·STRICT·PARALLEL SAFE 함수 `aml.wlf_lower_text_array(jsonb) → text[]`** 신설 — jsonb 문자열 배열을 소문자 `text[]` 로 정규화(배열이 아닌 jsonb 는 오류 대신 빈 배열 → 해당 행만 후보 제외). 소문자화를 인덱스 안에 둔 것이 핵심으로, dilisense 는 `normalized_tokens` 를 대문자로 적재하므로(표본 20,000행 중 18,804행 비소문자) 대소문자 구분 `@>` 로 갔으면 1.5M 행이 통째로 미탐이 된다. (2) **GIN 표현식 인덱스 2종** `gin_wle_tokens_lower`(`wlf_lower_text_array(normalized_tokens)`)·`gin_wle_phonetic_lower`(`wlf_lower_text_array(phonetic_codes)`) — 질의 술어를 배열 중복(`&&`)으로 전환(`?|` 는 Hibernate 네이티브 쿼리 파서가 `?` 를 바인드 마커로 읽어 사용 불가). (3) `SET LOCAL maintenance_work_mem='256MB'` + 빌드 후 `ANALYZE`. **실측 4,490ms→73ms**(다양한 이름)·7,944ms→1,320ms(상위 3% 공통 토큰). 재현율은 S2 술어 동치·S4 상위집합으로 구조적 보존. 스키마 컬럼 변경 없음·additive·멱등(`IF NOT EXISTS`). | V1~V63 |
| V65 | `V65__outbox_dead_lettered_status.sql` | **아웃박스 종료 상태 `DEAD_LETTERED` 도입(§3.15·§5.17)**: 재시도 상한(기본 5)이 서비스 상수로만 선언되고 claim 술어·전이 어디에도 없어, 상한을 넘긴 행이 `FAILED` 로 남아 백오프마다 다시 claim 됐다(라이브 실측 `attempt=48`, 외부 webhook 무기한 중복 호출). `aml_outbox_status_check` 를 `DROP CONSTRAINT IF EXISTS` 후 **5종**(`PENDING`/`DISPATCHING`/`DISPATCHED`/`FAILED`/`DEAD_LETTERED`)으로 재생성하고 `status` 컬럼 COMMENT 갱신 — `DEAD_LETTERED` 는 재시도 예산 소진 종료(DLQ)이자 **claim 대상 밖**이다(fds `fds_webhook_outbox` 의 DLQ 어휘와 동일). **데이터 전이 없음** — 기존 attempt 초과 행은 릴레이의 수렴 스윕이 코드 경로로 종료시킨다(DB 직접 수술 금지). additive·멱등. | V1~V64 |
| V66 | `V66__outbox_claim_token.sql` | **아웃박스 클레임 소유권 토큰 `claim_token`(§3.15)**: V65 까지의 전이 CAS 는 "`status='DISPATCHING'` + 리스 미만료"를 소유권으로 썼는데 이는 "누군가 정당하게 들고 있다"일 뿐 "내가 들고 있다"가 아니어서, 리스가 만료된 릴레이의 늦은 전이가 그 사이 재claim 한 **다른** 릴레이의 활성 리스를 풀거나(중복·병렬 배달) 배달 중인 행을 종료시켰다. `aml_outbox` 에 `claim_token uuid` NULL 허용 컬럼 additive 추가(`ADD COLUMN IF NOT EXISTS`) + COMMENT. claim 1회당 `gen_random_uuid()` 로 새로 발급(`claimBatch`·stale 회수 클레임)하고 전이 3종(`markDispatched`·`markFailed`·`markDeadLettered`)이 `claim_token IS NOT DISTINCT FROM :expectedClaimToken` 으로 대조해 **자기 클레임일 때만** 적용, 전이 성립 시 `claim_token = NULL`(클레임 해제). 기존 행은 NULL 로 남으며 stale 회수 클레임이 새 토큰을 발급해 가져간다(데이터 전이 불필요). additive·멱등. | V1~V65 |
| V67 | `V67__sanctions_sync_run_lease.sql` | **제재 sync run 리스(소스 단위 상호배제 + run 소유권) + unchanged 스킵 로컬 무결성 인덱스(§3.6)**: 스케줄러 sweep 와 admin REST 트리거(또는 두 인스턴스)가 겹치면, 늦게 실패한 run 의 보상(캐리오버 복원)이 먼저 승격한 run 의 `active_version` 에서 행을 빼내 이전 version 으로 되돌려 그 주체들이 제재 명단에서 사라졌다(미탐). 게다가 publisher version 이 같으면 다음 sync 가 unchanged 로 스킵해 구멍이 고착됐다. (1) `aml_watchlist_sources` 에 컬럼 2종 additive 추가(`ADD COLUMN IF NOT EXISTS`, 둘 다 nullable) — `sync_run_id uuid`(현재 동기화 중인 run 의 소유권 토큰, NULL=진행 중 run 없음)·`sync_lease_expires_at timestamptz`(리스 만료 시각, 만료된 리스만 다른 run 이 이어받음 → 프로세스 사망 시 자가 치유) + COMMENT 2건. 획득은 "리스가 비었거나 만료됐을 때만" 조건부 UPDATE, 배치 적재·승격·보상은 전부 `sync_run_id = :myRunId` CAS 통과 후에만 실행. 값의 계산은 DDL이 아니라 어댑터가 소유하며 DB `now()` + TTL로 고정되고 TTL 0 이하는 `-infinity` 즉시 만료로 처리한다(신규 Flyway 없음). 세션/트랜잭션 advisory lock 은 배치마다 커넥션·트랜잭션이 바뀌는 수십 분짜리 멀티 트랜잭션 run 을 감싸지 못해 채택하지 않았다. (2) 부분 인덱스 `ix_wle_active_version (tenant_id, source_code, version) WHERE status='ACTIVE'` — "ACTIVE 행은 전부 `active_version` 에 있다" 불변식 판정(min/max 두 번)을 위해. publisher version 이 같아도 이 불변식이 깨져 있으면 스킵하지 않고 재적재해 치유한다. (3) 빌드 후 `ANALYZE`. additive·멱등(`IF NOT EXISTS`). | V1~V66 |
...
>
> **bo-api(스키마 `bo`) CTR/STR 마이그레이션(참고 — 데모 백오피스 소관, 코드=truth)**: bo-api 는 `services/bo-api/src/main/resources/db/migration/` 에 별도 체인(V1 baseline·V2 seed·V3 hanpass_demo_scope·V4 fds_hanpass_connector)을 두며, CTR/STR 모니터링은 다음 3개를 additive 로 얹는다 — **V5 `V5__ctr_str_rules_foundation.sql`**(`backoffice.aml_ctr_thresholds`·`backoffice.aml_ph_banking_calendar` 생성 + `platform`·`tenant_demo` CTR 임계 PHP 500,000/KRW 10,000,000 + 2026 PH 고정일 공휴일 7종 시드; 룰 카탈로그는 코드), **V6 `V6__ctr_str_monitoring_audit_events.sql`**(`backoffice.bo_audit_logs` `chk_bo_audit_logs_event` CHECK 에 P4 이벤트코드 3종 추가 — `CTR_THRESHOLD_CHANGE_SUBMITTED`·`REPORT_RULE_ACTIVATE_SUBMITTED`·`AMLC_SUBMISSION_DELEGATED`, 기존 allowlist 전량 보존 후 append), **V7 `V7__ph_banking_calendar_2026_movable_holidays.sql`**(aml-svc V6 대칭 — `platform`·`tenant_demo` 2026 이동 공휴일 11종씩 additive 시드). bo-api `CTR_THRESHOLD`는 엔진 연결 시 aml-svc 결재 상신으로 위임하고, 비운영 미연결 환경에서만 스텁 스토어(`AmlStubStore`) fallback을 사용한다. `REPORT_RULE`은 bo-api 애플리케이션 enum + 스텁 결재 경계를 유지한다. 이어 국가위험 일일 수집이 **V8 `V8__country_risk_import_audit_event.sql`** 을 additive 로 얹는다 — `backoffice.bo_audit_logs` 의 `chk_bo_audit_logs_event` CHECK 재생성(V6 allowlist 전량 보존 verbatim + 신규 이벤트코드 1종 append): **`COUNTRY_RISK_IMPORT_TRIGGERED`**(bo-web `POST .../country-risk:import` 수동 트리거 감사, `AmlCountryRiskService#triggerImport` emit — `AuditEventConstraintTest` 가드). 이어 CTR/STR 룰 파라미터 편집이 **V9 `V9__aml_report_rule_params.sql`** 을 additive 로 얹는다 — ① `backoffice.aml_report_rule_params` 생성(aml-svc V22 §3.22e 와 동형, PK `(tenant_id, rule_code, param_key)`·`NUMERIC(20,6)` — 🔒 `REPORT_RULE_PARAM` 4-eyes EXECUTED 반영분의 데모 stub 로컬 폐루프 store, CTR 임계는 `aml_ctr_thresholds` 정본 재사용·비저장), ② `chk_bo_audit_logs_event` CHECK 재생성(V8 allowlist 전량 verbatim 보존 + 신규 이벤트코드 1종 append: **`REPORT_RULE_PARAM_CHANGE_SUBMITTED`**). bo-api `AmlApprovalDtos.SubjectType` 은 21→**22종**(`REPORT_RULE_PARAM` — 승인선 COMPLIANCE, 룰 단위 파라미터 셋 원자 제출)이 되며 엔진 `aml_approvals.subject_type` CHECK 는 `CTR_THRESHOLD` 포함 20종이다. 이어 RA 상세 관리자 액션(즉시 재이행)이 **V10 `V10__due_diligence_reissue_audit_event.sql`** 을 additive 로 얹는다 — `chk_bo_audit_logs_event` CHECK 재생성(V9 allowlist 전량 verbatim 보존 + 신규 이벤트코드 1종 append: **`DUE_DILIGENCE_REISSUE_REQUESTED`**, `AmlReissueService#requestReissue` emit — `AuditEventConstraintTest` 가드. 라이브 검증에서 allowlist 누락이 위임 성공 후 감사 기록 500 을 유발해 적발·수정). 이어 RA 당연고위험 등재가 **V11 `V11__hrr_registration_audit_event.sql`** 을 additive 로 얹는다 — ① `chk_bo_audit_logs_event` CHECK 재생성(V10 allowlist 전량 verbatim 보존 + 신규 이벤트코드 1종 append: **`HRR_REGISTRATION_SUBMITTED`**, `AmlHighRiskRegistryService#submitRegistration` emit — `AuditEventConstraintTest` 가드), ② `bo_approval_routes` 에 `('platform', 'HRR_REGISTRATION', 1, 'EXECUTIVE_APPROVAL', required)` 라우팅 seed 멱등 삽입(`ON CONFLICT DO NOTHING` — 승인선 실 결정은 서비스 `approvalLineFor` 소관, seed 는 bo-web 결재 라우팅 뷰 정합용). 이로써 bo-api `AmlApprovalDtos.SubjectType` 은 22→**23종**(`HRR_REGISTRATION`, `TRAVEL_RULE_EXCEPTION` 미보유이므로 V31 무영향), 엔진 `aml_approvals.subject_type` CHECK 는 V28 에서 21종이었다가 **V31 에서 20종**(`TRAVEL_RULE_EXCEPTION` 제거)이 된다. 이어 FDS 룰 변수 편집(SFDS-RULE-002, docs/plan/01)이 **V12 `V12__fds_rule_param_audit_event.sql`** 로 `chk_bo_audit_logs_event` 에 `FDS_RULE_PARAM_CHANGE_SUBMITTED` 1종을 append 한 뒤, 알림 lifecycle 처분(alert-triage-disposition)이 **V13 `V13__alert_disposition_audit_events.sql`** 을 additive 로 얹는다 — `chk_bo_audit_logs_event` CHECK 재생성(V12 allowlist 전량 verbatim 보존 + 신규 이벤트코드 **4종** append: **`AML_ALERT_TRIAGED`·`AML_ALERT_DISMISSED`·`AML_ALERT_ESCALATED`·`AML_ALERT_STR_RECOMMENDED`**, `AmlTmService` 의 알림 처분 위임 4액션 emit — `AuditEventConstraintTest` 가드). 4종 모두 단일 행위자 lifecycle 전이(비-4-eyes, 가정 G2 — 케이스 `:close`(EDD_CLOSE)·FDS CASE_CLOSE 만 2인 결재 유지)이며 오탐 종결(`AML_ALERT_DISMISSED`)이 룰 효과성 오탐율(§12-B.3)의 실 분모를 만든다. 이어 FDS 룰 아카이브가 **V21 `V21__fds_rule_archive_audit_event.sql`**(`FDS_RULE_ARCHIVE` 1종 append)을 얹은 뒤, 케이스↔STR 보고 연계·PEP 경영진 승인 상신이 **V22 `V22__case_str_draft_and_pep_approval_audit_events.sql`** 을 additive 로 얹는다 — `chk_bo_audit_logs_event` CHECK 재생성(V21 allowlist 전량 verbatim 보존 + 신규 이벤트코드 **2종** append): **`CASE_STR_DRAFT_LINKED`**(`AmlReportService#draftStrForCase` — `POST .../reports/str-drafts`, 기능정의서 §12-A.7 BR-002 케이스-연계 STR DRAFT 생성/연결 emit)·**`AML_PEP_APPROVAL_SUBMITTED`**(`AmlPepApprovalService#submit` — `POST customers/{ref}:submit-pep-approval` emit). 둘 다 **방출 중이었으나 allowlist 에 없어** 감사 INSERT 가 CHECK 제약 위반(SQLState 23514)으로 터졌고, 특히 `CASE_STR_DRAFT_LINKED` 는 aml-svc 위임 쓰기가 이미 커밋된 뒤 감사 단계에서 터져 **호출자는 500 인데 케이스-보고 링크는 실제로 맺어지는 비원자적 상태**를 만들었다(V10 `DUE_DILIGENCE_REISSUE_REQUESTED` 와 동일 계열 사고의 재발). 재발 방지로 bo-api 에 소스 파생 빌드-타임 게이트 `AuditEventEmissionAllowlistTest` 를 신설했다 — `src/main/java` 의 `AuditLogService.record*` 호출에서 event 인자를 괄호 균형 파싱으로 추출해 최신 CHECK 마이그레이션 allowlist 와 대조하며, DB·환경변수 없이 항상 돈다(손으로 유지되던 `AuditEventConstraintTest` 목록은 두 코드를 놓치고 있었다). 아울러 되돌릴 수 없는 외부 효과가 커밋된 뒤의 감사 append 는 `AuditLogService#recordAfterCommittedEffect`(REQUIRES_NEW 프록시 경유 + 실패 격리·ERROR 로그)로 분리해 업무 결과를 오염시키지 않게 했다 — 감사가 fail-closed 통제인 경로(`PiiRevealService` 의 `RAW_DATA_ACCESS`)는 기존 엄격 전파(`record`)를 유지한다.

> 이어 bo-api **V14 `V14__remove_travel_rule.sql`**은 Travel Rule 메뉴/권한을 제거하고, **V15 `V15__wlf_engine_menu.sql`**은 AML-WLF-005 `WLF 엔진 조절`(`/aml/wlf-engine`, 설정 영역 sort 435)과 `BO_SUPER_ADMIN`·`AML_POLICY_ADMIN` 권한을 additive 등록한다. WLF 설정값 자체는 bo-api DB에 복제하지 않고 aml-svc §3.14 Policy Pack만 사용한다. 같은 형태로 **V23 `V23__webhook_credential_menu.sql`**(2026-08-10)은 AML-WHK-001 `콜백 자격증명`(`/aml/webhook-credential`, 설정 영역 sort 495)을 `bo_menu` 에, `BO_SUPER_ADMIN`(전권)·`AML_POLICY_ADMIN`(read/create/update)·`AML_VIEWER`(read) 권한을 `bo_menu_permissions` 에 additive·멱등(`ON CONFLICT DO UPDATE`) 등록한다 — **런타임 인가는 계속 scope 기반**(bo-api `@PreAuthorize` — 읽기 `aml:case:read`\|`aml:admin:policy`, 쓰기 `aml:admin:policy`)이고 이 행들은 동적 메뉴 카탈로그를 bo-web 정적 NAV 트리(`lib/nav.ts` `aml-config-connect`)와 일치시키기 위한 것이다. 자격증명 값 자체는 bo-api DB에 복제하지 않고 aml-svc §3.15 `aml_api_credentials`(`credential_type=WEBHOOK`) 정본만 위임 조회한다(API §2.7a). 이어 기준통화 프로파일 apply 오케스트레이션(다통화, PLAN 20260818 U5)이 **V24 `V24__currency_profile_apply_audit_event.sql`** 을 additive 로 얹는다 — `chk_bo_audit_logs_event` CHECK 재생성(V22 allowlist 전량 verbatim 보존 + 신규 이벤트코드 **2종** append, V23 은 이 CHECK 를 재정의하지 않으므로 V22 를 직전 기준으로 삼는다): **`CURRENCY_PROFILE_APPLY_SUBMITTED`**(`CurrencyProfileApplyService#apply` — `POST .../tenants/{tenantId}/currency-profile:apply` 총괄 감사, `AuditLogService#recordAfterCommittedEffect` 경유·실패 격리, F-030 ② 계약)·**`FDS_TENANT_REGULATORY_CURRENCY_CHANGE_SUBMITTED`**(`FdsTenantCompliancePolicyService#submitRegulatoryCurrencyChange` — STEP `FDS_REGULATORY_CURRENCY` 위임 상신, V22 `FDS_POLICY_PACK_TOGGLE_SUBMITTED` 동렬). `aml-svc`(스키마 `aml`) DDL 변경 0. **(2026-09-04, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095)** HRR 등재 해제 경로가 **V25 `V25__hrr_deregistration_audit_event.sql`** 을 additive 로 얹는다 — `chk_bo_audit_logs_event` CHECK 재생성(V24 allowlist 전량 verbatim 보존 + 신규 이벤트코드 1종 append): **`HRR_DEREGISTRATION_SUBMITTED`**(`AmlHighRiskRegistryService` 등재 해제 위임 상신 emit — `AuditEventEmissionAllowlistTest` 가드). `aml-svc`(스키마 `aml`) 은 §7 표 아래 별도 마이그레이션(등재 해제 유스케이스는 기존 `aml_approvals`·`aml_high_risk_registry_items` 컬럼 재사용, 신규 DDL 0)으로 처리되며 본 절은 bo-api(`bo`) 감사 allowlist 항목만 다룬다.
>
> **aml-svc(스키마 `aml`) — 다통화(법인별 자국통화) 기준통화 프로파일 REST 계약(신규 마이그레이션 0, PLAN 20260818 U1).** `report_cutoff_time` 값 기입은 신규 전용 `PUT /api/v1/admin/aml/tenants/{tenantId}/policy-binding/report-cutoff-time`(항상-쓰기·명시 null 허용) 경로로 옮겨지고, 기존 `PATCH .../policy-binding` 은 거래성 이력 보유 테넌트의 `baseCurrency` 변경을 fail-closed(422 `AML.TENANT_CURRENCY_HISTORY_LOCKED`)로 거부한다. 신규 raw `GET .../policy-binding`(12키 read-back)·엔진 CTR 임계 read 2종(`GET .../ctr-thresholds[/{currency}]`)도 추가됐다 — 전부 `aml_tenants`/`aml_ctr_thresholds` 기존 컬럼 조합이며 신규 컬럼·마이그레이션은 없다(API §2.7 동기화).
>
...
| API 인증 자격증명 + webhook 콜백 URL | aml_api_credentials(구현 V2, credential_type 4종, `secret_ciphertext`·`webhook_url`(V17), 알려진 demo fingerprint ciphertext 격리 V45) — `WEBHOOK enabled` 행이 콜백 URL 정본(integration §3.4·API §8). aml_source_systems 에 webhook URL 없음. **`WEBHOOK` 행 등재·교체는 admin REST `GET\|PUT /api/v1/admin/aml/webhook-credential`(API §2.7a, 2026-08-10) — DDL 변경 0·`scopes=[]` 고정**(운영자 표면은 bo-api 위임 `GET\|PUT /api/v1/bo/aml/webhook-credential` → 화면 AML-WHK-001, 2026-08-10 — 이 엔진 경로가 여전히 유일한 등재 정본이고 bo-api 는 값을 복제하지 않는다. 미등록 테넌트 쓰기는 400 `unknown tenant` 로 거부). master key online rotation은 P1-03 |
...
| 일자 | 변경 | 비고 |
|---|---|---|
| 2026-09-22 | **§3.9 `aml_risk_models` 후주 신설(비-`tenant_demo` RA 모델 baseline v1 통화 팩 시드) + §3.6 註 갱신(공개 제재명단 필수 2종→6종, 코드=truth, aegis-aml 커밋 `50458a8`(U8)·`e45328f`(FX-2)).** (1) 비-`tenant_demo` 통화 팩(AUD/KRW/JPY)이 REST 최초 생성 불가 제약을 우회해 `aml_risk_models` 에 `KR_DEFAULT_RA` v1(ONBOARDING, APPROVED, is_default)·`KR_ONGOING_RA` v1(ONGOING, APPROVED)을 정본 `config/ra-models/baseline.json`(`tenant_demo` V2/V11/V12/V19/V20/V34/V35 최종 상태 재현, 통화·관할 중립)으로부터 멱등 시드하고 이후 버전 개정은 REST 4-eyes(`versions:copy→simulate→activate`)만 소유함을 §3.9 에 명문화(php `tenant_demo` 팩은 무변경). (2) §3.6 소스 행 생성 경로 註를 "필수 소스(OFAC_SDN·UN_CONSOLIDATED)만 등록"에서 **공개 제재명단 소스 6종(필수 2 + 비필수 `EU_CFSL`·`UK_OFSI`·`AU_DFAT`·`JP_MOF_FEFTA`, V60 이 `tenant_demo` 에 시드하는 것과 동일 집합) 중 미등록분 등록, 비필수 등록 실패는 §3.6a 등급 계약대로 흡수**로 갱신. 스키마·마이그레이션 변경 없음(팩 자산 행 삽입 + 시뮬레이터 서술만). | 코드 truth=aegis-aml `services/aml-svc/src/main/resources/db/currency/{aud,krw,jpy}/R__currency_reference_configuration.sql`·`config/ra-models/baseline.json`·`scripts/demo_ingest.py#ensure_watchlists`. `docs/aml-fds-workflow-guide.md` ⓪·⓪′ 동일 작업 단위, API 무변경(기존 REST 재사용). |
| 2026-09-22 | **§3.6 `aml_watchlist_sources` 소스 행 생성 경로 후주 신설(코드=truth, 마이그레이션 신규 없음, aegis-aml PLAN fix-20260922-au-profile-gaps U5·U6).** 비-`tenant_demo` 스택(다통화 프로파일 등)은 V8/V60/V62 시드가 없어 필수 공개 제재 소스가 0건이던 것을, 시뮬레이터 `demo_ingest.ensure_watchlists()` 가 sync 전에 미등록 필수 소스(OFAC_SDN·UN_CONSOLIDATED)만 `POST /api/v1/admin/aml/watchlist-sources` 로 REST 등록(CLAUDE.md §데모 시드 절대 금지 — 통화 팩은 이 테이블을 시드하지 않는다)하도록 역전파. 스키마·마이그레이션 변경 없음(서술만) | db-designer. 코드 truth=`scripts/demo_ingest.py#ensure_watchlists`(aegis-aml 커밋 `0aeda43`). `docs/aml-fds-workflow-guide.md` ⓪ 행 동일 작업 단위, API 무변경(기존 REST 재사용). |
| 2026-09-11 | **§3.9·§3.20 후주 정정 — 당연고위험(HRR) 레지스트리 floor 조회 포트 단일화 `findForcedFloor`(코드=truth, 마이그레이션 없음, aegis-aml PLAN 20260911-aml-hrr-floor-consolidation, 사용자 지시로 F-115·F-116 해제).** 구 부재판정 `isKnownUnregistered`(exists, 비잠금) → 등재 시 `find()` 전량 로드 2단 경로를 **단일 조회** `HighRiskRegistryReferenceItemJpaRepository.findByTenantIdAndSubjectRef`(인덱스 `ix_hrr_items_subject`, 비잠금·readOnly)로 통합 — 도메인 정적 팩토리 `HighRiskRegistry.forcedFloorFrom` 으로 조립한다. §3.19 `findForUpdate` 잠금 순서 불변식 무영향, 인덱스·스키마·마이그레이션 신규 없음(서술 정정만). | db-designer. 코드 truth=`application/port/out/HighRiskRegistryStorePort#findForcedFloor`·`domain/registry/HighRiskRegistry#forcedFloorFrom`·`adapter/out/persistence/{HighRiskRegistryJpaAdapter,HighRiskRegistryReferenceItemJpaRepository}`·`application/usecase/OnboardingRaDerivationService`. 설계서 §24 항목 7·PRD §12-A.4 BR-004 ① 동일 작업 단위, API §3.3 무변경. |
| 2026-09-10 | **§3.20 `ix_hrr_items_subject` 인덱스 활용 후주 신설 — 온보딩 RA 파생 조회 fast path(코드=truth, 마이그레이션 신규 없음, aegis-aml PLAN 20260910-aml-hrr-floor-followups).** F-115 QA 로그 후속(M2) — 온보딩 파생이 CDD 인입마다 헤더+아이템 전량 로드(`find`)를 수행하던 것을, 대다수 미등재 회원은 이 인덱스 기반 `EXISTS(tenant_id, subject_ref)` 부재판정(`HighRiskRegistryStorePort#isKnownUnregistered`)으로 전량 로드를 생략하도록 정정 — 등재가 입증되지 않은 경우만 `find` 폴백. 인덱스·스키마 신규 없음(기존 인덱스 활용 서술만). | db-designer. 코드 truth=`application/port/out/HighRiskRegistryStorePort#isKnownUnregistered`·`adapter/out/persistence/{HighRiskRegistryJpaAdapter,HighRiskRegistryReferenceItemJpaRepository}`·`application/usecase/OnboardingRaDerivationService`. API §3.3·PRD §12-A.4 BR-004 ①·§12-B.6 BR-005 동일 작업 단위. |
| 2026-09-10 | **§3.9 당연고위험(HRR) 레지스트리 floor 온보딩 파생 병합 후주 신설(코드=truth, 마이그레이션 신규 없음, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd).** 온보딩 파생(`OnboardingRaDerivationService`)이 대상 회원의 당연고위험 레지스트리(§5.33) 등재 여부를 조회하지 않아, 명단(WLF) 매치가 소멸한 등재 회원의 재CDD 가 `mandatory_high_risk=false` 로 착지하던 결함을 해소 — 국가위험/명단 매치 floor 파생 직후 레지스트리 floor(tier `HIGH→HIGH`·`VERY_HIGH→PROHIBITED`)를 병합(`mandatory_high_risk_reasons`에 참조 리스트 이름 추가·`factor_breakdown.forcedFloor.evidence`에 `{listType,subjectRef,tier}` 추가), 최초 CDD·재이행 re-CDD·국가정책 재평가 공통 적용. 조회는 부재판정 `isKnownUnregistered`(exists, 비잠금) 후 등재 시에만 `find()`(비잠금)를 사용(§3.19 잠금 순서 불변식 무영향). 스키마·마이그레이션 신규 없음(기존 V13 컬럼·`factor_breakdown` JSONB 재사용). | db-designer. 코드 truth=`application/usecase/OnboardingRaDerivationService`·`domain/registry/HighRiskRegistry#forcedFloorFor`. API §3.3(`mandatoryHighRiskReasons`)·PRD §5(d)·§5.1 BR-013·§12-A.4 BR-010·§12-B.6 BR-010 동일 작업 단위. |
| 2026-09-10 | **§3.22f `aml_member_cdd_history.history_type` CHECK 7종(`MEMBER_DECISION` 추가)·§5.36 7종·§7 V78 행·회원관리 원장 요약 「AML 심사결과」 타일(코드=truth, aegis-aml PLAN 20260910-aml-member-ledger-decision-history).** 회원 심사결과 확정 3경로(`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`)가 같은 트랜잭션 안에 `aml_member_cdd_history` 에 `MEMBER_DECISION` 1행을 append(`details={decision, source, decisionId, approvalId?, caseId?}`) + 기존 `aml_member_decisions` 전 행 backfill(멱등, `details->>'decisionId'` 존재 여부로 중복 방지). 적재 지점 3종→4종. `aml_customers.kyc_status` 는 계정계 소유로 무변경(원인 — AML 심사결과와 KYC 상태는 별개 축, PRD §12-A.10 BR-004). | db-designer. 코드 truth=`V78__member_cdd_history_member_decision.sql`·`domain/enums/CddHistoryType`·`application/usecase/{MemberLedgerService,MemberDecisionService}`. API §2.x(회원원장 read)·PRD §12-A.10 BR-001/BR-004 동일 작업 단위. |
| 2026-09-09 | **§3.22g `aml_member_decisions.source` CHECK `ADMIN_DIRECT` 추가·§7 V77 행(코드=truth, aegis-aml PLAN 20260909-aml-ra-direct-member-decision).** 당연고위험(HRR) 등재가 경영진 결재로 이미 EXECUTED 된 회원에 한해, RA 상세 관리자 액션이 4-eyes 결재 없이 회원 심사결과를 직접 확정(`source=ADMIN_DIRECT`, `approval_id`/`case_id`/`case_type`/`final_status` 모두 NULL)하는 새 append 경로를 허용. 제약 재정의만(DROP+ADD CHECK), 신규 테이블·컬럼·인덱스 없음, 파괴적 변경 0. | db-designer. 코드 truth=`V77__member_decisions_admin_direct_source.sql`·`domain/decision/MemberDecision#fromAdminDirect`·`application/usecase/DirectMemberDecisionService`. API §2.3·§8.1·PRD §12-A.4 BR-009 동일 작업 단위. |
| 2026-09-07 | **§3.6b `paused_at`/`paused_by`·§7 V76(코드=truth, aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit).** 재스크린 워커 인프라 타임아웃 서킷과 함께 운영자 일시정지 컬럼 additive. | db-designer. 코드 truth=`V76__rescreen_job_pause.sql`·`RescreenTargetJpaAdapter#claimDueTargets`. API §2.7·integration §3.1c·설계서 §10.5 동일 작업 단위. |
| 2026-09-06 | **§3.22h `aml_fds_transaction_decisions` 신설·§7 V75 행(코드=truth, aegis-aml PLAN 20260906-fds-block-aml-propagation).** FDS 차단 판정의 거래 단위 사실 기록(append-only, 알럿·케이스 무개설), `UNIQUE(tenant_id, fds_decision_id)` 멱등, 거래 조회 LATERAL 인덱스, RLS 2정책. | db-designer. 코드 truth=`V75__fds_transaction_decisions.sql`. API §2.4/§3.10a·integration §3.2 v4.17 동일 작업 단위. |
| 2026-09-04 | **§3.20 등재 해제 DELETE+version+1 주석·§5.16 `subject_ref` `DEREGISTER\|` 접두 표기·bo-api 감사 allowlist V25 등재(코드=truth, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095). aml-svc DDL 신규 0(기존 컬럼 재사용).** §3.20 `aml_high_risk_registry_items` 후주에 등재 해제 EXECUTED 시 해당 항목 DELETE + 헤더 `version+1` + 온보딩 CDD 스냅샷 재파생 서술 추가. §5.16 `HRR_REGISTRATION` 행에 해제 상신 `subject_ref` 접두 `DEREGISTER\|customerRef`(선례 `FP_WHITELIST` `REVOKE\|`·`TM_SCENARIO` `CUSTOM_RULE_RETIRE\|`) 표기 추가(CHECK 재생성 없음 — subjectType 신규 값 아님). bo-api(`bo`) 는 **V25 `V25__hrr_deregistration_audit_event.sql`** 을 additive 로 얹어 `chk_bo_audit_logs_event` CHECK 에 `HRR_DEREGISTRATION_SUBMITTED` 1종을 append. | 코드=truth. 근거=aegis-aml aml-svc `HighRiskCustomerRegistrationService`·`HighRiskRegistry#removeFromReferenceList`, bo-api `db/migration/V25__hrr_deregistration_audit_event.sql`·`AmlHighRiskRegistryService`. API §HRR admin surface·PRD §12-B.6 BR-009 동일 작업 단위. |
| 2026-09-04 | **§3.19 `aml_high_risk_registry` 헤더 행 writer 직렬화 잠금 주석 추가(코드=truth, PLAN 20260904-aml-hrr-registry-row-lock, aegis-aml F-094). DDL·마이그레이션 신규 0.** 결재 EXECUTED 실행 트랜잭션 내 `SELECT … FOR UPDATE` 로 헤더 행을 잠가 참조 리스트 변경·`HRR_REGISTRATION`·`PEP_APPROVAL` 세 writer 의 lost-update 를 차단한다는 서술을 추가했다. 잠금 순서는 `aml_approvals`→헤더→`aml_high_risk_registry_items`→`aml_customers`/`aml_risk_scores`. 컬럼·인덱스·제약 전부 무변경. | 코드=truth. 근거=aegis-aml aml-svc `HighRiskRegistryStorePort#findForUpdate`·`HighRiskRegistryJpaRepository#findByTenantIdForUpdate`·`HighRiskRegistryConcurrencyIntegrationTest`. API §HRR admin surface·PRD §12-B.6 BR-003 동일 작업 단위. |
...
| 2026-08-19 | **§7 bo-api 註 — 기준통화 프로파일 apply 감사 마이그레이션 V24 등재(코드=truth, PLAN 20260818-currency-profile-bo-setup U13). aml-svc(스키마 `aml`) DDL 변경 0.** bo-api 가 **V24 `V24__currency_profile_apply_audit_event.sql`** 을 additive 로 얹어 `chk_bo_audit_logs_event` CHECK 를 V22 allowlist 전량 보존 + `CURRENCY_PROFILE_APPLY_SUBMITTED`·`FDS_TENANT_REGULATORY_CURRENCY_CHANGE_SUBMITTED` 2종 append 로 재생성했다(V23 은 이 CHECK 를 재정의하지 않음). 종전 "변경 없음(신규 마이그레이션 0)" 서술을 이 행으로 교정한다 — bo-api 는 신규 마이그레이션 1건(V24), aml-svc 스키마는 무변경. | 코드=truth. 근거=bo-api `db/migration/V24__currency_profile_apply_audit_event.sql`·`aml/currencyprofile/service/CurrencyProfileApplyService`·`fds/service/FdsTenantCompliancePolicyService`. API §2.7·§3.16a·§4 동기화. |
| 2026-08-17 | **신고소득 presence·알럿 evidence window·관할 기한 blocked-state 정합화.** `kyc_evidence` 숫자 4키와 내부 presence marker, `aml_alerts.evidence.evidenceWindow` 동결 범위, PH만 계산하는 nullable `due_at`, 비PH 구조화 WARN, profile-owned cutoff 재적용을 명문화했다. | 코드=truth. DDL 신규 0. 근거=aegis-aml `DeclaredIncomePolicy`·`IdentityProjectionService`·`AlertEvidenceWindowResolver`·`ReportDeadlinePolicy`·V69 repeatable currency packs. API §3.4a/§3.6 동기화. |
| 2026-08-14 | **§3.3 `aml_customers` — CDD projection과 PEP 승인 pair의 양방향 컬럼 소유권 고정.** registration/re-CDD는 PEP pair·가입/생성 시각을 제외한 current-state만 원자 upsert하고, `PEP_APPROVAL EXECUTED`는 `is_pep`·`pep_approval_id` 두 컬럼만 tenant-scoped UPDATE한다. pair write 직후 current row refresh를 후속 registry RA full-save보다 앞세워 stale 승인 snapshot이 최신 re-CDD KYC를 되쓰지 않게 한다. | 코드=truth. 근거=aegis-aml `CustomerStorePort`·`CustomerJpaAdapter`·`RegisterCustomerService`·`PepApprovalService`·`PepApprovalIntegrationTest`. DDL/Flyway 신규 0. |
| 2026-08-14 | **§3.3 `aml_customers.kyc_evidence` — CDD 소득구간 미입력의 internal/current-state `UNKNOWN` projection 계약 역전파.** canonical `declaredIncomeBand` 입력 enum 4종은 그대로 유지하고, exact `customer.cdd.completed`에서 지원 KYC 차원 1개 이상이 남은 omission/null/blank만 projection `UNKNOWN`으로 정규화한다. sentinel-only 객체 금지, re-CDD full replace, raw event/hash 불변, replay 무부작용, no-backfill을 명문화했다. | 코드=truth. 근거=aegis-aml `IdentityProjectionService`·`DeclaredIncomeBandPolicy.UNKNOWN`·`CddOnboardingDecisionRestIntegrationTest`. JSONB 값 정책만 변경하며 DDL/Flyway 신규 0. API §CDD ingest/§3.9·기능정의서 v9.86 동기화. |
| 2026-08-13 | **§3.9 V12 註 — ONGOING `parameters.signalScaling` 선택키 사실 기록(코드=truth, aegis-aml `feature/pep-name-risk-score-scaling` `206b7558`). 스키마·DDL·컬럼·인덱스·제약 전부 무변경이며 신규 Flyway 파일 0 이다(실측).** 2차 상시 RA 의 알럿 기여도에 세 번째 항 `× m(nameScore) ≤ 1.0` 이 도입되면서 그 파라미터가 `aml_risk_models.parameters`(ONGOING) JSONB 안의 **선택키** `signalScaling` 으로 들어왔다 — 신규 컬럼·테이블이 아니라 기존 JSONB 키 추가이고, 키 부재 = 미적용(배수 항등원 1.0)이라 V12 시드를 포함한 기존 ACTIVE 정의는 파싱·동작 모두 바이트 동일하다. 스키마·불변식(`0 ≤ floorScore < ceilingScore ≤ 1` · `0 < minMultiplier ≤ maxMultiplier ≤ 1` · `rules` 비어있지 않음·중복 없음 · 룰코드 `(STR\|CTR\|FDS)_.+` · **`STR_SANCTION` 거부** · origin 키 패턴)과 읽기·쓰기 두 경로 fail-fast 를 V12 註에 명문화했다. 활성화 경로는 DB 시드가 아니라 **REST 4-eyes** 뿐이다(REST-only 원칙). 마이그레이션이 없다는 사실을 남기지 않으면 이후 스키마 확장과 V-파일 부재의 불일치를 누락으로 오독할 수 있다. | 코드=truth. 근거=aml-svc `domain/risk/{SignalScaling,OngoingRaParameters,RiskModelDefinitionValidator}`, `services/aml-svc/src/main/resources/db/migration` 신규 파일 없음(실측). 소프트웨어 설계서 §11.3a·API §2.3 동일 작업 단위. |
| 2026-08-13 | **§7 V48 註 — fan-out step 재무장 전이(`SUCCEEDED → RETRYING`) 사실 기록(코드=truth, aegis-aml main `3d3f8bc0`). 스키마·DDL·CHECK·인덱스 전부 무변경이며 신규 Flyway 파일 0 이다.** WLF 지연 성공 시 같은 job 의 신호 소비 step(`STR`·`ONGOING_RA`)을 `SUCCEEDED` 에서 `RETRYING`(attempt 0·due now·`last_error` NULL)으로 되돌리는 전이가 코드에 추가됐으나, 이 전이가 쓰는 상태값은 **V48 이 이미 정의한 `status` CHECK 6종 안에** 있고 대상 컬럼도 기존 컬럼뿐이다. 마이그레이션이 필요 없다는 사실 자체를 남기지 않으면 이후 "상태 전이가 늘었는데 V-파일이 없다" 를 누락으로 오독할 수 있어 V48 행에 註로 고정한다. 재무장된 행은 기존 claim 부분 인덱스(`ix_aml_txn_fanout_steps_claim`)에 자연 편입된다. 처리 계약 정본은 연동 §3.1b-1. | 코드=truth. 근거=aml-svc `application/port/out/FanoutJobStorePort.rearmSucceededSteps`·`adapter/out/persistence/FanoutJobJpaAdapter`(`UPDATE … WHERE status='SUCCEEDED'`)·`domain/fanout/NeutralFanoutSteps.signalConsumersOf`. `services/aml-svc/src/main/resources/db/migration` 에 신규 파일 없음(실측). 소프트웨어 설계서 §15.9 동일 작업 단위. |
| 2026-08-12 | **V64~V67 역전파 + `aml_outbox`·`aml_watchlist_sources` 컬럼 정합(코드=truth, aegis-aml main `596ec1ab`·`beba9987`·`a03e4c77`·`0f6c253e`, staging/round15).** (1) **§7 마이그레이션 표에 V64~V67 4행 추가** — V64 WLF 후보 조회 블로킹 GIN 2종 + `aml.wlf_lower_text_array(jsonb)` 함수(실측 4,490ms→73ms), V65 아웃박스 종료 상태 `DEAD_LETTERED`, V66 아웃박스 `claim_token`, V67 제재 sync run 리스(`sync_run_id`·`sync_lease_expires_at`) + `ix_wle_active_version`. §7 서두의 1:1 파일 범위를 `V41~V62`→**`V41~V67`** 로 갱신하고 V63~V67 을 additive 목록에 편입(직전 표는 V63 행만 있고 서두 범위가 밀려 있었다). (2) **§3.15 `aml_outbox`** — `status` 4종→**5종**(`DEAD_LETTERED`), `claim_token uuid` 행 신설, `attempt` 에 재시도 상한(claim 술어 내장·기본 5) 서술, `next_attempt_at` 의 `DISPATCHING` 중 리스 재해석 서술. **`published_at`→`dispatched_at` 컬럼명 정정**(V1 baseline·`OutboxJpaAdapter` 실제 컬럼과 불일치하던 드리프트). (3) **§5.17 outbox_status 5종 + 상태 기계·종료 상태 2종 註**. (4) **§3.6 `aml_watchlist_sources`** — `sync_run_id`·`sync_lease_expires_at` 2행 신설. (5) **§4 인덱스 설계 3행 추가**(`gin_wle_tokens_lower`·`gin_wle_phonetic_lower`·`ix_wle_active_version`). (6) **§7 표 누락분 V47 행 보정** — `V47__rls_tenant_isolation.sql`(P0-13 RLS)은 §1.2 서술과 V48 의존 표기가 이미 참조하고 있었는데 표에는 행이 없어 1:1 이 깨져 있었다. 이로써 **§7 표 = 실제 파일(V1~V37·V41~V67 sql + V38 Java) 완전 1:1**. (7) **§7 V62 註 (4) 「분할 임포트 미구현·후속 재설계 대상」 해제** — 스트리밍 인제스트로 대체(integration §7.4), 「미임포트·`active_version IS NULL`」 서술은 당시 상태 기록임을 명기. | 코드=truth. 근거=`services/aml-svc/src/main/resources/db/migration/V64~V67`·`domain/enums/OutboxStatus`·`domain/outbox/OutboxRetryPolicy`·`global/config/OutboxRetryConfig`·`adapter/out/persistence/OutboxJpaAdapter`·`application/usecase/SanctionsSyncLease`. integration §7.4·§8.1 동일 작업 단위 동기화 |
| 2026-08-10 | **§7 V62 註 — `DILISENSE_CONSOLIDATED` 현재 상태 「미임포트·`active_version=NULL`」 사실 기록(코드=truth, aegis-aml main `e29f9ab0`). DDL·마이그레이션 신규 0.** 사용자가 API 키를 열어 처음 실 피드에 연결한 결과 F-024/F-027 이 실 API 로 검증된 적 없던 사실이 드러났고, 정정(파서 snake_case 정합·전용 캡 1 GiB·공급자 널 센티널 `nan` 처리 — integration §7.4) 이후에도 **임포트는 여전히 성립하지 않는다**. V62 행 註에 다음을 사실대로 기입 — (1) 이 소스는 **한 번도 임포트 성공 이력이 없어** `active_version`·`last_imported_at` 이 **V62 시드 그대로 `NULL`**(`status=DISABLED`)이고 `aml_watchlist_entries` 의 해당 `source_code` 행도 **0건**, (2) 실패 계보 3단 — 키 미보유 `DilisenseConfigurationException`→`FAILED` → 실측 597,038,659 B 가 256 MiB 캡을 2.22배 초과해 `FAILED` → 캡 1 GiB 상향 후 라이브 sync 는 **HTTP 500 + `java.lang.OutOfMemoryError: Java heap space`**(84.7초, 컨테이너 생존, 1.479 GiB/2 GiB), (3) OOM 원인은 `parseFull` 이 엔트리 **1,503,937개**(레코드 1,025,647 + 별칭 478,290)를 힙 `List` 에 전량 적재하는 F-024 계약 vs 최대 힙 약 1 GiB, (4) ~~**분할 임포트는 미구현·후속 재설계 대상**~~ — **2026-08-12 해제**: 스트리밍 인제스트(`SanctionsStreamIngestTransaction`, aegis-aml main `ac3b4faa`)가 배치 단위 staging + 완주 후 승격으로 이 제약을 대체해 임포트가 성립한다(integration §7.4). 따라서 (1)~(3)·(5)의 「미임포트·`active_version IS NULL`」 서술은 **당시 상태 기록**이며 현재 상태 서술이 아니다. 스키마·컬럼·enum·인덱스 **무변경**(문서 註 전용). | 코드 truth=`services/aml-svc/.../adapter/out/feed/{DilisenseJsonlParser,DilisenseConsolidatedFeedAdapter}`·`db/migration/V62__dilisense_watchlist_source.sql`(무변경). 라이브 실증=aegis-aml 코디네이터 `POST /api/v1/admin/aml/watchlist-sources/DILISENSE_CONSOLIDATED/sync`. integration §7.4 「실 피드 스키마 실측 정정」 동기화(같은 커밋). |
| 2026-08-10 | **§7 bo-api 註 — AML-WHK-001 콜백 자격증명 메뉴/권한 마이그레이션 V23 등재(코드=truth, aegis-aml main `48e8e697`). aml-svc(스키마 `aml`) DDL 변경 0.** WEBHOOK 자격증명의 BO 운영자 표면이 신설되면서 bo-api 가 **V23 `V23__webhook_credential_menu.sql`** 을 additive·멱등으로 얹었다 — `bo_menu` 에 `aml-webhook-credential`(화면 `AML-WHK-001`, `/aml/webhook-credential`, console `aml`·영역 `설정`·sort 495) 1행, `bo_menu_permissions` 에 `BO_SUPER_ADMIN`(전권)·`AML_POLICY_ADMIN`(read/create/update)·`AML_VIEWER`(read) 3행(둘 다 `ON CONFLICT DO UPDATE`). **V15 `V15__wlf_engine_menu.sql`(AML-WLF-005)과 동형**이며 런타임 인가는 계속 scope 기반(bo-api `@PreAuthorize` — 읽기 `aml:case:read`\|`aml:admin:policy`, 쓰기 `aml:admin:policy`)이고 메뉴 행은 bo-web 정적 NAV(`lib/nav.ts` `aml-config-connect`)와의 정합용이다. **aml-svc 는 무변경** — `aml_api_credentials` 의 DDL·제약·컬럼·§7 표 어디도 바뀌지 않았고(같은 날 앞 행에서 이미 확인), 이번 라운드의 엔진 변경은 **미등록 테넌트 쓰기 거부**(`aml_api_credentials.tenant_id` → `aml_tenants` FK 위반 500 을 영속 어댑터 선검증으로 **400 `AML.VALIDATION_ERROR` `unknown tenant`** 로 전환)뿐인 **애플리케이션 레이어 가드**라 스키마·제약에 영향이 없다(FK 자체 무변경, 조회 경로는 미등록 테넌트도 정상 "미등재"). 자격증명 값은 bo-api DB에 복제하지 않는다. | 코드=truth. 근거=bo-api `db/migration/V23__webhook_credential_menu.sql`·`aml/webhook/controller/WebhookCredentialController`, aml-svc `adapter/out/persistence/WebhookCredentialAdminJpaAdapter#save`(테스트 `WebhookCredentialUnknownTenantIntegrationTest`), bo-web `lib/nav.ts`. API §2.7a(bo-api 위임 표면·미등록 테넌트 400), 기능정의서 §12.3 v9.80 동기화. |
| 2026-08-10 | **§3.15 `aml_api_credentials` — `WEBHOOK` 타입이 런타임 등재 대상이 됨을 반영(코드=truth, aegis-aml main `a0d1e5d9`). 마이그레이션 신규 0.** (1) **테이블 註 보강** — `WEBHOOK` 행은 인바운드 자격증명이 아니라 **아웃바운드 전용**이며 admin REST `GET\|PUT /api/v1/admin/aml/webhook-credential`(API §2.7a, 조회 `aml:case:read`/쓰기 `aml:admin:policy`·즉시 반영·4-eyes 없음)로 등재·교체된다(종전 유일 수단 DB 직접 INSERT 대체 — REST-only 원칙). 업서트는 테넌트당 한 행 제자리 갱신(대상 정렬 `enabled`→`webhook_url` 존재→`credential_id`, `credential_id`·`created_at` 보존, 신규 `webhook-primary`)이고 `webhook_url` 은 **NULL 등재 허용**(서명 시크릿만 — 아웃박스 `webhook_target_url` 오버라이드 이벤트용, V59). 저장 시 `POLICY_CHANGE` 감사(`detail.action=WEBHOOK_CREDENTIAL_SAVED`·`operation=CREATE\|REPLACE`, 시크릿 미기록·목적지 host 까지만). (2) **`scopes` 행 보강** — `WEBHOOK` row 는 저장 시 **`[]` 고정**(아웃바운드 서명 전용, 인바운드 호출 권한 0). (3) **provisioner 문단 범위 명시** — 기존 local/demo provisioner 서술은 **인바운드 credential 한정**이며, `WEBHOOK` 행은 profile 무관하게 admin REST 로만 생긴다(개발·데모는 시뮬레이터 ⓪⁗ `setup.webhook-credential` 이 그 REST 를 멱등 호출 — Flyway/DB 시드 아님). (4) **§8 정본 매핑 표** 에 등재 경로·`scopes=[]`·DDL 0 을 반영. **DDL·제약·컬럼·마이그레이션 전부 무변경** — 필요한 컬럼과 `credential_type` CHECK 의 `WEBHOOK` 이 V1/V2·V17 시점에 이미 존재해 엔티티 매핑만 추가됐다(§7 표 무변경). | 코드=truth. 근거=aml-svc `adapter/out/persistence/{WebhookCredentialAdminJpaAdapter,ApiCredentialJpaRepository#findWebhookCredentialsForAdmin,ApiCredentialJpaEntity}`·`application/usecase/WebhookCredentialAdminService`·`adapter/in/rest/WebhookCredentialAdminController`. 테스트 `WebhookCredentialAdminJpaAdapterTest`·`WebhookCredentialAdminIntegrationTest`. API §2.7a·§8.3, integration §3.4 동기화. 엔진 케이스 RA-C14~C18. |
| 2026-08-06 | **TM 알림 회수 축(`RETIRED`) + 회수 3컬럼 역전파(V63, 코드=truth, feature/aml-configurable-rule-retire).** 설정형 STR/CTR 룰 회수(`:retire` 4-eyes, API §2.4)가 그 룰의 TM 알림을 운영 목록·룰 효과성 통계에서 제외하되 **행은 보존**하도록 (1) **§3.10 `aml_alerts`** 에 `retired_at`/`retired_by`/`retire_reason` 3행 신설 + `status` 설명을 CHECK 7종(triage 6종 ∪ `RETIRED`)으로 갱신 + 회수 주석(행 미삭제·기본 제외·케이스 보유 알림 미회수·`ix_aml_alerts_scenario_code`·데모 정리는 REST 전용) 추가, (2) **§5.7 alert_status** 에 `RETIRED` 를 **triage 상태기계 밖의 별도 축**으로 명시(진입 경로 = 룰 회수 승인 실행 1개, `DISMISSED` 재사용 금지 — 업무 판단 vs 룰 무효화로 의미가 다르고 회수해도 `disposition_*` 보존), (3) **§7 마이그레이션 표에 V63 행 추가**(의존 V1~V62). 마이그레이션에 데모 데이터 DELETE 없음(REST-only). V57~V62 문서 미기재 각주(기왕 드리프트)는 본 작업 범위 밖으로 유지 — V63 1행만 추가. | 코드=truth. 근거=`services/aml-svc/.../db/migration/V63__alert_retirement.sql`·`domain/Alert`(retire/canRetire)·`domain/enums/AlertStatus`(RETIRED)·`adapter/out/persistence/{AlertJpaEntity,AlertJpaAdapter,AlertJpaRepository}`·`application/usecase/ConfigurableReportRuleService`(submitRetire/approveRetire). 테스트 `ConfigurableReportRuleRetireIntegrationTest`(실 PostgreSQL Flyway). API §2.4·§3.4a 동기화. |
| 2026-08-03 | **dilisense AML Database 소스 임포트 완결 역전파(F-024 잔여분 완결 — U7, feature/dilisense-source-import-completion).** §7 마이그레이션 표에 V61(레거시 TM 시나리오 10종 제거·`scenario_code` 자유형 전환)·V62(dilisense 소스 카탈로그 등록) 2행 추가 + V57~V60 위임 각주(AMLC 이관 V57·V58 / HRR 콜백 V59 / 제재 4소스 V60 — 각 소유 문서 브랜치 머지 시 채워짐) + 표 헤더 prose·1:1 파일 범위(문두·문말 2곳) `V41~V56` → `V41~V62` 갱신. | 코드 truth=`services/aml-svc/.../db/migration/{V61__remove_legacy_tm_scenarios,V62__dilisense_watchlist_source}.sql`. integration §7.4·API § 동기화(같은 커밋). |
| 2026-08-05 | **bo-api 감사 이벤트 allowlist 누락 2종 역전파(bo-api V22, 코드=truth, fix/bo-str-draft-link-and-tm-rulecode).** (1) **§7 bo-api 참고 註에 V21·V22 추가** — `V22__case_str_draft_and_pep_approval_audit_events.sql` 이 `chk_bo_audit_logs_event` CHECK 를 V21 allowlist 전량 verbatim 보존 후 재생성하며 **`CASE_STR_DRAFT_LINKED`**(`AmlReportService#draftStrForCase`)·**`AML_PEP_APPROVAL_SUBMITTED`**(`AmlPepApprovalService#submit`) 2종을 append(삭제·완화 없음). 둘 다 방출 중이었으나 미등재라 감사 INSERT 가 23514 로 터졌고, `CASE_STR_DRAFT_LINKED` 는 aml-svc 위임 쓰기 커밋 뒤에 터져 **호출자 500 ↔ 링크는 실제 생성**이라는 비원자적 상태를 만들었다(§12-A.7 BR-002 케이스↔STR 연계가 BO 경로에서 불능이던 원인, V10 동일 계열 사고 재발). (2) **재발 방지 2겹** — bo-api 신설 `AuditEventEmissionAllowlistTest`(소스 파생 빌드-타임 게이트: `src/main/java` 의 `AuditLogService.record*` 호출 event 인자 ⊆ 최신 CHECK allowlist, DB·env 불요) + `AuditLogService#recordAfterCommittedEffect`(되돌릴 수 없는 외부 효과 커밋 이후의 감사 실패를 REQUIRES_NEW 프록시 경유로 격리·ERROR 로그, 업무 결과 무오염). 감사가 fail-closed 통제인 경로(`PiiRevealService` `RAW_DATA_ACCESS`)는 기존 엄격 전파 유지. **엔진(aml-svc) 스키마·마이그레이션 변경 없음.** | 코드=truth. 근거=bo-api `db/migration/V22__case_str_draft_and_pep_approval_audit_events.sql`·`audit/service/AuditLogService`·`aml/reports/service/AmlReportService#draftStrForCase`·`test/.../AuditEventEmissionAllowlistTest`. 기능정의서 §12-A.7 BR-002 동기화. |
| 2026-08-16 | **신고소득 숫자 직접 수신 + 보고기한 관할화 역전파(V69, 사용자 지시로 F-029·F-040·F-041 해제).** (1) **신고소득 인입 계약 개편** — 밴드 코드(`declaredIncomeBand`) 대신 `declaredIncomeOperator`+`declaredIncomeAmount`+`declaredIncomeCurrency`+`declaredIncomePeriod` 를 직접 받는다. 상한을 알 수 있는 연산자(`<=`·`<`·`=`)만 평가하고 하한만 아는 신고(`>=`·`>`)는 미평가한다 — 룰이 `거래금액 > 소득 × multiplier` 라 소득을 크게 잡을수록 덜 발동하므로, 임의 상한을 지어내면 오탐이 는다(구 `OVER_10M_PHP` skip 과 같은 판단). 밴드는 레거시 폴백으로 남아 공유 사전 계약(F-041)은 불변. **부수 효과**: 밴드가 PHP 액면이라 비PHP 법인의 소득 룰이 영구 미평가였던 제약이 해소됐다(F-077 이 별도 요건으로 남긴 범위) — 단위 정합 가드는 유지되어 신고 통화 ≠ 테넌트 통화면 여전히 미평가다. (2) **보고기한 관할화** — `BankingCalendar` 의 `Asia/Manila`·17:00 하드코딩을 주입 가능하게 바꾸고, 시간대는 `timezone`(V53)·마감시각은 신규 `report_cutoff_time`(V69)에서 읽는다. PH의 legacy NULL만 17:00으로 해석하고 비PH는 기한을 계산하지 않는 것으로 2026-08-17 행에서 보정했다. 달력 생성 4곳이 `JurisdictionCalendarPolicy`를 공유한다. (3) 미상 sentinel 조건 보정 — 숫자로 신고한 회원이 밴드 키 부재를 이유로 `UNKNOWN` 으로 거짓 표기되던 결함(F-076 계약은 유지). | 코드 truth=`services/common-policy/.../DeclaredIncomePolicy`·`services/aml-svc/.../domain/report/{BankingCalendar,JurisdictionCalendarPolicy}`·`db/migration/V69__tenant_report_cutoff_time.sql`·`IdentityProjectionService`·`TransactionReportSideEffectRunner`. 최신 deadline gate는 위 2026-08-17 `ReportDeadlinePolicy` 행이 정본이다. |
| 2026-08-15 | **기준통화 프로파일 팩 — 통화별 분리 배포 구조 역전파(V68, feature/aml-multicurrency-reporting).** 통화 종속 기본값(CTR 임계·관할 비영업일·FDS 룰 금액)이 마이그레이션·스크립트에 PHP 액면으로 흩어져 있었고, `tenant_demo` 한 테넌트에 PHP·KRW 임계가 함께 심겨 있었다(V3). 테넌트 기준통화는 `base_currency` 단일값이 엔진 계약이라 KRW 행은 도달 불가 死데이터였다. (1) **정본 단일화** — `config/currency-profiles/<code>.json`(php·aud·jpy·krw)에 관할·시간대·전용 테넌트·CTR 임계·비영업일을 모으고, FDS 룰 금액은 **그 관할 CTR 임계 대비 비율**(`_ratios.json`)로 파생한다(환율 변환 아님 — PHP 는 기존 6개 금액을 정확히 재현, 회귀 0). (2) **Flyway 통화 팩** — `db/currency/<code>/R__currency_reference_configuration.sql`(생성물, `scripts/generate_currency_packs.py`)이 자기 테넌트·자기 통화 행만 적재한다(테넌트·소스시스템·PolicyPack·관할통화 바인딩·CTR 임계 1행·관할 공휴일). (3) **배포 선택 축은 Spring 프로파일이 아니라 Flyway 로케이션 속성** `aegis.currency-locations`(env `AEGIS_CURRENCY_LOCATIONS`) — currency-* 프로파일을 쓰면 로컬 자격증명 프로비저닝 allowlist(활성 프로파일이 전부 local/demo 여야 통과)를 벗어나 machine credential 이 발급되지 않고 전 인입이 401 이 된다(실측). 보안 가드를 넓히는 대신 프로파일 축을 쓰지 않는다. `locations` 를 재정의하는 모든 설정은 `${aegis.currency-locations:}` 자리표시자를 유지해야 한다(빠뜨리면 앞선 로케이션이 사라져 기동 실패·또는 팩 미로드 거짓 통과). (4) **V68** — 데모 시드가 남긴 통화 혼재 행 제거. 초안의 전역 삭제는 `ProductionMigrationIsolationIntegrationTest`(전방 마이그레이션이 same-id 고객 구성을 변조하지 않는다)가 잡아내, P0-02(V45)와 동일한 **지문 대조**로 원본 데모 시드일 때만 지우도록 좁혔다 — 엔진이 읽지 않는다는 사실과 마이그레이션이 고객 설정을 지워도 된다는 것은 별개다. V3 원문은 체크섬 보존을 위해 미수정. | 코드 truth=`config/currency-profiles/`·`scripts/{currency_profiles,generate_currency_packs}.py`·`services/aml-svc/.../db/currency/`·`db/migration/V68__drop_cross_currency_ctr_threshold.sql`·`application.yml`(currency-locations). 검증=`CurrencyProfilePackFlywayIntegrationTest`(팩 적용·통화 격리·타 프로파일 무접촉·교차통화 0) + python 계약 14건 + aml-svc 2,532건 전건 PASS + 실 스택 AUD 배포 실증(tenant_au/AUD 10,000 임계로 CTR·STR 발동). |
| 2026-08-15 | **다통화(법인별 자국통화) STR/CTR/TM 기준금액 역전파(코드=truth, feature/aml-multicurrency-reporting).** 호주(AUD)·일본(JPY)·한국(KRW) 법인이 각자 자국 통화를 금액 기준으로 쓰도록 엔진의 PHP 고정 경로를 테넌트 바인딩 통화로 교체했다. **스키마 변경 없음**(기존 `aml_tenants.base_currency` V53 + `aml_ctr_thresholds` PK `(tenant_id, currency)` 를 실제로 사용하게 된 것). (1) **§0 헤더 금액 행·§3.1 `base_currency` 설명·§5.6a·§3.10a `dsl` 행** — 금액 임계 정본을 `transaction.phpEquivalent`(PHP 환산)에서 **`transaction.baseEquivalent`(테넌트 기준통화 액면)** 로 정정. `phpEquivalent` 는 `base_currency='PHP'` 테넌트 전용 하위호환 별칭으로 격하(비PHP 테넌트 미노출 → PHP 액면 기존 룰이 자국통화 금액과 비교되는 오탐 구조적 차단). (2) **§7 V53 후주의 'FX 는 phase-2(A1)·KRW 테넌트 CTR/금액룰 미발동' 서술 폐기** — 업무 확정(2026-08-15)상 필요한 것은 합계·평균뿐이고, 인입이 `amounts.baseCurrency == 테넌트 바인딩 base` 를 강제해 테넌트 윈도우가 단일 통화이므로 환산 없이 성립한다. 환율 테이블·FX 포트는 도입하지 않는다. (3) canonical payload 는 전 통화 테넌트에 `baseEquivalent`+`baseCurrency` 를 싣고, 윈도우 집계 4종(CTR 일합계·최신 기준금액·STR 신호행·활동집계)이 `COALESCE(baseEquivalent, phpEquivalent)` 로 구 적재분까지 읽는다. CTR 임계는 **테넌트 reporting currency 행으로만** 조회한다(타 통화 임계·필리핀 ₱500,000 floor 미적용 → 미설정 통화는 CTR 미개설 fail-safe). (4) 소득 미스매치(F-041)는 밴드 통화(PHP)와 테넌트 통화가 다르면 **미평가** — 잠금 계약의 '단위 band=월소득 PHP ↔ 거래금액=PHP 일치' 를 강제한 것이며 환산으로 수치를 합성하지 않는다(자국통화 소득 밴드는 별도 요건). | 코드 truth=`application/usecase/{NeutralTransactionEventService,CtrEvaluationService,StrEvaluationService,TransactionReportSideEffectRunner,EvidenceTimelineService,ConfigurableReportRuleEvaluationService,ReportRelatedTransactionsBuilder}`·`adapter/out/persistence/CanonicalEventWindowAdapter`·`domain/tenant/TenantPolicyBinding`·`domain/tm/ConfigurableRuleDslPolicy`. 검증=`MultiCurrencyTenantReportingIntegrationTest`(AUD/JPY/KRW 3법인 폐루프·교차오염 0·일합계·미설정 fail-safe) + aml-svc 2,528건 전건 PASS. |
| 2026-08-05 | **PH STR 법정 보고 기한 DRAFT 개설 시 확정 역전파(코드=truth, fix/bo-str-draft-link-and-tm-rulecode; 2026-08-17 관할 gate 보정).** §3.12 `aml_regulatory_reports.due_at`은 CTR 전용이 아니라 PH STR에도 채워진다 — PH AMLC 계약대로 `RegulatoryReport.strDraft()`가 트리거 시각 +5영업일 17:00 PHT를 DRAFT 개설 시 freeze한다. 사유 누적은 기한을 이동시키지 않는다. 2026-08-17부터 이 경로는 `ReportDeadlinePolicy`를 거쳐 다른 관할에는 `NULL`을 보존한다. **스키마·마이그레이션 신규 없음**(기존 V4 `due_at` 재사용). | 코드=truth. 근거=aml-svc `RegulatoryReport#strDraft`·`StrEvaluationService#strDueAt`·`ReportDeadlinePolicy`. API §3.6 동기화. |
| 2026-08-01 | **레거시 TM 시나리오 정의 10종 제거 + `scenario_code` 자유형 전환 역전파(V61, 코드=truth, refactor/remove-legacy-tm-scenarios).** v9.21 확정 사실(TM 알림 발동 정본=CTR/STR 룰 카탈로그, F-025 실측)에 따라 거래 인입 경로에서 전혀 발동하지 않는 `aml_tm_scenarios` 레거시 정의 10종(닫힌 enum `TmScenario`, 실측 13행)을 제거하고 저작 경로는 존치한다. (1) **§3.10a** 헤더·서술을 "V61 자유형 전환"으로 갱신 — 정의 DELETE(테이블 DROP 아님)·CHECK 를 닫힌 10종 목록에서 형식 정규식 `^[A-Z][A-Z0-9_]{2,64}$` 로 재정의(예약 코드 `CUSTOM_RULE`는 유스케이스 단계에서 거부), 과거 hanpass ACTIVE 6종 시드 서술은 "과거 사실(제거됨)"으로 개정. (2) **§3.10 `aml_alerts.scenario_code`** 서술에 `ck_aml_alerts_scenario_code`(V7 합집합) 무변경 명시(FK 부재로 정의 DELETE 와 무관, 레거시 알럿 보유 환경 기동 실패 위험 회피 — 요구 밖 확장 배제). (3) **§5.6 tm_scenario** 를 닫힌 enum 10종 표 → 자유형 코드 서술로 전환(과거 10종 코드값·제거 경위 명기). (4) **§7 마이그레이션 표에 V61 행 추가**(`V61__remove_legacy_tm_scenarios.sql`, 의존 V1~V60) + 표 인근 각주로 V57~V60 4행 문서 미기재(기왕 드리프트·별건)를 명시(본 작업 범위 확대 금지 — V61 1행만 추가). 코드 계층(aml-svc `TmScenario`→자유형 String, `TmScenarioStorePort.findAllActive`→`findAll` 대체, `AdvancedDomainEvaluationService` 시나리오 평가 limb 제거·EDD limb 존치)·bo-api(`ScenarioCode`/`ScenarioTemplates`/`ScenarioDslCodec` per-code switch 제거→generic decode, `displayName`=코드 원문)·bo-web(`AmlScenarioCode` 자유형·라벨 맵 제거)는 API·기능정의서·software 문서에 대칭 반영(02-aml-api.md·02-aml-sass-functional-spec.md·02-amlSvc-sass.md 동일 커밋). | 코드=truth. 근거=`services/aml-svc/.../db/migration/V61__remove_legacy_tm_scenarios.sql`·`domain/tm/TmScenarioDefinition`·`application/port/out/TmScenarioStorePort`·`application/usecase/{TmScenarioService,AdvancedDomainEvaluationService}`. API §2.5a/§2.7/§3.4a/§3.4c·기능정의서 §7.1 BR-010/§12-A.6/§12-B.3·software §6.2/§12.1 동기화. |
| 2026-07-24 | **§3.10 `aml_alerts.evidence` ① 트리거 — 설정형(configurable) 룰 발동 행 additive 필드 역전파(코드=truth, feature/aml-str-rule-detail-condition, v9.36).** 설정형 룰 발동 알림 행의 트리거에 `ruleFamily`(STR\|CTR)·`ruleVersion`·`ruleSource=CUSTOM`·`condition`(룰 DSL 조건식의 사람이 읽는 요약 — locale 중립 결정적 문법·저장 시점 고정 문자열, 요약 실패 시 키 생략 fail-safe)이 실리고 최상위 `features`(관측 피처 스냅샷·비PII)·`parameters`·`policyPack` 맵이 보존됨을 명문화. 법정 카탈로그 룰(STR8/CTR2) 행은 무변경. **DDL 무변경 — JSONB 내부 확장(마이그레이션 없음)** | 코드 truth=aml-svc `ConfigurableReportRuleEvaluationService.evidence()`·`domain/tm/ConfigurableRuleDslSummarizer`. API §3.4a 동기화, 엔진 케이스 STR-C12 검증 |
...
| V47 | `V47__rls_tenant_isolation.sql` | **`SHARED` 배포 DB tenant 격리 방어선 — Row-Level Security(P0-13, §1.2)**: (1) 클러스터 전역 **NOLOGIN role `aegis_app_runtime`** 확보 + 권한 부여(멱등) — 애플리케이션 연결이 획득 시점에 `SET ROLE aegis_app_runtime` + `set_config('app.tenant_id'|'app.workspace_id'|'app.elevated', …)` 를 실행해(common-security `RlsSessionDataSource`) login user 가 superuser/owner 여도 세션이 non-owner 로 강등돼 RLS 가 강제된다. Flyway 는 감싸지 않은 원본 DataSource(owner)로 실행돼 정책 DDL·후속 데이터 마이그레이션이 전량 접근한다. (2) `tenant_id` 보유 전 테이블에 `ENABLE`+**`FORCE ROW LEVEL SECURITY`** + **정책 2종** — `aml_rls_tenant`(runtime: `tenant_id = current_setting('app.tenant_id', true)` OR `current_setting('app.elevated', true)='on'`)·`aml_rls_owner`(owner 전량 허용 = 마이그레이션·운영 정비용 명시적·감사 가능 escape). GUC 미설정 세션은 `current_setting(…, true)=NULL → false → 0 row`(fail-closed). (3) **격리 키는 `tenant_id` 단일 차원**(workspace 미도입, §1.1). AML `data_scope` 는 저장 격리키가 아니라 운영자 row-level 권한 필터이므로 RLS 키로 쓰지 않는다. (4) `tenant_id` 컬럼이 없는 글로벌/참조 테이블과 `flyway_schema_history` 는 DO 루프가 열거하지 않아 자동 제외되며, 가드 테스트 `RlsCoverageGuardIntegrationTest` 가 대상 전 테이블의 `relrowsecurity AND relforcerowsecurity` + 정책 2개 존재를 강제한다. **DO 루프 경과 후 신설되는 테이블은 자기 마이그레이션에서 동형 정책 2종을 직접 부여한다**(선례 V57). 멱등. **(2026-08-12 표 누락 보정 — §1.2 서술·V48 의존 표기는 이미 V47 을 참조하고 있었으나 본 표에 행이 없었다.)** | V1~V46 |
