...
CTR/STR 보고 룰 카탈로그(API §11, `AmlReportRuleCatalog`)의 **튜너블 파라미터(임계·변수·윈도우·밴드) per-tenant 오버라이드 store**다. 카탈로그 기본값이 정본이며, 행이 존재하는 `(tenant, rule, param)` 튜플만 기본값을 대체한다 — **무-오버라이드 시 기존 동작 완전 보존**. bo-api 🔒 `REPORT_RULE_PARAM` 4-eyes 승인(EXECUTED)이 위임하는 엔진 admin `POST /api/v1/admin/aml/report-rules/{ruleCode}:update-params`(API §2.7)가 룰 단위 파라미터 셋을 **원자 upsert** 하는 반영면이고, STR 평가 경로(`StrEvaluationService`→`StrSignalDeriver` 인자 주입, pure 유지)가 resolve 소비한다(bo-api 라이브 평가와 parity). **CTR 단건/일합산 임계는 이 테이블에 두지 않는다** — `aml_ctr_thresholds`(§3.22a)가 단일 정본(이중 정본 금지, absent=미발동 fail-safe 불변).
...
원장(`aml_customers`, §3.3)은 회원의 **현재 상태**만 upsert 로 보존한다. "회원(memberRef)이 **언제 어떤 유형의 실사를 어떤 결과로** 수행했는가"는 이 **불변(append-only) 이력**이 정본이다 — 회원원장 요약·CDD/EDD 히스토리 화면(AML-CDD-004 / AML-MBR-001, read-only)의 소스. 회원 키 `member_ref` = `aml_customers.customer_ref` = `originator.nationalIdentityKey`(엔진 targetRef) **단일 키**(CDD 인입·1차 RA·거래·WLF 전부 이 키로 연결). API §2.x `/admin/aml/members/{memberRef}/{ledger|cdd-history}`(엔진)·bo-api `/api/v1/bo/aml/members/{memberRef}/*` 위임 read 의 조회 대상이다.
...
| `history_type` | VARCHAR(32) | N | — | CHECK 4종 | §5.36 `cdd_history_type`(`CDD_INITIAL`/`CDD_REVIEW`/`EDD_OPENED`/`EDD_CLOSED`) — 엔진 `CddHistoryType` 와 1:1 |
...
PK: `(history_id)`. 인덱스 `ix_aml_member_cdd_history_member (tenant_id, member_ref, occurred_at DESC)`(회원별 최신순 페이지·요약 카운트)·`ix_aml_member_cdd_history_member_type (tenant_id, member_ref, history_type)`(유형 필터). 멀티테넌시 키 `(tenant_id, member_ref)` 선두. **append-only**(UPDATE/DELETE 없음, `append` 는 항상 신규 `history_id` INSERT). **raw PII 미적재** — `kyc_status`/`risk_grade` 는 enum 스냅샷, `details` 는 참조 토큰·사유 코드만(§19.2). 표시명·문서번호 미노출(프로필 마스킹 소관). 적재 지점 2종: (a) `customer.cdd.completed` 인입(`AmlEventIngestService`) — projection upsert **전** 원장 존재 여부로 `CDD_INITIAL`(최초)/`CDD_REVIEW`(재이행) 판정, (b) EDD 착수/종료(`CddEddService`) — `EDD_OPENED`/`EDD_CLOSED`(4-eyes 승인 실행). 정책·스냅샷 메타만.
...
| `subject_type` | VARCHAR(64) | N | enum,CHECK | §5.16 subject_type **20종**: `WLF_DECISION`/`FP_WHITELIST`/`RA_MODEL`/`RISK_OVERRIDE`/`EDD_CLOSE`/`STR_SUBMIT`/`CTR_SUBMIT`/`TRAVEL_RULE_EXCEPTION`/`WATCHLIST_IMPORT`/`COUNTRY_RISK`/`POLICY_PACK`/`SECRET_CHANGE`/`RELATIONSHIP_REJECT`/`TM_SCENARIO`/`CHECKLIST_CHANGE`/`PERIODIC_REVIEW_CHANGE`/`IRA_SUBMIT`/`HIGH_RISK_REGISTRY`/`PEP_APPROVAL`/`CTR_THRESHOLD` (§13.5). **API `ApprovalDto.subjectType` enum이 정본**(전수), DB는 이를 동기화. V3 16종 → V13 17종(`IRA_SUBMIT`) → V14 18종(`HIGH_RISK_REGISTRY`) → V18 18종 재정의 → **V24 19종(`PEP_APPROVAL`)** → **V23 20종(`CTR_THRESHOLD`, 현재 체인)**. bo-api 계층은 `REPORT_RULE`·`REPORT_RULE_PARAM` 포함 22종이며, `CTR_THRESHOLD`는 엔진 결재함으로 위임된다. |
...
### 5.16 subject_type — 결재 대상 (설계서 §13.5) — **20종(확정, 도메인 `ApprovalSubjectType` 1:1)**
...
> **CTR/STR 룰·임계 4-eyes 경계(코드=truth)**: `CTR_THRESHOLD`(CTR 규제 임계 변경)는 aml-svc 엔진 `ApprovalSubjectType`/`aml_approvals.subject_type` CHECK에 포함되는 **엔진 결재 대상**이며 승인선은 `REPORTING_OFFICER`다. bo-api `POST /api/v1/bo/aml/ctr-thresholds/{currency}:update`는 엔진 연결 시 aml-svc admin `POST /api/v1/admin/aml/ctr-thresholds/{currency}:update`로 상신을 위임하고, 결재 EXECUTED 시 엔진이 `aml_ctr_thresholds`를 반영한다(운영 엔진 미연결 fail-closed, 비운영 stub fallback). 반면 `REPORT_RULE`(룰 활성화)과 `REPORT_RULE_PARAM`(룰 단위 임계·변수 셋 변경, 승인선 `COMPLIANCE`, bo-api V9 감사 `REPORT_RULE_PARAM_CHANGE_SUBMITTED`)은 bo-api 애플리케이션 enum/스텁 결재 또는 EXECUTED 시 엔진 admin `:update-params` 위임 경로를 유지한다. 결재 subject_type 배정은 기능정의서 `docs/plan/03-bo-iam-approval-functional-spec.md` §4.2(REPORTING_OFFICER/COMPLIANCE) 정본.
...
> DB가 물리 정본(CHECK **4종**, V14 3종 → V24 `PEP_INDIVIDUALS` 추가). 도메인 enum `ReferenceListType`·bo-api 계약(가정 A4)과 1:1. `PEP_INDIVIDUALS`는 PEP 경영진 승인(§5.16 `PEP_APPROVAL`) EXECUTED 시 등재(tier HIGH) → RA 위험등급 HIGH 강제 상향(V24).
...
### 5.36 cdd_history_type (`aml_member_cdd_history.history_type`, §3.22f, V26)
`CDD_INITIAL`(최초 CDD 이행 — 원장 미존재 시 첫 `customer.cdd.completed` 인입) / `CDD_REVIEW`(재이행 CDD — 기존 회원 후속 재인입) / `EDD_OPENED`(EDD 착수 — 케이스 개시·트리거 병기) / `EDD_CLOSED`(EDD 종료 — 4-eyes 승인 실행·최종 상태 병기)

> DB가 물리 정본(CHECK **4종**, V26). 도메인 enum `CddHistoryType` 및 bo-api `MemberLedgerDtos.HistoryType` 와 1:1. `CDD_*` 는 원장 존재 여부로 판정(§3.22f 적재 지점 a), `EDD_*` 는 EDD 착수/종료(`CddEddService`, 적재 지점 b).
...
스키마: `aml`. 파일 위치: `services/aml-svc/src/main/resources/db/migration/`. **본 표는 실제 구현 파일(V1~V23, V26, fresh-DB Flyway 검증 통과)과 1:1 일치한다** — 구 누적 마이그레이션 체인(구 phase 단위 `V1~V25`)은 2026-06-30 `AML-ENGINE-FIX: consolidate Flyway baselines`(commit 9a3ac74)로 **검증된 최종 상태의 pg_dump 를 단일 `V1__baseline.sql`(schema-only DDL) + `V2__seed.sql`(data-only bootstrap/demo seed)로 통합(consolidate)**되었다. 이후 CTR/STR 모니터링 통합(feature/aml-ctr-str-monitoring, 2026-07-01)이 `V3`~`V6` 을, 이어 TM 룰코드(`V7`)·실 제재명단 수집(`V8`)·WLF 수취인 임계(`V9`)·워치리스트 브라우즈 인덱스(`V10`)·RA 모델 시나리오(`V11`)·2차 상시 RA(ONGOING) 모델 활성화(`V12`)·RA 당연고위험(HRR 강제 상향) 사유 영속(`V13`)·FP whitelist 등록 메타데이터 영속(`V14`)·케이스 발단(origin) 계보 `origin_screening_id`(`V15`)·국가위험 일일 웹 수집(`V16`)·국가위험 단일 ACTIVE 불변식(`V17`)·국가위험 수집 소스 제공자화(EU 집행위 기본·FATF 대안 provenance, `V18`)·1차 온보딩 RA 엔진 CDD 파생 파라미터 정본(`V19`)·1차 온보딩 RA 국가위험 등급 기반 강제 floor 파라미터(`V20`)·데모 국가위험 수동 기준선(`V21`)·CTR/STR 룰 튜너블 파라미터 오버라이드(`V22`)·CTR 임계 엔진 결재 subject 확장(`V23`)·회원원장 CDD/EDD append-only 이력 신설(`V26`)을 additive 로 얹었다. **(V번호 주: 구 changelog(2026-06-29)의 `V24__pep_approval.sql`(PEP)·`V25__periodic_review_policy.sql`(periodic)는 2026-06-30 consolidate 로 `V1__baseline.sql`+`V2__seed.sql`에 흡수·삭제되어 디스크에 부재하나, 그 논리적 V번호 점유(V24·V25)와의 재사용 checksum 충돌을 피하려 member-cdd-history 는 다음 자유 번호 `V26` 을 점유한다 — V24·V25 는 예약·공번.)** **(주의: `V19`/`V20` 은 RA 온보딩 파생·국가위험 floor 파라미터이며, 일부 구 이력 서술에 남아 있는 가상 `demo_ph_scenarios(V19)`·`demo_approval_seed(V20)` 는 디스크에 실재하지 않는다 — L1222 이력 註 참조.)** 아래는 코드(truth) 기준.
