...
| 일자 | 변경 | 비고 |
|---|---|---|
| 2026-08-10 | **§7 V62 註 — `DILISENSE_CONSOLIDATED` 현재 상태 「미임포트·`active_version=NULL`」 사실 기록(코드=truth, aegis-aml main `e29f9ab0`). DDL·마이그레이션 신규 0.** 사용자가 API 키를 열어 처음 실 피드에 연결한 결과 F-024/F-027 이 실 API 로 검증된 적 없던 사실이 드러났고, 정정(파서 snake_case 정합·전용 캡 1 GiB·공급자 널 센티널 `nan` 처리 — integration §7.4) 이후에도 **임포트는 여전히 성립하지 않는다**. V62 행 註에 다음을 사실대로 기입 — (1) 이 소스는 **한 번도 임포트 성공 이력이 없어** `active_version`·`last_imported_at` 이 **V62 시드 그대로 `NULL`**(`status=DISABLED`)이고 `aml_watchlist_entries` 의 해당 `source_code` 행도 **0건**, (2) 실패 계보 3단 — 키 미보유 `DilisenseConfigurationException`→`FAILED` → 실측 597,038,659 B 가 256 MiB 캡을 2.22배 초과해 `FAILED` → 캡 1 GiB 상향 후 라이브 sync 는 **HTTP 500 + `java.lang.OutOfMemoryError: Java heap space`**(84.7초, 컨테이너 생존, 1.479 GiB/2 GiB), (3) OOM 원인은 `parseFull` 이 엔트리 **1,503,937개**(레코드 1,025,647 + 별칭 478,290)를 힙 `List` 에 전량 적재하는 F-024 계약 vs 최대 힙 약 1 GiB, (4) **분할 임포트는 미구현·후속 재설계 대상**(`getConsolidatedFile?includes=<source_id>` 서버측 필터 동작만 실증), (5) `active_version IS NULL` 이라 **48h freshness fail-closed 게이트 범위 밖**이므로 타 소스 스크리닝을 차단하지 않는다. 스키마·컬럼·enum·인덱스 **무변경**(문서 註 전용). | 코드 truth=`services/aml-svc/.../adapter/out/feed/{DilisenseJsonlParser,DilisenseConsolidatedFeedAdapter}`·`db/migration/V62__dilisense_watchlist_source.sql`(무변경). 라이브 실증=aegis-aml 코디네이터 `POST /api/v1/admin/aml/watchlist-sources/DILISENSE_CONSOLIDATED/sync`. integration §7.4 「실 피드 스키마 실측 정정」 동기화(같은 커밋). |
