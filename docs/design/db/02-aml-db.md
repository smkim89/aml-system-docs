...
| `sync_lease_expires_at` | TIMESTAMPTZ | Y | NULL | | **(V67)** 위 리스의 만료 시각(TTL 설정 `aml.watchlist.sync-lease.ttl-seconds` 기본 900초). **만료된 리스만** 다른 run 이 이어받을 수 있어 프로세스 사망 시 자가 치유되며, 진행 중인 run 은 배치마다 갱신한다 |
| `created_at/created_by/updated_at/updated_by/trace_id` | (공통) | | | | |
