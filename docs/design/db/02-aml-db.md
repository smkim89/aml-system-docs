...
**`aml_cdd_onboarding_decisions`(V42)와의 관계**: 두 테이블은 서로 다른 생명주기를 갖는 별개 append 소스다 — 전자는 CDD 인입마다(재이행 포함) 새 스냅샷을 남기고, 본 테이블은 그 스냅샷이 `EDD_REQUIRED`인 상태에서 `EDD_CLOSE` 결재가 실행되거나(`caseType==EDD_REVIEW` **∧** 확정 전 현재 결정이 `EDD_REQUIRED`일 때만) `RELATIONSHIP_REJECT` 결재가 실행될 때만 행을 남긴다. 재이행(re-CDD)으로 새 `EDD_REQUIRED` 스냅샷이 과거 `APPROVED`/`REJECTED` 확정 행보다 최신이면 회원은 다시 보류 상태로 읽히고(현재 결정 재계산), 그 EDD 케이스가 다시 종결되면 새 확정 행이 추가로 append된다(동일 승인당 1행 제약은 approval_id 단위이므로 재이행 발 신규 결재는 신규 행).
