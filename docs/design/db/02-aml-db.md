...
| 버전 | 파일 | 내용 | 의존 |
|---|---|---|---|
| V71 | `V71__wlf_candidate_runtime_rls_recall.sql` | **WLF runtime RLS-safe 후보조회**: mutation 전 superuser를 요구하는 one-shot privileged migration. 전용 NOLOGIN/non-super/BYPASSRLS owner와 최소 ACL을 만들고 tenant/elevated guard·22023 입력 검증·고정 S1~S4/grouped/ranked/capped 3열 SECURITY DEFINER 함수를 생성한다. 선존 role의 잘못된 attribute/membership뿐 아니라 `aml` schema/table/sequence/routine/function ownership, unexpected direct/default ACL을 mutation 전에 거부하며, 종료 직전 exact ownership·2 table SELECT·helper EXECUTE·schema USAGE·PUBLIC/runtime function ACL을 재검증한다. 적용 runner는 동일 aml-svc bootJar/Flyway 11.1.1 inventory(Java V38+선택 currency repeatable)를 사용하고 `aegis_aml_app` non-super login을 runtime role membership+history-only direct ACL로 provision한다. 신규 RLS policy·planner GUC·timeout/cap/floor/시드 0. | V70 |
