...
> **RA 모델 템플릿(`aml_risk_models`) — `scenario` 컬럼(V11, 코드=truth).** §3.9 의 `model_code`·`model_version` 이 참조하는 RA 모델 정의 테이블 `aml_risk_models`(통합 `V1__baseline.sql` 생성, PK `(tenant_id, model_code, version)`)에 **`scenario VARCHAR(32) NOT NULL DEFAULT 'ONBOARDING'`** + CHECK `aml_risk_models_scenario_check (scenario IN ('ONBOARDING','ONGOING'))` 이 `V11__ra_model_scenario.sql` 로 추가되었다(§7). 어느 모델 정의가 어느 RA 흐름에 소비되는지를 **자기서술**한다 — `ONBOARDING`(1차·회원가입 CDD 완료 시점 위험평가, 정본 `KR_DEFAULT_RA`) / `ONGOING`(2차·상시). 기존 `KR_DEFAULT_RA`(v1 APPROVED·v2 DRAFT)는 DEFAULT `'ONBOARDING'` 백필. `KR_ONGOING_RA` v1 은 `scenario='ONGOING'`·`status=DRAFT`·`is_default=false` **placeholder** 1행만 시딩되며, 활성화·거래가중 재평가·주기 단축·EDD 자동 개시는 **다음 단계 예정**(기능정의서 §6.1 미정의 — placeholder 로만 선반영). 도메인 enum `RaScenario`(2종)·`RiskModel.scenario`·`RiskModelJpaEntity.scenario` 와 1:1. 정책 메타만(PII 없음).
...
스키마: `aml`. 파일 위치: `services/aml-svc/src/main/resources/db/migration/`. **본 표는 실제 구현 파일(V1~V11, fresh-DB Flyway 검증 통과)과 1:1 일치한다** — 구 누적 마이그레이션 체인(구 phase 단위 `V1~V25`)은 2026-06-30 `AML-ENGINE-FIX: consolidate Flyway baselines`(commit 9a3ac74)로 **검증된 최종 상태의 pg_dump 를 단일 `V1__baseline.sql`(schema-only DDL) + `V2__seed.sql`(data-only bootstrap/demo seed)로 통합(consolidate)**되었다. 이후 CTR/STR 모니터링 통합(feature/aml-ctr-str-monitoring, 2026-07-01)이 `V3`~`V6` 을, 이어 TM 룰코드(`V7`)·실 제재명단 수집(`V8`)·WLF 수취인 임계(`V9`)·워치리스트 브라우즈 인덱스(`V10`)·RA 모델 시나리오(`V11`)를 additive 로 얹었다. 아래는 코드(truth) 기준.
