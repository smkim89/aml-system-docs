...
| 일자 | 변경 | 비고 |
| 2026-07-07 | **RA 점수 목록 기본 modelVersion 무필터 역전파(코드=truth, fix/aml-ra-list-default-modelversion).** §2.7 `GET /api/v1/admin/aml/risk-scores` bo-api pass-through 서술에 "**`modelVersion` 미지정 ⇒ 무필터(1차 `ONBOARDING`+2차 `ONGOING` 혼합 평가 내역 뷰), bo-api 는 활성 버전을 기본 첨부하지 않음**" 명시. 기존 bo-api 는 미지정 시 `KR_DEFAULT_RA` 활성 버전(`resolveActiveModelVersion()`)을 엔진 질의에 강제 첨부해, 2026-07-06 `KR_DEFAULT_RA` v1→v2 활성 전환 후 기존 평가 내역(전부 v1)이 `modelVersion=v2` 필터에 전량 걸러져 `/aml/ra` 목록이 공백이 되고 2차 RA(`KR_ONGOING_RA`) 내역이 타 모델 버전 필터에 교차 오염되는 회귀가 발생 → 목록 위임 경로에서 미지정 시 무필터(`null`)로 위임하도록 수정(순수 optional 필터). `distribution`(활성 버전 기준 분포)·`simulate` 의 활성 버전 resolve 는 불변. | aegis-java-implementer(spec). 코드=truth. 근거=bo-api `aml/ra/service/AmlRaService.riskScores(RiskScoreListFilter,page,size)`(위임 경로 `explicit ? filter.modelVersion() : null`)·`resolveActiveModelVersion()`(distribution·simulate 전용). Flyway/DB 변경 없음. |
