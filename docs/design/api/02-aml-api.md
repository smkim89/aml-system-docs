...
| flat 키 | 타입 | 파생/설명 |
|---|---|---|
| `targetRef` | string | 주체 안정 토큰(`originator.nationalIdentityKey` 기반, subject_ref 동형) |
| `memberRef` | string | 비PII 회원 업무참조(`originator.partyReference`, 예 `M-9903`) — 화면 회원번호 열, FDS `subject_ref` 동형. 부재 시 키 미기록 |
...
| 메서드 | 경로 | scope | 4-eyes | 설명 | DB |
|---|---|---|---|---|---|
| GET | `/api/v1/admin/aml/ra-models?modelCode=` | `aml:admin:policy` | — | RA 모델 버전 목록. 응답 `RaModelSummary[]` — 각 행에 `scenario`(ONBOARDING/ONGOING, DB §3.9 `aml_risk_models.scenario`) 노출 | (정책 store) |
| POST | `/api/v1/admin/aml/ra-models` | `aml:admin:policy` | — | RA 모델 버전 초안(draft·결재 불필요). 요청 `DraftRequest{ modelCode, version, scenario?, weights, mediumThreshold, highThreshold, prohibitedThreshold }` — **`scenario`(ONBOARDING/ONGOING) 옵션, 미지정 시 `ONBOARDING`**(엔진 `RiskModelAdminController.draft` default). 응답 `RaModelSummary` | (정책 store) |
...
> **RA 모델 관리 DTO `scenario`(bo-api BFF, 코드=truth).** bo-api 의 RA 모델 관리 read model `RaModel`(`GET /api/v1/bo/aml/ra-models`)과 그 하위 `RaModelVersion` 은 model-level 축 **`scenario`(ONBOARDING/ONGOING, DB §3.9 `aml_risk_models.scenario` 1:1)** 를 노출한다 — `RaModelVersion.scenario` 는 소유 `RaModel.scenario` 를 승계(같은 modelCode 의 버전은 동일 scenario)해 버전 행이 모델 그룹 밖에서도 자기서술적이다. FE 는 이 값을 시나리오 배지 + 트리거·산출·주기 규칙 패널에서 소비한다. 엔진 응답에 `scenario` 부재 시 bo-api 는 `ONBOARDING` 으로 안전 기본 처리(graceful, `RaDtos.RaScenario`). enum 2종 = `ONBOARDING`(1차·온보딩) / `ONGOING`(2차·상시, 활성화·거래가중 재평가·주기 단축·EDD 자동 개시는 다음 단계 예정 — DRAFT placeholder `KR_ONGOING_RA` 자리만).
