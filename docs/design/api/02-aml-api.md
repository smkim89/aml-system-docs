...
> **CDD 신고소득 숫자 직접 수신(2026-08-16, 사용자 지시로 F-040·F-041 해제, 코드=truth).** `payload.kyc` 가 `declaredIncomeOperator`(`LTE`/`LT`/`EQ`/`GTE`/`GT` 또는 기호)·`declaredIncomeAmount`(양수)·`declaredIncomeCurrency`(ISO 4217)·`declaredIncomePeriod`(`MONTHLY`/`ANNUAL`)를 직접 싣는다. **상한을 알 수 있는 연산자만 평가**하며(`GTE`/`GT` 는 상한이 없어 미평가 — 임의 상한을 지어내면 임계가 낮아져 오탐), 신고 통화가 테넌트 기준통화와 다르면 미평가한다(환산 없음). `ANNUAL` 은 /12 월할. `declaredIncomeBand` 는 레거시 입력으로 계속 수용되며 숫자 신고가 없을 때만 쓰인다. 미신고 시 동작은 아래 선택 입력 경계 그대로다(소득 조건만 미평가). 정본 `docs/aml-data.md` §4.3.1.
>
...
`BankingCalendar`(도메인, I/O-free)가 모든 기한·일합산 경계 산정을 담당한다. `JurisdictionCalendarPolicy`가 테넌트 바인딩의 `timezone`(V53)과 `report_cutoff_time`(V69)을 한 번 해소해 달력에 주입한다. ▷ `bankingDayKey(instant)`=거래 instant 의 **테넌트 관할 캘린더 일자**(정산/집계 축, 주말/공휴일 instant 도 자기 일자로 매핑). ▷ `startOfDay`/`endOfDay`=같은 관할 자정 경계. ▷ `isBusinessDay(date)`=토/일 아님 AND 관할 override 아님(레거시 명칭 `aml_ph_banking_calendar`, §3.22b, 행은 tenant별). ▷ `plusBusinessDays(from,N)`=N영업일 전진(앵커를 먼저 영업일로 정규화). ▷ `dueAt(txnTime,N)`=거래 영업일 +N영업일 `<report_cutoff_time>` Instant. PH 프로필만 규제 근거가 확인된 `Asia/Manila`·17:00을 명시하며, AU/JP/KR 프로필의 마감시각은 승인 전 `NULL`로 둔다. DB 미설정·구 배포는 기존 `Asia/Manila`·17:00 기본값으로 폴백하되 이 값을 타 관할의 확정 규제로 간주하지 않는다. CTR·STR 달력 생성과 CTR 조회/evidence window는 동일 snapshot을 공유한다.
...
CTR 평가(`CtrEvaluationService`)는 거래의 **freeze 된 서버 파생 테넌트 기준통화 금액(`baseEquivalent`)을 재계산하지 않는다**(BR-501). neutral ingest는 canonical 저장의 바깥 transaction과 CTR/STR `REQUIRES_NEW` 경계 및 허용 범위 내 원천↔엔진 시계 오차에도 현재 거래를 잃지 않도록 command의 current-event 기준통화 금액을 우선 사용한다. 이 값은 canonical `amounts`에서 서버가 확정한 동일 동결값이며 PHP 테넌트만 `phpEquivalent` 하위호환 별칭을 함께 갖는다. 현재-event 신호가 없는 레거시·직접 TM 호출만 canonical 이벤트 윈도우의 `COALESCE(baseEquivalent, phpEquivalent)`로 폴백하며, 두 경로 모두 값이 없으면 금액 룰은 fail-safe 미발동한다. `CTR_SINGLE`=단건 기준통화 금액 ≥ 해당 통화 임계, `CTR_DAILY`=동일 **관할 영업일** 현금거래 합산 ≥ 임계(다건 보완재). 조회 구간과 evidence window는 `BankingCalendar.startOfDay/endOfDay`를 사용한다. (테넌트,주체,영업일)당 CTR DRAFT 정확히 1건(부분 UNIQUE `ux_aml_ctr_draft`, DB §3.12)이며 후속 현금거래는 `report_amount`에 정확히 1회 누적한다(`accumulateCtr`, 경합 시 재시도). `due_at`은 §11.2의 관할 영업일·마감시각으로 freeze한다.
