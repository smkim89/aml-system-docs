...
| POST | `/api/v1/admin/aml/watchlist-rescreen-jobs/{jobId}:pause` | `aml:admin:watchlist` | — | **재스크린 잡 일시정지(aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit)** — `paused_at/paused_by`(V76) 설정, 이후 워커 claim 에서 제외. 4-eyes 아님(운영 안전장치, `mandatory-sources` 동형), 감사 `RESCREEN_JOB_PAUSE` 1건. 잡 없음 404, 이미 정지 409 `AML.STATE_CONFLICT`. 응답 `{jobId, paused:true, pausedAt, pausedBy}` | `aml_wlf_rescreen_jobs` |
| POST | `/api/v1/admin/aml/watchlist-rescreen-jobs/{jobId}:resume` | `aml:admin:watchlist` | — | **재스크린 잡 재개** — `paused_at/paused_by` 해제, 감사 `RESCREEN_JOB_RESUME`. 정지 아님 409. 응답 `{jobId, paused:false, pausedAt:null, pausedBy:null}` | `aml_wlf_rescreen_jobs` |
