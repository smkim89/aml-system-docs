...
| 일자 | 변경 | 비고 |
|---|---|---|
| 2026-08-17 | **숫자 신고소득·동결 evidence window·관할 보고기한 fail-safe 정합화.** 숫자 amount 키 presence가 malformed/null일 때 legacy band 폴백을 막고, ANNUAL은 DECIMAL128 월할·원문 period provenance를 보존한다. TM alert는 `evidenceWindow`를 동결해 binding 변경 뒤에도 동일 관련거래를 재현한다. `reportDeadlineAt`은 PH AMLC만 계산하며 다른 관할은 DRAFT 유지+null+구조화 WARN으로 명문화했다. | 코드=truth. 근거=aegis-aml `DeclaredIncomePolicy`·`IdentityProjectionService`·`AlertEvidenceWindowResolver`·`ReportDeadlinePolicy`. DB §3.3/§3.10/§3.12 동기화. |
