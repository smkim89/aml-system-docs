...
|---|---|---|---|---|---|
| POST | `/api/v1/aml/events` | `aml:event:write` | Y | canonical AML event 수신. `customer.cdd.completed`는 동기 1차 RA의 불변 업무결정(`APPROVE`/`REJECT`/`EDD_REQUIRED`)과 score/model snapshot 및 같은 score의 `nextReviewDueAt`(다음 CDD 재이행 예정일)을 함께 반환하며 replay는 최초 결정을 그대로 반환 | `aml_canonical_events`,`aml_cdd_onboarding_decisions` |
...
> - **(d) 당연고위험(HRR) 레지스트리 floor 병합(2026-09-10, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)** = 위 (a)~(c) 직후 대상 `customerRef` 가 당연고위험 레지스트리(§HRR admin surface)에 등재돼 있으면(`HighRiskRegistry.classify`) 그 floor(tier `HIGH→HIGH`·`VERY_HIGH→PROHIBITED`)를 병합 — `mandatoryHighRiskReasons` 에 매치된 참조 리스트 이름 추가·`forcedFloorEvidence` 에 `{listType,subjectRef,tier}` 추가(§3.3). **최초 CDD 뿐 아니라 등재 이후의 모든 재파생(재이행 re-CDD·국가정책 재평가)에서 매회 병합**되므로, 명단(WLF) 매치가 그사이 사라진 재CDD 도 등재가 유지되는 한 `mandatoryHighRisk=true` 로 유지된다(해소는 등재 해제 결재뿐). 등재 없으면 diff 0.
...
> **회원 심사결과 확정은 별개 후속 계약(PLAN 20260903-aml-decision-status-webhooks, 코드=truth).** 위 `customer.cdd.completed` 인입 결정(`aml_cdd_onboarding_decisions`, V42)은 인입 시점의 **불변 스냅샷**이며 이후 어떤 4-eyes 결재도 이 행을 갱신하지 않는다(F-076). 계정계가 "결재를 통해 승인/거절이 완료된 시점"의 상태값을 읽으려면 별도로 **회원(고객 관계) 심사결과 확정**(append-only `aml_member_decisions`, V74/V77)을 쓴다 — 온보딩 보류(`EDD_REQUIRED`) 회원의 EDD 케이스 종결(`EDD_CLOSE`) 4-eyes 결재가 **EXECUTED**되거나 관계거절(`RELATIONSHIP_REJECT`) 결재가 **EXECUTED**될 때, 또는 **(2026-09-09 신설 — PLAN 20260909-aml-ra-direct-member-decision)** 당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 이미 **EXECUTED**된 회원에 대해 RA 상세 관리자 액션의 **직접 확정**(`POST /api/v1/admin/aml/customers/{customerRef}/decision`, `source=ADMIN_DIRECT`, 결재 없이 즉시)이 성공할 때 행이 append되고, 같은 트랜잭션에서 웹훅 `AmlMemberDecisionResolved`(§8.1)가 아웃박스 enqueue된다. 결재 **반려**는 행·웹훅 모두 없음(F-020 `rejectPathNeverEmits` 선례) — 직접 확정 경로는 결재 자체가 없으므로 반려 개념이 적용되지 않는다. 조회는 §2.3 `GET /api/v1/aml/customers/{customerRef}/decision`, DB는 §3.x `aml_member_decisions`(DB 문서), 목적지 우선순위·payload 는 §8.1/§12.1a(`docs/aml-data.md`).
...
> **적용본 존재 게이트 → `503 AML.SCREENING_UNAVAILABLE`(P0-06, 2026-08-27 정책 정정·2026-09-07 IMPORTING 제외)**: `POST /api/v1/aml/screen`(및 `POST /internal/v1/aml/screen`)은 실제 스크리닝 전 tenant의 **필수 source 정책**(DB §3.6a)을 해소한다. 각 활성 필수 source는 적용본(`activeVersion`)이 있거나 capability=`NOT_APPLICABLE`+유효 waiver여야 한다. 적용본이 있으면 effectiveReadiness가 `STALE`·`FAILED`이어도 마지막 적용 ACTIVE 엔트리로 판정(`IMPORTING` 은 제외)하며, refresh/freshness는 명단관리 운영 신호로만 남는다. **필수 정책이 없는 tenant**는 fallback=적용본 source ≥1건이면 통과, 0건이면 fail-closed다. 적용본 자체가 없거나 waiver가 유효하지 않을 때만 `NO_MATCH` 합성 대신 **503 `AML.SCREENING_UNAVAILABLE`**를 반환한다. 기존 `ScreeningReadinessReason` wire enum은 호환을 위해 유지하지만 적용본이 있는 STALE/FAILED(IMPORTING 제외)은 차단 사유로 반환하지 않는다. 코드 truth=`WatchlistReadinessGateAdapter`·`WlfScreeningService`; rescreen 파이프라인은 integration §. IMPORTING(임포트 진행 중) 소스는 적용본이 있어도 **적격에서 제외**한다(2026-09-07 사용자 결정 A — 스트리밍 임포트가 구버전 ACTIVE 행을 배치 단위로 옮기는 동안 버전 스코프 후보 recall 에서 대상이 빠질 수 있음; 해당 소스만으로 필수 정책을 충족하던 경우 503/HELD).
...
| POST | `/api/v1/admin/aml/customers/{customerRef}/decision` | `aml:admin:approval` | — | **회원 심사결과 직접 확정**(신규, PLAN 20260909-aml-ra-direct-member-decision) — 당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 이미 **EXECUTED**된 회원에 한해 추가 4-eyes 결재 없이 RA 상세 관리자 액션에서 즉시 확정한다. body `{decision: APPROVED\|REJECTED, reason?}`(actor=`X-User-Subject`). 게이트: ⓐ 회원 미존재 **404** ⓑ `decision` 값 오류 **400** ⓒ `reason` 검증 — `REJECTED`인데 `reason` 공백이면 **400**(`REJECTED` 한정, `APPROVED`는 `reason` 공백 허용), `reason`이 256자 초과면 `decision` 값과 무관하게 공통으로 **400** ⓓ HRR 등재 미완료 **409** `AML.STATE_CONFLICT` ⓔ 대기 중 `EDD_CLOSE`/`RELATIONSHIP_REJECT` 결재 존재 **409** `AML.STATE_CONFLICT`(HRR 등재 **해제** 결재가 대기 중이어도 등재 read-back `registered=true`이면 이 게이트에 걸리지 않고 직접 확정 가능 — F-095 경계, `deriveHrrFlow`는 해제 결재를 등재 단계 판정에 쓰지 않음) ⓕ 현재 결정과 요청 `decision`이 동일(중복 확정) **409** `AML.STATE_CONFLICT` ⓖ actor 미검증(`X-User-Subject` 미서명/공백/128자 초과/제어문자) **400**. 통과 시 `aml_member_decisions`에 `source=ADMIN_DIRECT`(`approvalId`/`caseId`/`caseType`/`finalStatus` 모두 `null`) 행을 append하고 같은 트랜잭션에서 웹훅 `AmlMemberDecisionResolved`(§8.1) 아웃박스 enqueue + 감사(`CASE_APPROVAL`, action `MEMBER_DECISION`, source `ADMIN_DIRECT`). 응답은 `MemberDecisionDto`(위와 동일 shape, 확정 후 `history[0]`). bo-api 미러 `POST /api/v1/bo/aml/customers/{ref}/decision`(scope `aml:admin:approval` 또는 `BO_SUPER_ADMIN`, fail-closed — 엔진 위임 미구성 시 `ENGINE_UNAVAILABLE`, 엔진 4xx는 상태·코드 그대로 전파) | `aml_member_decisions`,`aml_audit_events(CASE_APPROVAL)`,`aml_outbox(AmlMemberDecisionResolved)` |
...
> | `source` | enum | **`MemberDecisionSource`**: `INGEST`(비영속 — CDD 인입 스냅샷 파생)\|`EDD_CLOSE`\|`RELATIONSHIP_REJECT`\|`ADMIN_DIRECT`(2026-09-09 신설 — HRR 등재 EXECUTED 회원에 대한 관리자 직접 확정, 결재 없음) |
> | `reason` | string\|null | 결재 사유(확정 행 `EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`, 자유문 — `ADMIN_DIRECT`는 `APPROVED`일 때만 선택) 또는 **인입 스냅샷 `OnboardingDecisionReason` enum 원문**(INGEST — `aml_cdd_onboarding_decisions.reason` NOT NULL 이므로 non-null, 예 `ONBOARDING_RA_HELD`; 2라운드 QA B1 미러). 확정 행의 결재 사유 공란일 때만 `null` |
> | `decidedAt` | string(date-time) | 확정/인입 시각 |
> | `decidedBy` | string\|null | 결재 checker(확정 행, `ADMIN_DIRECT`는 직접 확정 actor) 또는 `null`(INGEST) |
> | `approvalId`/`caseId` | string(uuid)\|null | 결재 실행 근거(4-eyes 확정 행만 — `ADMIN_DIRECT`는 결재·케이스가 없으므로 `null`, INGEST도 `null`) |
> | `caseType`/`finalStatus` | enum\|null | 케이스 유형·종결 상태(4-eyes 확정 행만 — `ADMIN_DIRECT`는 `null`) |
...
>
> **회원 결정을 만들지 않는 4-eyes 결재(제외 명시)**: `HRR_REGISTRATION`(당연고위험 등재 — `AmlHighRiskRegistrationApproved` 콜백이 별도 담당, §8.1)·`PEP_APPROVAL`(PEP 관계 승인 — 등급·EDD 판정에 반영되어 EDD 케이스 처분으로 귀결)·`RISK_OVERRIDE`(등급 수동 조정 — 점수 이력이지 온보딩 승인/거절 아님)·`HIGH_RISK_REGISTRY`(참조 리스트 변경 — 회원 개별 결정 아님). 이들은 `history`에도 나타나지 않는다. **(2026-09-09 정정)** `HRR_REGISTRATION` 결재 **자체**는 여전히 회원 결정을 만들지 않지만, 그 결재가 `EXECUTED`(등재 완료)된 회원은 RA 상세 관리자 액션의 **직접 확정**(`POST /api/v1/admin/aml/customers/{customerRef}/decision`, `source=ADMIN_DIRECT`)이라는 별도 사람 액션으로 회원 결정을 만들 수 있다 — 이는 `HRR_REGISTRATION` 결재의 부수효과가 아니라 등재 상태를 전제조건(게이트 ⓓ)으로만 참조하는 독립 트랜잭션이다. `EDD_CLOSE`는 **온보딩 보류 발단**(케이스 `caseType==EDD_REVIEW` **∧** 확정 전 회원의 현재 결정이 `EDD_REQUIRED`)일 때만 회원 결정을 만든다 — 이미 `APPROVED`인 회원의 2차 RA(ONGOING)·FDS 발단 EDD_REVIEW 종결은 상시감시 결과라 회원 결정을 만들지 않는다(workflow-guide §① L65 정합). `RELATIONSHIP_REJECT`는 케이스 유형·현재 결정과 무관하게 항상 `REJECTED` 회원 결정을 만든다. **(설계 의도 명문화)** 직접 확정(`source=ADMIN_DIRECT`, `decision=APPROVED`)이 선행된 회원은 이후 진행 중이던 `EDD_CLOSE` 결재가 `EXECUTED`되어도 확정 전 현재 결정이 이미 `APPROVED`라 온보딩 보류 술어(`EDD_REQUIRED`)를 만족하지 못하므로 새 회원 결정 행·웹훅을 만들지 않는다(로그 1행만 남김) — 그 `EDD_CLOSE`가 근거로 삼은 EDD 케이스 자체는 회원 결정과 별개로 정상 종결 처리해야 한다.
...
| POST | `/internal/v1/aml/fds-decisions` | `fds-svc` | **FDS 차단 판정 통지 수신**(aegis-aml PLAN 20260906-fds-block-aml-propagation, body §3.10a `FdsDecisionNotifyRequest`). `eventType` 은 `fds.decision.blocked` 만 허용(그 외 400), `transactionRef`·`decisionId`·`outcome` 필수, `eventId` 부재 시 `decision-blocked:{decisionId}`. `RecordFdsTransactionDecisionUseCase` → `aml_fds_transaction_decisions` `UNIQUE(tenant_id, fds_decision_id)` 멱등 append(알럿·케이스 미개설, 감사 1건). 응답 `202 { recorded, decisionId }`(replay 는 `recorded=false`). 가드는 `fds-escalations` 와 동일 — `aml:internal:fds-escalation:write`, signed `X-Internal-Service=fds-svc`, body/header dataScope 일치. SQS `aml-fds-decision` 경로(`FdsDecisionConsumer` eventType 분기)와 동일 usecase. | `aml_fds_transaction_decisions` |
...
| POST | `/api/v1/admin/aml/watchlist-sources/{sourceCode}/sync` | `aml:admin:watchlist` | — (auto-apply, 4-eyes 아님) | **명단 소스 동기화 — 코드 7종**(`OFAC_SDN`·`UN_CONSOLIDATED`·`EU_CFSL`·`UK_OFSI`·`AU_DFAT`·`JP_MOF_FEFTA`·`DILISENSE_CONSOLIDATED`, `WatchlistFeedRouter` 라우팅). 공개 6종은 OFAC/UN/EU/UK StAX XML, AU 자체 XLSX reader, JP HTML→CSV 2단계 discovery 파서를 사용하고 dilisense는 JSON Lines 전량/delta를 사용한다 — fetch→파싱→멱등 upsert(외부키 `external_ref`, entry-id 승계)→**auto-apply**→DELISTED 보존→freshness 갱신. 외부망 장애는 예외 미전파(fail-safe, 200+`outcome=FAILED`)이며 마지막 성공 `activeVersion`·`lastImportedAt`은 보존된다. 적용본이 있으면 FAILED/48h 초과여도 그 엔트리로 RA/WLF 평가를 계속하고 운영자는 본 응답과 source 목록에서 refresh 이상을 별도 확인한다. **`DILISENSE_CONSOLIDATED`는 API 키 미설정 시 `outcome=FAILED`**지만 기존 적용본은 계속 사용된다. 응답 `WatchlistSyncResult`(sourceCode·outcome(APPLIED/UNCHANGED/FAILED)·activeVersion·ingestedCount·delistedCount·prunedCount·lastImportedAt). 스케줄러 일일 자동 + 본 엔드포인트 수동 트리거. | `aml_watchlist_sources`,`aml_watchlist_entries` |
...
| POST | `/api/v1/admin/aml/watchlist-rescreen-jobs/{jobId}:pause` | `aml:admin:watchlist` | — | **재스크린 잡 일시정지(aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit)** — `paused_at/paused_by`(V76) 설정, 이후 워커 claim 에서 제외. 4-eyes 아님(운영 안전장치, `mandatory-sources` 동형), 감사 `RESCREEN_JOB_PAUSE` 1건. 잡 없음 404, 이미 정지 409 `AML.STATE_CONFLICT`. 응답 `{jobId, paused:true, pausedAt, pausedBy}` | `aml_wlf_rescreen_jobs`, `aml_audit_events`(카테고리 `WATCHLIST_READINESS`, detail `action=RESCREEN_JOB_PAUSE|RESUME`) |
| POST | `/api/v1/admin/aml/watchlist-rescreen-jobs/{jobId}:resume` | `aml:admin:watchlist` | — | **재스크린 잡 재개** — `paused_at/paused_by` 해제, 감사 `RESCREEN_JOB_RESUME`. 정지 아님 409. 응답 `{jobId, paused:false, pausedAt:null, pausedBy:null}` | `aml_wlf_rescreen_jobs`, `aml_audit_events`(카테고리 `WATCHLIST_READINESS`, detail `action=RESCREEN_JOB_PAUSE|RESUME`) |
| POST | `/api/v1/admin/aml/watchlist-sources/{sourceCode}:override` | `aml:admin:watchlist` | — | **긴급 readiness 관리 override(P0-06)** — source를 시한부 `OVERRIDDEN`으로 전이해 운영자가 상태를 인지·승인했음을 기록한다. 요청/응답·만료·`WATCHLIST_READINESS` 감사 계약은 불변이다. 평가 가능성은 별도 `activeVersion` 존재 기준이므로 override가 적용본을 합성하지 않으며, 적용본이 없는 source를 통과시키지 않는다. 적용본이 있는 STALE/FAILED(IMPORTING 제외)은 override 없이도 마지막 명단으로 평가한다. | `aml_watchlist_sources`,`aml_audit_events(WATCHLIST_READINESS)` |
...
> **bo-api 위임(§10.4).** BO 화면 수동 트리거는 `POST /api/v1/bo/aml/watchlist-sources/{sourceCode}/sync`(scope `aml:admin:watchlist` or `BO_SUPER_ADMIN`, `AmlWatchlistController`) → `AmlEngineClient`로 위 엔진 `.../{code}/sync`에 순수 위임한다(응답 `WatchlistSyncResponse` 미러, 운영자 감사 `WATCHLIST_IMPORT_APPLIED`·trigger MANUAL). 제재명단 수집은 엔진 전용 표면이라 비위임(stub) 모드는 503 `AML.ENGINE_UNAVAILABLE`로 위조 성공·freshness 갱신을 만들지 않는다.
...
| GET | `/api/v1/admin/aml/members/{memberRef}/cdd-history?types=&page=&size=` | `aml:case:read` | — | 회원 CDD/EDD/심사결과 이력 페이지(최신순). **`types` 반복 파라미터**(예 `types=CDD_INITIAL&types=EDD_OPENED`, 생략 시 전체) enum **7종** `CDD_INITIAL·CDD_REVIEW·EDD_OPENED·EDD_CLOSED·CDD_REISSUE_REQUESTED·EDD_REISSUE_REQUESTED·MEMBER_DECISION`(DB §3.22f `history_type` = 엔진 `CddHistoryType`, §5.36, V27·V78). 잘못된 유형 토큰은 400. 응답 페이징 봉투 `{ rows, page, size, totalElements, totalPages }`, 행은 masked 스냅샷(`historyId·historyType·kycStatus·riskGrade·sourceEventId·traceId·actor·details·occurredAt`) — raw PII 미노출. `historyType=MEMBER_DECISION` 행의 `details` 키는 `decision`(`APPROVED\|REJECTED\|REPORTED`)·`source`(`EDD_CLOSE\|RELATIONSHIP_REJECT\|ADMIN_DIRECT`)·`decisionId`·`approvalId`(있을 때만)·`caseId`(있을 때만) — 회원 심사결과 확정(§2.3 `MemberDecisionDto`) 시 같은 트랜잭션에서 append(V78). **(2026-09-10 QA 정정)** `details.decision` 값 `APPROVED\|REJECTED\|REPORTED`는 **확정 행(`aml_member_decisions`) 한정** — CDD 인입 스냅샷의 `EDD_REQUIRED`는 `MEMBER_DECISION` 이력 대상이 아니다(확정 3출처(`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`)만 이 이력을 append 하며, `INGEST` 소스는 §2.3 병합 뷰에만 나타나고 본 이력에는 나타나지 않는다). **(2026-09-10 신설, PLAN 20260910-aml-member-history-decision-column, additive)** 각 행 말미에 `ingestDecision{decision, reason, riskGrade, requiredAction, scoreId, createdAt}\|null` — `historyType∈{CDD_INITIAL,CDD_REVIEW}` 이고 그 인입(`sourceEventId`)의 온보딩 판정 스냅샷(`aml_cdd_onboarding_decisions`, DB §3.22f, F-076 불변)이 있으면 `event_id` 조인으로 병기한 값(전 필드 String/Instant — enum 은 `name()`), 그 외 유형·스냅샷 부재는 `null`. 이력 행·판정 스냅샷 모두 무수정(읽기 시 병기), 기존 9필드 불변. **(2026-09-10 QA 명세 리뷰 정정)** 엔진은 `ingestDecision` 키 존재+명시 `null`, bo-api 미러는 `@JsonInclude(NON_NULL)` 로 키 생략 — 소비자는 생략=null 동일 처리(F-076 ③ 선례 동형) | `aml_member_cdd_history` |
...
| PUT | `/api/v1/admin/aml/high-risk-registry/reference-lists/{listType}` | `aml:admin:high-risk-registry` | 🔒4-eyes(`HIGH_RISK_REGISTRY`) | 참조 리스트 변경 상신(**대상 `listType` 교체** — 운영자 관리 3종 `PRODUCT`·`VASP`·`HIGH_NET_WORTH` 한정. 자동 등재 2종 `PEP_INDIVIDUALS`·`RA_HIGH_RISK_CUSTOMERS` 는 listType 상신 시 400 `AML.VALIDATION_ERROR`로 거부되며 bo-api 위임도 엔진 상태·코드를 보존해 동일하게 400 전달, `UPDATE\|<version>` subjectRef, 전체 staged payload drift guard) — 결재 EXECUTED 적용 시 자동 등재 2종은 적용 시점 저장본을 보존하고 관리 3종만 교체, version 은 적용 시점 현재+1(상신 시 `UPDATE\|<version>` 은 상신 시점 예상값) + 일치 고객 RA 강제 상향 재평가 트리거. **동시 결재(참조 리스트 변경·`HRR_REGISTRATION`·`PEP_APPROVAL`)는 레지스트리 헤더 행 잠금(DB §3.19)으로 직렬화되어 각 EXECUTED 가 version 을 정확히 +1 한다(2026-09-04).** | `aml_high_risk_registry`,`aml_high_risk_registry_items`,`aml_approvals` |
...
| POST | `/api/v1/admin/aml/high-risk-registry/registrations/{customerRef}:deregister` | `aml:admin:high-risk-registry` | 🔒4-eyes(`HRR_REGISTRATION`, `subjectRef=DEREGISTER\|customerRef`, 승인선 `EXECUTIVE_APPROVAL`) | **(2026-09-04 신설, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095)** `RA_HIGH_RISK_CUSTOMERS` 자동 등재 회원의 등재 해제 상신(오등재 정정). body `{makerId, reason, callbackUrl?}`. **신규 상신 = `202 {approvalId, customerRef, subjectType:HRR_REGISTRATION, action:DEREGISTER, approvalLine:EXECUTIVE_APPROVAL, status:SUBMITTED}`**. **멱등 no-op(미등재 또는 해제 pending) = `200`** — 202 응답과 동형인 6필드 본문(`{approvalId, customerRef, subjectType:HRR_REGISTRATION, action:DEREGISTER, approvalLine:EXECUTIVE_APPROVAL, status}`)을 그대로 반환하되 `approvalId=null`·`status:NOOP`. 재파생 사전조건(온보딩 CDD 스냅샷·원 CDD 스크리닝 행) 미충족 또는 독립 당연고위험 사유(다른 참조 리스트 멤버십·WLF/국가위험 유래 강제 floor 사유) 보유 시 `400`. 승인 EXECUTED 시 레지스트리 헤더 잠금 재획득 스냅샷에서 독립 사유를 재검증하고 위반 시 `409 AML.HRR_DEREGISTRATION_BLOCKED`(결재는 SUBMITTED 잔류, 재시도 가능) — 통과 시 항목 제거+`version+1`+해당 회원 온보딩 CDD 스냅샷 재파생을 시도하되, 재파생이 보류되면(워치리스트 stale·ACTIVE ONBOARDING 모델 부재) `409 AML.STATE_CONFLICT`(결재는 SUBMITTED 잔류, 재시도 가능 — 잠금 스냅샷 재검증 실패의 `409 AML.HRR_DEREGISTRATION_BLOCKED` 와 코드로 구분)를 반환한다. 잠금 스냅샷 재검증 시점에 이미 미등재 상태면 레지스트리 무변경·재파생·콜백 없이 그대로 EXECUTED(no-op) 처리한다(§12-B.6 BR-009, `02-aml-sass-functional-spec.md`) | `aml_high_risk_registry_items`,`aml_approvals`,`aml_risk_scores` |
| GET | `/api/v1/admin/aml/high-risk-registry/registrations/{customerRef}` | `aml:admin:high-risk-registry` | — | 당연고위험 등재 상태(미상신/PENDING/EXECUTED) read-back. 응답 `{customerRef, registered, pending, pendingApprovalId, deregistrationPending, pendingDeregistrationApprovalId}`(마지막 2필드는 **2026-09-04 additive**, aegis-aml F-095 — 해제 상신 대기중 여부·그 approvalId) — RA 상세 등재 결재 상태 패널, no-op 세분(`registered` vs `pending`) 판별 | `aml_high_risk_registry_items`,`aml_approvals` |
...
> **bo-api stub fallback 자동 등재 listType 가드(2026-09-04, PLAN 20260904-aml-hrr-registry-row-lock, aegis-aml F-094).** 엔진 미가용 비운영 stub fallback 경로(`AmlHighRiskRegistryService.update`)도 자동 등재 2종(`PEP_INDIVIDUALS`·`RA_HIGH_RISK_CUSTOMERS`) `PUT` 상신을 **400 `AML.BAD_REQUEST`** 로 거부한다 — 엔진 가용 시 passthrough 는 무변경으로 **400 `AML.VALIDATION_ERROR`** 를 전달한다(status 동일·code 상이는 수용된 비대칭이며 stub 전용이라 별도 정정 대상이 아니다).

> **bo-api 등재 해제 위임(2026-09-04 신설, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095).** `POST /api/v1/bo/aml/high-risk-registry/registrations/{customerRef}:deregister`(scope 등재 상신과 동일, body `HrrDeregistrationRequest{reason}` — R)가 위 엔진 `:deregister` 로 위임한다. 엔진 `202 SUBMITTED` → bo-api `202 {approvalId, memberRef, subjectType:HRR_REGISTRATION, action:DEREGISTER, approvalLine, status, makerId, alreadyDeregistered:false, pending:false, submittedAt}`. 엔진 `200 NOOP` → bo-api 가 read-back(`GET .../registrations/{customerRef}`)으로 미등재(`alreadyDeregistered=true`)/해제 pending(`pending=true`) 을 세분한 뒤 **`202`**(등재 상신 no-op 응답 계약과 동형, 위 no-op 계약 문단 참조)로 반환 — 엔진 `400`은 그대로 passthrough. 엔진 위임 본문은 `{makerId, reason}`(엔진 계약 1:1), Idempotency-Key `hrr-deregistration:<customerRef>`(엔진 미소비, 등재 위임 선례 동형). 감사 `HRR_DEREGISTRATION_SUBMITTED`(`DATA`, bo-api Flyway **V25** allowlist).
...
응답 `IngestEventResponse`: `{ eventId, accepted, idempotent, traceId, decision?, scoreId?, riskScore?, riskGrade?, requiredAction?, modelVersion?, reason?, mandatoryHighRisk?, mandatoryHighRiskReasons?, wlfHit?, nextReviewDueAt? }`. CDD 외 이벤트는 decision 계열과 `nextReviewDueAt`이 null이고, `customer.cdd.completed` ACCEPTED/REPLAYED는 V42 불변 온보딩 decision snapshot을 반환한다. `mandatoryHighRisk`/`mandatoryHighRiskReasons`/`wlfHit`는 **additive** 1차 RA 요약(20260718 사용자 지시, F-003 해제, §2.1 note 참조)이며, `nextReviewDueAt`(string(date-time)\|null)은 **additive** 다음 CDD 재이행 예정일(20260904)이다. 값은 같은 불변 RA score에서 읽고 HELD/score 부재 시 null이며 기존 필드 이름·순서·타입은 불변이다.
...
| `mandatoryHighRiskReasons` | array&lt;string&gt; | 강제 등급 상향(floor) 적용 사유 코드 배열(영문 코드 passthrough, raw PII 없음). 강제 상향 미적용 시 빈 배열. **값집합 — (a) 당연고위험(HRR) 레지스트리·명단 floor**: `SANCTION`/`PEP`(WLF 진성 매치 명단 유형)·등재된 **참조 리스트 이름**(`RA_HIGH_RISK_CUSTOMERS`/`PEP_INDIVIDUALS`/`HIGH_NET_WORTH`/`PRODUCT`/`VASP`, §12-B.6 AML-HRR-001 5종 — `HighRiskRegistry.matchedReferences` 이름 그대로 병기). **(2026-09-10, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)** 레지스트리 등재 floor 는 등재 **이후의 모든 온보딩 파생**(최초 CDD·재이행 re-CDD·국가정책 재평가)에서 스크리닝/국가위험 floor 와 병합되어 매회 재산출된다 — 명단(WLF) 매치가 그 사이 사라진 재CDD 응답(`POST /api/v1/aml/events` `IngestEventResponse.mandatoryHighRiskReasons`, §2.1 note 동일 값 소스)에도 등재 참조 리스트 이름이 그대로 남는다(해소는 §12-B.6 BR-009 등재 해제 결재뿐, 명단 매치 소멸로 자동 해소되지 않음). **(b) 국가위험 등급-기반 floor(V20·기능정의서 §5.1 (b′)·DB §7)**: **`HIGH_RISK_COUNTRY_NATIONALITY`**(고위험 국적)·**`HIGH_RISK_COUNTRY_RESIDENCE`**(고위험 거주국) 2종 — 국적/거주국 국가위험 ACTIVE 등급(`PROHIBITED→HIGH`·`HIGH→MEDIUM`)으로 결정적 상향, 해당 국가만 개별 병기. **국가위험 floor 는 HRR 과 구분**(모델 정본 `aml_risk_models.parameters.countryFloor` 소비, `forcedFloorEvidence` 원소 미추가) — FE(라벨 사전 `lib/aml-risk.ts`)는 국가위험 코드에 "국가위험 floor(HRR 아님)" 배지, HRR 코드에 "HRR" 배지를 매핑한다. 미지 코드는 raw 표기(graceful passthrough) |
| `forcedFloorEvidence` | array&lt;object&gt;\|null | **강제 상향(floor) 근거 참조**(#1). 원소 형태 2종이 혼재한다 — **(1) 스크리닝(WLF) 매치 근거**: `{ listType(string — 매치 명단 유형, 예 `SANCTION`/`PEP`), screeningId(string\|null — 매치 스크리닝 실행 참조 토큰), entryId(string\|null — 매치 명단 엔트리 참조 토큰), label(string\|null — 사유 라벨) }`. **(2) 당연고위험(HRR) 레지스트리 등재 근거(2026-09-10, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd)**: `{ listType(string — 등재 참조 리스트 이름, 예 `RA_HIGH_RISK_CUSTOMERS`/`PEP_INDIVIDUALS`/`HIGH_NET_WORTH`/`PRODUCT`/`VASP`), subjectRef(string — 레지스트리 등재 주체 참조 토큰), tier(string — 등재 tier, `HIGH`\|`VERY_HIGH`) }`(엔진 `HighRiskRegistry.forcedFloorFrom(subjectRef, items)` 파생(온보딩 경로는 포트 `findForcedFloor` 가 등재 아이템만 읽어 위임, 인스턴스 `forcedFloorFor` 도 같은 정적 팩토리로 위임), `CustomerRiskController`/`RiskScoreAdminController` passthrough) — screeningId/entryId/label 필드는 없다. **masked 참조 토큰만 — raw PII 없음**. **(2026-09-10 정정, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-followups)** bo-api 미러는 **6키 `{listType, screeningId, entryId, label, subjectRef, tier}`** 로 양쪽 원소 형태를 모두 보존한다 — explicit 필드 매퍼(`AmlRaService.forcedFloorEvidence()`)와 `factorBreakdown.forcedFloor.evidence` 폴백 매퍼(`AmlRaService.forcedFloorEvidenceMarkers()`) **두 경로 모두** `subjectRef`/`tier` 를 `RaDtos.ForcedFloorEvidence` 에 싣는다(스크리닝 원소는 `subjectRef`/`tier` 가 `null`, 레지스트리 원소는 `screeningId`/`entryId` 가 `null` — 소실 없음, 구 4키 고정 매핑 문구는 폐기). **국가위험 등급-기반 floor(`HIGH_RISK_COUNTRY_*`)는 이 배열에 원소를 추가하지 않고 `mandatoryHighRiskReasons` 코드로만 노출**(명단 매치가 아니므로 entry/screening 토큰 없음) — 명단 매치·레지스트리 등재가 모두 없으면 `null`/빈 배열. FE 는 명단 유형 뱃지 표시 + 명단 엔트리 브라우저 딥링크(§딥링크 계약)에 사용(딥링크는 스크리닝 원소 `entryId` 전제, 레지스트리 원소는 `entryId` 부재로 비적용) |
...
| `ingestedAt` | string(date-time)\|null | **회원 최초 인입 시각**(`aml_customers.created_at`, additive 2026-09-08) — 목록 `sort=INGESTED_AT_DESC` 의 정렬 키·화면 "인입시각" 컬럼. 고객 행이 없으면 null — 목록(`GET /api/v1/admin/aml/risk-scores`) 전용, 상세 §2.3·평가 POST 응답 미포함 |
...
### 3.10a FdsDecisionNotifyRequest → `POST /internal/v1/aml/fds-decisions` (DB `aml_fds_transaction_decisions`, aegis-aml PLAN 20260906-fds-block-aml-propagation)

| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `eventType` | string | Y | `fds.decision.blocked` 고정(그 외 400) |
| `eventId` | string | N | 멱등키. 부재 시 `decision-blocked:{decisionId}` |
| `decisionId` | string(UUID) | Y | FDS `fds_decisions.decision_id` → `fds_decision_id`(UNIQUE) |
| `fdsEventRef` | string | N | FDS 인입 `eventId` |
| `transactionRef` | string | Y | 계정계 거래참조(= AML `transactionReference`) — 거래 read model 조인 키 |
| `targetRef` | string | N | 회원 키(참조 토큰) |
| `outcome` | string | Y | FDS `DecisionOutcome`(현재 `BLOCK` 만 발행, raw pass-through) |
| `phase` | string | N | `INLINE`/`ASYNC` |
| `matchedRules[]` | `{ruleId, ruleName, versionNo, outcome}` | N | 발동 룰 전건(룰별 outcome 포함). fail-safe 판정은 `[]` |
| `reasonCodes[]` | string[] | N | FDS reason code |
| `decidedAt` | ISO-8601 | Y | FDS 판정 시각 |
...
| 503 | `AML.SCREENING_UNAVAILABLE` | WLF 엔진 장애·PII reveal 역참조 실패·**필수 source 적용본 부재/waiver 미승인(P0-06)**·**후보 recall SQL timeout**. 적용본이 있는 STALE/FAILED(IMPORTING 제외)은 503이 아니라 마지막 적용본으로 판정한다. 적용본 부재 계열 details는 `NO_MANDATORY_POLICY`/`NO_READY_SOURCE`/`MISSING_SOURCE`/`NOT_READY`/`FAILED`(실패했고 적용본 없음)/`NOT_APPLICABLE_UNAPPROVED`, 후보 timeout은 `CANDIDATE_QUERY_TIMEOUT`(§2.2) |
  ...
  schemas:
    ...
    IngestEventResponse:
      ...
      properties:
        ...
        traceId: { type: string }
        nextReviewDueAt: { type: string, format: date-time, nullable: true, description: 동일 RA score가 산출한 다음 CDD 재이행 예정일; HELD/score 부재 시 null }
...
| `AmlMemberDecisionResolved` | (신규, PLAN 20260903-aml-decision-status-webhooks) `EDD_CLOSE`/`RELATIONSHIP_REJECT` 4-eyes 결재 EXECUTED(**거절 시 미발행**), 또는 **(2026-09-09 신설, PLAN 20260909-aml-ra-direct-member-decision)** HRR 등재 EXECUTED 회원에 대한 RA 상세 관리자 직접 확정(`ADMIN_DIRECT`, 결재 없이 즉시) 성공 | Member Decision(§2.3) | `memberRef`,`decision`(`APPROVED`\|`REJECTED`\|`EDD_REQUIRED`\|`REPORTED`),`source`(`EDD_CLOSE`\|`RELATIONSHIP_REJECT`\|`ADMIN_DIRECT`),`reason`,`approvalId`,`caseId`,`caseType`,`finalStatus`,`scoreId`,`riskGrade`,`requiredAction`,`checkerId`,`decidedAt` — `source=ADMIN_DIRECT`는 `approvalId`/`caseId`/`caseType`/`finalStatus`가 모두 `null` |
...
| `AmlHighRiskRegistrationRevoked` | (신규, PLAN 20260904-aml-hrr-deregistration) `HRR_REGISTRATION`(`subjectRef=DEREGISTER\|customerRef`) 등재 해제 4-eyes 승인 EXECUTED(**거절 시 미발행**) | RA(등재 해제) | `memberRef`,`approvalId`,`decision`("REVOKED"),`checkerId`,`approvedAt` |

> 7종은 정본 콜백 집합(2026-07-24 `AmlHighRiskRegistrationApproved` 신설, 2026-09-03 `AmlMemberDecisionResolved`·`AmlScreeningWhitelistChanged` 신설 + `AmlScreeningResolved` data additive 보강, **2026-09-04 `AmlHighRiskRegistrationRevoked` 신설**). enum 코드값은 DB §5와 동일. payload는 token/hash·마스킹만(원문 미포함).
>
...
| 2026-09-10 | **§2.x 회원원장 read `GET .../members/{memberRef}/cdd-history` `types` enum 6종→7종(`MEMBER_DECISION` 추가)·`details` 키 명시(코드=truth, aegis-aml PLAN 20260910-aml-member-ledger-decision-history).** 회원 심사결과 확정(`EDD_CLOSE`/`RELATIONSHIP_REJECT`/`ADMIN_DIRECT`, §2.3)이 같은 트랜잭션 안에 `aml_member_cdd_history` 에 `MEMBER_DECISION` 1행을 append 한다 — `details={decision, source, decisionId, approvalId?, caseId?}`. 기존 `aml_member_decisions` 행은 V78 로 backfill(멱등). 엔드포인트 신규 없음(기존 `cdd-history` 응답 행 유형 additive), `GET .../customers/{ref}/decision`(§2.3) 계약 무변경. | api-designer. 코드 truth=aml-svc `application/usecase/{MemberLedgerService,MemberDecisionService}`·`domain/enums/CddHistoryType`. DB §3.22f·§5.36·§7(V78)·PRD §12-A.10 BR-001 동일 작업 단위. |
| 2026-09-10 | **§3.3 `RiskScoreResponse.forcedFloorEvidence` 필드 정정 — "명단 매치 floor 전용" 문구 폐기, 레지스트리 원소 형태 병기(QA 명세 리뷰 반영, 코드=truth, aegis-aml PLAN 20260910-aml-hrr-floor-on-recdd).** 원소가 스크리닝(WLF) 매치 근거 `{listType,screeningId,entryId,label}` 만이 아니라 당연고위험(HRR) 레지스트리 등재 근거 `{listType,subjectRef,tier}`(엔진 `HighRiskRegistry.forcedFloorFor` 파생(내부 `classify`/`matchedReferences`)) 도 **같은 배열에 병합**돼 내려옴을 명시 — 구 서술은 "명단 매치 floor 전용"으로 레지스트리 원소 형태를 누락했다. **bo-api 미러(`AmlRaService.forcedFloorEvidence()`)는 4키 고정 매핑**이라 레지스트리 원소의 `subjectRef`/`tier` 가 소실되고 `screeningId`/`entryId`=`null` 로 내려가는 계약 상 손실도 명문화(엔진 직접 조회만 원형 보존). 신규 필드·엔드포인트 없음(필드 서술 정정만). **(2026-09-10 QA 명세 리뷰 재정정)** 원소 조립 정본 메서드 표기를 `HighRiskRegistry.classify` → `HighRiskRegistry.forcedFloorFor`(내부에서 `classify`/`matchedReferences` 사용)로 정합. | api-designer. 코드 truth=aml-svc `application/usecase/OnboardingRaDerivationService`·`domain/registry/HighRiskRegistry#forcedFloorFor`·`adapter/in/rest/{CustomerRiskController,RiskScoreAdminController}`, bo-api `aml/ra/service/AmlRaService#forcedFloorEvidence`(약 2295~2316행). PRD §12-B.6 BR-010·§12-A.4 BR-010 동일 작업 단위. |
| 2026-09-11 | **§3.3 `forcedFloorEvidence` 레지스트리 원소 출처 문구 정정(API §3.3 출처 문구 정정, QA 명세 리뷰 반영, 코드=truth).** 레지스트리 원소 `{listType,subjectRef,tier}` 조립 정본 메서드 표기를 `HighRiskRegistry.forcedFloorFor`(내부 `classify`/`matchedReferences`) → `HighRiskRegistry.forcedFloorFrom(subjectRef, items)`(온보딩 경로는 포트 `findForcedFloor` 가 등재 아이템만 읽어 위임, 인스턴스 `forcedFloorFor` 도 같은 정적 팩토리로 위임)로 정정 — `forcedFloorFor`/`findForcedFloor` 양 경로가 공통으로 위임하는 정적 조립 메서드가 `forcedFloorFrom`임을 명확히 한다. 필드·키·엔드포인트 무변경(출처 서술 정정만). | api-designer. 코드 truth=aml-svc `domain/registry/HighRiskRegistry#{forcedFloorFrom,forcedFloorFor}`·`application/port/out/HighRiskRegistryStorePort#findForcedFloor`. |
| 2026-09-09 | **§2.1·§2.3·§8.1 회원 심사결과 직접 확정(`POST /api/v1/admin/aml/customers/{customerRef}/decision`, `source=ADMIN_DIRECT`) 신설(코드=truth, aegis-aml PLAN 20260909-aml-ra-direct-member-decision).** 당연고위험(HRR) 등재가 경영진 결재로 EXECUTED 된 회원에 한해 4-eyes 결재 없이 RA 상세 관리자 액션이 회원 심사결과를 직접 확정하는 엔드포인트 신설. §2.1 CDD 인입 후주·§2.3 `MemberDecisionDto.source` 값 목록·`GET .../decision` 제외 명시 문단·§8.1 `AmlMemberDecisionResolved` 발행 시점을 함께 갱신(기존 `EDD_CLOSE`/`RELATIONSHIP_REJECT` 경로 계약 무변경). bo-api 미러 `POST /api/v1/bo/aml/customers/{ref}/decision`(fail-closed) 신설. | api-designer. 코드 truth=aml-svc `adapter/in/rest/MemberDecisionAdminController`·`application/usecase/DirectMemberDecisionService`·`domain/decision/MemberDecision#fromAdminDirect`, bo-api `aml/ra/{controller/AmlRiskReadController,service/AmlRaService,dto/RaDtos}`. DB §3.22g·§7(V77)·PRD §12-A.4 BR-009 동일 작업 단위. |
| 2026-09-08 | **RA 점수 목록 `sort` 파라미터(`EVALUATED_AT_DESC`\|`INGESTED_AT_DESC`)·기본 정렬 2차 키(`scoreId` 문자열 desc)·응답 `ingestedAt` additive(코드=truth, aegis-aml PLAN 20260908-aml-ra-initial-list-ingest-sort).** 1차 RA 목록이 일괄 재평가 후에도 최근 인입 회원을 상단에 두도록 회원 최초 인입 시각 정렬을 추가했다. 기존 필드·필터·`count` 모집단 무변경. |
...
| 2026-09-04 | **HRR 레지스트리 헤더 행 잠금 직렬화 + bo-api stub fallback 자동 등재 listType 가드(코드=truth, PLAN 20260904-aml-hrr-registry-row-lock, aegis-aml F-094).** §12-B.6 admin surface `PUT .../reference-lists/{listType}` 행에 "동시 결재(참조 리스트 변경·`HRR_REGISTRATION`·`PEP_APPROVAL`)는 레지스트리 헤더 행 잠금(DB §3.19)으로 직렬화되어 각 EXECUTED 가 version 을 정확히 +1" 문장을 추가했다. 같은 절 bo-api 위임 노트에 "엔진 미가용 stub fallback 도 자동 등재 listType(`PEP_INDIVIDUALS`·`RA_HIGH_RISK_CUSTOMERS`) PUT 을 400(`AML.BAD_REQUEST`)으로 거부(엔진 passthrough 는 400 `AML.VALIDATION_ERROR` 무변경 — status 동일·code 상이는 수용)" 문장을 신설했다. 엔드포인트·스키마 무변경. | 코드=truth. 근거=aml-svc `HighRiskRegistryStorePort#findForUpdate`·`HighRiskRegistryJpaRepository#findByTenantIdForUpdate`·`HighRiskRegistryConcurrencyIntegrationTest`, bo-api `AmlHighRiskRegistryService#update`. DB §3.19·PRD §12-B.6 BR-003 동일 작업 단위. |
...
| 2026-09-04 | **CDD 이행 응답 다음 재이행 일자 노출(코드=truth, PLAN 20260904-aml-cdd-next-review-date).** §2.1/§3.1/OpenAPI `IngestEventResponse`에 additive `nextReviewDueAt`(date-time\|null)을 추가했다. `customer.cdd.completed` ACCEPTED는 방금 산출한 동일 RA score의 값을, exact REPLAYED는 저장 decision의 `scoreId`로 조회한 같은 score의 값을 반환한다. HELD·score 부재/조회 실패·CDD 외 이벤트는 null이며 재평가·DB/Flyway·주기 정책·운영 수동조정 계약은 무변경이다. 코드=truth. 근거=aml-svc `IngestAmlEventUseCase.RiskSummary`·`AmlEventIngestService.riskSummary`·`AmlEventController.IngestEventResponse`; `docs/aml-data.md` §12.1·워크플로우 가이드 §① 동기화. |
