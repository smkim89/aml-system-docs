...
> **적용본 존재 게이트 → `503 AML.SCREENING_UNAVAILABLE`(P0-06, 2026-08-27 정책 정정)**: `POST /api/v1/aml/screen`(및 `POST /internal/v1/aml/screen`)은 실제 스크리닝 전 tenant의 **필수 source 정책**(DB §3.6a)을 해소한다. 각 활성 필수 source는 적용본(`activeVersion`)이 있거나 capability=`NOT_APPLICABLE`+유효 waiver여야 한다. 적용본이 있으면 effectiveReadiness가 `STALE`·`FAILED`·`IMPORTING`이어도 마지막 적용 ACTIVE 엔트리로 판정하며, refresh/freshness는 명단관리 운영 신호로만 남는다. **필수 정책이 없는 tenant**는 fallback=적용본 source ≥1건이면 통과, 0건이면 fail-closed다. 적용본 자체가 없거나 waiver가 유효하지 않을 때만 `NO_MATCH` 합성 대신 **503 `AML.SCREENING_UNAVAILABLE`**를 반환한다. 기존 `ScreeningReadinessReason` wire enum은 호환을 위해 유지하지만 적용본이 있는 STALE/FAILED/IMPORTING은 차단 사유로 반환하지 않는다. 코드 truth=`WatchlistReadinessGateAdapter`·`WlfScreeningService`; rescreen 파이프라인은 integration §.
...
| POST | `/api/v1/admin/aml/watchlist-sources/{sourceCode}:override` | `aml:admin:watchlist` | — | **긴급 readiness 관리 override(P0-06)** — source를 시한부 `OVERRIDDEN`으로 전이해 운영자가 상태를 인지·승인했음을 기록한다. 요청/응답·만료·`WATCHLIST_READINESS` 감사 계약은 불변이다. 평가 가능성은 별도 `activeVersion` 존재 기준이므로 override가 적용본을 합성하지 않으며, 적용본이 없는 source를 통과시키지 않는다. 적용본이 있는 STALE/FAILED/IMPORTING은 override 없이도 마지막 명단으로 평가한다. | `aml_watchlist_sources`,`aml_audit_events(WATCHLIST_READINESS)` |
...
| 503 | `AML.SCREENING_UNAVAILABLE` | WLF 엔진 장애·PII reveal 역참조 실패·**필수 source 적용본 부재/waiver 미승인(P0-06)**·**후보 recall SQL timeout**. 적용본이 있는 STALE/FAILED/IMPORTING은 503이 아니라 마지막 적용본으로 판정한다. 적용본 부재 계열 details는 `NO_MANDATORY_POLICY`/`NO_READY_SOURCE`/`MISSING_SOURCE`/`NOT_READY`/`FAILED`(실패했고 적용본 없음)/`NOT_APPLICABLE_UNAPPROVED`, 후보 timeout은 `CANDIDATE_QUERY_TIMEOUT`(§2.2) |
