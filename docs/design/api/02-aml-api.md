...
| `ruleCode` | string\|null | TM_SCENARIO 타입 알림의 안정 발동 룰 코드. 잠금 기준선 `AmlReportRuleCode` 10종 또는 사용자 정의 `ConfigurableReportRule.ruleCode`이며 엔진 DB `scenario_code`를 JSON `ruleCode`로 노출한다. custom evidence는 `trigger.ruleSource=CUSTOM`·`ruleFamily`·`ruleVersion`·**`condition`**(DSL 조건식 사람이 읽는 요약, v9.36)을 병기한다. 비-TM 알림은 null. |
...
`ReportRuleOverviewRow`: `{ ruleCode, family("CTR"|"STR"), reportType, reasonCode, evaluationMode, actions, status, naturalLanguage, hitCount30d, draftCount, lastFiredAt, tuningRecommended, source("BUILT_IN"|"CUSTOM"), conditions[] }`. 룰 정의는 BUILT_IN이 카탈로그, CUSTOM이 `aml_configurable_report_rules`가 원천이다. 같은 custom 코드에 여러 버전이 있으면 **실제 평가 중인 ACTIVE를 우선 표시**하고, ACTIVE가 없을 때만 최신 DRAFT를 표시한다. custom DRAFT는 발동하지 않으므로 `draftCount=0`; `actions=["TM_ALERT"]`. `conditions[]`는 built-in resolved 파라미터 또는 custom safe DSL leaf를 표시한다.

> **발동 카운트 원천(코드=truth, 2026-08-05 F-1 드리프트 교정).** `hitCount30d`는 **BUILT_IN·CUSTOM 모두 실제 `aml_alerts.scenario_code`(=룰코드) lifecycle 집계**(엔진 `GET /aml/alerts`를 룰코드별로 fold, period 창 적용)가 정본이다. 직전 계약은 BUILT_IN만 bo-api 라이브 CTR/STR DRAFT store(비운영 stub 폴백)의 `firedRules` 집계를 쓰도록 기술했으나, **엔진 위임 배치에서 그 store는 비어 있고 엔진 보고 payload는 `firedRules` 키를 쓰지 않아**(CTR=`ruleCode`, STR=`reasonCodes`) BUILT_IN 전건이 0으로 표기되고 같은 응답의 CUSTOM 행만 실 건수를 보이는 모순이 있었다. 알림 집계에 없는 룰코드는 비운영 라이브 store fold로 폴백하며, 양쪽 모두 없으면 0이다(seed 없음). `draftCount`/`lastFiredAt`은 **비운영 라이브 store 전용**으로 남는다 — 엔진 보고 목록 계약 `ReportSummary`(§2.7)가 행에 발동 룰코드를 싣지 않아(룰코드는 상세 `reportPayload`에만 존재) 위임 경로에서는 룰별 보고 귀속이 불가능하다. 없는 수치를 만들지 않고 `0`/`null`을 유지한다(honest 0). 룰별 DRAFT 귀속이 필요해지면 엔진 `ReportSummary`에 발동 룰코드 투영을 추가하는 것이 선행이다(미구현·예정).
