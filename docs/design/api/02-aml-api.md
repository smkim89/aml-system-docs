...
`CountryRiskImportStatusDto`(GET `.../country-risk/import-status` — FATF 일일 수집 상태 패널): `{ sourceCode("FATF_DAILY"), provider, status(ACTIVE|DISABLED), activeVersion(현재 적용 canonical SHA-256 — 첫 적용 전 null), lastImportedAt, lastCheckedAt, lastStatus(APPLIED|SKIPPED_UNCHANGED|FAILED — 시도 전 null), lastError, recentRuns: CountryRiskImportRunDto[](최근 10건) }` — DB §3.22c `aml_country_risk_sources` + `aml_country_risk_import_runs` 1:1. `CountryRiskImportRunDto` = `{ startedAt, finishedAt, status, version, added[], upgraded[], downgraded[], suppressedManual[], error }`(run diff — ISO 코드 목록: 신규/상향/하향/수동보존).

`CountryRiskImportResultDto`(POST `.../country-risk:import` — 수동 트리거 동기 실행 결과, 엔진 `SyncResult` 1:1): `{ status(APPLIED|SKIPPED_UNCHANGED|FAILED), version, added[], upgraded[], downgraded[], delisted[](이번 run 에 양쪽 FATF 목록에서 이탈한 ISO), suppressedManual[](MANUAL 오버라이드 우선으로 건너뛴 ISO, 가정 A8), error(FAILED 시 fail-safe 사유 — 기존 등급 유지) }`.

> bo-api 위임 계약(`GET/POST /api/v1/bo/aml/country-risk*`, `CountryRiskDtos`): 등급표 행(`CountryRiskEntry`)은 `provenance`/`asOf` 를 그대로 통과하고(`sourceUrl` 은 화면 미노출·`version` int 파생·`policyPackCode` 부가), import-status/import 응답은 run diff 를 **카운트**(added/upgraded/downgraded/delisted/suppressedManual int)로 요약하며 소스 URL(blackUrl/greyUrl)을 병기한다. 수동 트리거는 `COUNTRY_RISK_IMPORT_TRIGGERED` 감사 이벤트(bo V8)를 남긴다.
