...
| POST | `/api/v1/admin/aml/customers/{customerRef}/decision` | `aml:admin:approval` | — | **회원 심사결과 직접 확정**(신규, PLAN 20260909-aml-ra-direct-member-decision) — 당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 이미 **EXECUTED**된 회원에 한해 추가 4-eyes 결재 없이 RA 상세 관리자 액션에서 즉시 확정한다. body `{decision: APPROVED\|REJECTED, reason?}`(actor=`X-User-Subject`). 게이트: ⓐ 회원 미존재 **404** ⓑ `decision` 값 오류 **400** ⓒ `REJECTED`인데 `reason` 공백/256자 초과 **400** ⓓ HRR 등재 미완료 **409** `AML.STATE_CONFLICT` ⓔ 대기 중 `EDD_CLOSE`/`RELATIONSHIP_REJECT` 결재 존재 **409** `AML.STATE_CONFLICT` ⓕ 현재 결정과 요청 `decision`이 동일(중복 확정) **409** `AML.STATE_CONFLICT` ⓖ actor 미검증(`X-User-Subject` 미서명/공백/128자 초과/제어문자) **400**. 통과 시 `aml_member_decisions`에 `source=ADMIN_DIRECT`(`approvalId`/`caseId`/`caseType`/`finalStatus` 모두 `null`) 행을 append하고 같은 트랜잭션에서 웹훅 `AmlMemberDecisionResolved`(§8.1) 아웃박스 enqueue + 감사(`CASE_APPROVAL`, action `MEMBER_DECISION`, source `ADMIN_DIRECT`). 응답은 `MemberDecisionDto`(위와 동일 shape, 확정 후 `history[0]`). bo-api 미러 `POST /api/v1/bo/aml/customers/{ref}/decision`(scope `aml:admin:approval` 또는 `BO_SUPER_ADMIN`, fail-closed — 엔진 위임 미구성 시 `ENGINE_UNAVAILABLE`, 엔진 4xx는 상태·코드 그대로 전파) | `aml_member_decisions`,`aml_audit_events(CASE_APPROVAL)` |
