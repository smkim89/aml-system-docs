...
> 룰 DSL/`rule_json`은 `{"type":"cmp","feature":"channel.type","op":"=","value":"<channel>"}` + `{"type":"cmp","feature":"transaction.phpEquivalent","op":">=","value":<임계>}`의 AND 결합이다(velocity 룰은 `{"type":"velocity","agg":"count","dimension":"counterparty","window":"24h"}`). `amountBase`(규제통화 환산 = 데모 `PHP`, 서버 파생 — PLAN 20260717 R2)는 통화 무관 비교 축으로 응답/필터에 병행 노출되며, `transaction.phpEquivalent`·`transaction.amountBase` 피처가 이 파생값 단일 원천을 발화 축으로 쓴다. 데모 룰은 `tenant_demo`가 없으면(=운영) 매칭 0건(gated).
...
| device | DeviceDto | △ | `deviceRef`↔`deviceId` 불일치 422, `os`/`version`/`ip` ≤64자·제어문자 금지(422) | `device_ref`·`canonical_payload.device` |
...
| canonicalPayload | string(JSON) | △ | 정규화 payload(마스킹·`transaction.phpEquivalent` 운반). **인입 시 raw PII 방어 스캔**(PAN=Luhn 유효 13–19자리·계좌=10자리 이상 연속 숫자런·이메일·KR 주민등록번호) 후 검출 시 reject; `instrumentRef`/`accountRef`/`*Ref`/`amount`/`amountBase`/`phpEquivalent`/`balanceBefore`/`balanceAfter` 등 토큰·금액 키는 예외(정당한 opaque 참조·수치) — §5.1 서술·§16.1 | `canonical_payload` |
...
>
> **transaction 통화 계약·서버측 규제통화 파생·422 규칙(코드=truth, PLAN 20260717 R2/R3 — `IngestEventService.validateTransactionContract`·`RegulatoryAmountPolicy`·`FdsIngestProperties`, aegis-aml `docs/aml-data.md` §11.7.6).** ① **회원 키 정본**: `subject.subjectRef` = AML CDD `partyReference` = 엔진 `targetRef`(단일 키 관통). ② **send/receive leg**: 클라이언트는 `sendAmount`/`sendCurrency`·`receiveAmount`/`receiveCurrency` 4필드로 통화·금액을 전송한다. 레거시 `amount`/`currency`는 하위호환으로 **send leg**에 매핑되며 — ⓐ 4필드만 있고 레거시 부재 → 그대로 사용, ⓑ 레거시만 있고 send leg 부재 → `amount`/`currency`를 `sendAmount`/`sendCurrency`로 역채움(202·파생 원천), ⓒ 둘 다 있으면 `amount≠sendAmount`·`currency≠sendCurrency` 불일치 시 422. `amountBase`/`baseCurrency` 클라 입력은 제거(전송 시 Jackson 무시). corridor 통화와 충돌 시 transaction 블록 값이 우선(corridor는 국가 축 정본). ③ **서버 파생**: 규제통화 파라미터 `fds.ingest.regulatory-currency`(데모 `PHP`) 기준으로 send leg 통화 = 규제통화 → `sendAmount`, 아니면 receive leg → `receiveAmount`, 둘 다 아니면 **미파생(fail-safe)** — `amount_base`/`base_currency` null 저장·금액 피처(`transaction.amountBase`/`phpEquivalent`) 미노출·금액 룰 미발동(인입은 202 유지). 파생값이 `fds_canonical_events.amount_base`(base_currency=규제통화)로 저장되고 velocity SUM·`transaction.amountBase`/`phpEquivalent` 피처의 단일 원천이다. ④ **422 규칙(`FDS-CONTRACT-VIOLATION`)**: leg-전무(4필드+레거시 모두 부재) transaction 블록 / `amount↔sendAmount`·`currency↔sendCurrency` 불일치 / `accountNoHash` 제어문자·64자 초과 형식 위반. 원문 계좌번호(숫자·하이픈만 8자 이상)는 `FDS-PII-REJECTED`(422, raw PII 클래스). **단 linkage-only 면제(A13)**: leg-전무여도 `transactionRef` 가 비어있지 않으면 금전 leg 를 주장하지 않는 연결 전용 블록(커머스/정산 물질화 키, `transactionRef=settlementRef` 직결)으로 보아 **202 로 수용**한다 — 채움·불일치·파생을 스킵하고 send/receive leg·`amount_base`/`base_currency` 를 전부 null 저장(fail-safe·금액 피처 미노출). 422 잔여 대상은 `transactionRef` 까지 부재한 퇴화 블록뿐이다. 검증은 persist 이전이며 rejection 은 예외를 던지지 않고 배치 per-item 으로 보고된다(202 성공 경로 무영향). 기존 적재 행 미백필·신선 스택 검증 전제(A9).

OriginatorDto(**정본**, AML Party §4.1 동형 부분집합): `partyReference`(string — 회원 키 단일 정본), `partyType`(string — `INDIVIDUAL`/`LEGAL_ENTITY`, A3 닫힌 매핑), `nationalIdentityKey`(string — fallback 토큰 파생 입력, 미영속), `fullNameLatin`/`dateOfBirth`/`gender`/`phone`(string, 수용 즉시 스캔 후 폐기), `nationality`/`residenceCountry`(string), `identification[]`(`idType`/`idNumberMasked`/`issuingCountry`, 수용 즉시 스캔 후 폐기), `kyc`(`occupation`/`sourceOfFunds`/`kycLevel`/`customerRiskRating`/`kycVerifiedAt`, `kycVerifiedAt` 만 소비), `registeredAt`(datetime, FDS 확장 필드). 상세 계약은 위 originator 주체 계약 note 참조.
...
TransactionDto: `transactionRef`(string ●), `transactionType`(enum `TransactionType` ●, **DB §4.19 폐쇄 12종 정본** — `WITHDRAWAL`/`DEPOSIT`/`TRANSFER`/`REMITTANCE`/`PAYMENT`/`REFUND`/`REVERSAL`/`CHARGE`/`SETTLEMENT`/`PAYOUT`/`EXCHANGE`/`ADJUSTMENT`, 자유 문자열 금지, §10 `TransactionType` schema), `direction`(enum `INBOUND`/`OUTBOUND`), `sendAmount`(decimal(24,8), ≥0 △), `sendCurrency`(string(12) △), `receiveAmount`(decimal(24,8), ≥0 △), `receiveCurrency`(string(12) △), `amount`/`currency`(레거시 하위호환 = send leg, 아래 통화 계약 note 참조), `corridor`(CorridorDto △, cross-border 송금 시), `status`. **송금/수취 leg 통화·금액 계약(코드=truth, PLAN 20260717 R2 — `TransactionDto`·`IngestEventService.validateTransactionContract`·`RegulatoryAmountPolicy`)**: 클라이언트는 `sendAmount`/`sendCurrency`·`receiveAmount`/`receiveCurrency` 4필드를 전송한다(국내송금=양측 PHP·동일 금액, 해외송금=send PHP·receive 상이). 구 `amountBase`/`baseCurrency` 클라 입력 필드는 **제거**되고(전송 시 Jackson 무시), `transaction.amountBase`(base 통화 = 규제통화 `fds.ingest.regulatory-currency`, 데모 `PHP`)는 이제 **서버가 파생**한다(§5.1 통화 계약 note). **PHP 환산 임계 feature `transaction.phpEquivalent`·`transaction.amountBase`는 서버 파생 `amount_base` 단일 원천**을 노출한다(구 `canonicalPayload.transaction.phpEquivalent` 독해 경로 제거).
...
CounterpartyDto: `counterpartyType`(string), `counterpartyRef`(string token — AML G9 안정키(이름+국가+전화), 원문 미수용), `country`, `institutionCode`(string ≤32 △ — 수취 기관 코드, feature `counterparty.institutionCode`(STRING)), `accountNoHash`(string ≤64 hex △ — 원천이 사전 해시한 계좌 식별). **원문 계좌번호 미수용(코드=truth, PLAN R3 A4)**: `accountNoHash` 값이 숫자·하이픈만으로 구성되고 8자 이상이면 raw 계좌번호로 판정해 `FDS-PII-REJECTED`(422); 제어문자 포함·64자 초과는 형식 위반 422(§6 `FDS-CONTRACT-VIOLATION`).
MerchantDto: `merchantRef`(string token), `mcc`(string), `country`.
DeviceDto(△): `deviceRef`(string token), `fingerprint`(string token), `deviceId`(string — `deviceRef` coalesce 대상, 둘 다 존재·불일치 422), `os`/`version`/`ip`(string ≤64자·제어문자 금지, 준식별자 원값 저장 — 위 device 공통 블록 note 참조). 이벤트 조회 응답(`GET /fds/events*`)도 `deviceRef`를 마스킹 토큰으로 반환해 DEVICE 시간 윈도우 룰의 현재/이전 거래 비교 근거를 제공한다(raw device fingerprint 미노출).
