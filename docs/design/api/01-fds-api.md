...
> 입력 설계서 핀: `docs/software/01-fdsSvc-sass.md` **v1.9** (§13 배포 모델·온보딩 프로비저닝·키 의미 재정의, §11.6.11 `deployment_model`, §11.6.11a `onboarding_status` 상태머신, §12.8 고객사 관리=배포 유형+온보딩, §11.6.1a `close_reason` 8종, §11.6.1 REOPEN 전이, §11.5 `subject_kind` 9종, §16.2 규제 팩 토글).
...
components:
  ...
  schemas:
    ...
    TenantDto:
      type: object
      description: >
        고객사 배포/온보딩 메타. bo-api 소유 고객사 레지스트리/온보딩(/api/v1/bo/fds/tenants/** + /onboarding/**)에서 노출.
...
| 고객사 알림 채널(TNT-002 ⑤ 알림·소스 탭) | `GET/PUT /admin/fds/notify-channels`(설계서 §13.2 alert channel, §4.8) |
