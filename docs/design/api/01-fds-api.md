...
| GET | `/api/v1/admin/fds/rules` | rule 목록(필터: `ruleSetId`,`status`,`channelScope`,`decisionOutcome`,`evaluationMode`,`ruleNo`(텍스트검색) — PRD §6.1 BR-001 5축). **`status` 미지정 시 `ARCHIVED` 기본 제외**(이력은 `status=ARCHIVED` 명시 조회로만 노출 — 룰 체계 개편(PLAN 20260717 U-F5), 사용자 지시로 룰 전량 대체). **bo-api 위임 plane 한정(20260906)**: bo-api `GET /api/v1/admin/fds/rules` 는 `includeSimulator`(boolean, 기본 false) 를 추가로 받아 이름이 `[SIM-` 로 시작하는 시뮬레이터/검증 러너 부산물 룰을 **기본 제외**한다(모든 status 포함 — 아카이브 이력의 `[SIM-MATRIX-V1] …` 동명 중복 노이즈 제거). 룰 효과 통계 scaffold 도 항상 제외. `includeSimulator=true` 가 엔진 원본 parity 경로이며 엔진 REST 자체는 이 파라미터를 모른다(엔진·검증 스크립트·잠금 러너 무영향). 단건 조회·통화 프로파일 apply 의 엔진 스캔은 필터 비적용 | `fds:admin:rule` | — |
...
## 2026-09-08 확장: 디시전트리 관리 및 이중 평가 API
