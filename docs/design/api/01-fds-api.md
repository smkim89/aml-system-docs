...
> 입력 DB: `docs/design/db/01-fds-db.md` v1.3 (스키마 `fds`, 멀티테넌시 `tenant_id/workspace_id/data_scope`, enum 코드값 §4, 컬럼/타입 §5, `subject_kind` 8종 `CASE_CLOSE` 포함 §5.23, `fds_cases.aml_case_id` §5.13, **배포 모델 `fds_tenants.deployment_model`(3종)·`onboarding_status`(8종)·`default_region`·`infra_ref` §4.1·§5.1, 구 `isolation_mode` 폐기**).
> 입력 설계서 핀: `docs/software/01-fdsSvc-sass.md` v1.5 (§13 배포 모델·온보딩 프로비저닝·키 의미 재정의, §11.6.11 `deployment_model`, §11.6.11a `onboarding_status` 상태머신, §12.8 고객사 관리=배포 유형+온보딩).
...
- 본 문서는 **fds-svc가 노출하는 HTTP API**를 확정한다. 두 부류로 나눈다.
  - **외부(고객사 시스템) API** — `/api/v1/fds/**`, `/api/v1/evidence/fds/**` (설계서 §12.8). API Key+HMAC / OAuth2 / mTLS 인증. 거래 시스템이 직접 호출.
...
- **4-eyes `subjectKind` = DB 정본 `fds_approval_requests.subject_kind` 8종(§5.23)과 1:1**. `ACTION`/`RULE`/`MAPPING`/`SECRET`/`GROUP`/`EXPORT`/`MERCHANT_NORMALIZE`/`CASE_CLOSE`. **case 종결(`POST /fds/cases/{caseId}/close`)은 `CASE_CLOSE`(subjectRef=`case_id`)로 매핑**한다(ACTION 아님, §8). integration·tasks는 이 8종을 따른다.
...
- **운영자 집계 API 소유 경계 = bo-api(§12)**. 대시보드(플랫폼·고객사별)·고객사 관리(목록/상세/등록/설정)·감사 조회 화면이 호출하는 집계 엔드포인트는 **bo-api가 소유·집약·인증**한다. fds-svc/aml-svc는 저수준 데이터 API만 제공하며, **본 엔진 API 명세(§4)에는 운영자 집계 엔드포인트(대시보드/고객사/감사)를 추가하지 않는다**. PRD/PPT의 해당 화면은 호출 대상을 bo-api로 명시한다.
...
| 헤더 | 매핑 컬럼 | 필수 | 설명 |
|---|---|---|---|
| `Tenant-Id` | `tenant_id` | 필수(외부) | SaaS 고객사 경계. Admin API는 위임 토큰 claim에서 주입 |
...
| GET | `/api/v1/fds/events/{eventId}` | event 상태·정규화 결과 조회(마스킹) | `fds:case:read` | — | — |
...
| GET | `/api/v1/fds/decisions` | decision 목록(필터: `transactionRef`,`decision`,`from/to`,`ruleNo`(적중 룰 번호),`subjectRef`(대상 토큰) · 페이지네이션) | `fds:case:read` | — | — |
...
| POST | `/api/v1/fds/cases/{caseId}/close` | case 종결(`close_reason`) | `fds:case:update` | 내부감사·규제 case 필수 |
...
| GET | `/api/v1/admin/fds/rules` | rule 목록(필터: `ruleSetId`,`status`,`channelScope`) | `fds:admin:rule` | — |
...
| GET | `/api/v1/admin/fds/connectors/{id}` | connector 단건 health·offset·lag·last_error 조회(`fds_connector_offsets`) | `fds:admin:source-system` | — |
| POST | `/api/v1/admin/fds/connectors/{id}/pause` | connector 일시중지(`connector_status`→`DISABLED`, ingest/poll suspend) | `fds:admin:source-system` | — |
| POST | `/api/v1/admin/fds/connectors/{id}/resume` | connector 재개(`connector_status`→`HEALTHY`, offset 유지 후 소비 재개) | `fds:admin:source-system` | — |
...
| 메서드 | 경로 | 설명 | scope | 4-eyes |
|---|---|---|---|---|
| GET | `/api/v1/admin/fds/approvals` | 결재 대기 목록(`fds_approval_requests`). 필터: `subjectKind`(enum subject_kind 8종), `status`(enum approval_status 8종), `maker`(상신자 token=`maker_subject`) + 페이지네이션(§3.2) | `fds:case:read` | — |
...
TransactionDto: `transactionRef`(string ●), `transactionType`(string ●), `direction`(enum `INBOUND`/`OUTBOUND`), `amount`(decimal(24,8), ≥0), `currency`(string(12)), `amountBase`(decimal(24,8)), `baseCurrency`, `status`.
...
| closeReason | string(64) | `close_reason` (DB VARCHAR(64) §5.13) |
...
`status`(enum case_status), `priority`(enum case_priority), `assignedTo`(string) — 모두 선택, 1개 이상 필수. 내부감사 case 종결은 결재 게이트.
...
응답 ActionResponse: `actionId`(uuid), `status`(enum action_status), `approvalRequestId`(uuid, null 가능), `idempotencyKey`.
...
### 5.15 ConnectorDto (GET /admin/fds/connectors/{id}) — `fds_connector_offsets` (§5.28)
커넥터 단건 health. raw payload·cursor 원문 비노출(cursor는 토큰화/요약, secret 없음).
| 필드 | 타입 | 매핑 | 설명 |
|---|---|---|---|
| connectorId | string(128) | `connector_id` | PK(`{id}`) |
...
> `{id}`=`connector_id`. data-scope 밖 connector → `FDS-DATASCOPE-DENIED`(403), 격리 밖/미존재 → `FDS-NOT-FOUND`(404).

### 5.16 ConnectorPauseResponse / ConnectorResumeResponse (POST /admin/fds/connectors/{id}/pause·/resume)
...
> 본 콜백은 outbound(fds-svc→고객사)다. adapter→fds-svc **내부 ack**(`FdsActionResult` 큐 소비로 outbox 상태 SENT→ACKED/FAILED 전이)는 별개 채널이며 외부 webhook 노출 대상이 아니다(integration 명세 §4.3).
...
components:
  ...
  schemas:
    ...
    SubjectKind:
      type: string
      enum: [ACTION, RULE, MAPPING, SECRET, GROUP, EXPORT, MERCHANT_NORMALIZE, CASE_CLOSE]
    ActionStatus:
      type: string
    ...
    TenantDto:
      type: object
      description: 고객사 배포/온보딩 메타. bo-api 소유 고객사 레지스트리/온보딩(/api/v1/bo/fds/tenants/** + /onboarding/**)에서 노출. fds-svc 엔진 API(§4)에는 미노출(소유 경계, §11.2·§12). isolationMode 필드 폐기.
...
paths:
  ...
  /admin/fds/connectors/{id}/resume:
    post:
...
| connector health / 단건 / 일시중지·재개 / replay | `GET /admin/fds/connectors`, `GET /admin/fds/connectors/{id}`, `/{id}/pause`, `/{id}/resume`, `/replay` |
...
아래 화면이 호출하는 집계 엔드포인트는 **bo-api가 소유·집약·인증**한다. 본 fds-svc 엔진 명세(§4)에는 추가하지 않는다(§1.1·§12). bo-api는 §11.1의 fds-svc 저수준 데이터 API(+ aml-svc·자체 고객사/감사 저장소)를 fan-out·집약하여 다음 경로를 노출한다.
...
| 고객사별 대시보드 | `GET /api/v1/bo/fds/tenants/{tenantId}/dashboard` | 동일 + `Tenant-Id` 위임 필터 |
| 고객사 목록/상세/등록(배포유형) | `GET/POST /api/v1/bo/fds/tenants`, `GET/PUT /api/v1/bo/fds/tenants/{tenantId}` | bo-api 고객사 레지스트리(`tenant_status`/`deployment_model`/`onboarding_status`/`region`/`infraRef`) |
...
> 위 `bo-api 소유` 경로(`/api/v1/bo/**`)는 bo-api API 명세에서 확정한다. PRD/PPT의 대시보드·고객사 관리·감사·온보딩 화면은 호출 대상을 **bo-api**로 명시하고, 과거 `GET /api/v1/admin/fds/dashboard|tenants|audit` 표기(엔진 직접 경로)는 폐기한다.
>
> **고객사 등록 = 배포 유형 선택 + 온보딩 신청/상태(격리 토글 아님, 정본 target-architecture §4.1)**: 고객사 등록은 '격리 방식(DB 분리/스키마 분리/공유) 라디오' 즉석 선택이 아니라 **배포 유형(`deployment_model`: `MANAGED_DEDICATED`/`SELF_HOSTED`/`SHARED`) 선택 + 온보딩 신청(`onboarding_status` 상태머신)** 흐름이다. 온보딩 프로비저닝 트리거·상태 조회·self-hosted 등록 콜백은 **bo-api 전용 `/onboarding/**`** 경로로만 노출하며, **본 fds-svc 엔진 API(§4)에는 온보딩 엔드포인트를 추가하지 않는다**(DB §9 소유 경계). fds-svc는 `fds_tenants`의 `deployment_model`/`onboarding_status`/`default_region`/`infra_ref`를 스키마로 보유하되 운영 변경은 bo-api 온보딩 워크플로우가 트리거한다. tenant_id 라우팅 의미: 전용 배포(`MANAGED_DEDICATED`/`SELF_HOSTED`)는 **배포=고객사 단일**(라우팅은 배포 엔드포인트 단위, 고객사 간 격리는 배포 경계가 보장), `SHARED`만 `Tenant-Id` 헤더 행 라우팅(integration 명세 확정).
...
- **운영자 집계 API 소유 경계(정본 결정)**: **대시보드(플랫폼·고객사별)·고객사 관리(목록/상세/등록/설정)·감사 조회는 bo-api가 소유·집약·인증**한다. fds-svc/aml-svc는 **저수준 데이터 API만** 제공한다. 따라서 본 엔진 API 명세(§4)에는 운영자 집계 엔드포인트(대시보드/고객사/감사)를 **추가하지 않는다**. PRD/PPT의 해당 화면은 호출 대상을 bo-api(`/api/v1/bo/**`)로 명시한다(§11.2).
