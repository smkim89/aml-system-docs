...
|---|---|---|---|---|---|
| POST | `/api/v1/fds/events` | canonical event 비동기 수신(큐 적재). 신규=**202 Accepted**, 멱등 재반환=200/201(§5.2). `fds_canonical_events` insert. **`ACCEPTED`는 전송 수리이며 업무 판정이 아니다** — 판정은 `/decisions/evaluate`(또는 `/events/evaluate`) 응답 정본(20260718, aegis-aml `docs/aml-data.md` §11.7.3) | API Key+HMAC / `fds:event:write` | 필수 | — |
