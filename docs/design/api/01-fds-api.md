...
> **고객 프로파일 스냅샷 확장(코드=truth, feature/fds-rule-nationality-metric-conditions, additive — aegis-aml `docs/aml-data.md` §11.7.1).** 국적별·가입경과일·KYC경과일 기반 룰(국적 VN 차단 등)을 실시간 판정에서 평가하도록 `SubjectDto` 에 비-PII 스냅샷 3필드(`nationality`/`registeredAt`/`kycCompletedAt`)를 추가했다 — ISO 국가코드·타임스탬프만 허용(원문 성명·신분증 미수용, §16.1). 인입 시 `fds_subjects` 마스터(`nationality`/`registered_at`/`kyc_completed_at`, DB §5.6·V10)로 **COALESCE upsert**(거래 이벤트의 미동봉 null 이 CDD 원천 값을 지우지 않음)되고, feature 컴퓨트가 `customer.nationality`(STRING)·`customer.signupAgeDays`/`customer.kycAgeDays`(NUMBER = floor((occurredAt−타임스탬프)/일), 음수 0 클램프)로 파생한다. 값 부재 시 feature 미노출(= 룰 non-matching), 기존 `customer.accountAgeDays`(`riskSignals.memberAgeDays` 원천, 소스 제공)는 별도 보존(무회귀). 회원 키 단일 정본: `subject.subjectRef` = originator.nationalIdentityKey(= memberRef).

SubjectDto: `subjectType`(enum subject_type ●), `subjectRef`(string token ●), `country`(string(8)), `nationality`(string(2) △ — 고객 국적 ISO-3166 alpha-2, 프로파일 스냅샷 → feature `customer.nationality`), `registeredAt`(datetime △ — 회원 가입 시각 → feature `customer.signupAgeDays`), `kycCompletedAt`(datetime △ — KYC 완료 시각 → feature `customer.kycAgeDays`). (기존 표기 `kycLevel`/`riskRating` 은 코드 `IngestEventRequest.SubjectDto` 에 존재하지 않아 정정 삭제 — 해당 속성은 `fds_subjects` 마스터 컬럼(DB §5.6)이며 인입 DTO 필드가 아니다.)
