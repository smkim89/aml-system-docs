...
- **서명 material 분리(정본)**: 아웃바운드 webhook 서명 = HMAC-SHA256(secret, `timestamp + "." + rawBody`)이며 **인바운드 공통 필터**(`IngestAuthenticationFilter`)의 material(`timestamp + "\n" + apiKey + "\n" + method + "\n" + requestURI + "\n" + [nonblank X-User-Subject + "\n"] + rawBody`)과 **다르다** — 혼용 금지. actor 없는 외부 FDS 요청은 optional segment가 없어 기존 material과 byte 호환되고, actor가 있는 내부/운영자 요청은 actor까지 서명된다. 양 엔진(fds/aml) 아웃바운드 서명 material·헤더는 동일(API FDS §9.3 / AML §8.3).
