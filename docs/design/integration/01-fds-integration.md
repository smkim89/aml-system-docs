...
| 일자 | 버전 | 변경 내용 | 비고 |
|---|---|---|---|
| 2026-09-06 | v2.2 | **§4.4a `fds.decision.blocked` 신설 — BLOCK 판정 시점 AML 거래 통지(코드=truth, aegis-aml PLAN 20260906-fds-block-aml-propagation).** 전용 아웃박스 `fds_aml_notify_outbox`(V36)+전용 릴레이(`!test` 프로파일), non-aws REST `POST /internal/v1/aml/fds-decisions` / aws `aml-fds-decision`. §3 이벤트 표 행 추가. 기존 §4.4 핸드오프 무변경. | integration-designer. 코드 truth=fds-svc `EvaluateDecisionService`·`AmlNotifyOutboxEmitter`·`AmlNotifyRelayService`·`AmlNotifyRelayScheduler`·`RestAmlHandoffPublisher#notifyDecision`. AML integration §3.2 v4.17·FDS DB §5.35a·V36 동일 작업 단위. |
