...
> 입력 API: `docs/design/api/01-fds-api.md` v2.0 (base path `/api/v1`, 헤더 `Tenant-Id/Workspace-Id/Source-System/Idempotency-Key`, scope 11종(전체 `fds:` prefix 통일), `ActionType` 22종 정본, `amlCaseRef`, `SubjectKind` **9종**(POLICY_PACK 포함, §1.1·§5.12·§8·§13), `DeploymentModel`(3종)·`OnboardingStatus`(8종)·`TenantDto`(`deploymentModel`/`onboardingStatus`/`region`/`infraRef`, `isolationMode` 폐기), 운영자 집계·온보딩 API는 bo-api 소유·엔진 API 미포함).
...
- 서버는 servlet normalized route로 filter/scope coverage를 판단하면서 raw path를 서명하고, dispatch 의미가 달라질 수 있는 dot/encoded-separator/matrix/double-slash 등 ambiguous path와 duplicate singleton header를 nonce 소비 전에 거부한다. signed REST client는 same-origin/cross-origin redirect를 자동 추종하지 않고 새 target은 새 nonce로 재서명한다. bo-api 공용 engine `RestClient`도 `DONT_FOLLOW`로 302 target 호출·machine header 전달을 막는다(bo-api→FDS signer는 P0-04 미완료).
...
- **적용·미완료 경계(2026-07-12)**: P0-01로 AML `/aml/v1/**` filter coverage는 완료됐다. 남은 미완료는 P0-04(내부 service-auth·bo-api→FDS signer), P0-14(multipart raw-byte signer), P1-02(machine credential 폐기·유예회전·last-used·rate/network/workload 통제)다. 본 연동 문서의 v2 계약만으로 이 잔여 호출 경로나 운영 lifecycle 적용 완료를 주장하지 않는다.
