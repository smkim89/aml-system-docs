...
> 입력 설계서: `docs/software/01-fdsSvc-sass.md` v1.9 (§8 Canonical Event Taxonomy, §11.2 action_type 22종·§11.2a 별칭 매핑, §11 Action/Case/결재, §11.5 subject_kind **11종**(POLICY_PACK·RULE_PARAM·TENANT_REGULATORY_CURRENCY 포함, r13 이격 2 교정), §11.6.1 case_status·REOPEN 전이, §11.6.1a close_reason **8종**(`FP_THRESHOLD`/`FP_NORMAL_PATTERN`/`FP_DATA_QUALITY`/`CONFIRMED_FRAUD`/`CONFIRMED_MULE`/`CONFIRMED_ATO`/`ESCALATED_AML`/`OTHER`), §11.6.11 `deployment_model`(3종)·§11.6.11a `onboarding_status`(8종) 상태머신, §12 외부 시스템 연동, §12.8 서비스 관리=배포 유형+온보딩, §13 배포 모델·온보딩 프로비저닝·멀티테넌시 키 재정의, §16 PII/규제, §16.2 named 규제 팩 카탈로그).
> 입력 DB: `docs/design/db/01-fds-db.md` **v2.0** (스키마 `fds`, 멀티테넌시 `tenant_id/workspace_id/data_scope`, enum 코드값 §4, action_type 22종 정본 §4.8, close_reason 8종 §4.11, subject_kind **11종** §5.23(r13 이격 2 교정), `fds_actions` outbox §5.12(`next_attempt_at` 백오프), `fds_cases.aml_case_id` §5.13, `fds_idempotency_keys` §5.33, **배포 모델 `fds_tenants.deployment_model`(3종, §4.1/§5.1)·`onboarding_status`(8종, §4.1a 상태머신)·`default_region`·`infra_ref`, 구 `isolation_mode` 폐기**, **corridor 4컬럼 `send_country`/`receive_country`/`send_currency`/`receive_currency` §5.5(Flyway V16)**, `fds_webhook_outbox` §5.35, **저장소 Flyway 실제 파일 매핑 §8**(배포/규제 팩 전환=`V2__phase1_foundation.sql`, V16 ph_data_grounding, V17/V18 데모 시드)).
> 입력 API: `docs/design/api/01-fds-api.md` v2.0 (base path `/api/v1`, 헤더 `Tenant-Id/Workspace-Id/Source-System/Idempotency-Key`, 외부/운영 scope 11종 + AML profile internal scope 1종 = 총 12종(전체 `fds:` prefix 통일), `ActionType` 22종 정본, `amlCaseRef`, `SubjectKind` **11종**(POLICY_PACK·RULE_PARAM·TENANT_REGULATORY_CURRENCY 포함, §1.1·§5.12·§8·§13, r13 이격 2 교정), `DeploymentModel`(3종)·`OnboardingStatus`(8종)·`TenantDto`(`deploymentModel`/`onboardingStatus`/`region`/`infraRef`, `isolationMode` 폐기), 운영자 집계·온보딩 API는 bo-api 소유·엔진 API 미포함).
...
> 책임 서비스: **`services/fds-svc`** (`com.aegis.fds.adapter.in.sqs` / `adapter.out.messaging`·`adapter.out.webhook` / `adapter.in.scheduled`). AML/STR/CTR 본 처리는 **aml-svc**, 운영자 IAM·결재 집약·감사는 **bo-api**, UI는 **bo-web**(bo-api 경유).
...
9. [규제 제출 흐름 (STR/CTR) — aml-svc 위임](#9-규제-제출-흐름-strctr--aml-svc-위임)
...
- 본 문서는 **fds-svc의 비동기 메시지 경계**(SQS in/out), **외부 커넥터 수집·정규화**, **action outbox 상태머신**, **규제 제출(STR/CTR) 위임 흐름**을 확정한다.
...
- **직렬화 규약**: 모든 큐 메시지 키는 **camelCase**로 직렬화하고 DB 컬럼(snake_case)과 1:1 매핑한다(예 `errorCode`↔`error_code`). `action_type` 마스터는 **API `ActionType` enum 22종(전수)** 이 정본이며, 설계서 §15의 도메인 verb(`SUSPEND_MERCHANT`/`SEND_SECURITY_ALERT`/`OPEN_*_CASE`)는 §8.2 정규화 매핑으로 환원해 enum 코드로만 전파한다.
- **운영자 집계 API 경계**: 대시보드·서비스 관리·감사 조회는 **bo-api**가 소유·집약·인증한다. fds-svc는 저수준 데이터 API·비동기 큐만 제공하며, 본 연동 명세는 운영자 집계 엔드포인트를 정의하지 않는다.
- **비동기 SQS 전제**(정본 §3): ingest·action relay·규제 제출은 큐 기반. REST Push(`POST /api/v1/fds/events`)는 수신 후 내부 큐 `fds-events`로 적재(API §4.1과 정합). 실시간 판단은 참조 기반 `POST /api/v1/fds/decisions/evaluate`와 additive one-call `POST /api/v1/fds/events/evaluate`가 동기다. one-call도 event 저장·큐 적재 후 ACTIVE inline 룰을 즉시 평가하며 기존 async consumer는 유지한다.
...
- REST inbound machine-auth는 raw path/query·Tenant-Id·FDS workspace·고정 9-key scopeContext·최종 body digest·±5분 timestamp·16-byte nonce를 함께 서명하고 nonce를 credential-wide 원자 소비한다([공통 인증 정본](../api/00-common-machine-auth.md)). 기본 TTL 15분은 `2×skew`보다 엄격히 길고, cleanup은 기본 1분마다 최대 `20×5000` row를 짧은 batch로 배수한다. `Source-System`만 source header 정본이다. 구 v1은 RFC3339 offset timestamp 호환을 유지하지만 신규 연동은 canonical UTC `Z`의 v2를 사용한다.
- 서버는 servlet normalized route로 filter/scope coverage를 판단하면서 raw path를 서명하고, dispatch 의미가 달라질 수 있는 dot/encoded-separator/matrix/double-slash 등 ambiguous path와 duplicate singleton header를 nonce 소비 전에 거부한다. signed REST client는 same-origin/cross-origin redirect를 자동 추종하지 않고 새 target은 새 nonce로 재서명한다. bo-api 공용 engine `RestClient`도 `DONT_FOLLOW`로 302 target 호출·machine header 전달을 막는다. P0-04부터 BO→FDS typed client와 FDS→AML REST fallback도 final URI·same body bytes를 v2로 서명한다.
- `X-Trace-Id`/`X-Correlation-Id`는 큐·로그의 관측성 계보로 계속 전파하지만 고정 9-key scopeContext에는 포함하지 않는다(`X-Trace-Id`는 서버 singleton 거부 목록에는 포함). local/demo credential provisioner와 bootstrap bypass는 명시적 `local|demo` positive profile에서만 opt-in 가능하며 Flyway business seed가 아니다.
- **적용 경계(2026-07-15)**: P0-04로 `/internal/v1/fds/**` v2-only 수신, BO→FDS typed signer, FDS→AML non-AWS REST fallback signer를 완료했다. credential은 exact `(purpose,tenant,workspace)` target에만 해소하고 local lifecycle은 AML/FDS bootstrap-off다. P0-14로 multipart raw-byte signer(nonce 유도 boundary·`sha-256(body)`+exact `Content-Type` v2 서명·서버 재-SHA-256 검증)와 production capability guard(security tier `prod`/`production`/`aws` vs transport aws/local 분리·startup fail-closed)를 완료했다. 남은 미완료는 P1-02(machine credential 생성·자동 유예회전·폐기·last-used·rate/network/workload 통제)다.
- **배포 모델(deployment topology) 라우팅**(설계서 §13.0/§13.0b, DB §2.1, 정본 target-architecture §4.1): AML/FDS는 **서비스별 전용 배포가 기본**이다(테넌트=서비스, 1 기관 : N 서비스). `fds_tenants.deployment_model` ∈ `MANAGED_DEDICATED`(매니지드 전용)/`SELF_HOSTED`(자체 인프라 설치형)/`SHARED`(소규모 공유). **전용 배포(`MANAGED_DEDICATED`/`SELF_HOSTED`)에서 서비스 간 격리는 배포 경계(전용 DB/스택)가 보장하므로 `tenant_id`는 단일 배포 내 상수이고 큐 라우팅은 배포 엔드포인트(전용 큐/계정) 단위**다. `SHARED`에서만 공유 큐 위에서 `tenant_id`가 서비스 간 행 라우팅으로 동작한다(§10.1). 큐 메시지의 `(tenantId, workspaceId)`는 어느 모델에서나 필수이며 — 전용 배포에서는 *배포 내부 분리*(workspace/data-scope), 공유 배포에서는 *서비스 격리 + 내부 분리* 의미로 쓰인다.
...
> **aws transport auto-config·startup fail-closed(P0-14 CC1, 코드=truth).** AWS SQS auto-config 는 **`aws` 프로파일에서만 활성**이다 — base `application.yml` 은 `spring.autoconfigure.exclude: SqsAutoConfiguration` 로 항상 제외하고, `application-aws.yml` 이 `spring.autoconfigure.exclude: []`(+`spring.cloud.aws.region`·`sqs.enabled`)로 exclusion 을 해제해 `SqsTemplate`·`fds-events` FIFO consumer/publisher 가 auto-wire 된다. 엔진은 startup 에 **security tier**(PROD=활성 프로파일 `prod`/`production`/`aws`)와 **transport**(aws SQS vs local)를 분리 판정해 `[capability]` 한 줄을 출력하고, `aws` transport 인데 `SqsTemplate` bean 부재 또는 큐명 미바인딩이면 **fail-closed**(`CapabilityStartupValidator`) — 큐에 붙지 못한 채 메시지를 조용히 유실하는 mis-provisioning 을 차단한다. Docker 배포 profile 은 `SPRING_PROFILES_ACTIVE` env 로 전달한다(exec-form literal `-Dspring.profiles.active=…` 제거, §설계 5). **LocalStack SQS smoke**(`SqsEventQueueSqsSmokeIntegrationTest`)로 publish/consume·redelivery·DLQ 를 검증하며 실 AWS prod 계정 검증은 phase-2(A1) 후속이다.

> **큐 토폴로지의 배포 범위(deployment scope)**: 아래 큐 세트는 **하나의 배포 단위로 프로비저닝**된다. 전용 배포(`MANAGED_DEDICATED`/`SELF_HOSTED`)는 서비스마다 **전용 큐 세트**(매니지드 전용 계정/VPC의 SQS, 또는 self-hosted 고객 인프라의 브로커)를 가지며 — 큐 자체가 서비스 격리 경계다. `SHARED`만 **공유 큐**를 두고 message attribute·FIFO group의 `tenantId`로 서비스를 분리한다(§1·§10.1). 따라서 논리명(`fds-events` 등)은 동일하되 물리 큐는 배포별로 분리된다(전용) 또는 단일(공유).
...
| `fds.decision.blocked` | `aml-fds-decision`(aws) / `POST {AML}/internal/v1/aml/fds-decisions`(non-aws) | **BLOCK 판정 → AML 거래 표기 통지**(aegis-aml PLAN 20260906-fds-block-aml-propagation) | **aml-svc** `FdsDecisionConsumer`(eventType 분기)·`FdsDecisionInternalController` | 판정 natural-key 신규 저장 시 전용 아웃박스 `fds_aml_notify_outbox`(V36) 원자 적재 → 전용 릴레이(`AmlNotifyRelayScheduler`, `!test`) 전송. sandbox·replay 미발행. AML 은 알럿·케이스 없이 `aml_fds_transaction_decisions` 기록 | `tenantId`,`eventType`,`eventId`(=`decision-blocked:{decisionId}`),`decisionId`,`fdsEventRef`,`transactionRef`,`targetRef`,`outcome`,`phase`,`matchedRules[{ruleId,ruleName,versionNo,outcome}]`,`reasonCodes[]`,`decidedAt`,`dataScope` |
| `FdsActionResult` | (내부 ack) | action adapter → fds-svc | Action relay | adapter 응답(SENT→ACKED/FAILED 전이) | `actionId`,`actionType`(action_type 22종, 필수),`status`(enum `action_status`, DB §4.9 / API §10 OpenAPI `ActionStatus` 7종, 통상 `ACKED`/`FAILED`),`errorCode`(nullable),`completedAt`(nullable, `completedAt`↔`completed_at`) |

> 콜백(webhook) 4종(`FdsDecisionCreated`/`FdsCaseOpened`/`FdsCaseStatusChanged`/`FdsActionResult`)의 핵심 payload·envelope는 **API §9.1/§9.2가 정본**이며 본 표는 이와 1:1 정렬한다. `FdsCaseOpened`(`priority`/`originDecisionId`)와 `FdsCaseStatusChanged`(`fromStatus`/`toStatus`/`closeReason`)는 별개 이벤트로 분리하고, `FdsActionResult`는 `actionType`(action_type 22종)을 필수 포함한다(`status`는 `action_status` enum 기준). 보조필드 `amlCaseRef`는 `data` 외 cross-ref 보존용으로만 사용한다(필수 payload 아님).
...
> **`recommendedActions`는 DB 컬럼 1:1 매핑이 아니다.** 정본 `fds_decisions`(DB §5.10)에는 `recommended_actions` 컬럼이 존재하지 않는다 — recommended action은 별도 저장 방식으로, 해당 decision으로 생성된 **`fds_actions` outbox row(DB §5.12)의 `action_type` 집합을 webhook 시점에 투영(projection)**한 **파생 배열**이다(`fds_actions.decision_id = fds_decisions.decision_id`로 조인). 각 원소는 enum `action_type`(22종, DB §4.8) 코드값만 사용하며, 도메인 verb(`SUSPEND_MERCHANT`/`SEND_SECURITY_ALERT`)는 §8.2 정규화 매핑으로 환원해 enum 코드로만 전파한다. (decision의 정본 저장은 `decision`/`matched_rules`/`feature_snapshot`(DB §5.10) + `fds_decision_reasons`(§5.11) + `fds_actions`(§5.12)로 분산되며, 단일 `recommended_actions` 컬럼은 없다.)
...
```json
{
  ...
  "subject":      { "subjectType": "PERSON", "subjectRef": "subj_hmac_123", "country": "PH",
                    "nationality": "PH", "registeredAt": "2025-11-20T00:00:00Z",
                    "kycCompletedAt": "2025-12-05T00:00:00Z" },
...
> **subject 프로파일 스냅샷(코드=truth, fds V10 — aegis-aml `docs/aml-data.md` §11.7.1, 전부 선택)**: `subject.nationality`(ISO-3166 alpha-2)·`subject.registeredAt`(회원 가입 시각)·`subject.kycCompletedAt`(KYC 완료 시각)은 국적 차원·가입경과일·KYC경과일 룰의 비-PII feature 원천이다(원문 성명·신분증 미수용 §16.1). `IngestController` 가 canonical payload `subject` 블록에 실어 투영하면 `StateProjectionService` 가 `fds_subjects` 마스터(`nationality`/`registered_at`/`kyc_completed_at`)로 **COALESCE upsert**(미동봉 null 이 CDD 원천 값을 지우지 않음)하고, `FeatureComputeAdapter` 가 `customer.nationality`/`customer.signupAgeDays`/`customer.kycAgeDays` feature 로 파생한다(부재 시 미노출 = 룰 non-matching). 시뮬레이터는 CDD 프로파일을 결정적으로 파생해 동봉한다(`scripts/demo_ingest.py` `fds_subject_profile()`).
...
- `actionType` ∈ enum `action_type`(DB §4.8, 22종). `targetRef`는 token. 자금/규제성 action은 `approvalRequestId` 필수(미승인 시 발행 금지, §8). **비자금성 action은 `approvalRequestId=null`**(API §5.7 ActionResponse와 동일, nullable).
- 응답(adapter→fds-svc) `FdsActionResult`: `{ "actionId", "actionType", "status": "ACKED|FAILED", "errorCode", "completedAt" }` → `fds_actions.status`/`completed_at`/`error_code` 갱신. `actionType`(action_type 22종)은 **필수**, `status`는 `action_status` enum(DB §4.9 / API §10 OpenAPI `ActionStatus` 7종) 기준이다(API §9.1 정합). 키는 camelCase(`errorCode`↔`error_code`, `completedAt`↔`completed_at`).

### 4.4 FDS→AML 핸드오프 (`aml-fds-decision`) — aml-svc 위임 (raw PII 미전파)

**정본 계약 = `02-aml-integration.md` §3.2**(인바운드 `aml-fds-decision`, D-07). 본 절은 그 단일 계약을 발행(fds-svc) 측에서 1:1로 기술하며, 봉투 키·멱등·산출은 AML §3.2가 정본이다(상호 참조). 봉투는 `eventType`별 2종 — `fds.case.escalated`(FDS fraud case → STR 후보 위임) / `fds.decision.applied`(FDS hold·block 결정 → AML EDD 트리거).
...
- **멱등·REST 인증**: SQS `messageDeduplicationId = eventId`(=`fdsEventId`) + 소비측 DB partial UNIQUE `(tenant_id, origin_fds_case_ref, fds_event_id) WHERE source_origin='FDS'`. `fds.case.escalated` 동기 fallback은 `POST /internal/v1/aml/fds-escalations`(둘 다 멱등, AML §3.2)이며 non-AWS/local에서 `aegis.fds.aml-handoff.base-url`이 설정되면 fds-svc REST handoff adapter가 같은 `eventId`·`fraudCaseRef/fdsCaseRef`·`targetRef`·`transactionRef`·`action`·`severity`·`suggestedCaseType`·`dataScope` 의미를 전달한다. 이 fallback은 exact target credential의 `aml:internal:fds-escalation:write`, signed `X-Internal-Service=fds-svc`, `X-Data-Scope=workspaceId`, `Idempotency-Key=eventId`, AML canonical `workspace=default`를 final URI와 한 번 직렬화한 body bytes에 결합한다. unsigned/spoof/tamper/replay는 401, valid wrong scope는 nonce 소비 뒤 403이다. 같은 업무 event는 새 nonce로 재전송해 단일 alert/case로 수렴한다. `fds.decision.applied`는 `aml-fds-decision` 큐 전용.
...
### 4.4a FDS 차단 판정 통지 (`fds.decision.blocked`) — 판정 시점·거래 단위 (aegis-aml PLAN 20260906-fds-block-aml-propagation)

§4.4 핸드오프(케이스/EDD 위임)와 **별개 채널**이다. `EvaluateDecisionService` 가 `outcome=BLOCK` 판정을 natural-key 신규 저장하면(정상·fail-safe 경로 모두, replay 제외) 같은 트랜잭션에서 `AmlNotifyOutboxEmitter.enqueueDecisionBlocked` 가 `fds.fds_aml_notify_outbox`(DB §5.35a, `UNIQUE(tenant_id, event_id)`, `event_id=decision-blocked:{decisionId}`)에 적재한다. sandbox(shadow-only) 워크스페이스는 적재하지 않는다. 전용 릴레이 `AmlNotifyRelayService`(`AmlNotifyRelayScheduler`, `@Profile("!test")` — 로컬 `local,demo` 에서도 동작)가 `AmlHandoffPort.notifyDecision` 으로 전송한다: non-aws=`RestAmlHandoffPublisher` → `POST {AML_SVC_BASE_URL}/internal/v1/aml/fds-decisions`(purpose `FDS_AML_ESCALATION` 머신 인증, `X-Internal-Service: fds-svc`, 서명 `X-Data-Scope`=workspaceId, `Idempotency-Key`=eventId) / aws=`SqsAmlHandoffPublisher` → `aml-fds-decision`. 실패는 지수 백오프 재시도 후 상한 초과 시 `DEAD_LETTERED`(+감사 `AML_NOTIFY_DEAD_LETTER`, 메트릭 `fds.aml_notify.{sent,failed,dlq}`). 봉투는 AML §3.2 `fds.decision.blocked` 행이 정본이며 `matchedRules` 는 룰별 `outcome` 을 포함한 발동 룰 전건, 원문 PII 없음. **`transactionRef` 가 없는 BLOCK 판정(비거래 family)은 AML 조인 키가 없어 미발행**(아웃박스 적재 0, INFO 로그만 — poison row 방지). 기존 `fds_webhook_outbox`·`WebhookRelayService`·`ActionRelayService.publishDecisionApplied` 는 무변경.
...
sequenceDiagram
    ...
    SRC->>API: POST events (Tenant/Workspace/Source/Idempotency + Timestamp + Auth-Version:2 + Nonce + Signature)
    API->>API: v2 HMAC 검증 → nonce 원자 consume → (tenant,ws,EVENT,key) 업무 멱등 조회
    alt 멱등 재요청
...
sequenceDiagram
    ...
    participant QH as aml-fds-decision (FIFO)
    participant AML as aml-svc FdsDecisionConsumer
    CORE->>DB: insert fds_cases(case_type=AML_REVIEW, status=OPEN, aml_case_id=NULL)
    CORE->>QH: fds.case.escalated(eventId=fdsEventId, fdsCaseRef, targetRef, action, severity, suggestedCaseType, dataScope)
    QH->>AML: deliver (group=tenant:eventId, dedupId=eventId)
    AML->>AML: 멱등(tenant_id, origin_fds_case_ref, fds_event_id) + 본 케이스 생성 + sanction/PEP + STR/CTR
...
서비스 콜백(`FdsDecisionCreated`/`FdsCaseOpened`/`FdsCaseStatusChanged`/`FdsActionResult`, §4.5)은 **액션 outbox(`fds_actions`)와 별개 채널**인 `fds_webhook_outbox`(DB §5.35, Flyway V15)에 적재되어 전용 스케줄드 디스패처로 **서비스 등록 URL로 서명 HTTP POST** 발행된다(구현 정본 `WebhookOutboxEmitter`(producer)/`WebhookRelayService`(`RelayWebhookUseCase`)/`HttpWebhookSenderAdapter`(전송)/`WebhookRelayScheduler`).
...
- **서명 material 분리(정본)**: 아웃바운드 webhook 서명 = HMAC-SHA256(secret, `timestamp + "." + rawBody`)이며 [인바운드 공통 v2](../api/00-common-machine-auth.md)의 preamble/version/METHOD/rawPath/rawQuery/Tenant-Id/고정 9-key scopeContext/content digest/timestamp/nonce canonical bytes와 **다르다** — 혼용 금지. 양 엔진(fds/aml) 아웃바운드 서명 material·헤더는 동일(API FDS §9.3 / AML §8.3).
...
#### 6.2.3 AML 판정 통지 릴레이 (`fds_aml_notify_outbox`, aegis-aml PLAN 20260906-fds-block-aml-propagation)

- **채널 분리**: §6.2.1(액션 디스패처)·§6.2.2(webhook 디스패처)와 별개 테이블·별개 릴레이(`AmlNotifyRelayService`, 4.4a). 두 기존 디스패처의 `aws` 한정 계약은 무변경.
- **구동 방식**: `AmlNotifyRelayScheduler`(`@Profile("!test")`)는 `@EnableScheduling`/`@Scheduled` 에 **의존하지 않고** Spring `SmartLifecycle` 로 전용 단일 스레드 실행기(`fds-aml-notify-relay-*`, 데몬)를 자체 구동한다 — 고정지연 `aegis.fds.aml-notify.relay-interval-ms`(기본 5000ms, PENDING) / `retry-interval-ms`(기본 30000ms, FAILED 백오프 경과) 로 `ElevatedDbContext` sweep. 틱마다 예외를 삼켜 다음 틱을 보장하고 `stop()` 시 10초 내 종료. 이유: fds `SchedulingConfig` 가 `@Profile("aws")` 라 로컬(`local,demo`)에는 스케줄링 인프라가 없는데 이 채널은 로컬 폐루프(FDS-C44·X-C09) 에서도 배달이 필요하다. `aws` 에서도 동일 실행기 하나만 돈다(이중 실행 없음).
- **재시도·DLQ**: 지수 백오프(§6.2.2 동형 상수), `MAX_RETRIES` 초과 → `DEAD_LETTERED` + 감사 `AML_NOTIFY_DEAD_LETTER` + 메트릭 `fds.aml_notify.{sent,failed,dlq}`. 성공 기준은 `AmlHandoffPort.notifyDecision` 이 예외 없이 반환(REST 2xx / SQS 발행).
- **후속 과제(기록)**: fds `@Profile("!test")` 잡 2종(`FdsAuditHashChainVerificationJob`·`MachineAuthNonceCleanupScheduler`)은 어떤 로컬 프로파일에서도 틱이 없다(활성 범위 오도) — 별건.

### 6.3 DLQ

- 각 큐는 전용 DLQ(`*-dlq`). `adapter/in/scheduled`에는 현재 `ActionRelayScheduler`·`WebhookRelayScheduler`(`aws` 한정) + `AmlNotifyRelayScheduler`(§6.2.3, `!test`·자체 구동)가 구현되어 있다(코드 truth). DLQ depth 폴러(`APPROXIMATE_NUMBER_OF_MESSAGES` → `fds.action.failed`/`fds.ingest.rejected` 메트릭 노출)는 **운영 관측 보강 예정**(미구현). 현 종단 처리·메트릭은 outbox 디스패처(`ActionRelayService`/`WebhookRelayService`) 내부에서 수행한다(§6.2.1/§6.2.2).
...
action 발행 전 대상 `target_system`의 capability(`fds_source_systems` 또는 tenant capability 설정) 검증. 미지원 시 **`OPEN_CASE`로 강등**(case-only). `action_type` 은 닫힌 enum(22종, 코드 truth)이며 본 매트릭스는 전수를 보존한다. **hanpass-ph(`tenant_demo`)가 실제 발행하는 action 은 송금·월렛 도메인 중심**(`HOLD_FUNDS`/`RELEASE_HOLD`/`BLOCK_TRANSACTION`/`BLOCK_WITHDRAWAL`/`CANCEL_TRANSACTION`/`SUSPEND_ACCOUNT`/`SUSPEND_INSTRUMENT`/`OPEN_CASE`/`OPEN_AML_CASE`/`SEND_ALERT`/`REGULATORY_REPORT` 등)이고, 카드 승인(`DECLINE_AUTHORIZATION`)·가맹점/셀러(`SUSPEND_MERCHANT`/`SUSPEND_SELLER_PAYOUT`/`INCREASE_RESERVE`)는 enum 보존하되 **hanpass-ph 미사용**(Phase 7 확장 슬롯).
...
| `REGULATORY_REPORT` | aml-svc 위임(§9) | `aml-fds-decision` 큐 |

> `SUSPEND_MERCHANT`·`SEND_SECURITY_ALERT`(§15.5/§15.8/§15.11 도메인 verb)는 정본 `action_type` enum(22종)에 독립 코드로 존재하지 않는다. 정규화 매핑(설계서 §11.2a / DB §4.8 주석 / API §3 action_type 마스터 규약)에 따라 `SUSPEND_MERCHANT`=`SUSPEND_INSTRUMENT`(대상=`MERCHANT_ACCOUNT`), `SEND_SECURITY_ALERT`=`SEND_ALERT`로 환원해 `fds_actions.action_type`에 저장한다. 자동 제어 불가 tenant는 `OPEN_CASE`(`case_type=MERCHANT_RISK`)로 강등한다.

### 8.3 4-eyes 게이트 (설계서 §11.4/§11.5, API §8 정합)

자금성(`HOLD_FUNDS`/`RELEASE_HOLD`/`CANCEL_TRANSACTION`/`REQUEST_REVERSAL`/`HOLD_SETTLEMENT`)·규제성(`REGULATORY_REPORT`/`OPEN_AML_CASE`)·정책변경(rule activate/rollback·group member·mapping·credential·export 최종본·merchant normalize)·**case 종결**·**규제 팩 토글**·**룰 파라미터 편집**·**테넌트 규제통화 전환**은 `fds_approval_requests`(`subject_kind` **11종**(r13 이격 2 교정 — 종전 "9종" 서술은 `RULE_PARAM`(SFDS-RULE-002)·`TENANT_REGULATORY_CURRENCY`(다통화, PLAN 20260818 U17) 2종 누락. 코드=truth `fds_approval_requests_subject_kind_check`, DB §5.23) ∈ `ACTION`/`RULE`/`MAPPING`/`SECRET`/`GROUP`/`EXPORT`/`MERCHANT_NORMALIZE`/`CASE_CLOSE`/`POLICY_PACK`/`RULE_PARAM`/`TENANT_REGULATORY_CURRENCY`) 생성 → checker(≠maker) 승인 후 relay. `subject_kind` enum은 DB 정본 `fds_approval_requests.subject_kind` **11종**(DB §5.23)·API §1.1·§5.12·§8·§13과 1:1이다. **case 종결(`POST /api/v1/fds/cases/{caseId}/close`)은 `CASE_CLOSE`(subjectRef=`case_id`)로 매핑**한다(ACTION 아님, API §8 정합). **규제 팩 토글(`POLICY_PACK`)은 `subjectRef=tenant_id`, `approvalLine=COMPLIANCE_MANAGER`**, named pack on/off·확장 활성화(설계서 §16.2). **테넌트 규제통화 전환(`TENANT_REGULATORY_CURRENCY`)은 `subjectRef=tenant_id`, `approvalLine=COMPLIANCE_MANAGER`**(`POST .../compliance/regulatory-currency:change`, 다통화 PLAN 20260818 U17 — 이력 가드 422 `FDS.TENANT_CURRENCY_HISTORY_LOCKED`). `approval_line` 기본값 요약은 아래 표를 따르며(정본 = API §8), 신규 `subject_kind` 추가 시 본 표와 동기화 필수.
...
| `RULE_PARAM` | `rule_id` | `COMPLIANCE_MANAGER` | 룰 변수(파라미터) 편집(SFDS-RULE-002, r13 이격 2 신규 등재) |
| `TENANT_REGULATORY_CURRENCY` | `tenant_id` | `COMPLIANCE_MANAGER` | 테넌트 규제통화 전환(다통화, PLAN 20260818 U17, DB V32) |

---

## 9. 규제 제출 흐름 (STR/CTR) — aml-svc 위임

설계서 §11.3·§16.2(특금법 STR/CTR)·DB §9 서비스 경계를 따른다. **fds-svc는 후보(origin)만 생성, 본 제출·증빙·재제출은 aml-svc 소유.**
...
> `reportKind` 는 `STR`/`CTR` 2종이다(코드 truth: fds `CaseType` 10종에 Travel Rule 케이스 타입 없음, aml `ReportType` 6종 = `STR`/`CTR`/`EDD_REGISTER`/`WLF_REGISTER`/`RA_REPORT`/`AUDIT_EXPORT`). 구 가상자산 Travel Rule(`TRAVEL_RULE`·`CRYPTO_TRAVEL_RULE`)은 Travel Rule 기능 전면 제거(aegis-aml 84997e1, fds V9·aml V31 DROP)로 enum·스키마에서 삭제됐다.

### 9.3 흐름·증빙·재제출

1. fds-svc: rule hit → `fds_decisions`(REPORT/FREEZE) → `fds_cases`(origin, `aml_case_id=NULL`) → `aml-fds-decision` 발행(`fds.case.escalated`/`fds.decision.applied`, §4.4). **token/hash·base 금액만, 원문 미전파.**
2. aml-svc: 본 케이스 생성·sanction/PEP·STR/CTR 제출 → `amlCaseRef` ack.
...
5. **재제출**: aml-svc 제출 실패/반려 시 `aml-fds-decision` 멱등 재발행(키=eventId=fdsEventId). fds-svc는 신규 case 생성 없이 동일 origin에 재handoff. DLQ 진입 시 §6.3 replay.

> fds-svc API에서 AML 본 처리 시도는 `FDS-AML-DELEGATED`(409)로 거부(API §6).
...
**tenant_id 라우팅 의미는 `deployment_model`에 따라 재정의된다**(설계서 §13.0b, DB §2.1·§2.2, 정본 target-architecture §4.1). `tenant_id`는 **서비스(테넌트) 격리 경계**의 식별자이며(계층: 기관 → 서비스(테넌트) → 워크스페이스), 격리의 1차 경계는 *배포*다. `tenant_id`는 전용 배포에서 *배포 내부 상수*, 공유 배포에서만 *서비스 간 행 라우팅 키*다.

| deployment_model | 큐 토폴로지 | `tenantId` 의미 | 서비스 간 격리 보장 |
|---|---|---|---|
| `MANAGED_DEDICATED`(기본) | 서비스 **전용** 큐 세트(전용 계정/VPC의 `fds-events`/`fds-actions`/…) | 배포 내 **단일 상수**(배포=서비스). 라우팅은 **배포 엔드포인트 단위** | 전용 DB/스택·전용 큐 **배포 경계** |
| `SELF_HOSTED` | 고객 인프라 내 **설치 인스턴스 전용** 큐(고객 측 브로커). 플랫폼 ↔ 인스턴스는 등록 콜백·라이선스 채널만 | 인스턴스 내 **단일 상수**(배포=서비스). 플랫폼은 행 라우팅 미수행 | 고객 인프라(물리) **배포 경계** |
...
- API key/OAuth2 client/webhook은 `(tenantId, workspaceId)` 바인딩(`fds_api_credentials`). `allowed_protocol_versions`는 기존 row `[v1,v2]`, 신규 row `[v2]`이며 service policy와 교집합만 허용한다. v2 nonce store PK도 같은 tenant/workspace/credential scope를 사용한다. cross-workspace 메시지 라우팅은 명시 scope 필요 → 위반 `FDS-AUTHZ-003`.
...
| secret | AES-GCM `secret_ciphertext`만 저장. raw secret은 발급 후 DB·로그·조회 응답에 미보존(API §5.13, DB §5.29) |
...
- **`FdsActionResult` 필드**: `actionId` / `actionType`(action_type 22종, 필수) / `status`(action_status) / `errorCode`(nullable, `errorCode`↔`error_code`) / `completedAt`(nullable, `completedAt`↔`completed_at`).
...
- **FIFO 규약**: `messageDeduplicationId = idempotencyKey`, `messageGroupId = tenantId:workspaceId:<orderKey>`(events=transactionRef, actions=targetRef). **`aml-fds-decision`은 `messageDeduplicationId = eventId`(=fdsEventId), `messageGroupId = tenantId:eventId`**(AML §3.2 멱등 계약 — 소비측 partial UNIQUE `(tenant_id, origin_fds_case_ref, fds_event_id) WHERE source_origin='FDS'`).
- **outbox 상태**: `action_status` = `PENDING`/`APPROVAL_REQUIRED`/`APPROVED`/`SENT`/`ACKED`/`FAILED`/`CANCELLED`(+sandbox shadow 미발행).
- **AML cross-ref 확정**: `fds_cases.aml_case_id VARCHAR(96) NULL` = API/ack `amlCaseRef`. **FDS→AML 핸드오프 봉투는 AML §3.2 정본 계약**(`aml-fds-decision`): `eventType` ∈ `fds.case.escalated`/`fds.decision.applied`, 키 = `tenantId`·`eventType`·`eventId`(=멱등키 fdsEventId)·`fdsCaseRef`·`fraudCaseRef`·`targetRef`·`transactionRef`·`action`·`severity`·`suggestedCaseType`·`dataScope`(소비측 `workspaceId`→`dataScope` 변환). 규제 제출 종류 `reportKind` ∈ `STR`/`CTR`은 §9.2 fds-svc 내부 origin 분류이며 봉투 키가 아니다.
...
- **4-eyes subject_kind(정본 DB §5.23 / API §1.1·§5.12·§8·§13, **11종**, r13 이격 2 교정)**: `ACTION`/`RULE`/`MAPPING`/`SECRET`/`GROUP`/`EXPORT`/`MERCHANT_NORMALIZE`/`CASE_CLOSE`/`POLICY_PACK`/`RULE_PARAM`/`TENANT_REGULATORY_CURRENCY`. approvalLine 정본(API §8): `ACTION`=`MAKER_CHECKER`(대규모 `EXECUTIVE_APPROVAL`), `RULE`=`COMPLIANCE_MANAGER`, `MAPPING`=`MAKER_CHECKER`(subjectRef=`source_system`), `SECRET`=`SECURITY_ADMIN`, `GROUP`=`RISK_MANAGER`, `EXPORT`=`COMPLIANCE_MANAGER`, `MERCHANT_NORMALIZE`=`RISK_MANAGER`(대규모 `EXECUTIVE_APPROVAL`), `CASE_CLOSE`=`COMPLIANCE_MANAGER`(subjectRef=`case_id`, ACTION 아님), `POLICY_PACK`=`COMPLIANCE_MANAGER`(subjectRef=`tenant_id`, 설계서 §16.2), `RULE_PARAM`=`COMPLIANCE_MANAGER`(subjectRef=`rule_id`), `TENANT_REGULATORY_CURRENCY`=`COMPLIANCE_MANAGER`(subjectRef=`tenant_id`, 다통화 PLAN 20260818 U17). §8.3에 인라인 요약 표 포함.
- **배포 모델 라우팅(deployment topology, 정본 target-architecture §4.1 / DB §2.1 / API §11.2)**: `deployment_model` ∈ `MANAGED_DEDICATED`(기본)/`SELF_HOSTED`/`SHARED`(enum `DeploymentModel` 3종). **전용 배포(`MANAGED_DEDICATED`/`SELF_HOSTED`)는 배포=서비스 단일** — 큐 라우팅은 배포 엔드포인트(전용 큐 세트/고객 인스턴스 브로커) 단위이고 `tenantId`는 배포 내 상수, 서비스 간 격리는 배포 경계(전용 DB/스택·고객 인프라)가 보장. **`SHARED`만** 공유 큐 위에서 `Tenant-Id` 헤더 행 라우팅(§10.1). `workspaceId`는 전 모델에서 서비스 내 환경(`retail`/`corporate`·`prod`/`sandbox`) 분리, `dataScope`는 메시지 미탑재 권한 필터. 구 `isolation_mode`(`SHARED`/`SCHEMA`/`DB`) 행/스키마 토글·격리 라디오 UI는 폐기.
...
- **서비스 등록 흐름**: 격리 토글이 아니라 **배포 유형 선택 + 온보딩 신청/상태**(읽기 표시). 화면 명칭: '배포 유형 선택', '온보딩 신청', '온보딩 상태'. `TenantDto` = `tenantId`/`deploymentModel`/`onboardingStatus`/`region`(=`default_region`)/`infraRef`(=`infra_ref`), `isolationMode` 필드 폐기.
...
| 일자 | 버전 | 변경 내용 | 비고 |
|---|---|---|---|
| 2026-09-06 | v2.2 | **§4.4a `fds.decision.blocked` 신설·§6.2.3 AML 판정 통지 릴레이 구동 방식(SmartLifecycle 자체 구동, `@EnableScheduling` 비의존 — PLAN Q11) — BLOCK 판정 시점 AML 거래 통지(코드=truth, aegis-aml PLAN 20260906-fds-block-aml-propagation).** 전용 아웃박스 `fds_aml_notify_outbox`(V36)+전용 릴레이(`!test` 프로파일), non-aws REST `POST /internal/v1/aml/fds-decisions` / aws `aml-fds-decision`. §3 이벤트 표 행 추가. 기존 §4.4 핸드오프 무변경. | integration-designer. 코드 truth=fds-svc `EvaluateDecisionService`·`AmlNotifyOutboxEmitter`·`AmlNotifyRelayService`·`AmlNotifyRelayScheduler`·`RestAmlHandoffPublisher#notifyDecision`. AML integration §3.2 v4.17·FDS DB §5.35a·V36 동일 작업 단위. |
| 2026-08-19 | v2.1 | **subject_kind 9종→11종 정합 교정(r13 이격 2 — 다통화 PLAN 20260818 U13 역전파).** 종전 "9종" 서술이 이미 코드에 있던 `RULE_PARAM`(SFDS-RULE-002)과 신규 `TENANT_REGULATORY_CURRENCY`(U17 — 테넌트 규제통화 4-eyes 전환, DB V32) 를 누락하고 있었다. 헤더 입력 핀(§도입부)·§8.3 approvalLine 인라인 표·§9 subject_kind 표 3곳을 11종·`TENANT_REGULATORY_CURRENCY`=`COMPLIANCE_MANAGER`(subjectRef=`tenant_id`)로 동기화. | 코드=truth. 근거=fds-svc `db/migration/V32__tenant_regulatory_currency_subject_kind.sql`·`adapter/in/rest/TenantAdminController`. DB `01-fds-db.md` §5.23·API `01-fds-api.md` §1.1·§5.12·§8·§13 동일 작업 단위. |
| 2026-07-13 | v3.1 | **P0-04 내부 REST·BO→FDS service-auth 완료.** BO typed 호출과 FDS→AML local fallback을 exact target machine credential/final URI/same bytes v2 signer로 전환하고, FDS internal profile 수신을 v2-only 최소 scope로 닫았다. local lifecycle은 bootstrap-off이며 SQS primary 경계는 불변이다. | integration-designer. 신규 scope=`fds:internal:customer-profile:write`, `aml:internal:fds-escalation:write` |
| 2026-07-12 | v3.0 | **P0-00 공통 inbound machine-auth wire v2 연동 전환.** REST sequence를 normalized servlet route/ambiguous path·duplicate singleton 거부→v2 HMAC→credential-wide nonce 원자 consume→업무 멱등 순으로 정정하고, canonical 공식은 `../api/00-common-machine-auth.md`를 단일 정본으로 참조했다. v1 offset 호환, TTL `>2×skew`, cleanup `20×5000/tick`, signed redirect 거부, trace/correlation의 9-key context 제외, local/demo positive provisioning을 동기화했다. P0-01/P0-04/P0-14와 P1-02 lifecycle은 미완료이며 FDS 구 hash 컬럼 표기를 AES-GCM `secret_ciphertext`로 정정했다. outbound webhook `timestamp + "." + rawBody`는 inbound v2와 분리해 유지했다. | integration-designer. 코드 truth=`common-security`, FDS V14, bo-api `RestClientConfig`/`RestClientConfigTest`, Python simulator transport |
| 2026-07-09 | v2.9 | **FDS 인입 subject 프로파일 스냅샷 역전파(코드=truth, feature/fds-rule-nationality-metric-conditions, fds V10).** §4.2 IngestEventMessage 예시 `subject` 블록에 `nationality`/`registeredAt`/`kycCompletedAt` 3필드 추가 + 프로파일 스냅샷 note 신설 — 비-PII(ISO 국가코드·타임스탬프만) 원천으로 `IngestController` canonical payload 투영 → `StateProjectionService` 가 `fds_subjects` COALESCE upsert(미동봉 null 이 CDD 원천 보존) → `FeatureComputeAdapter` 가 `customer.nationality`/`customer.signupAgeDays`/`customer.kycAgeDays` feature 파생(부재 시 미노출). 시뮬레이터 결정적 프로파일 동봉(`fds_subject_profile()`). API §5.1 `SubjectDto`·DB §5.6 V10 과 1:1. | integration-designer. 코드=truth. 근거=fds-svc `IngestController`(hasSubjectProfile)·`StateProjectionService`·`SubjectStateJpaAdapter`·`FeatureComputeAdapter`·aegis-aml 491f46e·`scripts/demo_ingest.py`. |
| 2026-07-09 | v2.8 | **Travel Rule 기능 전면 제거 정합(코드=truth).** (1) 헤더·개요·§1 원칙에서 `AML/STR/CTR/Travel Rule`→`AML/STR/CTR`. (2) §9 제목·TOC `규제 제출 흐름 (STR/CTR/Travel Rule)`→`(STR/CTR)`, 앵커 동기화. (3) §8.2 capability 매트릭스 — `ActionType` 23종→**22종**(코드 truth `ActionType` `REQUEST_TRAVEL_RULE_INFO` 제거), `REGULATORY_REPORT` / `REQUEST_TRAVEL_RULE_INFO` 행에서 travel action 제거, 거래소(Travel Rule) 미사용 서술 제거. 헤더·§1·§3.2·§4.3·§12 downstream 의 action_type "23종" 전수를 22종으로 정정. (4) §9.2 reportKind 표에서 `TRAVEL_RULE`/`CRYPTO_TRAVEL_RULE` 행 제거(fds `CaseType` 10종에 travel 케이스 없음·aml `ReportType` 6종=STR/CTR/EDD_REGISTER/WLF_REGISTER/RA_REPORT/AUDIT_EXPORT) + "enum 보존" 서술을 삭제 정정으로 교체. (5) §5.3 시퀀스·§9.3 흐름·§12 downstream 의 `STR/CTR/Travel Rule`→`STR/CTR`, `reportKind ∈ STR/CTR/TRAVEL_RULE`→`STR/CTR`, `FDS-AML-DELEGATED` 서술 현행화. **OPEN_AML_CASE/REGULATORY_REPORT aml-svc 위임 흐름 자체는 유지**(Travel Rule 만 제거). 과거 changelog 행은 역사 기록으로 보존. | 코드=truth. 근거=aegis-aml 84997e1(feature/remove-travel-rule)·fds `DecisionActionRouter`/`ActionType`(22종)·`CaseType`(10종)·aml `EventFamily`(19종)/`ReportType`(6종)·V31/V9/V14 DROP 마이그레이션. integration-designer |
...
| 2026-06-08 | v1.5 | **격리(isolation_mode) → 배포 모델(deployment topology) 재설계** 동기화(설계서 `01-fdsSvc-sass.md` v1.5 §13/§11.6.11/§11.6.11a/§12.8 + DB `01-fds-db.md` v1.3 §2.1/§2.2/§4.1/§5.1/§9 + API `01-fds-api.md` v1.5 §10/§11.2 + 정본 target-architecture §4.1). 본 문서는 연동 명세이므로 **온보딩 엔드포인트는 bo-api 전용으로 두고 fds-svc 비동기 큐·엔진 API에 추가하지 않으며 라우팅 의미·등록 콜백 연동만 재정의**한다. (1) **§1 원칙에 배포 모델 라우팅 항 추가** — `deployment_model`(`MANAGED_DEDICATED` 기본/`SELF_HOSTED`/`SHARED`), 전용 배포=배포=서비스 단일·라우팅은 배포 엔드포인트 단위·서비스 간 격리는 배포 경계 보장, `SHARED`만 공유 큐 위 `Tenant-Id` 헤더 행 라우팅. (2) **§10.1을 'deployment_model + tenant/workspace/data-scope'로 재작성** — 모델별 큐 토폴로지·`tenantId` 의미·격리 보장 표 신설(전용=전용/인스턴스 큐·단일 상수, SHARED=공유 큐 행 라우팅), `workspaceId`(서비스 내 환경)·`dataScope`(권한 필터, 메시지 미탑재) 의미 재정의, 온보딩=bo-api 워크플로우 소유 명시. (3) **§10.2 self-hosted 인스턴스 등록 콜백 연동 신설**(기존 PII 절은 §10.3으로 재번호) — `PACKAGE_ISSUED→CUSTOMER_DEPLOYED→REGISTERED` 시퀀스(Mermaid), 등록 콜백=bo-api 전용 `POST .../onboarding/register`(멱등·라이선스 ref만·PII/시크릿 금지), 등록 후 전용 큐 처리(공유 행 라우팅 미수신). (4) **§12 downstream에 배포 모델 라우팅·온보딩 상태머신(8종)·등록 콜백·서비스 등록 흐름·`TenantDto`(`deploymentModel`/`onboardingStatus`/`region`/`infraRef`, `isolationMode` 폐기) 확정**. (5) 입력 핀 갱신(설계서 v1.2→v1.5·DB v1.1→v1.3·API v1.x→v1.5). 폐기: `isolation_mode` 행/스키마 토글·격리 라디오 UI. ⚠ 오픈결정 승계: self-hosted 라이선스 발급·검증 방식(라이선스 서버 vs 오프라인 토큰)·매니지드 IaC 파이프라인 도구는 P8 인프라 설계 확정, `SHARED` 실제 제공 여부는 영업 정책 대기. | integration-designer |
