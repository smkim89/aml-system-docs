...
- **적용 경계(2026-07-13)**: P0-04로 `/internal/v1/fds/**` v2-only 수신, BO→FDS typed signer, FDS→AML non-AWS REST fallback signer를 완료했다. credential은 exact `(purpose,tenant,workspace)` target에만 해소하고 local lifecycle은 AML/FDS bootstrap-off다. 남은 미완료는 P0-14(multipart raw-byte signer), P1-02(machine credential 생성·자동 유예회전·폐기·last-used·rate/network/workload 통제)다.
