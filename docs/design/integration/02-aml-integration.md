...
> **인프라 타임아웃 서킷·일시정지(2026-09-07, aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit).** 재스크린 워커는 후보 조회 타임아웃 등 DB 인프라 실패를 주체 실패와 분리해 attempt 를 소모하지 않고 쿨다운 후 재시도하며, 연속 인프라 실패(기본 3) 시 남은 claim 을 해제하고 워커 서킷을 OPEN(쿨다운 300s→최대 1800s, half-open 1건)한다. 틱 예산(60s) 초과 시에도 해제. 운영자 `POST …/watchlist-rescreen-jobs/{jobId}:pause|:resume`(API §2.7)로 잡 단위 정지 가능(V76 `paused_at`). 재스크린 SLA·reconciliation·DLQ 계약은 무변경(정지 중 잡도 open job 집계에 남는다).
...
| `fds.decision.blocked` | **FDS `BLOCK` 판정(natural-key 신규 저장) → AML 거래 read model 표기**(aegis-aml PLAN 20260906-fds-block-aml-propagation) — 판정 시점 통지, 계정계 ACK 무관 | `tenantId`·`eventType`·`eventId`(=`decision-blocked:{decisionId}`, 멱등키)·`decisionId`·`fdsEventRef`·`transactionRef`(**필수**, 계정계 동일 거래참조)·`targetRef`·`outcome`·`phase`·`matchedRules[{ruleId,ruleName,versionNo,outcome}]`·`reasonCodes[]`·`decidedAt`·`dataScope` | **케이스·알럿 미개설**(EDD/STR 트리거 아님). `RecordFdsTransactionDecisionUseCase` 가 `aml_fds_transaction_decisions`(V75) 에 `UNIQUE(tenant_id, fds_decision_id)` 멱등 append + 감사 1건. 거래 조회(`GET /api/v1/aml/transactions`·alert related-transactions) 행 `fdsDecision` 로 노출 | `aml_fds_transaction_decisions` |
...
> `fds.decision.blocked` 의 동기 fallback(non-aws) 경로는 `POST /internal/v1/aml/fds-decisions`(`FdsDecisionInternalController`, API §3.10a)다 — 가드는 `fds-escalations` 와 동일(scope `aml:internal:fds-escalation:write`·`X-Internal-Service=fds-svc`·`X-Data-Scope`=body `dataScope`)이며 `Idempotency-Key`=`eventId`. SQS 경로는 `FdsDecisionConsumer` 가 `eventType` 으로 분기한다. 발행 측은 FDS 전용 아웃박스 `fds.fds_aml_notify_outbox`(FDS DB §5.35a) + 전용 릴레이(지수 백오프·DLQ, `!test` 프로파일 — 로컬 스택에서도 동작)이며 AML 장애가 FDS 판정 응답을 실패시키지 않는다.
...
| `webhook.callback.requested` | screening/case/report/member-decision 상태 변경 | `subjectRef`·`eventName`(API §8.1, aggregate `WEBHOOK`) | **SQS 미경유** — `OutboxRelayRouter`가 `WebhookSenderPort`로 서명 HTTP 전송. 콜백 URL 원천 = `aml_api_credentials`(`credential_type=WEBHOOK enabled`).`webhook_url`(DB §3.15, 구현 `V17`). 공유 secret = 동일 행 `secret_ciphertext`(서명 시점만 복호). **그 행의 런타임 등재·교체 경로 = admin REST `GET\|PUT /api/v1/admin/aml/webhook-credential`(API §2.7a, 2026-08-10 신설 — 조회 `aml:case:read`/쓰기 `aml:admin:policy`, upsert·즉시 반영·4-eyes 없음, AES-GCM 암호문 저장·응답에 시크릿 필드 없음)** — 종전 유일 수단이던 DB 직접 INSERT 를 대체한다(스키마 무변경). 목적지 없이 **서명 시크릿만** 등재하는 `webhook_url=NULL` 구성도 유효하다(요청별 `webhook_target_url` 오버라이드 이벤트용). **`aml_source_systems` 에는 webhook URL 컬럼 없음**(fds-svc `fds_api_credentials.webhook_url` 미러). **전송 직전 egress SSRF 재검증(P0-17, API §8 정본)** — `WebhookUrlPolicy` 위반·redirect(3xx) 미추종은 delivery 실패로 outbox `FAILED`+지수 backoff에 수렴. **(2026-09-03 신설, PLAN 20260903-aml-decision-status-webhooks)** 신규 2종 `AmlMemberDecisionResolved`(family `member`, `EDD_CLOSE`/`RELATIONSHIP_REJECT` 결재 EXECUTED)·`AmlScreeningWhitelistChanged`(family `screening`, `FP_WHITELIST` 결재 EXECUTED) + 기존 `AmlScreeningResolved` data additive 보강(API §8.1). **(2026-09-09 신설, PLAN 20260909-aml-ra-direct-member-decision)** `AmlMemberDecisionResolved` 는 세 번째 발행 트리거를 추가로 갖는다 — 당연고위험(HRR) 등재가 경영진 결재(`HRR_REGISTRATION`/`EXECUTIVE_APPROVAL`)로 이미 EXECUTED 된 회원에 대해 RA 상세 관리자 액션의 **직접 확정**(`POST /api/v1/admin/aml/customers/{customerRef}/decision`, `source=ADMIN_DIRECT`, 결재 없이 즉시)이 성공하는 즉시 발행되며, 목적지 우선순위는 기존 회원 결정 규칙(최신 CDD `callback_url` 우선)과 동일하다. `source=ADMIN_DIRECT` data 는 결재·케이스가 없어 `approvalId`/`caseId`/`caseType`/`finalStatus` 가 모두 `null`이다. 목적지 우선순위: 회원 결정=회원의 최신 `customer.cdd.completed` 인입 `callback_url`(`aml_canonical_events`, §12.1) → 없으면 테넌트 `webhook_url`; WLF(screening/whitelist)=원 스크리닝의 `aml_screening_results.callback_url`(V74) → 없으면 테넌트 `webhook_url`(HRR `AmlHighRiskRegistrationApproved` 요청별 URL 오버라이드와 동형 패턴). 결재 **반려**는 세 이벤트 모두 미발행. **(2026-09-04 신설, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095)** 신규 `AmlHighRiskRegistrationRevoked`(family `hrr`, `HRR_REGISTRATION`(`subjectRef=DEREGISTER\|customerRef`) 등재 해제 결재 EXECUTED) — 목적지는 `AmlHighRiskRegistrationApproved`와 동일하게 요청별 URL(등재 상신 결부 `callbackUrl`) 우선, 없으면 테넌트 `webhook_url`. 결재 **반려는 미발행**(기존 정책과 동일) |

> webhook 아웃박스 row 는 서비스 콜백 envelope(API §8.2 정본)을 발행한다(코드 truth `WebhookOutboxEmitter`). envelope 키: `schemaVersion`(`aml.webhook.v1`)·`eventName`(`AmlScreeningResolved`/`AmlCaseStatusChanged`/`AmlReportSubmitted`/`AmlHighRiskRegistrationApproved`/`AmlMemberDecisionResolved`/`AmlScreeningWhitelistChanged`/**`AmlHighRiskRegistrationRevoked`**(신규, PLAN 20260904-aml-hrr-deregistration))·`eventFamily`(`screening`/`case`/`report`/`hrr`/**`member`**(신규, PLAN 20260903-aml-decision-status-webhooks), **`eventName` 접두에서 서버 파생** — 입력 아님, `AmlHighRiskRegistrationRevoked`도 `AmlHighRiskRegistration*`→`hrr` 기존 접두 매핑 그대로 적용)·`eventId`·`tenantId`·`dataScope`·`occurredAt`·`traceId`·`data`. 모든 키 camelCase, `data`는 token/hash·마스킹만(원문 미포함). 서명·재시도·멱등은 API §8.3/§8.4 정본.
...
| 일자 | 버전 | 변경 | 비고 |
|---|---|---|---|
| 2026-09-09 | v4.19 | **§3.4 `webhook.callback.requested` — 회원 심사 직접 확정(`ADMIN_DIRECT`) 세 번째 발행 트리거 추가(코드=truth, aegis-aml PLAN 20260909-aml-ra-direct-member-decision).** 당연고위험(HRR) 등재 EXECUTED 회원에 대한 RA 상세 관리자 직접 확정(결재 없이 즉시)이 성공하면 기존 `AmlMemberDecisionResolved` 를 동일 목적지 우선순위 규칙으로 발행한다(`source=ADMIN_DIRECT`, `approvalId`/`caseId`/`caseType`/`finalStatus` 모두 `null`). 기존 `EDD_CLOSE`/`RELATIONSHIP_REJECT` 트리거·서명·재시도·아웃박스 라우팅 계약 무변경. | integration-designer. 코드 truth=aml-svc `application/usecase/{DirectMemberDecisionService,WebhookOutboxEmitter}`·`adapter/in/rest/MemberDecisionAdminController`. API §2.3·§8.1, DB §3.22g·§7(V77) 동일 작업 단위. |
| 2026-09-07 | v4.18 | **§3.1c 재스크린 워커 인프라 타임아웃 서킷·잡 일시정지 주석(코드=truth, aegis-aml PLAN 20260907-aml-rescreen-timeout-circuit).** 큐·이벤트 계약 무변경. | integration-designer. 코드 truth=`RescreenWorker`·`RescreenCircuitBreaker`. API §2.7·DB V76 동일 작업 단위. |
| 2026-09-06 | v4.17 | **§3.2 `fds.decision.blocked` 신설 — FDS 차단 판정의 AML 거래 전파(코드=truth, aegis-aml PLAN 20260906-fds-block-aml-propagation).** FDS `BLOCK` 판정 시점에 동일 `transactionRef` 로 통지, AML 은 알럿·케이스 없이 `aml_fds_transaction_decisions`(V75) 에 멱등 기록하고 거래 조회 행 `fdsDecision` 으로 노출. non-aws 동기 경로 `POST /internal/v1/aml/fds-decisions` 신설(가드 `fds-escalations` 동형). 기존 `fds.case.escalated`/`fds.decision.applied` 계약 무변경. | integration-designer. 코드 truth=fds-svc `AmlNotifyOutboxEmitter`·`AmlNotifyRelayService`·`RestAmlHandoffPublisher#notifyDecision`, aml-svc `FdsDecisionInternalController`·`FdsDecisionConsumer`·`FdsTransactionDecisionService`. API §2.4·§2.6·§3.10a, DB §3.22h·V75, FDS integration §4.4a·DB §5.35a·V36 동일 작업 단위. |
| 2026-09-04 | v4.16 | **§3.4 `webhook.callback.requested` — HRR 자동 등재 회원 등재 해제 웹훅 신설(코드=truth, PLAN 20260904-aml-hrr-deregistration, aegis-aml F-095).** 신규 `AmlHighRiskRegistrationRevoked`(family `hrr`, `HRR_REGISTRATION`(`subjectRef=DEREGISTER\|customerRef`) 등재 해제 결재 EXECUTED) — 목적지는 `AmlHighRiskRegistrationApproved`와 동형(요청별 URL 우선 → 없으면 테넌트 `webhook_url`), 결재 반려는 미발행. `eventName`·`eventFamily`(기존 `AmlHighRiskRegistration*`→`hrr` 접두 매핑 그대로 적용) 표에 반영. 서명·재시도·아웃박스 라우팅 계약 무변경. | integration-designer. 코드 truth=aml-svc `application/usecase/{HighRiskCustomerRegistrationService,OnboardingSnapshotReassessmentSupport,ApprovalDispatchService,WebhookOutboxEmitter}`. API §8.1, DB §3.20·§5.16 동일 작업 단위. |
| 2026-09-03 | v4.15 | **§3.4 `webhook.callback.requested` — 회원 심사결과 확정·WLF 결재 웹훅 2종 신설 + `AmlScreeningResolved` data 보강(코드=truth, PLAN 20260903-aml-decision-status-webhooks, 사용자 지시로 F-084 부분 해제).** 신규 `AmlMemberDecisionResolved`(family `member`, `EDD_CLOSE`/`RELATIONSHIP_REJECT` 결재 EXECUTED)·`AmlScreeningWhitelistChanged`(family `screening`, `FP_WHITELIST` 결재 EXECUTED) + 기존 `AmlScreeningResolved` data additive(transactionRef·targetType·score·approvalId·decidedBy·decidedAt·matchedEntries[]). 목적지 우선순위 확장 — 회원 결정=최신 CDD 인입 `callback_url`(`aml_canonical_events`) 우선, WLF=원 스크리닝의 `aml_screening_results.callback_url`(V74) 우선, 둘 다 없으면 테넌트 `webhook_url`(HRR `AmlHighRiskRegistrationApproved` 요청별 URL 패턴과 동형). `eventFamily` 파생 표에 `AmlMember*`→`member` 추가. 서명·재시도·아웃박스 라우팅 계약은 무변경. | integration-designer. 코드 truth=aml-svc `application/usecase/{MemberDecisionService,WebhookOutboxEmitter,CddEddService,CaseManagementService,ScreeningReviewService,FpWhitelistService}`·`domain/decision/MemberDecision`·`V74__member_decisions_and_screening_callback.sql`. API §2.3·§8.1·§8.2, DB §3.8·§3.22g 동일 작업 단위. |
