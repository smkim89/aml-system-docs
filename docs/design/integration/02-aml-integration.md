...
> webhook 아웃박스 row 는 서비스 콜백 envelope(API §8.2 정본)을 발행한다(코드 truth `WebhookOutboxEmitter`). envelope 키: `schemaVersion`(`aml.webhook.v1`)·`eventName`(`AmlScreeningResolved`/`AmlCaseStatusChanged`/`AmlReportSubmitted`/`AmlHighRiskRegistrationApproved`/`AmlMemberDecisionResolved`/`AmlScreeningWhitelistChanged`)·`eventFamily`(`screening`/`case`/`report`/`hrr`/**`member`**(신규, PLAN 20260903-aml-decision-status-webhooks), **`eventName` 접두에서 서버 파생** — 입력 아님)·`eventId`·`tenantId`·`dataScope`·`occurredAt`·`traceId`·`data`. 모든 키 camelCase, `data`는 token/hash·마스킹만(원문 미포함). 서명·재시도·멱등은 API §8.3/§8.4 정본.
