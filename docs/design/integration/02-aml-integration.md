...
| dilisense AML Database | `https://api.dilisense.com`(base-url 설정값) + `/v1/getConsolidatedFile`(전량)·`/v1/getConsolidatedDelta?previous=`(delta) | JSON Lines(줄당 1레코드). `x-api-key` 헤더 인증. 전량 하루 50회 쿼터. delta `previous` 2주 유효. **미실증(2026-07-31 — API 키 미보유, 문서 근거 developers.dilisense.com)**. |
