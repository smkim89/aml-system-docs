...
| BOA-S4-09 | BE | **RA 모델 관리 프록시** — server-side version copy·전체 DRAFT round-trip·scenario 표본 simulate·simulationId 기반 activate🔒·override🔒(하향) 위임. 엔진 미연결 write/simulate fail-closed | API / PRD §6.1(BR-001~006) | 2 | S1 |
| BOA-S4-10 | FE | **AML-RA-002 4탭(모델 저작)** — ①1차/2차 현재 ACTIVE·버전 목록 ②scenario별 typed factor/규칙 편집 ③실제 표본 시뮬레이션(등급 이동·설정 diff·운영 영향) ④등급 조정 이력. copy→save→simulate→activate 순서와 dirty/stale 상태를 강제 | PRD §6.1 / PPT 슬라이드 30~33 | 5 | S4-09 |
