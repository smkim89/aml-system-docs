...
- **`SHARED` 배포 저장 격리(RLS, P0-13)**: 공유 DB 행 격리는 PostgreSQL Row-Level Security 로 강제한다 — 연결 획득 시 새 login 없이 NOLOGIN role 로 `SET ROLE aegis_app_runtime` + `set_config('app.tenant_id'/'app.workspace_id'/'app.elevated', …)`, `tenant_id`(fds/bo 는 `+workspace_id`) 보유 전 테이블에 `FORCE ROW LEVEL SECURITY` + 정책 2종(runtime tenant 일치 OR elevated / owner 전량), GUC 미설정=fail-closed(0 row). 스케줄러·relay·provisioner·platform operator 는 `ElevatedDbContext` elevated escape(코드 실수 방어). AML WLF 후보조회는 non-leakproof GIN predicate가 barrier 아래에서 전량화되는 것을 막기 위해 V71의 고정 3열 SECURITY DEFINER 함수 하나만 예외로 둔다: NOLOGIN/non-super/BYPASSRLS owner, watchlist 두 테이블 SELECT 최소권한, tenant/elevated guard, PUBLIC revoke, 직접 RLS 정책 불변. V71은 별도 privileged init job으로 적용하고 application credential은 승격하지 않는다. 코드 truth=`common-security` `RlsSessionDataSource`, aml V47/V71·fds V18·bo V20, runbook `aegis-aml/docs/ops/db-rls-isolation.md`.
