...
- **`SHARED` 배포 저장 격리(RLS, P0-13)**: 공유 DB 행 격리는 PostgreSQL Row-Level Security 로 강제한다 — 연결 획득 시 NOLOGIN role 로 `SET ROLE aegis_app_runtime` + tenant/workspace/elevated GUC를 게시하고, tenant-keyed 전 테이블에 FORCE RLS + 정책 2종(runtime/owner)을 둔다. AML WLF는 non-leakproof GIN predicate가 barrier 아래에서 전량화되는 것을 막기 위해 V71의 고정 3열 SECURITY DEFINER 함수 하나만 예외로 둔다: NOLOGIN/non-super/BYPASSRLS owner, watchlist 두 테이블 SELECT+helper EXECUTE 최소권한, tenant/elevated guard, PUBLIC revoke, 직접 RLS 정책 불변. V71은 같은 release bootJar one-shot runner로 적용하며 선존 ownership/ACL을 mutation 전에 거부하고, `aegis_aml_app` non-super login으로 ordinary app을 기동한다; privileged credential을 application에 주입하지 않는다. 배포 증명은 같은 image ID, Java V38+currency Flyway inventory, active-version 대용량 feed, GIN3/no Seq/3초 plan artifact를 함께 고정한다. 코드 truth=`common-security` `RlsSessionDataSource`, aml V47/V71·fds V18·bo V20, runbook `aegis-aml/docs/ops/db-rls-isolation.md`.
