...
- **`SHARED` 배포 저장 격리(RLS, P0-13)**: 공유 DB 행 격리는 PostgreSQL RLS로 강제한다 — 연결 획득 시 NOLOGIN `aegis_app_runtime`으로 강등하고 tenant/workspace/elevated GUC를 게시하며 tenant-keyed 전 테이블에 FORCE RLS+runtime/owner 2정책을 둔다. AML WLF는 non-leakproof GIN predicate 전량화를 막기 위해 V71의 고정 3열 SECURITY DEFINER 함수 하나만 bounded 예외로 둔다: NOLOGIN/non-super/BYPASSRLS owner, 두 watchlist table SELECT+helper EXECUTE, tenant/elevated guard, PUBLIC revoke, 직접 RLS 정책 불변. Same-release one-shot은 canonical Java parser→normalized config/Base64 app envelope→privileged V71→별도 transaction app provision 순서이며 V71-only 상태는 재실행 가능하다. ordinary app은 `aegis_aml_app` non-super login으로 기동하고 privileged credential을 받지 않는다. 배포 증명은 tenant_demo/PHP, 동일 image `.Id`, Java V38+PHP inventory, `dilisense-<raw>` active-version 대용량 feed, GIN3/no Seq/3초 plan artifact를 함께 고정한다. 코드 truth=`common-security` `RlsSessionDataSource`, aml V47/V71·fds V18·bo V20, runbook `aegis-aml/docs/ops/db-rls-isolation.md`.
