"""
BO-AML-SAAS-Planning_v4.0.pptx 생성 (도형 기반 · 시나리오 흐름 종합 재구성)
=========================================================================
...
NAV = ["AML 종합 대시보드", "고객사 관리", "WLF 검토", "명단 소스·임포트", "국가위험 관리",
       "RA·CDD", "TM 알림·시나리오", "케이스 관리", "규제 보고",
       "Travel Rule", "Policy Pack", "결재 대기함", "감사·증적"]
# 기능 ID prefix → NAV active 인덱스
NAVIDX = {
    "DASH": 0, "TNT": 1, "WLF": 2, "WL": 3, "CTRY": 4, "RA": 5, "CDD": 5,
    "TM": 6, "CASE": 7, "REP": 8, "TR": 9, "PP": 10, "APR": 11, "AUD": 12,
}
...
def dash_001(p):
    ...
    y = wf.kpi_cards(s, y, [
        ("WLF 검토 필요", "18 건", "상위승인 2 · 오탐 확정 33", "blue"),
        ("RA 높음 등급", "1,204 명", "3% · 중간 12,880 · 낮음 28,900", "orange"),
        ("TM 미처리 알림", "126 건", "케이스 전환 42 · STR 권고 7", "green"),
        ("케이스 SLA 진행중", "58 건", "기한 임박 7 ⚠ · 기한 초과 0", "red"),
        ("결재 대기", "7 건", "만료 임박 2 ⚠ (클릭 → AML-APR-001)", "orange")])
    ...
    wf.info_panel(s, "AML-DASH-001", [
        ...
        "• 상단 고객사 선택 ▼ · 서비스 선택 ▼ + 기간 필터(1 고객사 : N 서비스)",
        "• 카드 WLF·RA·TM·케이스 SLA·결재 대기 (각 카드 클릭 → 해당 목록 드릴다운)",
        "• 결재 대기 KPI 카드 — 만료 임박 ⚠ · 클릭 → AML-APR-001 (PRD §2.1)",
        "• 규제 보고 STR 후보·CTR 데이터·제출 대기 / Travel Rule 누락·위험 지갑·예외 대기",
        "• 운영 알림 명단 갱신 누락·WLF 적체·결재 만료·재심사 임박 (각 상세 딥링크)",
        "• 명단 소스 신선도 / 최근 위험평가(모델 버전·평가 대상·신규 높음)",
        "• 집계 30~60초 캐시 · read-only · raw PII 미포함(카운트·지표만)",
        "▸ API GET /api/v1/bo/aml/dashboard · /tenants/{id}/dashboard (bo-api 소유)"])
...
def tnt_001(p):
    ...
    wf.info_panel(s, "AML-TNT-001", [
        ...
        "• 상태 운영중(ACTIVE)/정지(SUSPENDED)/해지(OFFBOARDING) — 운영 생명주기",
        "• 온보딩 진행 중은 onboardingStatus 조건으로 배지 렌더링 (status enum 별도)",
...
def tnt_002_basic(p):
    ...
    wf.info_panel(s, "AML-TNT-002", [
        ...
        "• 변경 가능 displayName · status · policyPackCode",
...
def tnt_002_source(p):
    ...
    y = wf.table_block(s, y, 1.75, "연결된 소스 시스템 [이 고객사]",
        ["소스 ID", "종류", "연결 상태", "최근 동기화", "동작"],
        [["src-core", "회원·계좌 (CORE)", "정상", "06-08 09:10", "▶"],
         ["src-txn", "거래 (TRANSACTION)", "정상", "06-08 09:11", "▶"],
         ["src-kyc", "KYC 데이터 (KYC)", "지연 ⚠", "06-08 08:40", "▶"]],
        [0.16, 0.30, 0.16, 0.22, 0.16])
    wf.callout(s, y, "소스 시스템 = 이 고객사의 데이터 인입 경로", [
        "회원·거래·KYC 피드를 정규 이벤트로 수집 → 스크리닝·RA·TM 입력",
        "명단 소스·임포트 이력은 AML-WL-001(명단 소스·임포트) 메뉴에서 관리"])
    wf.info_panel(s, "AML-TNT-002", [
        "• 권한 조회 aml:admin:policy (bo-api 소유)",
        "• 컬럼 소스 ID · 종류 · 연결 상태 · 최근 동기화 · ▶",
        "• 종류 회원·계좌(CORE) / 거래(TRANSACTION) / KYC",
        "• 연결 상태 정상 / 지연 ⚠ / 중단 — 인입 헬스",
        "• 이전 ← ② 배포·온보딩  /  다음 → ④ 정책팩",
    ...
    y = wf.two_panels(s, y, 1.70,
        ("[적용 정책팩]", ["항목", "값"],
         [["정책팩 코드", "한국 기본팩 (KR_DEFAULT)"],
          ...
          ["CTR 기준금액", "10,000,000 KRW"],
          ["RA 고위험 임계", "0.75"],
          ["관할(jurisdiction)", "KR"]],
         [0.42, 0.58]),
        ("[변경 상신]", ["항목", "값"],
         [["변경 대상", "정책팩 / 기준금액"],
          ["결재 종류", "정책팩 변경 (POLICY_PACK)"],
          ["상신 동작", "[변경 상신 → 결재(2인)]"],
          ["이력", "[변경 상신·이력 보기]"]],
         [0.42, 0.58]))
    wf.info_panel(s, "AML-TNT-002", [
        "• 권한 조회 aml:admin:policy · 변경 4-eyes (POLICY_PACK)",
        "• 정책팩 변경은 AML-PP-001(Policy Pack 관리)에서 상신 → 결재 후 effective",
        "• CTR 기준금액·RA 임계는 정책팩 버전에 종속",
        "• 이전 ← ③ 소스 시스템  (상세 4탭 끝)",
        "▸ API GET .../tenants/{tenantId}/policy-pack · POST .../policy-packs:change(2인) (bo-api)"])
...
def wlf_001(p):
    ...
    y = wf.table_block(s, y, 1.30, "검토 필요 큐 [고객사: 은행 A] (POSSIBLE_MATCH)",
    ...
    y = wf.two_panels(s, y, 1.5,
        ...
        ("점수 분해 (factor 기여)", ["factor", "기여"],
         ...
         [0.6, 0.4]))
    wf.callout(s, y, "① 검토 필요 시나리오 — 행 선택 → 근거·점수 확인 → 판정", [
        "판정 [확정 매칭 ▼](확정/오탐/자동낮춤) 사유 입력 후 [상신(2인)] · 자기승인 차단",
        "상위승인 필요 건은 [상위 승인 상신] → 다음 ② [상위 승인] 탭에서 4-eyes 처리",
        "오탐 면제는 [오탐 면제 등록(2인)] → FP_WHITELIST(만료 가능)"])
    wf.info_panel(s, "AML-WLF-001", [
        ...
        "• 대상유형 개인/법인/상대방/지갑주소",
        "• 검토필요 큐(POSSIBLE_MATCH) + 선택 후보 근거·점수 분해(master-detail)",
...
def wlf_003(p):
    ...
    wf.table_block(s, y, 1.85, "처리 이력 (4-eyes 증적 · 감사 연결)",
...
def wl_001(p, tab=0):
    ...
    if tab == 0:
        ...
        y = wf.table_block(s, y, 2.30, "명단 소스 [고객사: 은행 A]",
...
def ctry_001(p, tab=0):
    ...
    if tab == 0:
        y = wf.filters(s, y, ["위험 등급 전체", "근거 전체"])
        y = wf.table_block(s, y, 2.55, "국가위험 등급표 (정책 store · effective v7)",
            ["국가", "위험 등급", "근거", "버전", "적용 시점"],
            [["IR (이란)", "거래금지", "FATF blacklist·제재", "v7", "2026-03-01"],
             ["KP (북한)", "거래금지", "FATF blacklist·제재", "v7", "2026-03-01"],
             ["MM (미얀마)", "높음", "FATF greylist", "v7", "2026-03-01"],
             ["PH (필리핀)", "중간", "고위험 corridor", "v7", "2026-03-01"],
             ["KR (한국)", "낮음", "기본", "v7", "2026-03-01"]],
            [0.24, 0.16, 0.30, 0.10, 0.20])
        ...
        wf.info_panel(s, "AML-CTRY-001", [
            ...
            "• 탭 ① 국가위험 등급표 / ② 변경 상신·이력",
            "• 필터 위험 등급 / 근거 + 국가 텍스트",
            "• 컬럼 국가(ISO) · 위험 등급 · 근거 · 버전 · 적용 시점",
            "• 위험 등급 낮음/중간/높음/거래금지 (RA 등급과 동일 축)",
            "• 근거 FATF blacklist/greylist · 제재 · 고위험 corridor 등",
            "• 다음 → ② 변경 상신·이력",
...
def ra_001(p, tab=0):
    ...
    if tab == 0:
        ...
        y = wf.table_block(s, y, 2.45, "고위험 목록 (높음·거래금지 · 대상=마스킹 식별자)",
        ...
        wf.info_panel(s, "AML-RA-001", [
            ...
            "• 이전 ← ① 점수 분포  /  다음 → ③ 시뮬레이션",
            "▸ API GET .../ra-scores?grade=HIGH,PROHIBITED"])
    else:  # tab == 2
        ...
        y = wf.two_panels(s, y, 1.50,
            ...
            ("factor 변화 영향", ["factor", "영향"],
             [["고위험국가 가중치", "+5pt → 높음 증가"],
              ...
              ["오탐율", "v4: 4% → v5: 10%"]],
             [0.50, 0.50]))
        wf.callout(s, y, "③ 시뮬레이션 — 분석 전용(등급 변경 동작 없음 · 결재 불필요)", [
            "[시뮬레이션 실행] → factor 변화·등급 이동·오탐율 미리보기",
            "모델 활성화는 AML-RA-002 ② factor 편집 → [활성화 상신(2인·RA_MODEL)]",
            "이전 ← ② 고위험 목록  (RA 분포 3탭 끝)"])
        wf.info_panel(s, "AML-RA-001", [
            "• 권한 시뮬레이션 aml:admin:policy (결재 불필요·분석 전용)",
            "• 탭 ③ 시뮬레이션 (모델 초안 검증)",
            "• 설정 비교 모델 선택 / 샘플 기간 / 샘플 수",
            "• 결과 등급 이동(높음/중간/낮음 delta) + 오탐율 예상",
            "• factor 변화 영향 표 (변경 factor별 등급 이동 설명)",
            "• 실행 후 결재 없이 분석 결과만 표시",
            "• 이전 ← ② 고위험 목록  (RA 분포 3탭 끝)",
            "▸ API GET .../ra-models/{code}/simulate (결재 불필요)"])


RA_002_TABS = ["버전 목록", "factor 편집", "등급 조정 이력"]
...
def ra_002(p, tab=0):
    ...
    if tab == 0:
        ...
        wf.callout(s, y, "② factor 편집 — 자연어 미리보기 · 활성화 상신 (4-eyes)", [
            ...
            "[임시저장] [시뮬레이션(→AML-RA-001 ③)] [활성화 상신(2인·RA_MODEL)]"])
        wf.info_panel(s, "AML-RA-002", [
...
TM_001_TABS = ["알림 적체", "시나리오 목록"]


def tm_001(p, tab=0):
    titles = ["TM 알림 적체 / 시나리오 목록 — ① 알림 적체",
              "TM 알림 적체 / 시나리오 목록 — ② 시나리오 목록"]
    crumbs = ["AML Console > TM 알림·시나리오 > 알림 적체",
              "AML Console > TM 알림·시나리오 > 시나리오 목록"]
    s = frame(p, "AML-TM-001", crumbs[tab], titles[tab], search="대상 식별자 검색...")
    y = wf.CON_TOP
    y = wf.tab_chips(s, y, TM_001_TABS, active=tab)
    if tab == 0:
        y = wf.filters(s, y, ["시나리오 전체", "심각도 전체", "상태 전체", "기간 전체"])
        y = wf.table_block(s, y, 1.95, "TM 알림 적체 [고객사: 은행 A] · 대상=마스킹 식별자",
            ...
            [["alt-3301", "구조화거래", "cust_…12", "높음", "탐지", "[케이스]"],
             ...
             ["alt-3260", "고위험 corridor", "cust_…45", "중간", "탐지", "[케이스]"]],
            [0.14, 0.22, 0.20, 0.14, 0.16, 0.14])
        wf.callout(s, y, "① 알림 적체 — [케이스] 클릭 → AML-CASE-002 케이스 생성", [
            ...
            "다음 → ② 시나리오 목록 탭에서 시나리오 클릭 → AML-TM-002"])
        wf.info_panel(s, "AML-TM-001", [
            "• 권한 조회 aml:case:read / 케이스 전환 aml:case:update",
            "• 탭 ① 알림 적체 / ② 시나리오 목록",
            "• 필터 시나리오·심각도·상태·기간 + 대상 식별자",
            "• 컬럼 알림ID·시나리오·대상(식별자)·심각도·상태·동작",
            "• 심각도 낮음/중간/높음/매우높음",
            "• 상태 탐지/1차분류/케이스생성/기각/상위승인/STR권고",
            "• 다음 → ② 시나리오 목록",
            "▸ API GET /aml/alerts?status=PENDING"])
    else:  # tab == 1
        y = wf.filters(s, y, ["시나리오 상태 전체", "시나리오 유형 전체"])
        y = wf.table_block(s, y, 2.50, "TM 시나리오 목록 [고객사: 은행 A]",
            ["시나리오 코드", "시나리오명", "활성 버전", "상태", "탐지 건수(7일)", ""],
            [["STRUCTURING", "구조화거래", "v3", "활성", "12", "[편집 ▶]"],
             ["RAPID_MOVE", "급속이동", "v2", "활성", "5", "[편집 ▶]"],
             ["MULE_NETWORK", "뮬 네트워크", "v1", "활성", "8", "[편집 ▶]"],
             ["HIGH_RISK_CORRIDOR", "고위험 corridor", "v2", "활성", "3", "[편집 ▶]"],
             ["FAKE_MERCHANT", "허위가맹점", "v1", "초안(DRAFT)", "0", "[편집 ▶]"]],
            [0.22, 0.22, 0.14, 0.12, 0.18, 0.12])
        wf.callout(s, y, "② 시나리오 목록 — 시나리오 클릭 → AML-TM-002 빌더 상세", [
        ...
        wf.info_panel(s, "AML-TM-001", [
            ...
            "• 컬럼 시나리오 코드·명·활성 버전·상태·탐지 건수(7일)",
...
def tm_002(p):
    ...
    y = wf.CON_TOP
    y = wf.entry_banner(s, y, "AML-TM-001 ② 시나리오 목록에서 [시나리오 편집 ▶] 클릭 → 시나리오 빌더")
...
def case_001(p):
    ...
    y = wf.table_block(s, y, 2.35, "케이스 목록 [고객사: 은행 A] · 대상=마스킹 식별자",
...
def rep_001(p, tab=0):
    ...
    if tab == 0:
        ...
        y = wf.table_block(s, y, 2.35, "STR 후보 목록 [고객사: 은행 A] · 대상=마스킹 식별자",
            ["보고ID", "대상(식별자)", "케이스", "상태", "발단", "제출 참조"],
            [["rep-220", "cust_…501", "case-771", "검토중", "구조화거래 의심", "▶"],
             ["rep-215", "ent_…77", "case-760", "승인", "WLF 확정·PEP", "(제출 대기) ▶"],
             ["rep-210", "cust_…90", "case-732", "초안", "TM 고위험 escalation", "▶"]],
            [0.13, 0.18, 0.14, 0.12, 0.28, 0.15])
        wf.callout(s, y, "① STR 후보 — 행 클릭 → AML-REP-002 (보고 상세·제출 4-eyes)", [
            "STR 후보 발단 WLF 확정·EDD 거절·TM 고위험·FDS escalation·무역 증빙 불일치·수동",
            "다음 → ② CTR 데이터 탭"])
        wf.info_panel(s, "AML-REP-001", [
            "• 권한 조회 aml:case:read / 초안 생성 aml:case:update",
            "• 탭 ① STR 후보 / ② CTR 데이터 / ③ 제출 이력",
            "• 필터 상태 / 기간 + 대상 식별자",
            "• 컬럼 보고ID·대상(식별자)·케이스·상태·발단·제출 참조",
            "• 상태 초안/검토중/승인/제출완료/반려/취소",
            "• 다음 → ② CTR 데이터",
            "▸ API GET/POST .../reports?type=STR"])
    elif tab == 1:
        y = wf.filters(s, y, ["기간 전체", "상태 전체"])
        y = wf.kpi_cards(s, y, [
            ("이번달 CTR 대상", "21 건", "고액현금거래 1,000만원↑", "blue"),
            ("제출 완료", "18 건", "FIU 수신 완료", "green"),
            ("제출 대기", "3 건", "오늘 제출 예정", "orange"),
            ("기준금액", "1,000만원", "한국 기본팩 v9", "red")])
        y = wf.table_block(s, y, 1.65, "CTR 데이터 [고객사: 은행 A] · 대상=마스킹 식별자",
            ["보고ID", "대상(식별자)", "거래금액", "거래일", "상태", "제출 참조"],
            [["rep-218", "cust_…12", "2,500만원", "06-05", "제출완료", "FIU-2026-000218"],
             ["rep-217", "cust_…45", "1,200만원", "06-06", "제출 대기", "▶"],
             ["rep-216", "ent_…80", "5,000만원", "06-06", "검토중", "▶"]],
            [0.13, 0.18, 0.16, 0.12, 0.14, 0.27])
        wf.callout(s, y, "② CTR 데이터 — 고액현금거래 기준금액 policy pack(AML-PP-001) 연동", [
            "CTR 기준금액 한국 기본팩(KR_DEFAULT) effective version 따름",
            "이전 ← ① STR 후보  /  다음 → ③ 제출 이력"])
        wf.info_panel(s, "AML-REP-001", [
            "• 권한 조회 aml:case:read",
            "• 탭 ② CTR 데이터",
            "• KPI 이번달 CTR 대상·제출 완료·대기·기준금액",
            "• CTR 기준금액 한국 기본팩(KR_DEFAULT) effective(AML-PP-001)",
            "• 컬럼 보고ID·대상·거래금액·거래일·상태·제출 참조",
            "• 이전 ← ① STR 후보  /  다음 → ③ 제출 이력",
            "▸ API GET .../reports?type=CTR"])
    else:  # tab == 2
        y = wf.filters(s, y, ["보고 종류 전체", "기간 전체"])
        y = wf.table_block(s, y, 2.55, "제출 이력 (STR/CTR 제출 완료 기록)",
            ["보고ID", "종류", "대상(식별자)", "제출일", "제출 참조", "상태"],
            [["rep-218", "CTR", "cust_…12", "06-05 14:30", "FIU-2026-000218", "제출완료"],
             ["rep-200", "STR", "cust_…77", "06-01 10:15", "FIU-2026-000200", "제출완료"],
             ["rep-195", "CTR", "ent_…50", "05-28 09:00", "FIU-2026-000195", "제출완료"],
             ["rep-190", "STR", "cust_…33", "05-20 11:30", "FIU-2026-000190", "제출완료"]],
            [0.13, 0.10, 0.18, 0.18, 0.28, 0.13])
        wf.callout(s, y, "③ 제출 이력 — 제출 참조번호·증빙 manifest hash 감사 보존", [
            "제출 참조(submittedRef)·제출 시각·증빙 manifest hash 증적 저장",
            "이전 ← ② CTR 데이터  (규제 보고 3탭 끝)"])
        wf.info_panel(s, "AML-REP-001", [
            "• 권한 조회 aml:case:read",
            "• 탭 ③ 제출 이력",
            "• 필터 보고 종류(STR/CTR) / 기간",
            "• 컬럼 보고ID·종류·대상·제출일·제출 참조·상태",
            "• 제출 참조번호(FIU-YYYY-XXXXXX) 증적 보존",
            "• 이전 ← ② CTR 데이터  (규제 보고 3탭 끝)",
            "▸ API GET .../reports?status=SUBMITTED"])
    ...
    if tab == 0:
        ...
        y = wf.two_panels(s, y, 1.45,
            ...
            ("증빙 manifest", ["항목", "값"],
             ...
             [0.42, 0.58]))
        y = wf.table_block(s, y, 1.15, "보고 본문 (요약 · PII는 hash/token)",
            ["항목", "내용"],
            [["의심 거래 요약", "최근 7일 9건 분할 입금 합계 9,500만원(보고 기준 회피 의심)"],
             ["WLF 근거", "제재 명단 유사도 0.92(scr-9f3a 확정)"],
             ["RA 근거", "위험 등급 높음(88점) · 고위험국가·UBO 불명"]],
            [0.26, 0.74])
        wf.callout(s, y, "① 보고 본문 — 편집(초안 결재 불필요) · 제출 4-eyes", [
            "[본문 편집(초안)] (결재 불필요) · [제출 상신(2인·보고 책임자)] → 승인 → 외부 제출",
            "다음 → ② 첨부 증빙 탭"])
        wf.info_panel(s, "AML-REP-002", [
            ...
            "• 초안 편집 결재 불필요 / 제출 4-eyes(STR_SUBMIT)",
            "• 다음 → ② 첨부 증빙",
            "▸ API GET .../reports/{id} · PATCH (본문 편집)"])
    elif tab == 1:
        ...
        y = wf.two_panels(s, y, 1.55,
            ("제출 정보", ["항목", "값"],
             ...
             [0.40, 0.60]),
            ("제출 시나리오", ["동작", "결과"],
             [["[제출 상신(2인)]", "보고 책임자 승인 → 외부 제출(EXECUTED)"],
              ["[반려]", "초안으로 회신"],
              ["제출 완료 후", "submittedRef · 케이스 REPORTED"]],
             [0.38, 0.62]))
        wf.callout(s, y, "③ 제출 이력 — 4-eyes 제출(STR_SUBMIT) · 증적 보존", [
            "[제출 상신(2인·보고 책임자)] → 승인(maker≠checker) → 외부 제출(EXECUTED)",
            "제출 후 submittedRef·제출 시각·manifest hash 증적 저장 · 케이스 상태 보고(REPORTED)",
            "이전 ← ② 첨부 증빙  (보고 상세 3탭 끝)"])
        wf.info_panel(s, "AML-REP-002", [
            "• 권한 제출 aml:case:update (2인·보고 책임자)",
            "• 탭 ③ 제출 이력",
            "• 제출 4-eyes(STR_SUBMIT·CTR_SUBMIT) maker≠checker",
            "• 제출 결과 submittedRef·시각·manifest hash 증적",
            "• 케이스 상태 보고(REPORTED) 연동",
            "• 제출 실패·재제출 상태·이력 보존(아웃박스 BE)",
            "• 이전 ← ② 첨부 증빙  (보고 상세 3탭 끝)",
            "▸ API POST .../reports/{id}:submit(2인) · GET (이력)"])
    ...
    if tab == 0:
        y = wf.two_panels(s, y, 1.55,
            ("적용 Policy Pack [고객사: 은행 A]", ["항목", "값"],
             [["적용 팩", "한국 기본팩(KR_DEFAULT)"], ["effective 버전", "v9"],
              ["jurisdiction 확장", "—"], ["적용 시점", "2026-04-01"]],
             [0.42, 0.58]),
            ("보고 기준금액 (effective)", ["기준", "값"],
             [["CTR 고액현금거래", "1,000만원 이상"], ["STR 보고 기준", "의심 시"],
              ["Travel Rule 기준", "100만원 상당 이상"], ["분할 의심 임계", "9,000만원/7일"]],
             [0.56, 0.44]))
        y = wf.table_block(s, y, 1.4, "Policy Pack 영역별 기본 반영 (한국 기본팩)",
            ["영역", "기본 반영 기준"],
            [["CDD", "고객확인·실소유자·자금출처/거래목적"],
             ["STR / CTR", "의심거래 후보·보고 / 고액현금거래 수집·검증"],
             ["Sanctions·PEP / VASP", "명단 필터링·정치인 관리 / Travel Rule·지갑 risk"],
             ["Privacy·Audit", "최소수집·목적제한 / append-only 증적 보존"]],
            [0.26, 0.74])
        wf.callout(s, y, "① 적용 팩 / 기준금액 — 변경은 ② 변경 상신·이력 탭에서 4-eyes", [
            "CTR/STR/Travel Rule 기준금액은 AML-REP-001/002·AML-TM-002에 연동",
            "다음 → ② 변경 상신·이력"])
        wf.info_panel(s, "AML-PP-001", [
            "• 권한 조회·변경 상신 aml:admin:policy",
            "• 탭 ① 적용 팩 / 기준금액 · ② 변경 상신·이력",
            "• 적용 팩 한국 기본팩(KR_DEFAULT) + 고객사 jurisdiction 확장",
            "• 보고 기준금액 CTR·STR·Travel Rule·분할 의심 임계(effective)",
            "• 영역 CDD/STR/CTR/Sanctions/PEP/VASP/Privacy/Audit",
            "• 다음 → ② 변경 상신·이력",
            "▸ API GET .../policy-packs · /tenants/{id}/policy-pack"])
    else:  # tab == 1
        ...
        y = wf.table_block(s, y, 2.10, "변경 상신 이력 (4-eyes 증적 · POLICY_PACK)",
            ["결재ID", "변경 내용", "상신자", "상태", "처리일"],
            [["apr-490", "CTR 기준금액 800만원 → 1,000만원 (고시 개정)", "이감리", "승인", "2026-04-01"],
        ...
        wf.info_panel(s, "AML-PP-001", [
            ...
            "▸ API POST .../policy-packs:change(2인·POLICY_PACK) · GET (이력)"])
...
SCREENS = [
    ...
    lambda p: ra_001(p, 2),            # AML-RA-001 ③ 시뮬레이션
    ...
    lambda p: ra_002(p, 2),            # AML-RA-002 ③ 등급 조정 이력
    ...
    lambda p: tm_001(p, 1),            # AML-TM-001 ② 시나리오 목록
...
def build():
    ...
    wf.cover_slide(p,
        ...
        ["정본 PRD: docs/plan/02-aml-sass-functional-spec.md (FS-AML-SAAS-001 v5.6)",
         "기능 ID 전수 23화면 (목록→상세→액션→결과 흐름 연결 + 고객사 관리 3종: 목록·상세[4탭]·등록)",
         "좌 75% 와이어프레임(실제 도형) + 우 25% 기능 설명",
         "WLF·명단·국가위험·RA/CDD·TM·케이스·규제 보고·Travel Rule·Policy Pack·4-eyes 결재·고객사 관리",
         "용어 고객사(tenant)·서비스(workspace) · enum 괄호 병기 · 책임 경계 명시",
         "버전 BO-AML-SAAS-Planning v5.6 (드릴다운 진입 트리거 배너 + RA 순서 재배치 · 슬라이드 47+)"],
    ...
    for fn in SCREENS:
        fn(p)
    out = "/Users/smkim/workspace/smkim89/aml-system-docs/docs/plan/BO-AML-SAAS-Planning_v5.6.pptx"
    p.save(out)
